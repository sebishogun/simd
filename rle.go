...
// it wrote.
//
// It is a plain Go loop and there is no kernel behind it, which is the honest
// shape rather than an omission: expansion's output position depends on the
// running total of the lengths, so it is a serial prefix. docs/tutorial.md
// makes the same point about ExpandInto — compression's serial half is the
// store, which an instruction fixes, and expansion's is the load, which none
// does.
//
// It stops when dst is full and allocates nothing.
func RunLengthDecodeInt32(dst, values, lengths []int32) int {
	n := min(len(values), len(lengths))
	out := 0
