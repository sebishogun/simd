...
| `make fuzz` | every `Fuzz*` in the tree, discovered per package, `FUZZTIME` seconds each — today `FuzzDifferential` (public API), `FuzzKernelsAgainstReference` (every kernel of every tier vs the reference) and `FuzzJSONMasksMatchesSeparateCalls` | adversarial bit patterns: signalling NaNs, denormals, exponent boundaries |
