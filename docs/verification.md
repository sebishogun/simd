...
| `make test-cross` | arm64, s390x, ppc64le under docker + qemu, hermetic (`--network=none`, `CGO_ENABLED=0`, `-vet=off`, `-short`), `simdinfo -require-accelerated` first | the correctness half of the non-amd64 backends; three memory-corruption bugs shipped past a green amd64 suite and only appeared here |
