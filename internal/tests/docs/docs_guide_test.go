...
// Every simd.Something named in a prose document has to be a function that
// exists.
//
// The README, the tutorial, the API reference and the guides in docs/guide
// are fragments rather than whole programs — a snippet says `simd.Add(a, b)`
// and lets the surrounding prose say what a and b are, which is the right
// shape for documentation and the wrong shape for a compiler. So they cannot
// simply be built.
//
// What can be checked is the part that actually rots: a function gets renamed,
// and the document goes on naming the old one. Nothing fails, the snippet is
...
func TestGuidesNameRealFunctions(t *testing.T) {
	documents := []string{
		...
		filepath.Join("docs", "tutorial.md"),
	}
	...
	for _, e := range entries {
		if strings.HasSuffix(e.Name(), ".md") { // README.md included
			documents = append(documents, filepath.Join("docs", "guide", e.Name()))
		}
	}
	...
	for _, rel := range documents {
		for _, m := range ref.FindAllStringSubmatch(readDoc(t, rel), -1) {
			checked++
			if !exported[m[1]] {
	...
	if checked == 0 {
		t.Fatal("no simd.* references found in the prose documents; either the " +
			"documents moved or this test stopped looking at them")
	}
}
