// Code generated by simdgen. DO NOT EDIT.
//
// Source:  csrc/bytes.c
// Target:  arm64/neon
// Clang:   clang version 22.1.8
// Command: clang --target=aarch64-linux-gnu -march=armv8-a -O3 -ffreestanding -fno-builtin -fno-builtin-memset -fno-builtin-memcpy -fno-jump-tables -fno-math-errno -fno-asynchronous-unwind-tables -fno-exceptions -fno-rtti -fomit-frame-pointer -fno-stack-protector -mllvm -inline-threshold=1000 -ffp-contract=off -ffixed-x28 -ffixed-x27 -ffixed-x18 -c ../csrc/bytes.c -o ../tools/build/tmp/bytes_arm64_neon.o
//
// The instruction bytes below are the compiled function verbatim. Branches
// inside a function are PC-relative and remain correct because the whole
// body moves as a unit.

//go:build arm64 && !purego

#include "textflag.h"

// func countByteNEON(b []byte, c byte) int
TEXT ·countByteNEON(SB), NOSPLIT|NOFRAME, $0-40
	MOVD $ret+32(FP), R0
	MOVD b_base+0(FP), R1
	MOVBU c+24(FP), R2
	MOVD b_len+8(FP), R3
	WORD $0xfc1e0fea
	WORD $0xf100807f
	WORD $0x6d0123e9
	WORD $0x5400008a
	WORD $0xaa1f03e8
	WORD $0xaa1f03ea
	WORD $0x1400003c
	WORD $0x4e010c40
	WORD $0xaa1f03ea
	WORD $0xaa1f03e8
	WORD $0x52801fe9
	WORD $0x14000005
	WORD $0x9100810c
	WORD $0x8b0a03ea
	WORD $0xeb03019f
	WORD $0x5400066c
	WORD $0xcb08006b
	WORD $0xd345fd6c
	WORD $0xf103fd9f
	WORD $0x9a89318b
	WORD $0xb4ffff0c
	WORD $0x6f00e401
	WORD $0x6f00e402
	WORD $0x8b08002c
	WORD $0xaa0b03ed
	WORD $0xacc10d84
	WORD $0xf10005ad
	WORD $0x6e208c84
	WORD $0x6e208c63
	WORD $0x6e238442
	WORD $0x6e248421
	WORD $0x54ffff41
	WORD $0x2f08a423
	WORD $0x2f08a444
	WORD $0x8b0b1508
	WORD $0x6f08a421
	WORD $0x6f08a442
	WORD $0x2f10a465
	WORD $0x6f10a463
	WORD $0x6f10a486
	WORD $0x6f10a427
	WORD $0x6f10a450
	WORD $0x2f10a484
	WORD $0x2f10a421
	WORD $0x2f10a442
	WORD $0x6ea60072
	WORD $0x2ea60063
	WORD $0x6eb000f1
	WORD $0x2eb000e7
	WORD $0x6ea400b0
	WORD $0x6ea20033
	WORD $0x2ea20021
	WORD $0x2ea400a2
	WORD $0x4ef18644
	WORD $0x4ee78463
	WORD $0x4ef38605
	WORD $0x4ee18441
	WORD $0x4ee484a2
	WORD $0x4ee38421
	WORD $0x4ee28421
	WORD $0x5ef1b821
	WORD $0x9e66002b
	WORD $0x9100810c
	WORD $0x8b0a016a
	WORD $0xeb03019f
	WORD $0x54fff9ed
	WORD $0xeb080069
	WORD $0x5400148d
	WORD $0xf100213f
	WORD $0x54001363
	WORD $0xf100813f
	WORD $0x54000062
	WORD $0xaa1f03eb
	WORD $0x14000070
	WORD $0x6f00e401
	WORD $0x6f00e400
	WORD $0x5280002b
	WORD $0x6f00e405
	WORD $0x6f00e410
	WORD $0x927d052c
	WORD $0x6f00e403
	WORD $0x6f00e404
	WORD $0x927be92d
	WORD $0x6f00e402
	WORD $0x6f00e412
	WORD $0x6f00e406
	WORD $0x4e081d41
	WORD $0x8b01010a
	WORD $0x6f00e415
	WORD $0x6f00e407
	WORD $0x9100414a
	WORD $0x6f00e416
	WORD $0x6f00e413
	WORD $0x6f00e414
	WORD $0x6f00e411
	WORD $0x6f00e417
	WORD $0x4e010c58
	WORD $0x4e080d79
	WORD $0x927be92b
	WORD $0xad7fed5a
	WORD $0xf10081ad
	WORD $0x9100814a
	WORD $0x6e388f5a
	WORD $0x6e388f7b
	WORD $0x6f08a75c
	WORD $0x2f08a75a
	WORD $0x6f08a769
	WORD $0x2f08a77b
	WORD $0x6f10a79d
	WORD $0x2f10a79c
	WORD $0x6f10a75f
	WORD $0x2f10a75a
	WORD $0x6f20a7be
	WORD $0x2f20a7bd
	WORD $0x6f20a788
	WORD $0x2f20a79c
	WORD $0x6f20a7ea
	WORD $0x2f20a7ff
	WORD $0x4e391fbd
	WORD $0x4e391fde
	WORD $0x4e391d08
	WORD $0x4e391f9c
	WORD $0x4e391d4a
	WORD $0x4e391fff
	WORD $0x4efd8442
	WORD $0x6f20a75d
	WORD $0x4efe8652
	WORD $0x2f10a53e
	WORD $0x4ee88484
	WORD $0x2f10a768
	WORD $0x6f10a529
	WORD $0x6f10a77b
	WORD $0x2f20a75a
	WORD $0x4e391fbd
	WORD $0x4eea8610
	WORD $0x4efc8463
	WORD $0x2f20a7ca
	WORD $0x4eff8400
	WORD $0x6f20a7de
	WORD $0x2f20a53c
	WORD $0x6f20a529
	WORD $0x6f20a77f
	WORD $0x4efd84a5
	WORD $0x2f20a51d
	WORD $0x6f20a508
	WORD $0x2f20a77b
	WORD $0x4e391f5a
	WORD $0x4e391d4a
	WORD $0x4e391f9c
	WORD $0x4e391d29
	WORD $0x4e391fff
	WORD $0x4e391fde
	WORD $0x4e391fbd
	WORD $0x4e391d08
	WORD $0x4e391f7b
	WORD $0x4efa8421
	WORD $0x4eea8673
	WORD $0x4ee986f7
	WORD $0x4efc8631
	WORD $0x4eff86d6
	WORD $0x4efe8694
	WORD $0x4ee886b5
	WORD $0x4efd84c6
	WORD $0x4efb84e7
	WORD $0x54fff7e1
	WORD $0x4ef086d0
	WORD $0x4ef286f2
	WORD $0xeb0b013f
	WORD $0x4ee586a5
	WORD $0x4ee184c1
	WORD $0x4ee38663
	WORD $0x4ee48684
	WORD $0x4ee084e0
	WORD $0x4ee28622
	WORD $0x4ee38421
	WORD $0x4ef28603
	WORD $0x4ee28400
	WORD $0x4ee484a2
	WORD $0x4ee08420
	WORD $0x4ee38441
	WORD $0x4ee18400
	WORD $0x5ef1b800
	WORD $0x9e66000a
	WORD $0x54000600
	WORD $0xb400066c
	WORD $0x6f00e400
	WORD $0x6f00e401
	WORD $0x8b08016c
	WORD $0x0e010c42
	WORD $0x6f00e403
	WORD $0x8b0c002c
	WORD $0x6f00e404
	WORD $0x4e081d40
	WORD $0x5280002a
	WORD $0x4e080d45
	WORD $0x927df12a
	WORD $0x8b0a0108
	WORD $0xcb0a016b
	WORD $0xfc408586
	WORD $0xb100216b
	WORD $0x2e228cc6
	WORD $0x2f08a4c6
	WORD $0x6f10a4c7
	WORD $0x2f10a4c6
	WORD $0x6f20a4f0
	WORD $0x2f20a4d1
	WORD $0x6f20a4c6
	WORD $0x2f20a4e7
	WORD $0x4e251e10
	WORD $0x4e251e31
	WORD $0x4e251cc6
	WORD $0x4e251ce7
	WORD $0x4ef08484
	WORD $0x4ee68463
	WORD $0x4ef18400
	WORD $0x4ee78421
	WORD $0x54fffdc1
	WORD $0x4ee18400
	WORD $0x4ee48461
	WORD $0xeb0a013f
	WORD $0x4ee18400
	WORD $0x5ef1b800
	WORD $0x9e66000a
	WORD $0x54000100
	WORD $0xcb080069
	WORD $0x8b080028
	WORD $0x3840150b
	WORD $0x6b22017f
	WORD $0x9a8a154a
	WORD $0xf1000529
	WORD $0x54ffff81
	WORD $0x6d4123e9
	WORD $0xf900000a
	WORD $0xfc4207ea
	WORD $0xd65f03c0
	WORD $0x8b0b0108
	WORD $0x17fffff4
	RET

// func indexByteNEON(b []byte, c byte) int
TEXT ·indexByteNEON(SB), NOSPLIT|NOFRAME, $0-40
	MOVD $ret+32(FP), R0
	MOVD b_base+0(FP), R1
	MOVBU c+24(FP), R2
	MOVD b_len+8(FP), R3
	WORD $0xf101007f
	WORD $0x540001aa
	WORD $0xaa1f03e8
	WORD $0xeb03011f
	WORD $0x540000ea
	WORD $0x38686829
	WORD $0x6b22013f
	WORD $0x54000500
	WORD $0x91000508
	WORD $0xeb08007f
	WORD $0x54ffff61
	WORD $0x92800008
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x4e010c40
	WORD $0xaa1f03e9
	WORD $0x5280080a
	WORD $0x14000005
	WORD $0x9101010a
	WORD $0x91010129
	WORD $0xeb03015f
	WORD $0x54fffdcc
	WORD $0x8b090028
	WORD $0xad400901
	WORD $0xad411103
	WORD $0xaa0a03e8
	WORD $0x6e208c21
	WORD $0x6e208c42
	WORD $0x6e208c63
	WORD $0x6e208c84
	WORD $0x4ea31c21
	WORD $0x4ea41c42
	WORD $0x4ea11c41
	WORD $0x6e30a821
	WORD $0x1e26002b
	WORD $0x3607fdeb
	WORD $0xaa1f03ea
	WORD $0xaa0103eb
	WORD $0x3869696c
	WORD $0x6b22019f
	WORD $0x540000c0
	WORD $0xd100054a
	WORD $0x9100056b
	WORD $0xb101015f
	WORD $0x54ffff41
	WORD $0x17ffffe5
	WORD $0xcb0a0128
	WORD $0xf9000008
	WORD $0xd65f03c0
	RET

// func lastIndexByteNEON(b []byte, c byte) int
TEXT ·lastIndexByteNEON(SB), NOSPLIT|NOFRAME, $0-40
	MOVD $ret+32(FP), R0
	MOVD b_base+0(FP), R1
	MOVBU c+24(FP), R2
	MOVD b_len+8(FP), R3
	WORD $0xf101007f
	WORD $0x540004cb
	WORD $0x4e010c40
	WORD $0x14000005
	WORD $0xd1010068
	WORD $0xf101fc7f
	WORD $0xaa0803e3
	WORD $0x5400042d
	WORD $0x8b030028
	WORD $0xad7f0502
	WORD $0xad7e0d04
	WORD $0x4e200821
	WORD $0x4e200842
	WORD $0x4e200863
	WORD $0x4e200884
	WORD $0x6e014021
	WORD $0x6e024042
	WORD $0x6e034063
	WORD $0x6e044084
	WORD $0x6e208c21
	WORD $0x6e208c42
	WORD $0x6e208c63
	WORD $0x6e208c84
	WORD $0x4ea31c21
	WORD $0x4ea41c42
	WORD $0x4ea11c41
	WORD $0x6e30a821
	WORD $0x1e260028
	WORD $0x3607fd08
	WORD $0xd1000429
	WORD $0x52800028
	WORD $0x3863692a
	WORD $0x6b22015f
	WORD $0x54000200
	WORD $0x91000508
	WORD $0xd1000529
	WORD $0xf101051f
	WORD $0x54ffff41
	WORD $0x17ffffde
	WORD $0xaa0303e8
	WORD $0xd100042a
	WORD $0xf1000509
	WORD $0x5400014b
	WORD $0x38686948
	WORD $0x6b22011f
	WORD $0xaa0903e8
	WORD $0x54ffff61
	WORD $0xf9000009
	WORD $0xd65f03c0
	WORD $0xcb080069
	WORD $0xf9000009
	WORD $0xd65f03c0
	WORD $0x92800009
	WORD $0xf9000009
	WORD $0xd65f03c0
	RET

// func popCountNEON(b []byte) int
TEXT ·popCountNEON(SB), NOSPLIT|NOFRAME, $0-32
	MOVD $ret+24(FP), R0
	MOVD b_base+0(FP), R1
	MOVD b_len+8(FP), R2
	WORD $0x6f00e400
	WORD $0x6f00e401
	WORD $0xf100405f
	WORD $0x6f00e403
	WORD $0x6f00e402
	WORD $0x6f00e404
	WORD $0x5400016a
	WORD $0xaa1f03e8
	WORD $0xeb0203ff
	WORD $0x5400030b
	WORD $0x6f00e405
	WORD $0x6f00e406
	WORD $0x6f00e407
	WORD $0xb2400109
	WORD $0xeb02013f
	WORD $0x540003ab
	WORD $0x14000020
	WORD $0x52800208
	WORD $0xaa0103e9
	WORD $0x3cc10525
	WORD $0x91004108
	WORD $0xeb02011f
	WORD $0x4e2058a5
	WORD $0x6f08a4a6
	WORD $0x2f08a4a5
	WORD $0x6e661084
	WORD $0x2e661042
	WORD $0x6e651063
	WORD $0x2e651021
	WORD $0x54fffecd
	WORD $0x927ce848
	WORD $0xeb02011f
	WORD $0x54fffd4a
	WORD $0x38686829
	WORD $0x6f00e405
	WORD $0x9e670120
	WORD $0x0e205806
	WORD $0x6f00e400
	WORD $0x6e0404c0
	WORD $0x6f00e406
	WORD $0x6f00e407
	WORD $0xb2400109
	WORD $0xeb02013f
	WORD $0x540000aa
	WORD $0x38696829
	WORD $0x9e670130
	WORD $0x0e205a10
	WORD $0x6e0c0600
	WORD $0xb27f0109
	WORD $0xeb02013f
	WORD $0x5400078b
	WORD $0xb2400509
	WORD $0xeb02013f
	WORD $0x5400080b
	WORD $0xb27e0109
	WORD $0xeb02013f
	WORD $0x5400088b
	WORD $0x528000a9
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x5400090b
	WORD $0xb27f0509
	WORD $0xeb02013f
	WORD $0x5400098b
	WORD $0xb2400909
	WORD $0xeb02013f
	WORD $0x54000a0b
	WORD $0xb27d0109
	WORD $0xeb02013f
	WORD $0x54000a8b
	WORD $0x52800129
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x54000b0b
	WORD $0x52800149
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x54000b8b
	WORD $0x52800169
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x54000c0b
	WORD $0xb27e0509
	WORD $0xeb02013f
	WORD $0x54000c8b
	WORD $0x528001a9
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x54000d0b
	WORD $0xb27f0909
	WORD $0xeb02013f
	WORD $0x54000d8b
	WORD $0xb2400d08
	WORD $0xeb02011f
	WORD $0x540000aa
	WORD $0x38686828
	WORD $0x9e670110
	WORD $0x0e205a10
	WORD $0x6e1c0607
	WORD $0x4ea384a3
	WORD $0x4ea484e4
	WORD $0x4ea18400
	WORD $0x4ea284c1
	WORD $0x4ea18400
	WORD $0x4ea48461
	WORD $0x4ea18400
	WORD $0x4eb1b800
	WORD $0x1e260008
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x38696829
	WORD $0x9e670130
	WORD $0x0e205a10
	WORD $0x6e140600
	WORD $0xb2400509
	WORD $0xeb02013f
	WORD $0x54fff84a
	WORD $0x38696829
	WORD $0x9e670130
	WORD $0x0e205a10
	WORD $0x6e1c0600
	WORD $0xb27e0109
	WORD $0xeb02013f
	WORD $0x54fff7ca
	WORD $0x38696829
	WORD $0x9e670130
	WORD $0x0e205a10
	WORD $0x6e040605
	WORD $0x528000a9
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x54fff74a
	WORD $0x38696829
	WORD $0x9e670130
	WORD $0x0e205a10
	WORD $0x6e0c0605
	WORD $0xb27f0509
	WORD $0xeb02013f
	WORD $0x54fff6ca
	WORD $0x38696829
	WORD $0x9e670130
	WORD $0x0e205a10
	WORD $0x6e140605
	WORD $0xb2400909
	WORD $0xeb02013f
	WORD $0x54fff64a
	WORD $0x38696829
	WORD $0x9e670130
	WORD $0x0e205a10
	WORD $0x6e1c0605
	WORD $0xb27d0109
	WORD $0xeb02013f
	WORD $0x54fff5ca
	WORD $0x38696829
	WORD $0x9e670130
	WORD $0x0e205a10
	WORD $0x6e040606
	WORD $0x52800129
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x54fff54a
	WORD $0x38696829
	WORD $0x9e670130
	WORD $0x0e205a10
	WORD $0x6e0c0606
	WORD $0x52800149
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x54fff4ca
	WORD $0x38696829
	WORD $0x9e670130
	WORD $0x0e205a10
	WORD $0x6e140606
	WORD $0x52800169
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x54fff44a
	WORD $0x38696829
	WORD $0x9e670130
	WORD $0x0e205a10
	WORD $0x6e1c0606
	WORD $0xb27e0509
	WORD $0xeb02013f
	WORD $0x54fff3ca
	WORD $0x38696829
	WORD $0x9e670130
	WORD $0x0e205a10
	WORD $0x6e040607
	WORD $0x528001a9
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x54fff34a
	WORD $0x38696829
	WORD $0x9e670130
	WORD $0x0e205a10
	WORD $0x6e0c0607
	WORD $0xb27f0909
	WORD $0xeb02013f
	WORD $0x54fff2ca
	WORD $0x38696829
	WORD $0x9e670130
	WORD $0x0e205a10
	WORD $0x6e140607
	WORD $0xb2400d08
	WORD $0xeb02011f
	WORD $0x54fff24b
	WORD $0x17ffff95
	RET

// func maskBitsNEON(dst []byte, b []byte, c byte)
TEXT ·maskBitsNEON(SB), NOSPLIT|NOFRAME, $0-49
	MOVD dst_base+0(FP), R0
	MOVD b_base+24(FP), R1
	MOVBU c+48(FP), R2
	MOVD b_len+32(FP), R3
	// Constant pool appended below the body; the displacements
	// above were patched to reach it and the aligned loads made
	// unaligned, since nothing promises alignment inside a TEXT.
	WORD $0xf101007f
	WORD $0x5400006a
	WORD $0xaa1f03e9
	WORD $0x14000027
	WORD $0x4e010c40
	WORD $0x100006e8
	WORD $0xaa1f03e9
	WORD $0x3dc04501
	WORD $0xaa0003e8
	WORD $0x8b09002a
	WORD $0xad408943
	WORD $0x3dc00144
	WORD $0x3dc00d45
	WORD $0x9102012a
	WORD $0x91010129
	WORD $0x6e208c84
	WORD $0xeb03015f
	WORD $0x6e208c42
	WORD $0x6e208c63
	WORD $0x6e208ca5
	WORD $0x4e211c84
	WORD $0x4e211c42
	WORD $0x4e211c63
	WORD $0x4e211ca5
	WORD $0x6e044090
	WORD $0x6e024046
	WORD $0x6e034067
	WORD $0x6e0540b1
	WORD $0x4e103884
	WORD $0x4e063842
	WORD $0x4e073863
	WORD $0x4e1138a5
	WORD $0x4e71b884
	WORD $0x4e71b842
	WORD $0x4e71b863
	WORD $0x4e71b8a5
	WORD $0x7d000104
	WORD $0x7d000902
	WORD $0x7d000503
	WORD $0x7d000d05
	WORD $0x91002108
	WORD $0x54fffc0d
	WORD $0xeb03013f
	WORD $0x5400018b
	WORD $0xd65f03c0
	WORD $0x3869682b
	WORD $0x91000529
	WORD $0x6b22017f
	WORD $0x1a9f17eb
	WORD $0xeb09007f
	WORD $0x1aca216a
	WORD $0x3868680b
	WORD $0x2a0b014a
	WORD $0x3828680a
	WORD $0x54fffec0
	WORD $0xd343fd28
	WORD $0xf240092a
	WORD $0x54fffe81
	WORD $0x3828681f
	WORD $0x17fffff2
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x00000002
	WORD $0x00000000
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x00000006
	WORD $0x00000000
	WORD $0x00000003
	WORD $0x00000000
	WORD $0x00000006
	WORD $0x00000000
	WORD $0xcfcfcfcf
	WORD $0xcfcfcfcf
	WORD $0x33333333
	WORD $0x33333333
	WORD $0x67676767
	WORD $0x67676767
	WORD $0x19191919
	WORD $0x19191919
	WORD $0xffffff00
	WORD $0xffffff01
	WORD $0xffffff02
	WORD $0xffffff03
	WORD $0xffffff04
	WORD $0xffffff05
	WORD $0xffffff06
	WORD $0xffffff07
	WORD $0xffffff08
	WORD $0xffffff09
	WORD $0xffffff0a
	WORD $0xffffff0b
	WORD $0xffffff0c
	WORD $0xffffff0d
	WORD $0xffffff0e
	WORD $0xffffff0f
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0xffffff00
	WORD $0xffffff01
	WORD $0xffffff02
	WORD $0xffffff03
	WORD $0xffffff04
	WORD $0xffffff05
	WORD $0xffffff06
	WORD $0xffffff07
	WORD $0xffffff08
	WORD $0xffffff09
	WORD $0xffffff0a
	WORD $0xffffff0b
	WORD $0xffffff0c
	WORD $0xffffff0d
	WORD $0xffffff0e
	WORD $0xffffff0f
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x33323130
	WORD $0x37363534
	WORD $0x62613938
	WORD $0x66656463
	RET

// func maskBitsLessNEON(dst []byte, b []byte, c byte)
TEXT ·maskBitsLessNEON(SB), NOSPLIT|NOFRAME, $0-49
	MOVD dst_base+0(FP), R0
	MOVD b_base+24(FP), R1
	MOVBU c+48(FP), R2
	MOVD b_len+32(FP), R3
	// Constant pool appended below the body; the displacements
	// above were patched to reach it and the aligned loads made
	// unaligned, since nothing promises alignment inside a TEXT.
	WORD $0xf101007f
	WORD $0x5400006a
	WORD $0xaa1f03e9
	WORD $0x14000027
	WORD $0x4e010c40
	WORD $0x100006e8
	WORD $0xaa1f03e9
	WORD $0x3dc04901
	WORD $0xaa0003e8
	WORD $0x8b09002a
	WORD $0xad408943
	WORD $0x3dc00144
	WORD $0x3dc00d45
	WORD $0x9102012a
	WORD $0x91010129
	WORD $0x6e243404
	WORD $0xeb03015f
	WORD $0x6e223402
	WORD $0x6e233403
	WORD $0x6e253405
	WORD $0x4e211c84
	WORD $0x4e211c42
	WORD $0x4e211c63
	WORD $0x4e211ca5
	WORD $0x6e044090
	WORD $0x6e024046
	WORD $0x6e034067
	WORD $0x6e0540b1
	WORD $0x4e103884
	WORD $0x4e063842
	WORD $0x4e073863
	WORD $0x4e1138a5
	WORD $0x4e71b884
	WORD $0x4e71b842
	WORD $0x4e71b863
	WORD $0x4e71b8a5
	WORD $0x7d000104
	WORD $0x7d000902
	WORD $0x7d000503
	WORD $0x7d000d05
	WORD $0x91002108
	WORD $0x54fffc0d
	WORD $0xeb03013f
	WORD $0x5400018b
	WORD $0xd65f03c0
	WORD $0x3869682b
	WORD $0x91000529
	WORD $0x6b22017f
	WORD $0x1a9f27eb
	WORD $0xeb09007f
	WORD $0x1aca216a
	WORD $0x3868680b
	WORD $0x2a0b014a
	WORD $0x3828680a
	WORD $0x54fffec0
	WORD $0xd343fd28
	WORD $0xf240092a
	WORD $0x54fffe81
	WORD $0x3828681f
	WORD $0x17fffff2
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x00000002
	WORD $0x00000000
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x00000006
	WORD $0x00000000
	WORD $0x00000003
	WORD $0x00000000
	WORD $0x00000006
	WORD $0x00000000
	WORD $0xcfcfcfcf
	WORD $0xcfcfcfcf
	WORD $0x33333333
	WORD $0x33333333
	WORD $0x67676767
	WORD $0x67676767
	WORD $0x19191919
	WORD $0x19191919
	WORD $0xffffff00
	WORD $0xffffff01
	WORD $0xffffff02
	WORD $0xffffff03
	WORD $0xffffff04
	WORD $0xffffff05
	WORD $0xffffff06
	WORD $0xffffff07
	WORD $0xffffff08
	WORD $0xffffff09
	WORD $0xffffff0a
	WORD $0xffffff0b
	WORD $0xffffff0c
	WORD $0xffffff0d
	WORD $0xffffff0e
	WORD $0xffffff0f
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0xffffff00
	WORD $0xffffff01
	WORD $0xffffff02
	WORD $0xffffff03
	WORD $0xffffff04
	WORD $0xffffff05
	WORD $0xffffff06
	WORD $0xffffff07
	WORD $0xffffff08
	WORD $0xffffff09
	WORD $0xffffff0a
	WORD $0xffffff0b
	WORD $0xffffff0c
	WORD $0xffffff0d
	WORD $0xffffff0e
	WORD $0xffffff0f
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x33323130
	WORD $0x37363534
	WORD $0x62613938
	WORD $0x66656463
	RET

// func maskBitsAny4NEON(dst []byte, b []byte, chars uint32)
TEXT ·maskBitsAny4NEON(SB), NOSPLIT|NOFRAME, $0-52
	MOVD dst_base+0(FP), R0
	MOVD b_base+24(FP), R1
	MOVWU chars+48(FP), R2
	MOVD b_len+32(FP), R3
	// Constant pool appended below the body; the displacements
	// above were patched to reach it and the aligned loads made
	// unaligned, since nothing promises alignment inside a TEXT.
	WORD $0x53187c48
	WORD $0xf101007f
	WORD $0x5400006a
	WORD $0xaa1f03ea
	WORD $0x14000043
	WORD $0x53087c49
	WORD $0x53107c4b
	WORD $0x4e010c40
	WORD $0x4e010d03
	WORD $0xaa1f03ea
	WORD $0x4e010d21
	WORD $0x4e010d62
	WORD $0x10000a89
	WORD $0x3dc04d24
	WORD $0xaa0003e9
	WORD $0x8b0a002b
	WORD $0xad414167
	WORD $0xad401965
	WORD $0x9102014b
	WORD $0xeb03017f
	WORD $0x9101014a
	WORD $0x6e208cf1
	WORD $0x6e218cf5
	WORD $0x6e228cf8
	WORD $0x6e238ce7
	WORD $0x6e208cd2
	WORD $0x6e208cb3
	WORD $0x6e208e14
	WORD $0x6e218cd6
	WORD $0x6e218cb7
	WORD $0x6e218e19
	WORD $0x6e228cda
	WORD $0x6e238cc6
	WORD $0x4eb51e31
	WORD $0x6e228cb5
	WORD $0x6e238ca5
	WORD $0x4eb81ce7
	WORD $0x6e228e18
	WORD $0x6e238e10
	WORD $0x4eb61e52
	WORD $0x4eba1cc6
	WORD $0x4eb71e73
	WORD $0x4eb51ca5
	WORD $0x4eb91e94
	WORD $0x4eb81e10
	WORD $0x4eb11ce7
	WORD $0x4eb21cc6
	WORD $0x4eb31ca5
	WORD $0x4eb41e10
	WORD $0x4e241ce7
	WORD $0x4e241cc6
	WORD $0x4e241ca5
	WORD $0x4e241e10
	WORD $0x6e0740f1
	WORD $0x6e0640d2
	WORD $0x6e0540b3
	WORD $0x6e104214
	WORD $0x4e1138e7
	WORD $0x4e1238c6
	WORD $0x4e1338a5
	WORD $0x4e143a10
	WORD $0x4e71b8e7
	WORD $0x4e71b8c6
	WORD $0x4e71b8a5
	WORD $0x4e71ba10
	WORD $0x7d000927
	WORD $0x7d000526
	WORD $0x7d000125
	WORD $0x7d000d30
	WORD $0x91002129
	WORD $0x54fff92d
	WORD $0xeb03015f
	WORD $0x540002ca
	WORD $0x53105c49
	WORD $0x53083c4b
	WORD $0x14000008
	WORD $0x1ace21ad
	WORD $0x386c680e
	WORD $0x9100054a
	WORD $0xeb0a007f
	WORD $0x2a0d01cd
	WORD $0x382c680d
	WORD $0x54000180
	WORD $0x386a682c
	WORD $0x6b22019f
	WORD $0x7a4b1184
	WORD $0x7a491184
	WORD $0x7a4c1104
	WORD $0xd343fd4c
	WORD $0x1a9f17ed
	WORD $0xf240094e
	WORD $0x54fffe21
	WORD $0x382c681f
	WORD $0x17ffffef
	WORD $0xd65f03c0
	WORD $0x00000000
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x00000002
	WORD $0x00000000
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x00000006
	WORD $0x00000000
	WORD $0x00000003
	WORD $0x00000000
	WORD $0x00000006
	WORD $0x00000000
	WORD $0xcfcfcfcf
	WORD $0xcfcfcfcf
	WORD $0x33333333
	WORD $0x33333333
	WORD $0x67676767
	WORD $0x67676767
	WORD $0x19191919
	WORD $0x19191919
	WORD $0xffffff00
	WORD $0xffffff01
	WORD $0xffffff02
	WORD $0xffffff03
	WORD $0xffffff04
	WORD $0xffffff05
	WORD $0xffffff06
	WORD $0xffffff07
	WORD $0xffffff08
	WORD $0xffffff09
	WORD $0xffffff0a
	WORD $0xffffff0b
	WORD $0xffffff0c
	WORD $0xffffff0d
	WORD $0xffffff0e
	WORD $0xffffff0f
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0xffffff00
	WORD $0xffffff01
	WORD $0xffffff02
	WORD $0xffffff03
	WORD $0xffffff04
	WORD $0xffffff05
	WORD $0xffffff06
	WORD $0xffffff07
	WORD $0xffffff08
	WORD $0xffffff09
	WORD $0xffffff0a
	WORD $0xffffff0b
	WORD $0xffffff0c
	WORD $0xffffff0d
	WORD $0xffffff0e
	WORD $0xffffff0f
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x33323130
	WORD $0x37363534
	WORD $0x62613938
	WORD $0x66656463
	RET

// func maskBitsAnyNEON(dst []byte, b []byte, chars uint64)
TEXT ·maskBitsAnyNEON(SB), NOSPLIT|NOFRAME, $0-56
	MOVD dst_base+0(FP), R0
	MOVD b_base+24(FP), R1
	MOVD chars+48(FP), R2
	MOVD b_len+32(FP), R3
	// Constant pool appended below the body; the displacements
	// above were patched to reach it and the aligned loads made
	// unaligned, since nothing promises alignment inside a TEXT.
	WORD $0x6dbd33ed
	WORD $0xd378fc48
	WORD $0xf101007f
	WORD $0x6d012beb
	WORD $0x6d0223e9
	WORD $0x5400006a
	WORD $0xaa1f03ea
	WORD $0x14000073
	WORD $0xd348fc49
	WORD $0xd368fc4b
	WORD $0xd350fc4c
	WORD $0xd360fc4d
	WORD $0xd358fc4e
	WORD $0x4e010c41
	WORD $0x4e010d20
	WORD $0xd370fc49
	WORD $0x4e010d82
	WORD $0x4e010dc3
	WORD $0x4e010da4
	WORD $0x4e010d65
	WORD $0x4e010d26
	WORD $0x4e010d07
	WORD $0x10001149
	WORD $0x3dc05130
	WORD $0xaa1f03ea
	WORD $0xaa0003e9
	WORD $0x8b0a002b
	WORD $0xad404d72
	WORD $0xad414574
	WORD $0x9102014b
	WORD $0xeb03017f
	WORD $0x9101014a
	WORD $0x6e218e56
	WORD $0x6e208e59
	WORD $0x6e228e5e
	WORD $0x6e218e37
	WORD $0x6e208e3a
	WORD $0x6e228e3d
	WORD $0x6e238e5f
	WORD $0x6e238e28
	WORD $0x6e218e75
	WORD $0x6e208e78
	WORD $0x6e218e9b
	WORD $0x6e208e9c
	WORD $0x4eba1ef7
	WORD $0x4eb91ed6
	WORD $0x6e228e99
	WORD $0x6e238e9a
	WORD $0x4ebd1d1d
	WORD $0x4ebe1ffe
	WORD $0x6e248e9f
	WORD $0x6e258e88
	WORD $0x4eb81eb5
	WORD $0x4ebc1f78
	WORD $0x6e228e7b
	WORD $0x6e238e7c
	WORD $0x4eb91f59
	WORD $0x6e248e7a
	WORD $0x6e258e69
	WORD $0x6e248e2a
	WORD $0x6e248e4b
	WORD $0x6e258e4c
	WORD $0x6e258e2d
	WORD $0x4ebf1d1f
	WORD $0x6e268e88
	WORD $0x4ebb1f9b
	WORD $0x4eba1d3a
	WORD $0x6e268e7c
	WORD $0x4eb81f38
	WORD $0x4eab1d99
	WORD $0x6e268e49
	WORD $0x4eaa1daa
	WORD $0x4ebf1d1f
	WORD $0x6e268e28
	WORD $0x4eb51f75
	WORD $0x4eba1f9a
	WORD $0x4eb71fb7
	WORD $0x4eb61fd6
	WORD $0x4eb91d39
	WORD $0x6e278e94
	WORD $0x4eaa1d1b
	WORD $0x4eb81ff8
	WORD $0x6e278e73
	WORD $0x4eb51f55
	WORD $0x6e278e52
	WORD $0x6e278e31
	WORD $0x4eb61f36
	WORD $0x4eb71f77
	WORD $0x4eb81e94
	WORD $0x4eb51e73
	WORD $0x4eb61e52
	WORD $0x4eb71e31
	WORD $0x4f0f5694
	WORD $0x4f0f5673
	WORD $0x4f0f5652
	WORD $0x4f0f5631
	WORD $0x4e20aa94
	WORD $0x4e20aa73
	WORD $0x4e20aa52
	WORD $0x4e20aa31
	WORD $0x4e301e94
	WORD $0x4e301e73
	WORD $0x4e301e52
	WORD $0x4e301e31
	WORD $0x6e144295
	WORD $0x6e134276
	WORD $0x6e124257
	WORD $0x6e114238
	WORD $0x4e153a94
	WORD $0x4e163a73
	WORD $0x4e173a52
	WORD $0x4e183a31
	WORD $0x4e71ba94
	WORD $0x4e71ba73
	WORD $0x4e71ba52
	WORD $0x4e71ba31
	WORD $0x7d000934
	WORD $0x7d000533
	WORD $0x7d000132
	WORD $0x7d000d31
	WORD $0x91002129
	WORD $0x54fff42d
	WORD $0xeb03015f
	WORD $0x540003ca
	WORD $0xd370dc49
	WORD $0xd368bc4b
	WORD $0xd3609c4c
	WORD $0x53187c4d
	WORD $0x53105c4e
	WORD $0x53083c4f
	WORD $0x14000008
	WORD $0x1ac42231
	WORD $0x38706804
	WORD $0x9100054a
	WORD $0xeb0a007f
	WORD $0x2a110091
	WORD $0x38306811
	WORD $0x54000200
	WORD $0x386a6830
	WORD $0x6b22021f
	WORD $0x7a4f1204
	WORD $0x7a4e1204
	WORD $0x7a4d1204
	WORD $0x7a4c1204
	WORD $0x7a4b1204
	WORD $0x7a491204
	WORD $0xfa501104
	WORD $0xd343fd50
	WORD $0x1a9f17f1
	WORD $0xf2400944
	WORD $0x54fffda1
	WORD $0x3830681f
	WORD $0x17ffffeb
	WORD $0x6d4223e9
	WORD $0x6d412beb
	WORD $0x6cc333ed
	WORD $0xd65f03c0
	WORD $0x00000000
	WORD $0x00000000
	WORD $0x00000000
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x00000002
	WORD $0x00000000
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x00000006
	WORD $0x00000000
	WORD $0x00000003
	WORD $0x00000000
	WORD $0x00000006
	WORD $0x00000000
	WORD $0xcfcfcfcf
	WORD $0xcfcfcfcf
	WORD $0x33333333
	WORD $0x33333333
	WORD $0x67676767
	WORD $0x67676767
	WORD $0x19191919
	WORD $0x19191919
	WORD $0xffffff00
	WORD $0xffffff01
	WORD $0xffffff02
	WORD $0xffffff03
	WORD $0xffffff04
	WORD $0xffffff05
	WORD $0xffffff06
	WORD $0xffffff07
	WORD $0xffffff08
	WORD $0xffffff09
	WORD $0xffffff0a
	WORD $0xffffff0b
	WORD $0xffffff0c
	WORD $0xffffff0d
	WORD $0xffffff0e
	WORD $0xffffff0f
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0xffffff00
	WORD $0xffffff01
	WORD $0xffffff02
	WORD $0xffffff03
	WORD $0xffffff04
	WORD $0xffffff05
	WORD $0xffffff06
	WORD $0xffffff07
	WORD $0xffffff08
	WORD $0xffffff09
	WORD $0xffffff0a
	WORD $0xffffff0b
	WORD $0xffffff0c
	WORD $0xffffff0d
	WORD $0xffffff0e
	WORD $0xffffff0f
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x33323130
	WORD $0x37363534
	WORD $0x62613938
	WORD $0x66656463
	RET

// func hammingU8NEON(a []byte, b []byte) int
TEXT ·hammingU8NEON(SB), NOSPLIT|NOFRAME, $0-56
	MOVD $ret+48(FP), R0
	MOVD a_base+0(FP), R1
	MOVD b_base+24(FP), R2
	MOVD a_len+8(FP), R3
	WORD $0x6f00e400
	WORD $0x6f00e401
	WORD $0xf100407f
	WORD $0x6f00e403
	WORD $0x6f00e402
	WORD $0x6f00e404
	WORD $0x5400016a
	WORD $0xaa1f03e8
	WORD $0xeb0303ff
	WORD $0x5400034b
	WORD $0x6f00e405
	WORD $0x6f00e406
	WORD $0x6f00e407
	WORD $0xb2400109
	WORD $0xeb03013f
	WORD $0x5400042b
	WORD $0x14000026
	WORD $0xaa1f03e8
	WORD $0x3ce86825
	WORD $0x3ce86846
	WORD $0x91008109
	WORD $0xeb03013f
	WORD $0x91004108
	WORD $0x6e251cc5
	WORD $0x4e2058a5
	WORD $0x6f08a4a6
	WORD $0x2f08a4a5
	WORD $0x6e661084
	WORD $0x2e661042
	WORD $0x6e651063
	WORD $0x2e651021
	WORD $0x54fffe6d
	WORD $0x927ce868
	WORD $0xeb03011f
	WORD $0x54fffd0a
	WORD $0x38686829
	WORD $0x3868684a
	WORD $0x6f00e405
	WORD $0x4a090149
	WORD $0x1e270120
	WORD $0x0e205806
	WORD $0x6f00e400
	WORD $0x6e0404c0
	WORD $0x6f00e406
	WORD $0x6f00e407
	WORD $0xb2400109
	WORD $0xeb03013f
	WORD $0x540000ea
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x1e270130
	WORD $0x0e205a10
	WORD $0x6e0c0600
	WORD $0xb27f0109
	WORD $0xeb03013f
	WORD $0x540007cb
	WORD $0xb2400509
	WORD $0xeb03013f
	WORD $0x5400088b
	WORD $0xb27e0109
	WORD $0xeb03013f
	WORD $0x5400094b
	WORD $0x528000a9
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54000a0b
	WORD $0xb27f0509
	WORD $0xeb03013f
	WORD $0x54000acb
	WORD $0xb2400909
	WORD $0xeb03013f
	WORD $0x54000b8b
	WORD $0xb27d0109
	WORD $0xeb03013f
	WORD $0x54000c4b
	WORD $0x52800129
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54000d0b
	WORD $0x52800149
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54000dcb
	WORD $0x52800169
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54000e8b
	WORD $0xb27e0509
	WORD $0xeb03013f
	WORD $0x54000f4b
	WORD $0x528001a9
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x5400100b
	WORD $0xb27f0909
	WORD $0xeb03013f
	WORD $0x540010cb
	WORD $0xb2400d08
	WORD $0xeb03011f
	WORD $0x540000ea
	WORD $0x38686829
	WORD $0x38686848
	WORD $0x4a090108
	WORD $0x1e270110
	WORD $0x0e205a10
	WORD $0x6e1c0607
	WORD $0x4ea384a3
	WORD $0x4ea484e4
	WORD $0x4ea18400
	WORD $0x4ea284c1
	WORD $0x4ea18400
	WORD $0x4ea48461
	WORD $0x4ea18400
	WORD $0x4eb1b800
	WORD $0x1e260008
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x1e270130
	WORD $0x0e205a10
	WORD $0x6e140600
	WORD $0xb2400509
	WORD $0xeb03013f
	WORD $0x54fff7ca
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x1e270130
	WORD $0x0e205a10
	WORD $0x6e1c0600
	WORD $0xb27e0109
	WORD $0xeb03013f
	WORD $0x54fff70a
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x1e270130
	WORD $0x0e205a10
	WORD $0x6e040605
	WORD $0x528000a9
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54fff64a
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x1e270130
	WORD $0x0e205a10
	WORD $0x6e0c0605
	WORD $0xb27f0509
	WORD $0xeb03013f
	WORD $0x54fff58a
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x1e270130
	WORD $0x0e205a10
	WORD $0x6e140605
	WORD $0xb2400909
	WORD $0xeb03013f
	WORD $0x54fff4ca
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x1e270130
	WORD $0x0e205a10
	WORD $0x6e1c0605
	WORD $0xb27d0109
	WORD $0xeb03013f
	WORD $0x54fff40a
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x1e270130
	WORD $0x0e205a10
	WORD $0x6e040606
	WORD $0x52800129
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54fff34a
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x1e270130
	WORD $0x0e205a10
	WORD $0x6e0c0606
	WORD $0x52800149
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54fff28a
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x1e270130
	WORD $0x0e205a10
	WORD $0x6e140606
	WORD $0x52800169
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54fff1ca
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x1e270130
	WORD $0x0e205a10
	WORD $0x6e1c0606
	WORD $0xb27e0509
	WORD $0xeb03013f
	WORD $0x54fff10a
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x1e270130
	WORD $0x0e205a10
	WORD $0x6e040607
	WORD $0x528001a9
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54fff04a
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x1e270130
	WORD $0x0e205a10
	WORD $0x6e0c0607
	WORD $0xb27f0909
	WORD $0xeb03013f
	WORD $0x54ffef8a
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x1e270130
	WORD $0x0e205a10
	WORD $0x6e140607
	WORD $0xb2400d08
	WORD $0xeb03011f
	WORD $0x54ffeecb
	WORD $0x17ffff7b
	RET

// func hammingU64NEON(a []uint64, b []uint64) int
TEXT ·hammingU64NEON(SB), NOSPLIT|NOFRAME, $0-56
	MOVD $ret+48(FP), R0
	MOVD a_base+0(FP), R1
	MOVD b_base+24(FP), R2
	MOVD a_len+8(FP), R3
	WORD $0x6f00e401
	WORD $0x6f00e400
	WORD $0xf100407f
	WORD $0x6f00e403
	WORD $0x6f00e402
	WORD $0x6f00e404
	WORD $0x5400016a
	WORD $0xaa1f03e8
	WORD $0xeb0303ff
	WORD $0x5400092b
	WORD $0x6f00e405
	WORD $0x6f00e406
	WORD $0x6f00e407
	WORD $0xb2400109
	WORD $0xeb03013f
	WORD $0x54000a2b
	WORD $0x14000057
	WORD $0x52800208
	WORD $0xaa0203e9
	WORD $0xaa0103ea
	WORD $0xad431546
	WORD $0x91004108
	WORD $0xad431d30
	WORD $0xeb03011f
	WORD $0xad424552
	WORD $0xad425536
	WORD $0x6e261e06
	WORD $0x6e251ce5
	WORD $0xad411d50
	WORD $0xacc44d54
	WORD $0xad415d38
	WORD $0x6e321ed2
	WORD $0xacc4653a
	WORD $0x6e311eb1
	WORD $0x4e2058a5
	WORD $0x4e2058c6
	WORD $0x6e271ee7
	WORD $0x6e301f10
	WORD $0x4e205a52
	WORD $0x6e341f54
	WORD $0x6e331f33
	WORD $0x4e205a31
	WORD $0x4e2058e7
	WORD $0x4e205a10
	WORD $0x6e2028a5
	WORD $0x4e205a73
	WORD $0x4e205a94
	WORD $0x6e2028c6
	WORD $0x6e202a52
	WORD $0x6e202a31
	WORD $0x6e6028a5
	WORD $0x6e2028e7
	WORD $0x6e202a10
	WORD $0x6e6028c6
	WORD $0x6e202a73
	WORD $0x6e202a94
	WORD $0x6e602a52
	WORD $0x6e602a31
	WORD $0x6ea028a5
	WORD $0x6e6028e7
	WORD $0x6e602a10
	WORD $0x6ea028c6
	WORD $0x6e602a73
	WORD $0x6e602a94
	WORD $0x6ea02a52
	WORD $0x6ea02a31
	WORD $0x6ea028e7
	WORD $0x6ea02a10
	WORD $0x4e8518c5
	WORD $0x6ea02a73
	WORD $0x6ea02a94
	WORD $0x4e911a51
	WORD $0x4e871a07
	WORD $0x4e931a86
	WORD $0x4ea58484
	WORD $0x4eb18442
	WORD $0x4ea78463
	WORD $0x4ea68400
	WORD $0x54fff8cd
	WORD $0x927ce868
	WORD $0xeb03011f
	WORD $0x54fff72a
	WORD $0xf8687829
	WORD $0xf868784a
	WORD $0xca090149
	WORD $0x9e670121
	WORD $0x0e205825
	WORD $0x6f00e401
	WORD $0x0e31b8a6
	WORD $0x6f00e405
	WORD $0x6e0404c1
	WORD $0x6f00e406
	WORD $0x6f00e407
	WORD $0xb2400109
	WORD $0xeb03013f
	WORD $0x5400010a
	WORD $0xf869782a
	WORD $0xf8697849
	WORD $0xca0a0129
	WORD $0x9e670130
	WORD $0x0e205a10
	WORD $0x0e31ba10
	WORD $0x6e0c0601
	WORD $0xb27f0109
	WORD $0xeb03013f
	WORD $0x540007eb
	WORD $0xb2400509
	WORD $0xeb03013f
	WORD $0x540008cb
	WORD $0xb27e0109
	WORD $0xeb03013f
	WORD $0x540009ab
	WORD $0x528000a9
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54000a8b
	WORD $0xb27f0509
	WORD $0xeb03013f
	WORD $0x54000b6b
	WORD $0xb2400909
	WORD $0xeb03013f
	WORD $0x54000c4b
	WORD $0xb27d0109
	WORD $0xeb03013f
	WORD $0x54000d2b
	WORD $0x52800129
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54000e0b
	WORD $0x52800149
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54000eeb
	WORD $0x52800169
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54000fcb
	WORD $0xb27e0509
	WORD $0xeb03013f
	WORD $0x540010ab
	WORD $0x528001a9
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x5400118b
	WORD $0xb27f0909
	WORD $0xeb03013f
	WORD $0x5400126b
	WORD $0xb2400d08
	WORD $0xeb03011f
	WORD $0x5400010a
	WORD $0xf8687829
	WORD $0xf8687848
	WORD $0xca090108
	WORD $0x9e670110
	WORD $0x0e205a10
	WORD $0x0e31ba10
	WORD $0x6e1c0607
	WORD $0x4ea384a3
	WORD $0x4ea484e4
	WORD $0x4ea08420
	WORD $0x4ea284c1
	WORD $0x4ea18400
	WORD $0x4ea48461
	WORD $0x4ea18400
	WORD $0x4eb1b800
	WORD $0x1e260008
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0xf869782a
	WORD $0xf8697849
	WORD $0xca0a0129
	WORD $0x9e670130
	WORD $0x0e205a10
	WORD $0x0e31ba10
	WORD $0x6e140601
	WORD $0xb2400509
	WORD $0xeb03013f
	WORD $0x54fff78a
	WORD $0xf869782a
	WORD $0xf8697849
	WORD $0xca0a0129
	WORD $0x9e670130
	WORD $0x0e205a10
	WORD $0x0e31ba10
	WORD $0x6e1c0601
	WORD $0xb27e0109
	WORD $0xeb03013f
	WORD $0x54fff6aa
	WORD $0xf869782a
	WORD $0xf8697849
	WORD $0xca0a0129
	WORD $0x9e670130
	WORD $0x0e205a10
	WORD $0x0e31ba10
	WORD $0x6e040605
	WORD $0x528000a9
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54fff5ca
	WORD $0xf869782a
	WORD $0xf8697849
	WORD $0xca0a0129
	WORD $0x9e670130
	WORD $0x0e205a10
	WORD $0x0e31ba10
	WORD $0x6e0c0605
	WORD $0xb27f0509
	WORD $0xeb03013f
	WORD $0x54fff4ea
	WORD $0xf869782a
	WORD $0xf8697849
	WORD $0xca0a0129
	WORD $0x9e670130
	WORD $0x0e205a10
	WORD $0x0e31ba10
	WORD $0x6e140605
	WORD $0xb2400909
	WORD $0xeb03013f
	WORD $0x54fff40a
	WORD $0xf869782a
	WORD $0xf8697849
	WORD $0xca0a0129
	WORD $0x9e670130
	WORD $0x0e205a10
	WORD $0x0e31ba10
	WORD $0x6e1c0605
	WORD $0xb27d0109
	WORD $0xeb03013f
	WORD $0x54fff32a
	WORD $0xf869782a
	WORD $0xf8697849
	WORD $0xca0a0129
	WORD $0x9e670130
	WORD $0x0e205a10
	WORD $0x0e31ba10
	WORD $0x6e040606
	WORD $0x52800129
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54fff24a
	WORD $0xf869782a
	WORD $0xf8697849
	WORD $0xca0a0129
	WORD $0x9e670130
	WORD $0x0e205a10
	WORD $0x0e31ba10
	WORD $0x6e0c0606
	WORD $0x52800149
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54fff16a
	WORD $0xf869782a
	WORD $0xf8697849
	WORD $0xca0a0129
	WORD $0x9e670130
	WORD $0x0e205a10
	WORD $0x0e31ba10
	WORD $0x6e140606
	WORD $0x52800169
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54fff08a
	WORD $0xf869782a
	WORD $0xf8697849
	WORD $0xca0a0129
	WORD $0x9e670130
	WORD $0x0e205a10
	WORD $0x0e31ba10
	WORD $0x6e1c0606
	WORD $0xb27e0509
	WORD $0xeb03013f
	WORD $0x54ffefaa
	WORD $0xf869782a
	WORD $0xf8697849
	WORD $0xca0a0129
	WORD $0x9e670130
	WORD $0x0e205a10
	WORD $0x0e31ba10
	WORD $0x6e040607
	WORD $0x528001a9
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54ffeeca
	WORD $0xf869782a
	WORD $0xf8697849
	WORD $0xca0a0129
	WORD $0x9e670130
	WORD $0x0e205a10
	WORD $0x0e31ba10
	WORD $0x6e0c0607
	WORD $0xb27f0909
	WORD $0xeb03013f
	WORD $0x54ffedea
	WORD $0xf869782a
	WORD $0xf8697849
	WORD $0xca0a0129
	WORD $0x9e670130
	WORD $0x0e205a10
	WORD $0x0e31ba10
	WORD $0x6e140607
	WORD $0xb2400d08
	WORD $0xeb03011f
	WORD $0x54ffed0b
	WORD $0x17ffff6e
	RET

// func grayscaleU8NEON(dst []byte, r []byte, g []byte, b []byte)
TEXT ·grayscaleU8NEON(SB), NOSPLIT|NOFRAME, $0-96
	MOVD dst_base+0(FP), R0
	MOVD r_base+24(FP), R1
	MOVD g_base+48(FP), R2
	MOVD b_base+72(FP), R3
	MOVD dst_len+8(FP), R4
	// Constant pool appended below the body; the displacements
	// above were patched to reach it and the aligned loads made
	// unaligned, since nothing promises alignment inside a TEXT.
	WORD $0xf100049f
	WORD $0x54000dcb
	WORD $0xf100109f
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x14000057
	WORD $0xf100409f
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x14000035
	WORD $0x5289916a
	WORD $0x5292c8cb
	WORD $0x5283a5ec
	WORD $0x4f042401
	WORD $0x4e020d40
	WORD $0x4e020d62
	WORD $0x4e020d83
	WORD $0x10000bea
	WORD $0x927e0489
	WORD $0x3dc02944
	WORD $0x927ce888
	WORD $0xaa0103ea
	WORD $0xaa0203eb
	WORD $0xaa0303ec
	WORD $0xaa0003ed
	WORD $0x927ce88e
	WORD $0x3cc10545
	WORD $0xf10041ce
	WORD $0x3cc10567
	WORD $0x6f08a4a6
	WORD $0x2f08a4a5
	WORD $0x6f08a4f1
	WORD $0x2f08a4e7
	WORD $0x3cc10593
	WORD $0x6e60c0d0
	WORD $0x2e60c0c6
	WORD $0x6e60c0b2
	WORD $0x2e60c0a5
	WORD $0x6f08a674
	WORD $0x6e628230
	WORD $0x2e628226
	WORD $0x6e6280f2
	WORD $0x2f08a671
	WORD $0x2e6280e5
	WORD $0x6e638290
	WORD $0x2e638286
	WORD $0x6e638232
	WORD $0x2e638225
	WORD $0x4ea18607
	WORD $0x4ea184c6
	WORD $0x4ea18650
	WORD $0x4ea184a5
	WORD $0x6f3004f4
	WORD $0x6f3004d3
	WORD $0x6f300612
	WORD $0x6f3004b1
	WORD $0x4e046225
	WORD $0x3c8105a5
	WORD $0x54fffc01
	WORD $0xeb08009f
	WORD $0x54000660
	WORD $0xb40003e9
	WORD $0x52899169
	WORD $0x5292c8ca
	WORD $0x5283a5eb
	WORD $0x0e020d20
	WORD $0x0e020d41
	WORD $0x0e020d62
	WORD $0x6f00e403
	WORD $0xaa0803ed
	WORD $0x927ef088
	WORD $0xcb0801a9
	WORD $0x8b0d000a
	WORD $0x8b0d006b
	WORD $0x8b0d004c
	WORD $0x8b0d002d
	WORD $0xbc4045a4
	WORD $0xb1001129
	WORD $0xbc404585
	WORD $0x2f08a484
	WORD $0xbc404566
	WORD $0x2f08a4a5
	WORD $0x2e60c084
	WORD $0x2e6180a4
	WORD $0x2f08a4c5
	WORD $0x2e6280a4
	WORD $0x2e634084
	WORD $0x0e001884
	WORD $0x0d9f8144
	WORD $0x54fffe61
	WORD $0xeb08009f
	WORD $0x54000280
	WORD $0xcb080089
	WORD $0x8b08000a
	WORD $0x8b08006b
	WORD $0x8b08004c
	WORD $0x8b080028
	WORD $0x5289916d
	WORD $0x5292c8ce
	WORD $0x5283a5ef
	WORD $0x38401590
	WORD $0xf1000529
	WORD $0x38401511
	WORD $0x1b0e7e10
	WORD $0x38401560
	WORD $0x1b0d4230
	WORD $0x1b0f4010
	WORD $0x11402210
	WORD $0x53107e10
	WORD $0x38001550
	WORD $0x54fffec1
	WORD $0xd65f03c0
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x00000002
	WORD $0x00000000
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x00000006
	WORD $0x00000000
	WORD $0x00000003
	WORD $0x00000000
	WORD $0x00000006
	WORD $0x00000000
	WORD $0xcfcfcfcf
	WORD $0xcfcfcfcf
	WORD $0x33333333
	WORD $0x33333333
	WORD $0x67676767
	WORD $0x67676767
	WORD $0x19191919
	WORD $0x19191919
	WORD $0xffffff00
	WORD $0xffffff01
	WORD $0xffffff02
	WORD $0xffffff03
	WORD $0xffffff04
	WORD $0xffffff05
	WORD $0xffffff06
	WORD $0xffffff07
	WORD $0xffffff08
	WORD $0xffffff09
	WORD $0xffffff0a
	WORD $0xffffff0b
	WORD $0xffffff0c
	WORD $0xffffff0d
	WORD $0xffffff0e
	WORD $0xffffff0f
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0xffffff00
	WORD $0xffffff01
	WORD $0xffffff02
	WORD $0xffffff03
	WORD $0xffffff04
	WORD $0xffffff05
	WORD $0xffffff06
	WORD $0xffffff07
	WORD $0xffffff08
	WORD $0xffffff09
	WORD $0xffffff0a
	WORD $0xffffff0b
	WORD $0xffffff0c
	WORD $0xffffff0d
	WORD $0xffffff0e
	WORD $0xffffff0f
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x33323130
	WORD $0x37363534
	WORD $0x62613938
	WORD $0x66656463
	RET

// func rgbToUVU8NEON(u []byte, v []byte, r []byte, g []byte, b []byte)
TEXT ·rgbToUVU8NEON(SB), NOSPLIT|NOFRAME, $0-120
	MOVD u_base+0(FP), R0
	MOVD v_base+24(FP), R1
	MOVD r_base+48(FP), R2
	MOVD g_base+72(FP), R3
	MOVD b_base+96(FP), R4
	MOVD u_len+8(FP), R5
	// Constant pool appended below the body; the displacements
	// above were patched to reach it and the aligned loads made
	// unaligned, since nothing promises alignment inside a TEXT.
	WORD $0xf10004bf
	WORD $0x54001b0b
	WORD $0xfc1c0fee
	WORD $0xf10010bf
	WORD $0x6d0133ed
	WORD $0x6d022beb
	WORD $0x6d0323e9
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x140000ac
	WORD $0xf10040bf
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x14000077
	WORD $0x100019cb
	WORD $0x100019ac
	WORD $0x1000198a
	WORD $0x3dc03161
	WORD $0x529a99ab
	WORD $0x3dc03582
	WORD $0x5295666c
	WORD $0x0e020d63
	WORD $0x52929a2b
	WORD $0x0e020d84
	WORD $0x5290000c
	WORD $0x0e020d65
	WORD $0x529d65eb
	WORD $0x72a0100c
	WORD $0x6f00e627
	WORD $0x0e020d66
	WORD $0x3dc02d40
	WORD $0x100017aa
	WORD $0x1000178b
	WORD $0x4e040d91
	WORD $0x3dc03950
	WORD $0x3dc03d72
	WORD $0x927e04a9
	WORD $0x927ce8a8
	WORD $0xaa0203ea
	WORD $0xaa0303eb
	WORD $0xaa0403ec
	WORD $0xaa0003ed
	WORD $0xaa0103ee
	WORD $0x927ce8af
	WORD $0x3cc10554
	WORD $0xf10041ef
	WORD $0x3cc10575
	WORD $0x4e10029b
	WORD $0x4e02029a
	WORD $0x4e000293
	WORD $0x4e1002b9
	WORD $0x4e0202bc
	WORD $0x4e010296
	WORD $0x4e0002b4
	WORD $0x3cc10597
	WORD $0x4e0102b5
	WORD $0x4e1002fe
	WORD $0x0e612b68
	WORD $0x4e0202fd
	WORD $0x0e612b2c
	WORD $0x0e612b49
	WORD $0x0e612b9c
	WORD $0x4f2f577b
	WORD $0x4e0102ff
	WORD $0x0e612aca
	WORD $0x0e612ab5
	WORD $0x4f2f575a
	WORD $0x4e0002f8
	WORD $0x0e612a6b
	WORD $0x0e612a94
	WORD $0x4f2f56d6
	WORD $0x0e612bd9
	WORD $0x0e63c108
	WORD $0x4f2f5673
	WORD $0x0e65819b
	WORD $0x0e612bb7
	WORD $0x0e63c129
	WORD $0x0e65839a
	WORD $0x0e612bed
	WORD $0x0e63c14a
	WORD $0x4f2f57de
	WORD $0x0e6582b6
	WORD $0x0e612b0e
	WORD $0x0e63c16b
	WORD $0x4f2f57bd
	WORD $0x0e658293
	WORD $0x4f2f57ff
	WORD $0x0e648188
	WORD $0x0e66833b
	WORD $0x4f2f5718
	WORD $0x0e648389
	WORD $0x4eb187dc
	WORD $0x0e6682fa
	WORD $0x0e6482aa
	WORD $0x4eb187b5
	WORD $0x0e6681b6
	WORD $0x0e64828b
	WORD $0x4eb187f4
	WORD $0x0e6681d3
	WORD $0x4ebc8517
	WORD $0x4eb18718
	WORD $0x4eb18779
	WORD $0x4eb58535
	WORD $0x4eb1875a
	WORD $0x4eb48554
	WORD $0x4eb186d6
	WORD $0x4eb88578
	WORD $0x6f3006f7
	WORD $0x4eb18673
	WORD $0x6f300739
	WORD $0x6f3006b5
	WORD $0x6f30075a
	WORD $0x6f300694
	WORD $0x6f3006d6
	WORD $0x6f300718
	WORD $0x6ea76efe
	WORD $0x6f300673
	WORD $0x6ea76f2b
	WORD $0x6ea76ebd
	WORD $0x6ea76f4a
	WORD $0x6ea76e9c
	WORD $0x6ea76ec9
	WORD $0x6ea76f1b
	WORD $0x6ea76e68
	WORD $0x4e126373
	WORD $0x4e126114
	WORD $0x3c8105b3
	WORD $0x3c8105d4
	WORD $0x54fff581
	WORD $0xeb0800bf
	WORD $0x54000a60
	WORD $0xb4000649
	WORD $0x529a99a9
	WORD $0x5295666a
	WORD $0x5290000b
	WORD $0x52929a2c
	WORD $0x0e020d20
	WORD $0x529d65e9
	WORD $0x72a0100b
	WORD $0x0e020d41
	WORD $0x0e020d82
	WORD $0x0e020d23
	WORD $0x6f00e624
	WORD $0x4e040d65
	WORD $0xaa0803ee
	WORD $0x927ef0a8
	WORD $0xcb0801c9
	WORD $0x8b0e002a
	WORD $0x8b0e000b
	WORD $0x8b0e008c
	WORD $0x8b0e006d
	WORD $0x8b0e004e
	WORD $0xbc4045c6
	WORD $0xb1001129
	WORD $0xbc4045a7
	WORD $0x2f08a4c6
	WORD $0xbc404590
	WORD $0x2f08a4e7
	WORD $0x2f08a610
	WORD $0x2f1fa4d1
	WORD $0x0e60c0c6
	WORD $0x2f1fa612
	WORD $0x0e6280f1
	WORD $0x0e6180e6
	WORD $0x4ea58647
	WORD $0x0e638211
	WORD $0x4ea784c6
	WORD $0x4ea58627
	WORD $0x6f3004c6
	WORD $0x6f3004e7
	WORD $0x6ea46cc6
	WORD $0x6ea46ce7
	WORD $0x0e6128c6
	WORD $0x0e6128e7
	WORD $0x0e0018c6
	WORD $0x0e0018e7
	WORD $0x0d9f8166
	WORD $0x0d9f8147
	WORD $0x54fffcc1
	WORD $0xeb0800bf
	WORD $0x54000420
	WORD $0x52900009
	WORD $0xcb0800aa
	WORD $0x8b08002b
	WORD $0x72a01009
	WORD $0x8b08000c
	WORD $0x8b08008d
	WORD $0x8b08006e
	WORD $0x8b080048
	WORD $0x1285664f
	WORD $0x128a9990
	WORD $0x128d65d1
	WORD $0x12829a00
	WORD $0x52801fe1
	WORD $0x38401502
	WORD $0x384015c3
	WORD $0x1b0f2444
	WORD $0x384015a5
	WORD $0x1b101064
	WORD $0x0b023d22
	WORD $0x1b110862
	WORD $0x1b0008a2
	WORD $0x0b053c83
	WORD $0x53107c63
	WORD $0x53107c42
	WORD $0x7103fc7f
	WORD $0x1a813063
	WORD $0x7103fc5f
	WORD $0x1a813042
	WORD $0xf100054a
	WORD $0x38001583
	WORD $0x38001562
	WORD $0x54fffdc1
	WORD $0x6d4323e9
	WORD $0x6d422beb
	WORD $0x6d4133ed
	WORD $0xfc4407ee
	WORD $0xd65f03c0
	WORD $0x00000000
	WORD $0x00000000
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x00000002
	WORD $0x00000000
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x00000006
	WORD $0x00000000
	WORD $0x00000003
	WORD $0x00000000
	WORD $0x00000006
	WORD $0x00000000
	WORD $0xcfcfcfcf
	WORD $0xcfcfcfcf
	WORD $0x33333333
	WORD $0x33333333
	WORD $0x67676767
	WORD $0x67676767
	WORD $0x19191919
	WORD $0x19191919
	WORD $0xffffff00
	WORD $0xffffff01
	WORD $0xffffff02
	WORD $0xffffff03
	WORD $0xffffff04
	WORD $0xffffff05
	WORD $0xffffff06
	WORD $0xffffff07
	WORD $0xffffff08
	WORD $0xffffff09
	WORD $0xffffff0a
	WORD $0xffffff0b
	WORD $0xffffff0c
	WORD $0xffffff0d
	WORD $0xffffff0e
	WORD $0xffffff0f
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0xffffff00
	WORD $0xffffff01
	WORD $0xffffff02
	WORD $0xffffff03
	WORD $0xffffff04
	WORD $0xffffff05
	WORD $0xffffff06
	WORD $0xffffff07
	WORD $0xffffff08
	WORD $0xffffff09
	WORD $0xffffff0a
	WORD $0xffffff0b
	WORD $0xffffff0c
	WORD $0xffffff0d
	WORD $0xffffff0e
	WORD $0xffffff0f
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x33323130
	WORD $0x37363534
	WORD $0x62613938
	WORD $0x66656463
	RET

// func isASCIINEON(b []byte) bool
TEXT ·isASCIINEON(SB), NOSPLIT|NOFRAME, $0-25
	MOVD $ret+24(FP), R0
	MOVD b_base+0(FP), R1
	MOVD b_len+8(FP), R2
	WORD $0x4f04e400
	WORD $0xaa1f03e8
	WORD $0x91020109
	WORD $0xeb02013f
	WORD $0x540002ac
	WORD $0x8b080028
	WORD $0xad400901
	WORD $0xad411103
	WORD $0xad421506
	WORD $0xad434107
	WORD $0xaa0903e8
	WORD $0x4ea21c82
	WORD $0x4ea11c61
	WORD $0x4ea51e03
	WORD $0x4ea61ce4
	WORD $0x4ea21c62
	WORD $0x4ea11c81
	WORD $0x4ea21c21
	WORD $0x4e201c21
	WORD $0x6e014022
	WORD $0x0ea21c21
	WORD $0x9e66002a
	WORD $0xb4fffd8a
	WORD $0x3900001f
	WORD $0xd65f03c0
	WORD $0x6f00e400
	WORD $0x6f00e401
	WORD $0x91008109
	WORD $0xeb02013f
	WORD $0x5400006d
	WORD $0x6f00e402
	WORD $0x1400000d
	WORD $0x4f04e403
	WORD $0x6f00e402
	WORD $0x8b080029
	WORD $0xad401125
	WORD $0x91010109
	WORD $0xeb02013f
	WORD $0x91008108
	WORD $0x4e231ca5
	WORD $0x4e231c84
	WORD $0x4ea21c82
	WORD $0x4ea11ca1
	WORD $0x54fffeed
	WORD $0x6f00e403
	WORD $0xeb02011f
	WORD $0x54000f6b
	WORD $0xb2400109
	WORD $0xeb02013f
	WORD $0x54000feb
	WORD $0xb27f0109
	WORD $0xeb02013f
	WORD $0x5400104b
	WORD $0xb2400509
	WORD $0xeb02013f
	WORD $0x540010ab
	WORD $0xb27e0109
	WORD $0xeb02013f
	WORD $0x5400110b
	WORD $0x528000a9
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x5400116b
	WORD $0xb27f0509
	WORD $0xeb02013f
	WORD $0x540011cb
	WORD $0xb2400909
	WORD $0xeb02013f
	WORD $0x5400122b
	WORD $0xb27d0109
	WORD $0xeb02013f
	WORD $0x5400128b
	WORD $0x52800129
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x540012eb
	WORD $0x52800149
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x5400134b
	WORD $0x52800169
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x540013ab
	WORD $0xb27e0509
	WORD $0xeb02013f
	WORD $0x5400140b
	WORD $0x528001a9
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x5400146b
	WORD $0xb27f0909
	WORD $0xeb02013f
	WORD $0x540014cb
	WORD $0xb2400d09
	WORD $0xeb02013f
	WORD $0x5400152b
	WORD $0xb27c0109
	WORD $0xeb02013f
	WORD $0x5400158b
	WORD $0x52800229
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x540015eb
	WORD $0x52800249
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x5400164b
	WORD $0x52800269
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x540016ab
	WORD $0x52800289
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x5400170b
	WORD $0x528002a9
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x5400176b
	WORD $0x528002c9
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x540017cb
	WORD $0x528002e9
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x5400182b
	WORD $0xb27d0509
	WORD $0xeb02013f
	WORD $0x5400188b
	WORD $0x52800329
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x540018eb
	WORD $0x52800349
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x5400194b
	WORD $0x52800369
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x540019ab
	WORD $0xb27e0909
	WORD $0xeb02013f
	WORD $0x54001a0b
	WORD $0x528003a9
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x54001a6b
	WORD $0xb27f0d09
	WORD $0xeb02013f
	WORD $0x54001acb
	WORD $0xb2401108
	WORD $0xeb02011f
	WORD $0x5400008a
	WORD $0x38686828
	WORD $0x12196108
	WORD $0x4e1f1d03
	WORD $0x4ea11c00
	WORD $0x4ea21c61
	WORD $0x4ea11c00
	WORD $0x6e004001
	WORD $0x0ea11c00
	WORD $0x9e660008
	WORD $0xf100011f
	WORD $0x1a9f17e8
	WORD $0x39000008
	WORD $0xd65f03c0
	WORD $0x6f00e400
	WORD $0x38686829
	WORD $0x12196129
	WORD $0x4e011d20
	WORD $0xb2400109
	WORD $0xeb02013f
	WORD $0x54fff06a
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e031d20
	WORD $0xb27f0109
	WORD $0xeb02013f
	WORD $0x54fff00a
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e051d20
	WORD $0xb2400509
	WORD $0xeb02013f
	WORD $0x54ffefaa
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e071d20
	WORD $0xb27e0109
	WORD $0xeb02013f
	WORD $0x54ffef4a
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e091d20
	WORD $0x528000a9
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x54ffeeea
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e0b1d20
	WORD $0xb27f0509
	WORD $0xeb02013f
	WORD $0x54ffee8a
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e0d1d20
	WORD $0xb2400909
	WORD $0xeb02013f
	WORD $0x54ffee2a
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e0f1d20
	WORD $0xb27d0109
	WORD $0xeb02013f
	WORD $0x54ffedca
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e111d20
	WORD $0x52800129
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x54ffed6a
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e131d20
	WORD $0x52800149
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x54ffed0a
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e151d20
	WORD $0x52800169
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x54ffecaa
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e171d20
	WORD $0xb27e0509
	WORD $0xeb02013f
	WORD $0x54ffec4a
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e191d20
	WORD $0x528001a9
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x54ffebea
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e1b1d20
	WORD $0xb27f0909
	WORD $0xeb02013f
	WORD $0x54ffeb8a
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e1d1d20
	WORD $0xb2400d09
	WORD $0xeb02013f
	WORD $0x54ffeb2a
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e1f1d20
	WORD $0xb27c0109
	WORD $0xeb02013f
	WORD $0x54ffeaca
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e011d23
	WORD $0x52800229
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x54ffea6a
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e031d23
	WORD $0x52800249
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x54ffea0a
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e051d23
	WORD $0x52800269
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x54ffe9aa
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e071d23
	WORD $0x52800289
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x54ffe94a
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e091d23
	WORD $0x528002a9
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x54ffe8ea
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e0b1d23
	WORD $0x528002c9
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x54ffe88a
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e0d1d23
	WORD $0x528002e9
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x54ffe82a
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e0f1d23
	WORD $0xb27d0509
	WORD $0xeb02013f
	WORD $0x54ffe7ca
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e111d23
	WORD $0x52800329
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x54ffe76a
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e131d23
	WORD $0x52800349
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x54ffe70a
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e151d23
	WORD $0x52800369
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x54ffe6aa
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e171d23
	WORD $0xb27e0909
	WORD $0xeb02013f
	WORD $0x54ffe64a
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e191d23
	WORD $0x528003a9
	WORD $0xaa090109
	WORD $0xeb02013f
	WORD $0x54ffe5ea
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e1b1d23
	WORD $0xb27f0d09
	WORD $0xeb02013f
	WORD $0x54ffe58a
	WORD $0x38696829
	WORD $0x12196129
	WORD $0x4e1d1d23
	WORD $0xb2401108
	WORD $0xeb02011f
	WORD $0x54ffe52b
	WORD $0x17ffff2b
	RET

// func validUTF8NEON(b []byte) bool
TEXT ·validUTF8NEON(SB), NOSPLIT|NOFRAME, $0-25
	MOVD $ret+24(FP), R0
	MOVD b_base+0(FP), R1
	MOVD b_len+8(FP), R2
	WORD $0xf100805f
	WORD $0x540000ca
	WORD $0xaa1f03e9
	WORD $0x52800028
	WORD $0x35000ee8
	WORD $0x39000008
	WORD $0xd65f03c0
	WORD $0x6dbc3bef
	WORD $0x6f00e400
	WORD $0x4f05e7e1
	WORD $0x52800408
	WORD $0x4f06e7e2
	WORD $0x4f07e5e3
	WORD $0xaa0103e9
	WORD $0x4f06e404
	WORD $0x4f07e7c5
	WORD $0x6d0133ed
	WORD $0x4f07e406
	WORD $0x4f05e407
	WORD $0x6d022beb
	WORD $0x4f07e5b0
	WORD $0x4f04e7f1
	WORD $0x6d0323e9
	WORD $0x4f07e612
	WORD $0x4f04e613
	WORD $0x4f07e694
	WORD $0x4f04e5f5
	WORD $0x6f00e419
	WORD $0x6f00e416
	WORD $0x6f00e417
	WORD $0x14000006
	WORD $0x4eb81f19
	WORD $0x91008108
	WORD $0x91008129
	WORD $0xeb02011f
	WORD $0x5400098c
	WORD $0x4ea01c1a
	WORD $0xad406120
	WORD $0x4eb91f1b
	WORD $0x4eba1c1a
	WORD $0x4ebb1f5a
	WORD $0x6e1a435b
	WORD $0x0ebb1f5a
	WORD $0x9e66034a
	WORD $0xf201c15f
	WORD $0x54fffe40
	WORD $0x6e00733b
	WORD $0x6e18701c
	WORD $0x6e18681d
	WORD $0x6e006b3e
	WORD $0x6e18781a
	WORD $0x6e007b39
	WORD $0x6e313709
	WORD $0x6e31340a
	WORD $0x6e35370d
	WORD $0x6e35340e
	WORD $0x6e22377b
	WORD $0x6e22379c
	WORD $0x6e2337bd
	WORD $0x6e2337de
	WORD $0x6e21375f
	WORD $0x6e213728
	WORD $0x6e308f2b
	WORD $0x6e308f4c
	WORD $0x6e268f2f
	WORD $0x4ebd1f9c
	WORD $0x6e3834fd
	WORD $0x4ebe1f7b
	WORD $0x6e268f5e
	WORD $0x4e2c1d29
	WORD $0x6e348f4c
	WORD $0x4ebc1ffc
	WORD $0x6e20367f
	WORD $0x4e3e1fbd
	WORD $0x6e328f3e
	WORD $0x4ebb1d1b
	WORD $0x4e2b1d48
	WORD $0x6e328f4a
	WORD $0x6e38366b
	WORD $0x4e2c1dac
	WORD $0x4e251f4d
	WORD $0x6e34375a
	WORD $0x4e3e1ffe
	WORD $0x6e348f3f
	WORD $0x4e2a1d6a
	WORD $0x6e2034eb
	WORD $0x4eac1f5a
	WORD $0x4e3f1ddf
	WORD $0x4e251f2e
	WORD $0x6e343739
	WORD $0x4e2f1d6b
	WORD $0x4ea91d49
	WORD $0x4ea81fde
	WORD $0x4e383488
	WORD $0x6e248daa
	WORD $0x4ebf1f39
	WORD $0x4e20349f
	WORD $0x6e248dcc
	WORD $0x4ea91f5a
	WORD $0x6e3c1d1c
	WORD $0x4eaa1fbd
	WORD $0x6e3b1ffb
	WORD $0x4eac1d7f
	WORD $0x4ebe1f39
	WORD $0x4ebd1f5a
	WORD $0x4eb71f97
	WORD $0x4ebf1f39
	WORD $0x4eb61f76
	WORD $0x4eb71f57
	WORD $0x4eb61f36
	WORD $0x17ffffb1
	WORD $0x4eb71ec0
	WORD $0x6d4323e9
	WORD $0x6d422beb
	WORD $0x927be449
	WORD $0x6d4133ed
	WORD $0x6e004001
	WORD $0x0ea11c00
	WORD $0x9e660008
	WORD $0xf100011f
	WORD $0x1a9f17e8
	WORD $0x6cc43bef
	WORD $0x34fff168
	WORD $0xb40001e9
	WORD $0xd1000528
	WORD $0x38e8682a
	WORD $0x3101015f
	WORD $0x540001ea
	WORD $0xd1000928
	WORD $0x38e8682a
	WORD $0x3101055f
	WORD $0x5400016c
	WORD $0xd1000d28
	WORD $0xd1001129
	WORD $0x38e8682a
	WORD $0x3101015f
	WORD $0x9a88b128
	WORD $0x14000005
	WORD $0xaa1f03e8
	WORD $0x14000003
	WORD $0x8b0d0108
	WORD $0x91000508
	WORD $0xeb02011f
	WORD $0x5400070a
	WORD $0x8b080029
	WORD $0x39c0012a
	WORD $0x36ffff6a
	WORD $0x12001d4b
	WORD $0x5103096a
	WORD $0x7100795f
	WORD $0x54000082
	WORD $0x5280002c
	WORD $0x5280002d
	WORD $0x1400000c
	WORD $0x121c0d6c
	WORD $0x7103819f
	WORD $0x54000081
	WORD $0x5280002c
	WORD $0x5280004d
	WORD $0x14000006
	WORD $0x5103c16c
	WORD $0x7100119f
	WORD $0x54000468
	WORD $0x2a1f03ec
	WORD $0x5280006d
	WORD $0x8b0801ae
	WORD $0xeb0201df
	WORD $0x540003ca
	WORD $0x39c0052e
	WORD $0x310105df
	WORD $0x5400036c
	WORD $0x7103817f
	WORD $0x12001dce
	WORD $0x54000061
	WORD $0x710281df
	WORD $0x540002c3
	WORD $0x7103b57f
	WORD $0x54000061
	WORD $0x71027ddf
	WORD $0x54000248
	WORD $0x7103c17f
	WORD $0x54000061
	WORD $0x710241df
	WORD $0x540001c3
	WORD $0x7103d17f
	WORD $0x54000061
	WORD $0x71023ddf
	WORD $0x54000148
	WORD $0x7100795f
	WORD $0x54fff9e3
	WORD $0x39c0092a
	WORD $0x3101015f
	WORD $0x540000aa
	WORD $0x3707f96c
	WORD $0x39c00d29
	WORD $0x3101013f
	WORD $0x54fff90b
	WORD $0x3900001f
	WORD $0xd65f03c0
	WORD $0x52800028
	WORD $0x39000008
	WORD $0xd65f03c0
	RET

// func indexNonASCIINEON(b []byte) int
TEXT ·indexNonASCIINEON(SB), NOSPLIT|NOFRAME, $0-32
	MOVD $ret+24(FP), R0
	MOVD b_base+0(FP), R1
	MOVD b_len+8(FP), R2
	WORD $0xf101005f
	WORD $0x5400016a
	WORD $0xaa1f03e9
	WORD $0xeb02013f
	WORD $0x540000ca
	WORD $0x38e96828
	WORD $0x37f80508
	WORD $0x91000529
	WORD $0xeb09005f
	WORD $0x54ffff81
	WORD $0xf9000002
	WORD $0xd65f03c0
	WORD $0xaa1f03e8
	WORD $0x5280080a
	WORD $0x14000005
	WORD $0x9101012a
	WORD $0x91010108
	WORD $0xeb02015f
	WORD $0x54fffe2c
	WORD $0x8b080029
	WORD $0xad410520
	WORD $0xad400d22
	WORD $0x4ea11c61
	WORD $0x4ea01c40
	WORD $0x4ea11c00
	WORD $0x6e004001
	WORD $0x0ea11c00
	WORD $0x9e660009
	WORD $0xaa498129
	WORD $0xd350fd2b
	WORD $0x2a0b0129
	WORD $0x2a49212b
	WORD $0xaa0a03e9
	WORD $0x363ffdcb
	WORD $0xaa1f03ea
	WORD $0xaa0103eb
	WORD $0x38e8696c
	WORD $0x37f800cc
	WORD $0xd100054a
	WORD $0x9100056b
	WORD $0xb101015f
	WORD $0x54ffff61
	WORD $0x17ffffe5
	WORD $0xcb0a0102
	WORD $0xf9000002
	WORD $0xd65f03c0
	WORD $0xf9000009
	WORD $0xd65f03c0
	RET

// func indexNonASCII16NEON(b []uint16) int
TEXT ·indexNonASCII16NEON(SB), NOSPLIT|NOFRAME, $0-32
	MOVD $ret+24(FP), R0
	MOVD b_base+0(FP), R1
	MOVD b_len+8(FP), R2
	WORD $0xf100805f
	WORD $0x5400018a
	WORD $0xaa1f03ea
	WORD $0xeb02015f
	WORD $0x540000ea
	WORD $0x786a7828
	WORD $0x7101fd1f
	WORD $0x54001028
	WORD $0x9100054a
	WORD $0xeb0a005f
	WORD $0x54ffff61
	WORD $0xf9000002
	WORD $0xd65f03c0
	WORD $0xaa1f03e8
	WORD $0x91008029
	WORD $0x14000007
	WORD $0x9101010b
	WORD $0x9100810a
	WORD $0x91010129
	WORD $0xeb02017f
	WORD $0xaa0a03e8
	WORD $0x54fffdcc
	WORD $0xad400520
	WORD $0xad7f0d22
	WORD $0x4ea11c61
	WORD $0x4ea01c40
	WORD $0x4ea11c00
	WORD $0x6e004001
	WORD $0x0ea11c00
	WORD $0x9e66000a
	WORD $0xd360fd4b
	WORD $0x2a0b014a
	WORD $0x2a4a414a
	WORD $0x12003d4a
	WORD $0x7102015f
	WORD $0x54fffda3
	WORD $0x785e012a
	WORD $0x7101fd5f
	WORD $0x54000c08
	WORD $0x785e212a
	WORD $0x7101fd5f
	WORD $0x54000c28
	WORD $0x785e412a
	WORD $0x7101fd5f
	WORD $0x54000c28
	WORD $0x785e612a
	WORD $0x7101fd5f
	WORD $0x54000c28
	WORD $0x785e812a
	WORD $0x7101fd5f
	WORD $0x54000c28
	WORD $0x785ea12a
	WORD $0x7101fd5f
	WORD $0x54000c28
	WORD $0x785ec12a
	WORD $0x7101fd5f
	WORD $0x54000c28
	WORD $0x785ee12a
	WORD $0x7101fd5f
	WORD $0x54000c28
	WORD $0x8b08042a
	WORD $0x7940214a
	WORD $0x7101fd5f
	WORD $0x54000c08
	WORD $0x785f212a
	WORD $0x7101fd5f
	WORD $0x54000c08
	WORD $0x785f412a
	WORD $0x7101fd5f
	WORD $0x54000c08
	WORD $0x785f612a
	WORD $0x7101fd5f
	WORD $0x54000c08
	WORD $0x785f812a
	WORD $0x7101fd5f
	WORD $0x54000c08
	WORD $0x785fa12a
	WORD $0x7101fd5f
	WORD $0x54000c08
	WORD $0x785fc12a
	WORD $0x7101fd5f
	WORD $0x54000c08
	WORD $0x785fe12a
	WORD $0x7101fd5f
	WORD $0x54000c08
	WORD $0x7940012a
	WORD $0x7101fd5f
	WORD $0x54000c08
	WORD $0x7940052a
	WORD $0x7101fd5f
	WORD $0x54000c08
	WORD $0x7940092a
	WORD $0x7101fd5f
	WORD $0x54000c08
	WORD $0x79400d2a
	WORD $0x7101fd5f
	WORD $0x54000c08
	WORD $0x7940112a
	WORD $0x7101fd5f
	WORD $0x54000c08
	WORD $0x7940152a
	WORD $0x7101fd5f
	WORD $0x54000c08
	WORD $0x7940192a
	WORD $0x7101fd5f
	WORD $0x54000c08
	WORD $0x79401d2a
	WORD $0x7101fd5f
	WORD $0x54000c08
	WORD $0x7940212a
	WORD $0x7101fd5f
	WORD $0x54000c08
	WORD $0x7940252a
	WORD $0x7101fd5f
	WORD $0x54000c08
	WORD $0x7940292a
	WORD $0x7101fd5f
	WORD $0x54000c08
	WORD $0x79402d2a
	WORD $0x7101fd5f
	WORD $0x54000c08
	WORD $0x7940312a
	WORD $0x7101fd5f
	WORD $0x54000c08
	WORD $0x7940352a
	WORD $0x7101fd5f
	WORD $0x54000c08
	WORD $0x7940392a
	WORD $0x7101fd5f
	WORD $0x54000c08
	WORD $0x79403d2a
	WORD $0x7101fd5f
	WORD $0x54fff189
	WORD $0x91007d08
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0xf900000a
	WORD $0xd65f03c0
	WORD $0x91000508
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91000908
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91000d08
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91001108
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91001508
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91001908
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91001d08
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91002108
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91002508
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91002908
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91002d08
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91003108
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91003508
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91003908
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91003d08
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91004108
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91004508
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91004908
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91004d08
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91005108
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91005508
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91005908
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91005d08
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91006108
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91006508
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91006908
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91006d08
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91007108
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91007508
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x91007908
	WORD $0xf9000008
	WORD $0xd65f03c0
	RET

// func widenU8U16NEON(dst []uint16, s []byte)
TEXT ·widenU8U16NEON(SB), NOSPLIT|NOFRAME, $0-48
	MOVD dst_base+0(FP), R0
	MOVD s_base+24(FP), R1
	MOVD dst_len+8(FP), R2
	WORD $0xf100045f
	WORD $0x540005cb
	WORD $0xf100205f
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x14000023
	WORD $0xf100805f
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x14000013
	WORD $0x927d0449
	WORD $0x927be448
	WORD $0x9100800a
	WORD $0x9100402b
	WORD $0x927be44c
	WORD $0xad7f8560
	WORD $0xf100818c
	WORD $0x9100816b
	WORD $0x6f08a402
	WORD $0x2f08a400
	WORD $0x6f08a423
	WORD $0x2f08a421
	WORD $0xad3f0940
	WORD $0xac820d41
	WORD $0x54fffee1
	WORD $0xeb08005f
	WORD $0x540002a0
	WORD $0xb40001a9
	WORD $0xaa0803eb
	WORD $0x927dec48
	WORD $0x8b0b0409
	WORD $0xcb08016a
	WORD $0x8b0b002b
	WORD $0xfc408560
	WORD $0xb100214a
	WORD $0x2f08a400
	WORD $0x3c810520
	WORD $0x54ffff81
	WORD $0xeb08005f
	WORD $0x54000100
	WORD $0x8b080409
	WORD $0xcb08004a
	WORD $0x8b080028
	WORD $0x3840150b
	WORD $0xf100054a
	WORD $0x7800252b
	WORD $0x54ffffa1
	WORD $0xd65f03c0
	RET

// func narrowU16U8NEON(dst []byte, s []uint16)
TEXT ·narrowU16U8NEON(SB), NOSPLIT|NOFRAME, $0-48
	MOVD dst_base+0(FP), R0
	MOVD s_base+24(FP), R1
	MOVD dst_len+8(FP), R2
	WORD $0xf100045f
	WORD $0x5400058b
	WORD $0xf100105f
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x14000021
	WORD $0xf100805f
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x14000011
	WORD $0x927e0849
	WORD $0x927be448
	WORD $0x9100400a
	WORD $0x9100802b
	WORD $0x927be44c
	WORD $0xad7f0161
	WORD $0xf100818c
	WORD $0xacc20963
	WORD $0x4e001820
	WORD $0x4e021861
	WORD $0xad3f8540
	WORD $0x9100814a
	WORD $0x54ffff21
	WORD $0xeb08005f
	WORD $0x540002a0
	WORD $0xb40001a9
	WORD $0xaa0803eb
	WORD $0x927ef048
	WORD $0x8b0b0429
	WORD $0xcb08016a
	WORD $0x8b0b000b
	WORD $0xfc408520
	WORD $0xb100114a
	WORD $0x0e001800
	WORD $0x0d9f8160
	WORD $0x54ffff81
	WORD $0xeb08005f
	WORD $0x54000100
	WORD $0x8b080429
	WORD $0xcb08004a
	WORD $0x8b080008
	WORD $0x3840252b
	WORD $0xf100054a
	WORD $0x3800150b
	WORD $0x54ffffa1
	WORD $0xd65f03c0
	RET

// func widenU8U32NEON(dst []uint32, s []byte)
TEXT ·widenU8U32NEON(SB), NOSPLIT|NOFRAME, $0-48
	MOVD dst_base+0(FP), R0
	MOVD s_base+24(FP), R1
	MOVD dst_len+8(FP), R2
	// Constant pool appended below the body; the displacements
	// above were patched to reach it and the aligned loads made
	// unaligned, since nothing promises alignment inside a TEXT.
	WORD $0xf100045f
	WORD $0x540007ab
	WORD $0xf100105f
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x14000032
	WORD $0xf100805f
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x14000021
	WORD $0x100006ca
	WORD $0x100006ab
	WORD $0x1000068c
	WORD $0x3dc01540
	WORD $0x1000064a
	WORD $0x3dc01961
	WORD $0x3dc01d82
	WORD $0x3dc02143
	WORD $0x927e0849
	WORD $0x927be448
	WORD $0x9101000a
	WORD $0x9100402b
	WORD $0x927be44c
	WORD $0xad7f9564
	WORD $0xf100818c
	WORD $0x9100816b
	WORD $0x4e030086
	WORD $0x4e020087
	WORD $0x4e010090
	WORD $0x4e000084
	WORD $0x4e0300b1
	WORD $0xad3f1947
	WORD $0x4e0200a6
	WORD $0x4e0100a7
	WORD $0x4e0000a5
	WORD $0xad3e4144
	WORD $0xad014546
	WORD $0xac841d45
	WORD $0x54fffe21
	WORD $0xeb08005f
	WORD $0x540002c0
	WORD $0xb40001c9
	WORD $0xaa0803eb
	WORD $0x927ef048
	WORD $0x8b0b0809
	WORD $0xcb08016a
	WORD $0x8b0b002b
	WORD $0xbc404560
	WORD $0xb100114a
	WORD $0x2f08a400
	WORD $0x2f10a400
	WORD $0x3c810520
	WORD $0x54ffff61
	WORD $0xeb08005f
	WORD $0x54000100
	WORD $0x8b080809
	WORD $0xcb08004a
	WORD $0x8b080028
	WORD $0x3840150b
	WORD $0xf100054a
	WORD $0xb800452b
	WORD $0x54ffffa1
	WORD $0xd65f03c0
	WORD $0x00000000
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x00000002
	WORD $0x00000000
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x00000006
	WORD $0x00000000
	WORD $0x00000003
	WORD $0x00000000
	WORD $0x00000006
	WORD $0x00000000
	WORD $0xcfcfcfcf
	WORD $0xcfcfcfcf
	WORD $0x33333333
	WORD $0x33333333
	WORD $0x67676767
	WORD $0x67676767
	WORD $0x19191919
	WORD $0x19191919
	WORD $0xffffff00
	WORD $0xffffff01
	WORD $0xffffff02
	WORD $0xffffff03
	WORD $0xffffff04
	WORD $0xffffff05
	WORD $0xffffff06
	WORD $0xffffff07
	WORD $0xffffff08
	WORD $0xffffff09
	WORD $0xffffff0a
	WORD $0xffffff0b
	WORD $0xffffff0c
	WORD $0xffffff0d
	WORD $0xffffff0e
	WORD $0xffffff0f
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0xffffff00
	WORD $0xffffff01
	WORD $0xffffff02
	WORD $0xffffff03
	WORD $0xffffff04
	WORD $0xffffff05
	WORD $0xffffff06
	WORD $0xffffff07
	WORD $0xffffff08
	WORD $0xffffff09
	WORD $0xffffff0a
	WORD $0xffffff0b
	WORD $0xffffff0c
	WORD $0xffffff0d
	WORD $0xffffff0e
	WORD $0xffffff0f
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x33323130
	WORD $0x37363534
	WORD $0x62613938
	WORD $0x66656463
	RET

// func narrowU32U8NEON(dst []byte, s []uint32)
TEXT ·narrowU32U8NEON(SB), NOSPLIT|NOFRAME, $0-48
	MOVD dst_base+0(FP), R0
	MOVD s_base+24(FP), R1
	MOVD dst_len+8(FP), R2
	// Constant pool appended below the body; the displacements
	// above were patched to reach it and the aligned loads made
	// unaligned, since nothing promises alignment inside a TEXT.
	WORD $0xf100045f
	WORD $0x5400064b
	WORD $0xf100105f
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x14000027
	WORD $0xf100805f
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x14000016
	WORD $0x1000054a
	WORD $0x927e0849
	WORD $0x927be448
	WORD $0x3dc02540
	WORD $0x9100400a
	WORD $0x9101002b
	WORD $0x927be44c
	WORD $0xad7e0961
	WORD $0xf100818c
	WORD $0xad404570
	WORD $0xad7f1163
	WORD $0xad414d72
	WORD $0x9102016b
	WORD $0x4e006021
	WORD $0x4e006202
	WORD $0xad3f8941
	WORD $0x9100814a
	WORD $0x54fffec1
	WORD $0xeb08005f
	WORD $0x540002c0
	WORD $0xb40001c9
	WORD $0xaa0803eb
	WORD $0x927ef048
	WORD $0x8b0b0829
	WORD $0xcb08016a
	WORD $0x8b0b000b
	WORD $0x3cc10520
	WORD $0xb100114a
	WORD $0x0e612800
	WORD $0x0e001800
	WORD $0x0d9f8160
	WORD $0x54ffff61
	WORD $0xeb08005f
	WORD $0x54000100
	WORD $0x8b080829
	WORD $0xcb08004a
	WORD $0x8b080008
	WORD $0xb840452b
	WORD $0xf100054a
	WORD $0x3800150b
	WORD $0x54ffffa1
	WORD $0xd65f03c0
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x00000002
	WORD $0x00000000
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x00000006
	WORD $0x00000000
	WORD $0x00000003
	WORD $0x00000000
	WORD $0x00000006
	WORD $0x00000000
	WORD $0xcfcfcfcf
	WORD $0xcfcfcfcf
	WORD $0x33333333
	WORD $0x33333333
	WORD $0x67676767
	WORD $0x67676767
	WORD $0x19191919
	WORD $0x19191919
	WORD $0xffffff00
	WORD $0xffffff01
	WORD $0xffffff02
	WORD $0xffffff03
	WORD $0xffffff04
	WORD $0xffffff05
	WORD $0xffffff06
	WORD $0xffffff07
	WORD $0xffffff08
	WORD $0xffffff09
	WORD $0xffffff0a
	WORD $0xffffff0b
	WORD $0xffffff0c
	WORD $0xffffff0d
	WORD $0xffffff0e
	WORD $0xffffff0f
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0xffffff00
	WORD $0xffffff01
	WORD $0xffffff02
	WORD $0xffffff03
	WORD $0xffffff04
	WORD $0xffffff05
	WORD $0xffffff06
	WORD $0xffffff07
	WORD $0xffffff08
	WORD $0xffffff09
	WORD $0xffffff0a
	WORD $0xffffff0b
	WORD $0xffffff0c
	WORD $0xffffff0d
	WORD $0xffffff0e
	WORD $0xffffff0f
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x33323130
	WORD $0x37363534
	WORD $0x62613938
	WORD $0x66656463
	RET

// func equalBytesNEON(a []byte, b []byte) bool
TEXT ·equalBytesNEON(SB), NOSPLIT|NOFRAME, $0-49
	MOVD $ret+48(FP), R0
	MOVD a_base+0(FP), R1
	MOVD b_base+24(FP), R2
	MOVD a_len+8(FP), R3
	WORD $0xaa1f03e8
	WORD $0x91020109
	WORD $0xeb03013f
	WORD $0x5400042c
	WORD $0x8b08002a
	WORD $0x8b080048
	WORD $0xad400141
	WORD $0xad400d02
	WORD $0xad411145
	WORD $0xad411907
	WORD $0x6e211c41
	WORD $0x6e201c60
	WORD $0xad420d42
	WORD $0xad424510
	WORD $0x6e251ce5
	WORD $0xad434953
	WORD $0x6e241cc4
	WORD $0xad435115
	WORD $0xaa0903e8
	WORD $0x6e231e23
	WORD $0x6e221e02
	WORD $0x4ea11ca1
	WORD $0x4ea01c80
	WORD $0x6e331ea6
	WORD $0x6e321e87
	WORD $0x4ea31ce3
	WORD $0x4ea21cc2
	WORD $0x4ea01c60
	WORD $0x4ea11c41
	WORD $0x4ea01c20
	WORD $0x6e004001
	WORD $0x0ea11c00
	WORD $0x9e66000a
	WORD $0xb4fffc0a
	WORD $0x3900001f
	WORD $0xd65f03c0
	WORD $0x6f00e400
	WORD $0x6f00e401
	WORD $0x91008109
	WORD $0x6f00e402
	WORD $0xeb03013f
	WORD $0x540001ac
	WORD $0x8b080029
	WORD $0x8b08004a
	WORD $0xad400d24
	WORD $0x91010109
	WORD $0xad401546
	WORD $0xeb03013f
	WORD $0x91008108
	WORD $0x6e241cc4
	WORD $0x6e231ca3
	WORD $0x4ea21c62
	WORD $0x4ea11c81
	WORD $0x54fffead
	WORD $0x6f00e403
	WORD $0xeb03011f
	WORD $0x54000f8b
	WORD $0xb2400109
	WORD $0xeb03013f
	WORD $0x5400102b
	WORD $0xb27f0109
	WORD $0xeb03013f
	WORD $0x540010ab
	WORD $0xb2400509
	WORD $0xeb03013f
	WORD $0x5400112b
	WORD $0xb27e0109
	WORD $0xeb03013f
	WORD $0x540011ab
	WORD $0x528000a9
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x5400122b
	WORD $0xb27f0509
	WORD $0xeb03013f
	WORD $0x540012ab
	WORD $0xb2400909
	WORD $0xeb03013f
	WORD $0x5400132b
	WORD $0xb27d0109
	WORD $0xeb03013f
	WORD $0x540013ab
	WORD $0x52800129
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x5400142b
	WORD $0x52800149
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x540014ab
	WORD $0x52800169
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x5400152b
	WORD $0xb27e0509
	WORD $0xeb03013f
	WORD $0x540015ab
	WORD $0x528001a9
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x5400162b
	WORD $0xb27f0909
	WORD $0xeb03013f
	WORD $0x540016ab
	WORD $0xb2400d09
	WORD $0xeb03013f
	WORD $0x5400172b
	WORD $0xb27c0109
	WORD $0xeb03013f
	WORD $0x540017ab
	WORD $0x52800229
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x5400182b
	WORD $0x52800249
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x540018ab
	WORD $0x52800269
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x5400192b
	WORD $0x52800289
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x540019ab
	WORD $0x528002a9
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54001a2b
	WORD $0x528002c9
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54001aab
	WORD $0x528002e9
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54001b2b
	WORD $0xb27d0509
	WORD $0xeb03013f
	WORD $0x54001bab
	WORD $0x52800329
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54001c2b
	WORD $0x52800349
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54001cab
	WORD $0x52800369
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54001d2b
	WORD $0xb27e0909
	WORD $0xeb03013f
	WORD $0x54001dab
	WORD $0x528003a9
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54001e2b
	WORD $0xb27f0d09
	WORD $0xeb03013f
	WORD $0x54001eab
	WORD $0xb2401108
	WORD $0xeb03011f
	WORD $0x540000aa
	WORD $0x38686829
	WORD $0x38686848
	WORD $0x4a090108
	WORD $0x4e1f1d03
	WORD $0x4ea11c00
	WORD $0x4ea21c61
	WORD $0x4ea11c00
	WORD $0x6e004001
	WORD $0x0ea11c00
	WORD $0x9e660008
	WORD $0xf100011f
	WORD $0x1a9f17e8
	WORD $0x39000008
	WORD $0xd65f03c0
	WORD $0x6f00e400
	WORD $0x38686829
	WORD $0x3868684a
	WORD $0x4a090149
	WORD $0x4e011d20
	WORD $0xb2400109
	WORD $0xeb03013f
	WORD $0x54fff02a
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e031d20
	WORD $0xb27f0109
	WORD $0xeb03013f
	WORD $0x54ffefaa
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e051d20
	WORD $0xb2400509
	WORD $0xeb03013f
	WORD $0x54ffef2a
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e071d20
	WORD $0xb27e0109
	WORD $0xeb03013f
	WORD $0x54ffeeaa
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e091d20
	WORD $0x528000a9
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54ffee2a
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e0b1d20
	WORD $0xb27f0509
	WORD $0xeb03013f
	WORD $0x54ffedaa
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e0d1d20
	WORD $0xb2400909
	WORD $0xeb03013f
	WORD $0x54ffed2a
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e0f1d20
	WORD $0xb27d0109
	WORD $0xeb03013f
	WORD $0x54ffecaa
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e111d20
	WORD $0x52800129
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54ffec2a
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e131d20
	WORD $0x52800149
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54ffebaa
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e151d20
	WORD $0x52800169
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54ffeb2a
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e171d20
	WORD $0xb27e0509
	WORD $0xeb03013f
	WORD $0x54ffeaaa
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e191d20
	WORD $0x528001a9
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54ffea2a
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e1b1d20
	WORD $0xb27f0909
	WORD $0xeb03013f
	WORD $0x54ffe9aa
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e1d1d20
	WORD $0xb2400d09
	WORD $0xeb03013f
	WORD $0x54ffe92a
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e1f1d20
	WORD $0xb27c0109
	WORD $0xeb03013f
	WORD $0x54ffe8aa
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e011d23
	WORD $0x52800229
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54ffe82a
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e031d23
	WORD $0x52800249
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54ffe7aa
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e051d23
	WORD $0x52800269
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54ffe72a
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e071d23
	WORD $0x52800289
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54ffe6aa
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e091d23
	WORD $0x528002a9
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54ffe62a
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e0b1d23
	WORD $0x528002c9
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54ffe5aa
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e0d1d23
	WORD $0x528002e9
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54ffe52a
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e0f1d23
	WORD $0xb27d0509
	WORD $0xeb03013f
	WORD $0x54ffe4aa
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e111d23
	WORD $0x52800329
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54ffe42a
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e131d23
	WORD $0x52800349
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54ffe3aa
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e151d23
	WORD $0x52800369
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54ffe32a
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e171d23
	WORD $0xb27e0909
	WORD $0xeb03013f
	WORD $0x54ffe2aa
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e191d23
	WORD $0x528003a9
	WORD $0xaa090109
	WORD $0xeb03013f
	WORD $0x54ffe22a
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e1b1d23
	WORD $0xb27f0d09
	WORD $0xeb03013f
	WORD $0x54ffe1aa
	WORD $0x3869682a
	WORD $0x38696849
	WORD $0x4a0a0129
	WORD $0x4e1d1d23
	WORD $0xb2401108
	WORD $0xeb03011f
	WORD $0x54ffe12b
	WORD $0x17ffff0c
	RET

// func bitAndNEON(dst []byte, a []byte, b []byte)
TEXT ·bitAndNEON(SB), NOSPLIT|NOFRAME, $0-72
	MOVD dst_base+0(FP), R0
	MOVD a_base+24(FP), R1
	MOVD b_base+48(FP), R2
	MOVD dst_len+8(FP), R3
	WORD $0xf100047f
	WORD $0x5400068b
	WORD $0xf100207f
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x14000026
	WORD $0xf100807f
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x14000014
	WORD $0x927d0469
	WORD $0x927be468
	WORD $0x9100400a
	WORD $0x9100404b
	WORD $0x9100402c
	WORD $0x927be46d
	WORD $0xad7f8d60
	WORD $0xf10081ad
	WORD $0xad7f8981
	WORD $0x9100816b
	WORD $0x9100818c
	WORD $0x4e211c00
	WORD $0x4e221c61
	WORD $0xad3f8540
	WORD $0x9100814a
	WORD $0x54fffee1
	WORD $0xeb08007f
	WORD $0x54000340
	WORD $0xb40001e9
	WORD $0xaa0803ec
	WORD $0x927dec68
	WORD $0xcb080189
	WORD $0x8b0c000a
	WORD $0x8b0c004b
	WORD $0x8b0c002c
	WORD $0xfc408580
	WORD $0xb1002129
	WORD $0xfc408561
	WORD $0x0e201c20
	WORD $0xfc008540
	WORD $0x54ffff61
	WORD $0xeb08007f
	WORD $0x54000160
	WORD $0xcb080069
	WORD $0x8b08000a
	WORD $0x8b08004b
	WORD $0x8b080028
	WORD $0x3840150c
	WORD $0xf1000529
	WORD $0x3840156d
	WORD $0x0a0c01ac
	WORD $0x3800154c
	WORD $0x54ffff61
	WORD $0xd65f03c0
	RET

// func bitOrNEON(dst []byte, a []byte, b []byte)
TEXT ·bitOrNEON(SB), NOSPLIT|NOFRAME, $0-72
	MOVD dst_base+0(FP), R0
	MOVD a_base+24(FP), R1
	MOVD b_base+48(FP), R2
	MOVD dst_len+8(FP), R3
	WORD $0xf100047f
	WORD $0x5400068b
	WORD $0xf100207f
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x14000026
	WORD $0xf100807f
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x14000014
	WORD $0x927d0469
	WORD $0x927be468
	WORD $0x9100400a
	WORD $0x9100404b
	WORD $0x9100402c
	WORD $0x927be46d
	WORD $0xad7f8d60
	WORD $0xf10081ad
	WORD $0xad7f8981
	WORD $0x9100816b
	WORD $0x9100818c
	WORD $0x4ea11c00
	WORD $0x4ea21c61
	WORD $0xad3f8540
	WORD $0x9100814a
	WORD $0x54fffee1
	WORD $0xeb08007f
	WORD $0x54000340
	WORD $0xb40001e9
	WORD $0xaa0803ec
	WORD $0x927dec68
	WORD $0xcb080189
	WORD $0x8b0c000a
	WORD $0x8b0c004b
	WORD $0x8b0c002c
	WORD $0xfc408580
	WORD $0xb1002129
	WORD $0xfc408561
	WORD $0x0ea01c20
	WORD $0xfc008540
	WORD $0x54ffff61
	WORD $0xeb08007f
	WORD $0x54000160
	WORD $0xcb080069
	WORD $0x8b08000a
	WORD $0x8b08004b
	WORD $0x8b080028
	WORD $0x3840150c
	WORD $0xf1000529
	WORD $0x3840156d
	WORD $0x2a0c01ac
	WORD $0x3800154c
	WORD $0x54ffff61
	WORD $0xd65f03c0
	RET

// func bitXorNEON(dst []byte, a []byte, b []byte)
TEXT ·bitXorNEON(SB), NOSPLIT|NOFRAME, $0-72
	MOVD dst_base+0(FP), R0
	MOVD a_base+24(FP), R1
	MOVD b_base+48(FP), R2
	MOVD dst_len+8(FP), R3
	WORD $0xf100047f
	WORD $0x5400068b
	WORD $0xf100207f
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x14000026
	WORD $0xf100807f
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x14000014
	WORD $0x927d0469
	WORD $0x927be468
	WORD $0x9100400a
	WORD $0x9100404b
	WORD $0x9100402c
	WORD $0x927be46d
	WORD $0xad7f8d60
	WORD $0xf10081ad
	WORD $0xad7f8981
	WORD $0x9100816b
	WORD $0x9100818c
	WORD $0x6e211c00
	WORD $0x6e221c61
	WORD $0xad3f8540
	WORD $0x9100814a
	WORD $0x54fffee1
	WORD $0xeb08007f
	WORD $0x54000340
	WORD $0xb40001e9
	WORD $0xaa0803ec
	WORD $0x927dec68
	WORD $0xcb080189
	WORD $0x8b0c000a
	WORD $0x8b0c004b
	WORD $0x8b0c002c
	WORD $0xfc408580
	WORD $0xb1002129
	WORD $0xfc408561
	WORD $0x2e201c20
	WORD $0xfc008540
	WORD $0x54ffff61
	WORD $0xeb08007f
	WORD $0x54000160
	WORD $0xcb080069
	WORD $0x8b08000a
	WORD $0x8b08004b
	WORD $0x8b080028
	WORD $0x3840150c
	WORD $0xf1000529
	WORD $0x3840156d
	WORD $0x4a0c01ac
	WORD $0x3800154c
	WORD $0x54ffff61
	WORD $0xd65f03c0
	RET

// func bitAndNotNEON(dst []byte, a []byte, b []byte)
TEXT ·bitAndNotNEON(SB), NOSPLIT|NOFRAME, $0-72
	MOVD dst_base+0(FP), R0
	MOVD a_base+24(FP), R1
	MOVD b_base+48(FP), R2
	MOVD dst_len+8(FP), R3
	WORD $0xf100047f
	WORD $0x5400068b
	WORD $0xf100207f
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x14000026
	WORD $0xf100807f
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x14000014
	WORD $0x927d0469
	WORD $0x927be468
	WORD $0x9100400a
	WORD $0x9100404b
	WORD $0x9100402c
	WORD $0x927be46d
	WORD $0xad7f8d60
	WORD $0xf10081ad
	WORD $0xad7f8981
	WORD $0x9100816b
	WORD $0x9100818c
	WORD $0x4e601c20
	WORD $0x4e631c41
	WORD $0xad3f8540
	WORD $0x9100814a
	WORD $0x54fffee1
	WORD $0xeb08007f
	WORD $0x54000340
	WORD $0xb40001e9
	WORD $0xaa0803ec
	WORD $0x927dec68
	WORD $0xcb080189
	WORD $0x8b0c000a
	WORD $0x8b0c004b
	WORD $0x8b0c002c
	WORD $0xfc408580
	WORD $0xb1002129
	WORD $0xfc408561
	WORD $0x0e611c00
	WORD $0xfc008540
	WORD $0x54ffff61
	WORD $0xeb08007f
	WORD $0x54000160
	WORD $0xcb080069
	WORD $0x8b08000a
	WORD $0x8b08004b
	WORD $0x8b080028
	WORD $0x3840150c
	WORD $0xf1000529
	WORD $0x3840156d
	WORD $0x0a2d018c
	WORD $0x3800154c
	WORD $0x54ffff61
	WORD $0xd65f03c0
	RET

// func bitNotNEON(dst []byte, b []byte)
TEXT ·bitNotNEON(SB), NOSPLIT|NOFRAME, $0-48
	MOVD dst_base+0(FP), R0
	MOVD b_base+24(FP), R1
	MOVD dst_len+8(FP), R2
	WORD $0xf100045f
	WORD $0x540005ab
	WORD $0xf100205f
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x14000021
	WORD $0xf100805f
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x14000011
	WORD $0x927d0449
	WORD $0x927be448
	WORD $0x9100400a
	WORD $0x9100402b
	WORD $0x927be44c
	WORD $0xad7f8560
	WORD $0xf100818c
	WORD $0x9100816b
	WORD $0x6e205800
	WORD $0x6e205821
	WORD $0xad3f8540
	WORD $0x9100814a
	WORD $0x54ffff21
	WORD $0xeb08005f
	WORD $0x540002c0
	WORD $0xb40001a9
	WORD $0xaa0803eb
	WORD $0x927dec48
	WORD $0xcb080169
	WORD $0x8b0b000a
	WORD $0x8b0b002b
	WORD $0xfc408560
	WORD $0xb1002129
	WORD $0x2e205800
	WORD $0xfc008540
	WORD $0x54ffff81
	WORD $0xeb08005f
	WORD $0x54000120
	WORD $0xcb080049
	WORD $0x8b08000a
	WORD $0x8b080028
	WORD $0x3840150b
	WORD $0xf1000529
	WORD $0x2a2b03eb
	WORD $0x3800154b
	WORD $0x54ffff81
	WORD $0xd65f03c0
	RET

// func fillBytesNEON(dst []byte, v byte)
TEXT ·fillBytesNEON(SB), NOSPLIT|NOFRAME, $0-25
	MOVD dst_base+0(FP), R0
	MOVBU v+24(FP), R1
	MOVD dst_len+8(FP), R2
	WORD $0xf100045f
	WORD $0x5400048b
	WORD $0xf100205f
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x1400001b
	WORD $0xf100805f
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x1400000d
	WORD $0x4e010c20
	WORD $0x927d0449
	WORD $0x927be448
	WORD $0x9100400a
	WORD $0x927be44b
	WORD $0xf100816b
	WORD $0xad3f8140
	WORD $0x9100814a
	WORD $0x54ffffa1
	WORD $0xeb08005f
	WORD $0x54000220
	WORD $0xb4000169
	WORD $0x0e010c20
	WORD $0xaa0803ea
	WORD $0x927dec48
	WORD $0xcb080149
	WORD $0x8b0a000a
	WORD $0xb1002129
	WORD $0xfc008540
	WORD $0x54ffffc1
	WORD $0xeb08005f
	WORD $0x540000c0
	WORD $0xcb080049
	WORD $0x8b080008
	WORD $0xf1000529
	WORD $0x38001501
	WORD $0x54ffffc1
	WORD $0xd65f03c0
	RET

// func compareBytesNEON(a []byte, b []byte) int
TEXT ·compareBytesNEON(SB), NOSPLIT|NOFRAME, $0-56
	MOVD $ret+48(FP), R0
	MOVD a_base+0(FP), R1
	MOVD b_base+24(FP), R2
	MOVD a_len+8(FP), R3
	MOVD b_len+32(FP), R4
	WORD $0xeb04007f
	WORD $0xaa1f03eb
	WORD $0x9a84b06a
	WORD $0xaa0b03ec
	WORD $0x9101016b
	WORD $0xaa0203e8
	WORD $0xeb0a017f
	WORD $0xaa0103e9
	WORD $0x540002ec
	WORD $0xad400520
	WORD $0x91010102
	WORD $0xad400d02
	WORD $0x91010121
	WORD $0xad411524
	WORD $0xad411d06
	WORD $0x6e201c40
	WORD $0x6e211c61
	WORD $0x6e241cc2
	WORD $0x6e251ce3
	WORD $0x4ea01c40
	WORD $0x4ea11c61
	WORD $0x4ea01c20
	WORD $0x6e004001
	WORD $0x0ea11c00
	WORD $0x9e66000d
	WORD $0xaa4d81ad
	WORD $0xd350fdae
	WORD $0x2a0e01ad
	WORD $0x2a4d21ad
	WORD $0x72001dbf
	WORD $0x54fffca0
	WORD $0xeb0c014a
	WORD $0x540000ed
	WORD $0x3840152b
	WORD $0x3840150c
	WORD $0x6b0c017f
	WORD $0x54000101
	WORD $0xf100054a
	WORD $0x54ffff61
	WORD $0xeb04007f
	WORD $0x9a9fd7e8
	WORD $0xda9fa108
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x92800008
	WORD $0xda883508
	WORD $0xf9000008
	WORD $0xd65f03c0
	RET

// func commonPrefixNEON(a []byte, b []byte) int
TEXT ·commonPrefixNEON(SB), NOSPLIT|NOFRAME, $0-56
	MOVD $ret+48(FP), R0
	MOVD a_base+0(FP), R1
	MOVD b_base+24(FP), R2
	MOVD a_len+8(FP), R3
	MOVD b_len+32(FP), R4
	WORD $0xeb04007f
	WORD $0xaa1f03ea
	WORD $0x9a84b069
	WORD $0xaa0a03e8
	WORD $0x9101014a
	WORD $0xeb09015f
	WORD $0x540002ec
	WORD $0x8b08002b
	WORD $0x8b08004c
	WORD $0xad400560
	WORD $0xad400d82
	WORD $0xad411564
	WORD $0xad411d86
	WORD $0x6e201c40
	WORD $0x6e211c61
	WORD $0x6e241cc2
	WORD $0x6e251ce3
	WORD $0x4ea01c40
	WORD $0x4ea11c61
	WORD $0x4ea01c20
	WORD $0x6e004001
	WORD $0x0ea11c00
	WORD $0x9e66000b
	WORD $0xaa4b816b
	WORD $0xd350fd6c
	WORD $0x2a0c016b
	WORD $0x2a4b216b
	WORD $0x72001d7f
	WORD $0x54fffce0
	WORD $0xeb09011f
	WORD $0x5400012a
	WORD $0x3868682a
	WORD $0x3868684b
	WORD $0x6b0b015f
	WORD $0x540000a1
	WORD $0x91000508
	WORD $0xeb08013f
	WORD $0x54ffff41
	WORD $0xaa0903e8
	WORD $0xf9000008
	WORD $0xd65f03c0
	RET

// func equalFoldASCIINEON(a []byte, b []byte) bool
TEXT ·equalFoldASCIINEON(SB), NOSPLIT|NOFRAME, $0-49
	MOVD $ret+48(FP), R0
	MOVD a_base+0(FP), R1
	MOVD b_base+24(FP), R2
	MOVD a_len+8(FP), R3
	WORD $0xd10103ff
	WORD $0x4f05e7e0
	WORD $0x4f00e741
	WORD $0xaa1f03e9
	WORD $0x4f01e402
	WORD $0x6d022beb
	WORD $0x6d0323e9
	WORD $0x91020128
	WORD $0xeb03011f
	WORD $0x54000c8c
	WORD $0x8b09002a
	WORD $0x8b090049
	WORD $0xad401145
	WORD $0xad401927
	WORD $0xad414552
	WORD $0xad415523
	WORD $0x4e208490
	WORD $0x4e2084b3
	WORD $0x4e2084f4
	WORD $0x4e2084d6
	WORD $0x4e208657
	WORD $0x4e208638
	WORD $0x4e2086b9
	WORD $0x6e303430
	WORD $0x6e333433
	WORD $0x6e343434
	WORD $0x6e363436
	WORD $0x6e383438
	WORD $0x6e373437
	WORD $0x6e393439
	WORD $0x4e221e10
	WORD $0x4e221e7a
	WORD $0x4e221e9b
	WORD $0x4e221ed6
	WORD $0x4e221ef7
	WORD $0x4e221f18
	WORD $0x4e221f39
	WORD $0xad425153
	WORD $0x4e248604
	WORD $0x4e258745
	WORD $0x4e278770
	WORD $0x4e318707
	WORD $0x4e3286f1
	WORD $0x4e2686c6
	WORD $0x4e358732
	WORD $0xad426538
	WORD $0xad43695b
	WORD $0x4e208476
	WORD $0xad43713d
	WORD $0x4e208695
	WORD $0x4e208677
	WORD $0x4e20873e
	WORD $0x4e20871f
	WORD $0x4e208748
	WORD $0x4e208769
	WORD $0x6e363436
	WORD $0x4e2087aa
	WORD $0x4e20878b
	WORD $0x6e353435
	WORD $0x6e373437
	WORD $0x6e3f343f
	WORD $0x6e3e343e
	WORD $0x6e283428
	WORD $0x6e293429
	WORD $0x4e221ed6
	WORD $0x6e2b342b
	WORD $0x6e2a342a
	WORD $0x4e221eb5
	WORD $0x4e221ef7
	WORD $0x4e221fde
	WORD $0x4e221fff
	WORD $0x4e221d08
	WORD $0x4e221d29
	WORD $0x4e2386c3
	WORD $0x4e221d4a
	WORD $0x4e221d6b
	WORD $0xaa0803e9
	WORD $0x4e3386f3
	WORD $0x4e3486b4
	WORD $0x4e3887f5
	WORD $0x4e3987d6
	WORD $0x4e3a8517
	WORD $0x4e3b8538
	WORD $0x4e3c8579
	WORD $0x4e3d855a
	WORD $0x6e251e05
	WORD $0x6e241cc4
	WORD $0x6e311c63
	WORD $0x6e271e46
	WORD $0x6e341ec7
	WORD $0x6e331eb0
	WORD $0x6e381f51
	WORD $0x6e371f32
	WORD $0x4ea41cc4
	WORD $0x4ea51c63
	WORD $0x4ea71e45
	WORD $0x4eb01e26
	WORD $0x4ea41ca4
	WORD $0x4ea31cc3
	WORD $0x4ea41c63
	WORD $0x6e034064
	WORD $0x0ea41c63
	WORD $0x9e66006a
	WORD $0xb4fff40a
	WORD $0x6d4323e9
	WORD $0x3900001f
	WORD $0x6d422beb
	WORD $0x910103ff
	WORD $0xd65f03c0
	WORD $0x6f00e400
	WORD $0x6f00e401
	WORD $0x91008128
	WORD $0xeb03011f
	WORD $0x5400006d
	WORD $0x6f00e402
	WORD $0x14000021
	WORD $0x4f05e7e3
	WORD $0x4f00e744
	WORD $0x4f01e405
	WORD $0x6f00e402
	WORD $0x8b090028
	WORD $0x8b09004a
	WORD $0xad401907
	WORD $0x91010128
	WORD $0xad404151
	WORD $0xeb03011f
	WORD $0x91008129
	WORD $0x4e2384d2
	WORD $0x4e2384f3
	WORD $0x4e238634
	WORD $0x4e238615
	WORD $0x6e323492
	WORD $0x6e333493
	WORD $0x6e353495
	WORD $0x6e343494
	WORD $0x4e251e52
	WORD $0x4e251e73
	WORD $0x4e251e94
	WORD $0x4e251eb5
	WORD $0x4e268646
	WORD $0x4e278667
	WORD $0x4e3086b0
	WORD $0x4e318691
	WORD $0x6e271e27
	WORD $0x6e261e06
	WORD $0x4ea21cc2
	WORD $0x4ea11ce1
	WORD $0x54fffcad
	WORD $0x6f00e403
	WORD $0xaa1f03e8
	WORD $0x8b09002a
	WORD $0x8b09004b
	WORD $0x910003ec
	WORD $0x14000004
	WORD $0x91000508
	WORD $0xf100811f
	WORD $0x54000280
	WORD $0x8b08012d
	WORD $0xeb0301bf
	WORD $0x54ffff6a
	WORD $0x3868694d
	WORD $0x3868696e
	WORD $0xad000fe0
	WORD $0x510105af
	WORD $0x110081b0
	WORD $0x710069ff
	WORD $0x510105cf
	WORD $0x1a8d320d
	WORD $0x110081d0
	WORD $0x710069ff
	WORD $0x1a8e320e
	WORD $0x9240110f
	WORD $0x4a0d01cd
	WORD $0x382f698d
	WORD $0xad400fe0
	WORD $0x17ffffeb
	WORD $0x4ea11c00
	WORD $0x4ea21c61
	WORD $0x4ea11c00
	WORD $0x6e004001
	WORD $0x0ea11c00
	WORD $0x9e660008
	WORD $0xf100011f
	WORD $0x1a9f17e8
	WORD $0x6d4323e9
	WORD $0x39000008
	WORD $0x6d422beb
	WORD $0x910103ff
	WORD $0xd65f03c0
	RET

// func indexAnyNEON(b []byte, chars []byte) int
TEXT ·indexAnyNEON(SB), NOSPLIT|NOFRAME, $0-56
	MOVD $ret+48(FP), R0
	MOVD b_base+0(FP), R1
	MOVD chars_base+24(FP), R2
	MOVD b_len+8(FP), R3
	MOVD chars_len+32(FP), R4
	WORD $0xf100049f
	WORD $0x5400038b
	WORD $0xaa1f03e9
	WORD $0xaa0903e8
	WORD $0x91008129
	WORD $0xeb03013f
	WORD $0x5400026c
	WORD $0x6f00e400
	WORD $0x6f00e401
	WORD $0x8b08002a
	WORD $0xad400943
	WORD $0xaa0203ea
	WORD $0xaa0403eb
	WORD $0x4ddfc144
	WORD $0xf100056b
	WORD $0x6e248c65
	WORD $0x6e248c44
	WORD $0x4ea01c80
	WORD $0x4ea11ca1
	WORD $0x54ffff41
	WORD $0x4ea01c20
	WORD $0x6e004001
	WORD $0x0ea11c00
	WORD $0x9e66000a
	WORD $0xb4fffd6a
	WORD $0xeb03011f
	WORD $0x92800009
	WORD $0x5400012b
	WORD $0x14000016
	WORD $0xf1007c7f
	WORD $0x528003e8
	WORD $0x9a88c068
	WORD $0x927be508
	WORD $0xeb03011f
	WORD $0x92800009
	WORD $0x540001ea
	WORD $0xf100049f
	WORD $0x540001ab
	WORD $0x38686829
	WORD $0xaa0203ea
	WORD $0xaa0403eb
	WORD $0x3840154c
	WORD $0x6b0c013f
	WORD $0x54000120
	WORD $0xf100056b
	WORD $0x54ffff81
	WORD $0x91000508
	WORD $0x92800009
	WORD $0xeb03011f
	WORD $0x54fffea1
	WORD $0xf9000009
	WORD $0xd65f03c0
	WORD $0xf9000008
	WORD $0xd65f03c0
	RET

// func indexAnyOrLessNEON(b []byte, chars []byte, lo byte) int
TEXT ·indexAnyOrLessNEON(SB), NOSPLIT|NOFRAME, $0-64
	MOVD $ret+56(FP), R0
	MOVD b_base+0(FP), R1
	MOVD chars_base+24(FP), R2
	MOVD b_len+8(FP), R3
	MOVD chars_len+32(FP), R4
	MOVBU lo+48(FP), R5
	WORD $0x4e010ca0
	WORD $0xf100009f
	WORD $0x5400032d
	WORD $0xaa1f03e9
	WORD $0xaa0903e8
	WORD $0x91008129
	WORD $0xeb03013f
	WORD $0x5400044c
	WORD $0x8b08002a
	WORD $0xaa0403eb
	WORD $0xad400d41
	WORD $0xaa0203ea
	WORD $0x6e233402
	WORD $0x6e213404
	WORD $0x4ddfc145
	WORD $0xf100056b
	WORD $0x6e258c26
	WORD $0x6e258c65
	WORD $0x4ea21ca2
	WORD $0x4ea41cc4
	WORD $0x54ffff41
	WORD $0x4ea21c81
	WORD $0x6e014022
	WORD $0x0ea21c21
	WORD $0x9e66002a
	WORD $0xb4fffd6a
	WORD $0x1400000f
	WORD $0xaa1f03e9
	WORD $0xaa0903e8
	WORD $0x91008129
	WORD $0xeb03013f
	WORD $0x5400014c
	WORD $0x8b08002a
	WORD $0xad400941
	WORD $0x6e223402
	WORD $0x6e213401
	WORD $0x4ea21c21
	WORD $0x6e014022
	WORD $0x0ea21c21
	WORD $0x9e66002a
	WORD $0xb4fffe8a
	WORD $0xeb03011f
	WORD $0x5400034a
	WORD $0xf100009f
	WORD $0x5400026d
	WORD $0x38686829
	WORD $0xaa0203ea
	WORD $0xaa0403eb
	WORD $0x6b25013f
	WORD $0x54000223
	WORD $0x3840154c
	WORD $0x6b0c013f
	WORD $0x540001c0
	WORD $0xf100056b
	WORD $0x54ffff81
	WORD $0x91000508
	WORD $0x92800009
	WORD $0xeb03011f
	WORD $0x54fffe61
	WORD $0x1400000a
	WORD $0x91000508
	WORD $0xeb08007f
	WORD $0x540000c0
	WORD $0x38686829
	WORD $0x6b25013f
	WORD $0x54ffff62
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x92800009
	WORD $0xf9000009
	WORD $0xd65f03c0
	RET

// func indexNotAnyNEON(b []byte, chars []byte) int
TEXT ·indexNotAnyNEON(SB), NOSPLIT|NOFRAME, $0-56
	MOVD $ret+48(FP), R0
	MOVD b_base+0(FP), R1
	MOVD chars_base+24(FP), R2
	MOVD b_len+8(FP), R3
	MOVD chars_len+32(FP), R4
	WORD $0xb40003e4
	WORD $0xf100049f
	WORD $0x5400042b
	WORD $0xaa1f03e9
	WORD $0xaa0903e8
	WORD $0x91008129
	WORD $0xeb03013f
	WORD $0x5400026c
	WORD $0x6f07e7e0
	WORD $0x6f07e7e1
	WORD $0x8b08002a
	WORD $0xad400943
	WORD $0xaa0203ea
	WORD $0xaa0403eb
	WORD $0x4ddfc144
	WORD $0xf100056b
	WORD $0x6e248c45
	WORD $0x6e248c64
	WORD $0x4e651c00
	WORD $0x4e641c21
	WORD $0x54ffff41
	WORD $0x4ea01c20
	WORD $0x6e004001
	WORD $0x0ea11c00
	WORD $0x9e66000a
	WORD $0xb4fffd6a
	WORD $0xeb03011f
	WORD $0xda9fb109
	WORD $0x5400016b
	WORD $0xf9000009
	WORD $0xd65f03c0
	WORD $0xf100007f
	WORD $0xda9f13e9
	WORD $0xf9000009
	WORD $0xd65f03c0
	WORD $0xaa1f03e8
	WORD $0xeb0303ff
	WORD $0xda9fb3e9
	WORD $0x54fffeea
	WORD $0xf100049f
	WORD $0x54fffeab
	WORD $0x0f00e420
	WORD $0x4f00e421
	WORD $0x927dec89
	WORD $0x927d048a
	WORD $0x927be48b
	WORD $0xcb0903ec
	WORD $0x3868682d
	WORD $0xf100209f
	WORD $0x54000082
	WORD $0xaa1f03f0
	WORD $0x2a1f03ee
	WORD $0x14000035
	WORD $0xf100809f
	WORD $0x54000082
	WORD $0xaa1f03ef
	WORD $0x2a1f03ee
	WORD $0x1400001d
	WORD $0x6f00e402
	WORD $0x6f00e403
	WORD $0x9100404e
	WORD $0x4e010da4
	WORD $0x927be48f
	WORD $0xad7f99c5
	WORD $0xf10081ef
	WORD $0x910081ce
	WORD $0x6e258c85
	WORD $0x6e268c86
	WORD $0x4e211ca5
	WORD $0x4e211cc6
	WORD $0x4ea51c42
	WORD $0x4ea61c63
	WORD $0x54fffee1
	WORD $0x4ea21c62
	WORD $0xeb0b009f
	WORD $0x6e024043
	WORD $0x0ea31c42
	WORD $0x9e66004e
	WORD $0xaa4e81ce
	WORD $0xaa4e41ce
	WORD $0xd348fdcf
	WORD $0x2a0f01ce
	WORD $0x540003e0
	WORD $0x927be48f
	WORD $0x927be490
	WORD $0xb400028a
	WORD $0x6f00e402
	WORD $0x0e010da3
	WORD $0x4e011dc2
	WORD $0x8b0f018e
	WORD $0x8b0f004f
	WORD $0xfc4085e4
	WORD $0xb10021ce
	WORD $0x2e248c64
	WORD $0x0e201c84
	WORD $0x0ea41c42
	WORD $0x54ffff61
	WORD $0x9e66004e
	WORD $0xeb09009f
	WORD $0x927dec90
	WORD $0xaa4e81ce
	WORD $0xaa4e41ce
	WORD $0xd348fdcf
	WORD $0x2a0f01ce
	WORD $0x54000120
	WORD $0xcb10008f
	WORD $0x8b100050
	WORD $0x38401611
	WORD $0x6b1101bf
	WORD $0x1a9f17f1
	WORD $0xf10005ef
	WORD $0x2a1101ce
	WORD $0x54ffff61
	WORD $0x72001ddf
	WORD $0x540000e0
	WORD $0x91000508
	WORD $0xeb03011f
	WORD $0x54fff741
	WORD $0x92800009
	WORD $0xf9000009
	WORD $0xd65f03c0
	WORD $0xf9000008
	WORD $0xd65f03c0
	RET

// func lastIndexNotAnyNEON(b []byte, chars []byte) int
TEXT ·lastIndexNotAnyNEON(SB), NOSPLIT|NOFRAME, $0-56
	MOVD $ret+48(FP), R0
	MOVD b_base+0(FP), R1
	MOVD chars_base+24(FP), R2
	MOVD b_len+8(FP), R3
	MOVD chars_len+32(FP), R4
	WORD $0xb4000d64
	WORD $0xf100049f
	WORD $0x54000ceb
	WORD $0xaa0303e9
	WORD $0xaa0903e8
	WORD $0xf1008129
	WORD $0x5400026b
	WORD $0x6f07e7e0
	WORD $0x6f07e7e1
	WORD $0x8b09002a
	WORD $0xad400943
	WORD $0xaa0203ea
	WORD $0xaa0403eb
	WORD $0x4ddfc144
	WORD $0xf100056b
	WORD $0x6e248c45
	WORD $0x6e248c64
	WORD $0x4e651c00
	WORD $0x4e641c21
	WORD $0x54ffff41
	WORD $0x4ea01c20
	WORD $0x6e004001
	WORD $0x0ea11c00
	WORD $0x9e66000a
	WORD $0xb4fffd8a
	WORD $0x0f00e420
	WORD $0x4f00e421
	WORD $0xeb03011f
	WORD $0x927dec89
	WORD $0x9a83b108
	WORD $0x927d048a
	WORD $0x927be48b
	WORD $0xcb0903ec
	WORD $0x14000003
	WORD $0x72001ddf
	WORD $0x54000980
	WORD $0xf1000508
	WORD $0x5400092b
	WORD $0x3868682d
	WORD $0xf100209f
	WORD $0x54000082
	WORD $0xaa1f03f0
	WORD $0x2a1f03ee
	WORD $0x14000035
	WORD $0xf100809f
	WORD $0x54000082
	WORD $0xaa1f03ef
	WORD $0x2a1f03ee
	WORD $0x1400001d
	WORD $0x6f00e402
	WORD $0x6f00e403
	WORD $0x9100404e
	WORD $0x4e010da4
	WORD $0x927be48f
	WORD $0xad7f99c5
	WORD $0xf10081ef
	WORD $0x910081ce
	WORD $0x6e258c85
	WORD $0x6e268c86
	WORD $0x4e211ca5
	WORD $0x4e211cc6
	WORD $0x4ea51c42
	WORD $0x4ea61c63
	WORD $0x54fffee1
	WORD $0x4ea21c62
	WORD $0xeb0b009f
	WORD $0x6e024043
	WORD $0x0ea31c42
	WORD $0x9e66004e
	WORD $0xaa4e81ce
	WORD $0xaa4e41ce
	WORD $0xd348fdcf
	WORD $0x2a0f01ce
	WORD $0x54fffb20
	WORD $0x927be48f
	WORD $0x927be490
	WORD $0xb400028a
	WORD $0x6f00e402
	WORD $0x0e010da3
	WORD $0x4e011dc2
	WORD $0x8b0f018e
	WORD $0x8b0f004f
	WORD $0xfc4085e4
	WORD $0xb10021ce
	WORD $0x2e248c64
	WORD $0x0e201c84
	WORD $0x0ea41c42
	WORD $0x54ffff61
	WORD $0x9e66004e
	WORD $0xeb09009f
	WORD $0x927dec90
	WORD $0xaa4e81ce
	WORD $0xaa4e41ce
	WORD $0xd348fdcf
	WORD $0x2a0f01ce
	WORD $0x54fff860
	WORD $0xcb10008f
	WORD $0x8b100050
	WORD $0x38401611
	WORD $0x6b1101bf
	WORD $0x1a9f17f1
	WORD $0xf10005ef
	WORD $0x2a1101ce
	WORD $0x54ffff61
	WORD $0x17ffffba
	WORD $0xf100007f
	WORD $0x5400008d
	WORD $0xd1000468
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x92800008
	WORD $0xf9000008
	WORD $0xd65f03c0
	RET

// func countAnyNEON(b []byte, chars []byte) int
TEXT ·countAnyNEON(SB), NOSPLIT|NOFRAME, $0-56
	MOVD $ret+48(FP), R0
	MOVD b_base+0(FP), R1
	MOVD chars_base+24(FP), R2
	MOVD b_len+8(FP), R3
	MOVD chars_len+32(FP), R4
	WORD $0xf100049f
	WORD $0x5400244b
	WORD $0x4f000420
	WORD $0x0f00e421
	WORD $0xaa1f03e8
	WORD $0x4f00e422
	WORD $0xaa1f03e9
	WORD $0x528000aa
	WORD $0x5280012b
	WORD $0x5280014c
	WORD $0x5280016d
	WORD $0x528001ae
	WORD $0x1400000e
	WORD $0x4ea68606
	WORD $0x4ea78647
	WORD $0x4ea48463
	WORD $0x4ea58624
	WORD $0x4ea48463
	WORD $0x4ea784c4
	WORD $0x4ea48463
	WORD $0x4eb1b863
	WORD $0x1e26006f
	WORD $0x8b0f0108
	WORD $0x91000529
	WORD $0xeb04013f
	WORD $0x54002160
	WORD $0x3869684f
	WORD $0xb4000889
	WORD $0xf100213f
	WORD $0x54000082
	WORD $0xaa1f03f0
	WORD $0x2a1f03f1
	WORD $0x14000036
	WORD $0xf100813f
	WORD $0x54000082
	WORD $0xaa1f03f0
	WORD $0x2a1f03f1
	WORD $0x1400001d
	WORD $0x6f00e403
	WORD $0x6f00e404
	WORD $0x927be931
	WORD $0x4e010de5
	WORD $0x927d0525
	WORD $0x927be530
	WORD $0x91004046
	WORD $0xad7f9cc6
	WORD $0xf1008231
	WORD $0x910080c6
	WORD $0x6e258cc6
	WORD $0x6e258ce7
	WORD $0x4e221cc6
	WORD $0x4e221ce7
	WORD $0x4ea61c63
	WORD $0x4ea71c84
	WORD $0x54fffee1
	WORD $0x4ea31c83
	WORD $0xeb10013f
	WORD $0x6e034064
	WORD $0x0ea41c63
	WORD $0x9e660071
	WORD $0xaa518231
	WORD $0xaa514231
	WORD $0xd348fe26
	WORD $0x2a060231
	WORD $0x540003a0
	WORD $0xb40002a5
	WORD $0x6f00e403
	WORD $0x0e010de4
	WORD $0x8b100045
	WORD $0x4e011e23
	WORD $0x927df131
	WORD $0xcb110211
	WORD $0x927ded30
	WORD $0xfc4084a5
	WORD $0xb1002231
	WORD $0x2e248ca5
	WORD $0x0e211ca5
	WORD $0x0ea51c63
	WORD $0x54ffff61
	WORD $0x9e660071
	WORD $0xeb10013f
	WORD $0xaa518231
	WORD $0xaa514231
	WORD $0xd348fe25
	WORD $0x2a050231
	WORD $0x54000100
	WORD $0x38706845
	WORD $0x91000610
	WORD $0x6b0f00bf
	WORD $0x1a9f17e5
	WORD $0xeb10013f
	WORD $0x2a050231
	WORD $0x54ffff41
	WORD $0x72001e3f
	WORD $0x54fff721
	WORD $0x6f00e403
	WORD $0x6f00e404
	WORD $0xf100407f
	WORD $0x6f00e406
	WORD $0x6f00e405
	WORD $0x6f00e407
	WORD $0x5400016a
	WORD $0xaa1f03f0
	WORD $0xeb0303ff
	WORD $0x5400042b
	WORD $0x6f00e410
	WORD $0x6f00e411
	WORD $0x6f00e412
	WORD $0xb2400211
	WORD $0xeb03023f
	WORD $0x540004cb
	WORD $0x14000029
	WORD $0x4e010df0
	WORD $0xaa0103f0
	WORD $0x52800211
	WORD $0x3cc10611
	WORD $0x91004231
	WORD $0xeb03023f
	WORD $0x6e308e31
	WORD $0x6f08a632
	WORD $0x2f08a631
	WORD $0x2f10a653
	WORD $0x6f10a652
	WORD $0x2f10a634
	WORD $0x6f10a631
	WORD $0x4e201e73
	WORD $0x4e201e52
	WORD $0x4e201e94
	WORD $0x4e201e31
	WORD $0x4eb284e7
	WORD $0x4eb384a5
	WORD $0x4eb48484
	WORD $0x4eb184c6
	WORD $0x54fffdcd
	WORD $0x927cec70
	WORD $0xeb03021f
	WORD $0x54fffc2a
	WORD $0x38706831
	WORD $0x6f00e403
	WORD $0x6f00e410
	WORD $0x6b0f023f
	WORD $0x1a9f17f1
	WORD $0x4e041e23
	WORD $0x6f00e411
	WORD $0x6f00e412
	WORD $0xb2400211
	WORD $0xeb03023f
	WORD $0x540000aa
	WORD $0x38716831
	WORD $0x6b0f023f
	WORD $0x1a9f17f1
	WORD $0x4e0c1e23
	WORD $0xb27f0211
	WORD $0xeb03023f
	WORD $0x5400052b
	WORD $0xb2400611
	WORD $0xeb03023f
	WORD $0x540005ab
	WORD $0xb27e0211
	WORD $0xeb03023f
	WORD $0x5400062b
	WORD $0xaa0a0211
	WORD $0xeb03023f
	WORD $0x540006ab
	WORD $0xb27f0611
	WORD $0xeb03023f
	WORD $0x5400072b
	WORD $0xb2400a11
	WORD $0xeb03023f
	WORD $0x540007ab
	WORD $0xb27d0211
	WORD $0xeb03023f
	WORD $0x5400082b
	WORD $0xaa0b0211
	WORD $0xeb03023f
	WORD $0x540008ab
	WORD $0xaa0c0211
	WORD $0xeb03023f
	WORD $0x5400092b
	WORD $0xaa0d0211
	WORD $0xeb03023f
	WORD $0x540009ab
	WORD $0xb27e0611
	WORD $0xeb03023f
	WORD $0x54000a2b
	WORD $0xaa0e0211
	WORD $0xeb03023f
	WORD $0x54000aab
	WORD $0xb27f0a11
	WORD $0xeb03023f
	WORD $0x54000b2b
	WORD $0xb2400e10
	WORD $0xeb03021f
	WORD $0x54ffe98a
	WORD $0x1400005c
	WORD $0x38716831
	WORD $0x6b0f023f
	WORD $0x1a9f17f1
	WORD $0x4e141e23
	WORD $0xb2400611
	WORD $0xeb03023f
	WORD $0x54fffaaa
	WORD $0x38716831
	WORD $0x6b0f023f
	WORD $0x1a9f17f1
	WORD $0x4e1c1e23
	WORD $0xb27e0211
	WORD $0xeb03023f
	WORD $0x54fffa2a
	WORD $0x38716831
	WORD $0x6b0f023f
	WORD $0x1a9f17f1
	WORD $0x4e041e30
	WORD $0xaa0a0211
	WORD $0xeb03023f
	WORD $0x54fff9aa
	WORD $0x38716831
	WORD $0x6b0f023f
	WORD $0x1a9f17f1
	WORD $0x4e0c1e30
	WORD $0xb27f0611
	WORD $0xeb03023f
	WORD $0x54fff92a
	WORD $0x38716831
	WORD $0x6b0f023f
	WORD $0x1a9f17f1
	WORD $0x4e141e30
	WORD $0xb2400a11
	WORD $0xeb03023f
	WORD $0x54fff8aa
	WORD $0x38716831
	WORD $0x6b0f023f
	WORD $0x1a9f17f1
	WORD $0x4e1c1e30
	WORD $0xb27d0211
	WORD $0xeb03023f
	WORD $0x54fff82a
	WORD $0x38716831
	WORD $0x6b0f023f
	WORD $0x1a9f17f1
	WORD $0x4e041e31
	WORD $0xaa0b0211
	WORD $0xeb03023f
	WORD $0x54fff7aa
	WORD $0x38716831
	WORD $0x6b0f023f
	WORD $0x1a9f17f1
	WORD $0x4e0c1e31
	WORD $0xaa0c0211
	WORD $0xeb03023f
	WORD $0x54fff72a
	WORD $0x38716831
	WORD $0x6b0f023f
	WORD $0x1a9f17f1
	WORD $0x4e141e31
	WORD $0xaa0d0211
	WORD $0xeb03023f
	WORD $0x54fff6aa
	WORD $0x38716831
	WORD $0x6b0f023f
	WORD $0x1a9f17f1
	WORD $0x4e1c1e31
	WORD $0xb27e0611
	WORD $0xeb03023f
	WORD $0x54fff62a
	WORD $0x38716831
	WORD $0x6b0f023f
	WORD $0x1a9f17f1
	WORD $0x4e041e32
	WORD $0xaa0e0211
	WORD $0xeb03023f
	WORD $0x54fff5aa
	WORD $0x38716831
	WORD $0x6b0f023f
	WORD $0x1a9f17f1
	WORD $0x4e0c1e32
	WORD $0xb27f0a11
	WORD $0xeb03023f
	WORD $0x54fff52a
	WORD $0x38716831
	WORD $0x6b0f023f
	WORD $0x1a9f17f1
	WORD $0x4e141e32
	WORD $0xb2400e10
	WORD $0xeb03021f
	WORD $0x54ffde0a
	WORD $0x38706830
	WORD $0x6b0f021f
	WORD $0x1a9f17ef
	WORD $0x4e1c1df2
	WORD $0x17fffeeb
	WORD $0xaa1f03e8
	WORD $0xf9000008
	WORD $0xd65f03c0
	RET

// func jsonMasksNEON(dst []byte, b []byte, want uint32)
TEXT ·jsonMasksNEON(SB), NOSPLIT|NOFRAME, $0-52
	MOVD dst_base+0(FP), R0
	MOVD b_base+24(FP), R1
	MOVD b_len+32(FP), R2
	MOVWU want+48(FP), R3
	// Constant pool appended below the body; the displacements
	// above were patched to reach it and the aligned loads made
	// unaligned, since nothing promises alignment inside a TEXT.
	WORD $0xf81f0ff3
	WORD $0xb100fc48
	WORD $0x9101f849
	WORD $0x9a884128
	WORD $0xf101005f
	WORD $0x9346fd0b
	WORD $0x8b0b0569
	WORD $0xd37df16a
	WORD $0xd37ced68
	WORD $0xd37be96b
	WORD $0xd37df129
	WORD $0x540004aa
	WORD $0xaa1f03ec
	WORD $0xb1001c4d
	WORD $0x9100384e
	WORD $0x9a8d41cd
	WORD $0x9343fdae
	WORD $0xeb0a01cd
	WORD $0x5400376a
	WORD $0x37001b63
	WORD $0x8b0b01cf
	WORD $0x8b0901d0
	WORD $0x8b0801d1
	WORD $0x8b0a01c4
	WORD $0x8b0f000e
	WORD $0x8b10000f
	WORD $0x8b110010
	WORD $0x8b040011
	WORD $0x14000007
	WORD $0xb10005ad
	WORD $0x910005ce
	WORD $0x910005ef
	WORD $0x91000610
	WORD $0x91000631
	WORD $0x54003562
	WORD $0x370800a3
	WORD $0x371000c3
	WORD $0x371800e3
	WORD $0x3627fee3
	WORD $0x14000007
	WORD $0x3900023f
	WORD $0x3617ff83
	WORD $0x3900021f
	WORD $0x361fff63
	WORD $0x390001ff
	WORD $0x3627fe03
	WORD $0x390001df
	WORD $0x17ffffee
	WORD $0x4f01e400
	WORD $0x4f07e6e1
	WORD $0x100060cc
	WORD $0x4f00e442
	WORD $0x4f00e5a3
	WORD $0x3dc04191
	WORD $0x4f06e7e4
	WORD $0x4f02e765
	WORD $0xaa1f03ed
	WORD $0x4f02e7a6
	WORD $0x4f02e787
	WORD $0xaa0003ee
	WORD $0x4f01e450
	WORD $0x14000007
	WORD $0x910201af
	WORD $0x910101ac
	WORD $0x910021ce
	WORD $0xeb0201ff
	WORD $0xaa0c03ed
	WORD $0x54fff94c
	WORD $0x8b0d002c
	WORD $0xad414993
	WORD $0xad405195
	WORD $0x370000c3
	WORD $0x370803c3
	WORD $0x371006e3
	WORD $0x37180b83
	WORD $0x3627fe63
	WORD $0x14000074
	WORD $0x6e308eb6
	WORD $0x6e308e97
	WORD $0x6e308e78
	WORD $0x6e308e59
	WORD $0x4e311ed6
	WORD $0x4e311ef7
	WORD $0x4e311f18
	WORD $0x4e311f39
	WORD $0x6e1642da
	WORD $0x6e1742fb
	WORD $0x6e18431c
	WORD $0x6e19433d
	WORD $0x4e1a3ad6
	WORD $0x4e1b3af7
	WORD $0x4e1c3b18
	WORD $0x4e1d3b39
	WORD $0x4e71bad6
	WORD $0x4e71baf7
	WORD $0x4e71bb18
	WORD $0x4e71bb39
	WORD $0x7d0001d6
	WORD $0x7d0005d7
	WORD $0x7d0009d8
	WORD $0x7d000dd9
	WORD $0x360ffc83
	WORD $0x6e278eb6
	WORD $0x6e278e97
	WORD $0x8b0a01cc
	WORD $0x6e278e78
	WORD $0x6e278e59
	WORD $0x4e311ed6
	WORD $0x4e311ef7
	WORD $0x4e311f18
	WORD $0x4e311f39
	WORD $0x6e1642da
	WORD $0x6e1742fb
	WORD $0x6e18431c
	WORD $0x6e19433d
	WORD $0x4e1a3ad6
	WORD $0x4e1b3af7
	WORD $0x4e1c3b18
	WORD $0x4e1d3b39
	WORD $0x4e71bad6
	WORD $0x4e71baf7
	WORD $0x4e71bb18
	WORD $0x4e71bb39
	WORD $0x7d000196
	WORD $0x7d000597
	WORD $0x7d000998
	WORD $0x7d000d99
	WORD $0x3617f963
	WORD $0x4e241eb6
	WORD $0x4e241e97
	WORD $0x8b0801cc
	WORD $0x4e241e78
	WORD $0x4e241e59
	WORD $0x6e258eda
	WORD $0x6e268ed6
	WORD $0x6e258efb
	WORD $0x6e268ef7
	WORD $0x6e258f1c
	WORD $0x6e268f18
	WORD $0x6e258f3d
	WORD $0x6e268f39
	WORD $0x4eb61f56
	WORD $0x4eb71f77
	WORD $0x4eb81f98
	WORD $0x4eb91fb9
	WORD $0x4e311ed6
	WORD $0x4e311ef7
	WORD $0x4e311f18
	WORD $0x4e311f39
	WORD $0x6e1642da
	WORD $0x6e1742fb
	WORD $0x6e18431c
	WORD $0x6e19433d
	WORD $0x4e1a3ad6
	WORD $0x4e1b3af7
	WORD $0x4e1c3b18
	WORD $0x4e1d3b39
	WORD $0x4e71bad6
	WORD $0x4e71baf7
	WORD $0x4e71bb18
	WORD $0x4e71bb39
	WORD $0x7d000196
	WORD $0x7d000597
	WORD $0x7d000998
	WORD $0x7d000d99
	WORD $0x361ff4c3
	WORD $0x6e353416
	WORD $0x6e343417
	WORD $0x8b0901cc
	WORD $0x6e333418
	WORD $0x6e323419
	WORD $0x4e311ed6
	WORD $0x4e311ef7
	WORD $0x4e311f18
	WORD $0x4e311f39
	WORD $0x6e1642da
	WORD $0x6e1742fb
	WORD $0x6e18431c
	WORD $0x6e19433d
	WORD $0x4e1a3ad6
	WORD $0x4e1b3af7
	WORD $0x4e1c3b18
	WORD $0x4e1d3b39
	WORD $0x4e71bad6
	WORD $0x4e71baf7
	WORD $0x4e71bb18
	WORD $0x4e71bb39
	WORD $0x7d000196
	WORD $0x7d000597
	WORD $0x7d000998
	WORD $0x7d000d99
	WORD $0x3627efe3
	WORD $0x6e208e56
	WORD $0x6e208e77
	WORD $0x8b0b01cc
	WORD $0x6e208eb8
	WORD $0x4e2186b9
	WORD $0x6e238eb5
	WORD $0x6e208e9a
	WORD $0x4e21869b
	WORD $0x6e238e94
	WORD $0x4e21865c
	WORD $0x4e21867d
	WORD $0x6e238e73
	WORD $0x6e238e52
	WORD $0x6e393459
	WORD $0x4eb51f15
	WORD $0x6e3b3458
	WORD $0x4eb41f54
	WORD $0x6e3d345a
	WORD $0x4eb31ef3
	WORD $0x6e3c3457
	WORD $0x4eb21ed2
	WORD $0x4eb91eb5
	WORD $0x4eb81e94
	WORD $0x4eba1e73
	WORD $0x4eb71e52
	WORD $0x4e311eb5
	WORD $0x4e311e94
	WORD $0x4e311e73
	WORD $0x4e311e52
	WORD $0x6e1542b6
	WORD $0x6e144297
	WORD $0x6e134278
	WORD $0x6e124259
	WORD $0x4e163ab5
	WORD $0x4e173a94
	WORD $0x4e183a73
	WORD $0x4e193a52
	WORD $0x4e71bab5
	WORD $0x4e71ba94
	WORD $0x4e71ba73
	WORD $0x4e71ba52
	WORD $0x7d000195
	WORD $0x7d000594
	WORD $0x7d000993
	WORD $0x7d000d92
	WORD $0x17ffff51
	WORD $0x37080223
	WORD $0x8b0e000e
	WORD $0x14000004
	WORD $0xb10005ad
	WORD $0x910005ce
	WORD $0x54001b42
	WORD $0x390001df
	WORD $0x37100083
	WORD $0x371800a3
	WORD $0x3627ff43
	WORD $0x14000005
	WORD $0x382869df
	WORD $0x361fffa3
	WORD $0x382969df
	WORD $0x3627fea3
	WORD $0x382b69df
	WORD $0x17fffff3
	WORD $0xcb0e014d
	WORD $0xf10021bf
	WORD $0x37100143
	WORD $0x371802a3
	WORD $0x37200563
	WORD $0x54001843
	WORD $0xf100815f
	WORD $0x54001803
	WORD $0xf10081bf
	WORD $0x54001442
	WORD $0xaa1f03ef
	WORD $0x140000af
	WORD $0x371802e3
	WORD $0x37200583
	WORD $0x540009e2
	WORD $0xcb0a01cd
	WORD $0x8b0e000e
	WORD $0x390001df
	WORD $0xb10005ad
	WORD $0x382a69df
	WORD $0x382869df
	WORD $0x910005ce
	WORD $0x54ffff63
	WORD $0x140000b7
	WORD $0x37200583
	WORD $0x54000982
	WORD $0xcb0a01cd
	WORD $0x8b0e000e
	WORD $0x390001df
	WORD $0xb10005ad
	WORD $0x382a69df
	WORD $0x382969df
	WORD $0x910005ce
	WORD $0x54ffff63
	WORD $0x140000ac
	WORD $0x37200583
	WORD $0x54000962
	WORD $0xcb0a01cd
	WORD $0x8b0e000e
	WORD $0x390001df
	WORD $0xb10005ad
	WORD $0x382a69df
	WORD $0x382869df
	WORD $0x382969df
	WORD $0x910005ce
	WORD $0x54ffff43
	WORD $0x140000a0
	WORD $0x54000942
	WORD $0xcb0a01cd
	WORD $0x8b0e000e
	WORD $0x390001df
	WORD $0xb10005ad
	WORD $0x382a69df
	WORD $0x382b69df
	WORD $0x910005ce
	WORD $0x54ffff63
	WORD $0x14000096
	WORD $0x54000962
	WORD $0xcb0a01cd
	WORD $0x8b0e000e
	WORD $0x390001df
	WORD $0xb10005ad
	WORD $0x382a69df
	WORD $0x382869df
	WORD $0x382b69df
	WORD $0x910005ce
	WORD $0x54ffff43
	WORD $0x1400008b
	WORD $0x540009a2
	WORD $0xcb0a01cd
	WORD $0x8b0e000e
	WORD $0x390001df
	WORD $0xb10005ad
	WORD $0x382a69df
	WORD $0x382969df
	WORD $0x382b69df
	WORD $0x910005ce
	WORD $0x54ffff43
	WORD $0x14000080
	WORD $0x540009e2
	WORD $0xcb0a01cd
	WORD $0x8b0e000e
	WORD $0x8b0a01cf
	WORD $0x380015df
	WORD $0xb10005ad
	WORD $0x390001ff
	WORD $0x8b0a01ef
	WORD $0x390001ff
	WORD $0x8b0a01ef
	WORD $0x390001ff
	WORD $0x382a69ff
	WORD $0x54fffee3
	WORD $0x14000072
	WORD $0xf100815f
	WORD $0x54fff623
	WORD $0xf100811f
	WORD $0x54fff5e3
	WORD $0xf10081bf
	WORD $0x54002442
	WORD $0xaa1f03ef
	WORD $0x14000131
	WORD $0xf100815f
	WORD $0x54fff683
	WORD $0xf100813f
	WORD $0x54fff643
	WORD $0xf100811f
	WORD $0x54fff603
	WORD $0xf10081bf
	WORD $0x540015a2
	WORD $0xaa1f03ef
	WORD $0x140000bc
	WORD $0xf100815f
	WORD $0x54fff6a3
	WORD $0xf100811f
	WORD $0x54fff663
	WORD $0xf100813f
	WORD $0x54fff623
	WORD $0xf10081bf
	WORD $0x54001862
	WORD $0xaa1f03ef
	WORD $0x140000d2
	WORD $0xf100815f
	WORD $0x54fff6c3
	WORD $0x9101f84f
	WORD $0xf101fdff
	WORD $0x54fff663
	WORD $0xf100813f
	WORD $0x54fff623
	WORD $0xf10081bf
	WORD $0x54001c62
	WORD $0xaa1f03ef
	WORD $0x140000f2
	WORD $0xf100815f
	WORD $0x54fff6a3
	WORD $0xf100811f
	WORD $0x54fff663
	WORD $0x9101f84f
	WORD $0xf101fdff
	WORD $0x54fff603
	WORD $0xf100813f
	WORD $0x54fff5c3
	WORD $0xf10081bf
	WORD $0x540022c2
	WORD $0xaa1f03ef
	WORD $0x14000128
	WORD $0xf100815f
	WORD $0x54fff663
	WORD $0xf100813f
	WORD $0x54fff623
	WORD $0x9101f84f
	WORD $0xf101fdff
	WORD $0x54fff5c3
	WORD $0xf100811f
	WORD $0x54fff583
	WORD $0xf10081bf
	WORD $0x540026e2
	WORD $0xaa1f03ef
	WORD $0x14000149
	WORD $0xf100815f
	WORD $0x54fff623
	WORD $0xf100811f
	WORD $0x54fff5e3
	WORD $0xf100813f
	WORD $0x54fff5a3
	WORD $0x9101f84f
	WORD $0xf101fdff
	WORD $0x54fff543
	WORD $0xf10081bf
	WORD $0x54002b02
	WORD $0xaa1f03ef
	WORD $0x14000169
	WORD $0x6f00e400
	WORD $0x8b0001d1
	WORD $0x927d05b0
	WORD $0x927be9af
	WORD $0x91004231
	WORD $0x927be9a4
	WORD $0xad3f8220
	WORD $0x8b0a0225
	WORD $0xf1008084
	WORD $0x91008231
	WORD $0xad3f80a0
	WORD $0x54ffff61
	WORD $0xeb0f01bf
	WORD $0x540002c0
	WORD $0xb4002db0
	WORD $0x6f00e400
	WORD $0x927df1b0
	WORD $0x8b0e01f1
	WORD $0x8b1001ce
	WORD $0xcb1001ef
	WORD $0x8b110011
	WORD $0xfd000220
	WORD $0xb10021ef
	WORD $0xfc2a6a20
	WORD $0x91002231
	WORD $0x54ffff81
	WORD $0xeb1001bf
	WORD $0x54000100
	WORD $0xcb0a01cd
	WORD $0x8b0e000e
	WORD $0x390001df
	WORD $0xb10005ad
	WORD $0x382a69df
	WORD $0x910005ce
	WORD $0x54ffff83
	WORD $0xeb02019f
	WORD $0x5400090a
	WORD $0xd284c010
	WORD $0xd28000b1
	WORD $0x8b0a000a
	WORD $0x8b08000d
	WORD $0x8b09000e
	WORD $0x8b0b000b
	WORD $0x5280002f
	WORD $0xf2c00030
	WORD $0xf2c000b1
	WORD $0x14000004
	WORD $0x9100058c
	WORD $0xeb0c005f
	WORD $0x54000760
	WORD $0xd343fd84
	WORD $0xf2400985
	WORD $0x540000e1
	WORD $0x370005c3
	WORD $0x370805e3
	WORD $0x37100603
	WORD $0x37180623
	WORD $0x36200043
	WORD $0x3824697f
	WORD $0x386c6826
	WORD $0x1ac521e5
	WORD $0x36000083
	WORD $0x710088df
	WORD $0xaa0003e7
	WORD $0x540000a0
	WORD $0x360800e3
	WORD $0x710170df
	WORD $0xaa0a03e7
	WORD $0x54000081
	WORD $0x386468f3
	WORD $0x2a050273
	WORD $0x382468f3
	WORD $0x371000c3
	WORD $0x36180203
	WORD $0x71007cdf
	WORD $0xaa0903e7
	WORD $0x54000129
	WORD $0x1400000c
	WORD $0x51016cc7
	WORD $0x710088ff
	WORD $0x54ffff28
	WORD $0x9ac721e7
	WORD $0xea1100ff
	WORD $0xaa0803e7
	WORD $0x54fffea0
	WORD $0x8b070007
	WORD $0x386468f3
	WORD $0x2a050273
	WORD $0x382468f3
	WORD $0x3627fac3
	WORD $0x710080df
	WORD $0x54fffa88
	WORD $0x9ac621e6
	WORD $0xea1000df
	WORD $0x54fffa20
	WORD $0x38646966
	WORD $0x2a0500c5
	WORD $0x38246965
	WORD $0x17ffffcd
	WORD $0x3824681f
	WORD $0x360ffa63
	WORD $0x3824695f
	WORD $0x3617fa43
	WORD $0x382469bf
	WORD $0x361ffa23
	WORD $0x382469df
	WORD $0x3727fa03
	WORD $0x17ffffd0
	WORD $0xf84107f3
	WORD $0xd65f03c0
	WORD $0x6f00e400
	WORD $0x8b0001d1
	WORD $0x927d05b0
	WORD $0x927be9af
	WORD $0x91004231
	WORD $0x927be9a4
	WORD $0x8b0a0225
	WORD $0xad3f8220
	WORD $0xf1008084
	WORD $0xad3f80a0
	WORD $0x8b090225
	WORD $0x91008231
	WORD $0xad3f80a0
	WORD $0x54ffff21
	WORD $0xeb0f01bf
	WORD $0x54fff4c0
	WORD $0xb4001ff0
	WORD $0x6f00e400
	WORD $0x927df1b0
	WORD $0x8b0e01f1
	WORD $0x8b1001ce
	WORD $0xcb1001ef
	WORD $0x8b110011
	WORD $0xfd000220
	WORD $0xb10021ef
	WORD $0xfc2a6a20
	WORD $0xfc296a20
	WORD $0x91002231
	WORD $0x54ffff61
	WORD $0xeb1001bf
	WORD $0x54ffdc61
	WORD $0x17ffff96
	WORD $0x6f00e400
	WORD $0x927d05b0
	WORD $0x927be9af
	WORD $0x8b0e0011
	WORD $0x927be9a4
	WORD $0x8b0a0225
	WORD $0xac810220
	WORD $0xf1008084
	WORD $0xad0000a0
	WORD $0x8b0a00a5
	WORD $0xad0000a0
	WORD $0x8b0a00a5
	WORD $0xad0000a0
	WORD $0x54ffff01
	WORD $0xeb0f01bf
	WORD $0x54fff0c0
	WORD $0xb4001c30
	WORD $0x6f00e400
	WORD $0x927df1b0
	WORD $0x8b0901c4
	WORD $0x8b0801c5
	WORD $0x8b0a01c6
	WORD $0x8b1001d1
	WORD $0x8b040004
	WORD $0x8b050005
	WORD $0x8b060006
	WORD $0x8b0e000e
	WORD $0x927df1a7
	WORD $0xd10020e7
	WORD $0xfc2f69c0
	WORD $0x910021ce
	WORD $0xfc2f68c0
	WORD $0xeb0701ff
	WORD $0x910020c6
	WORD $0xfc2f68a0
	WORD $0x910020a5
	WORD $0xfc2f6880
	WORD $0x91002084
	WORD $0x54fffec1
	WORD $0xeb1001bf
	WORD $0xaa1103ee
	WORD $0x54ffd861
	WORD $0x17ffff6b
	WORD $0x6f00e400
	WORD $0x8b0001d1
	WORD $0x927d05b0
	WORD $0x927be9af
	WORD $0x91004231
	WORD $0x927be9a4
	WORD $0x8b0a0225
	WORD $0xad3f8220
	WORD $0xf1008084
	WORD $0xad3f80a0
	WORD $0x8b0b0225
	WORD $0x91008231
	WORD $0xad3f80a0
	WORD $0x54ffff21
	WORD $0xeb0f01bf
	WORD $0x54ffeb60
	WORD $0xb4001710
	WORD $0x6f00e400
	WORD $0x927df1b0
	WORD $0x8b0e01f1
	WORD $0x8b1001ce
	WORD $0xcb1001ef
	WORD $0x8b110011
	WORD $0xfd000220
	WORD $0xb10021ef
	WORD $0xfc2a6a20
	WORD $0xfc2b6a20
	WORD $0x91002231
	WORD $0x54ffff61
	WORD $0xeb1001bf
	WORD $0x54ffd5c1
	WORD $0x17ffff4b
	WORD $0x6f00e400
	WORD $0x8b0001d1
	WORD $0x927d05b0
	WORD $0x927be9af
	WORD $0x91004231
	WORD $0x927be9a4
	WORD $0x8b0a0225
	WORD $0xad3f8220
	WORD $0xf1008084
	WORD $0xad3f80a0
	WORD $0x8b080225
	WORD $0x91008231
	WORD $0xad3f80a0
	WORD $0x54ffff21
	WORD $0xeb0f01bf
	WORD $0x54ffe760
	WORD $0xb4001350
	WORD $0x6f00e400
	WORD $0x927df1b0
	WORD $0x8b0e01f1
	WORD $0x8b1001ce
	WORD $0xcb1001ef
	WORD $0x8b110011
	WORD $0xfd000220
	WORD $0xb10021ef
	WORD $0xfc2a6a20
	WORD $0xfc286a20
	WORD $0x91002231
	WORD $0x54ffff61
	WORD $0xeb1001bf
	WORD $0x54ffcda1
	WORD $0x17ffff2b
	WORD $0x6f00e400
	WORD $0x8b0001cf
	WORD $0x927d05b0
	WORD $0x910041f1
	WORD $0x927be9af
	WORD $0x927be9a5
	WORD $0x8b0a0224
	WORD $0x8b080226
	WORD $0xad3f8220
	WORD $0xf10080a5
	WORD $0xad3f8080
	WORD $0x91008084
	WORD $0xad3f80c0
	WORD $0x8b0b0226
	WORD $0x91008231
	WORD $0xad3f80c0
	WORD $0x54fffee1
	WORD $0xeb0f01bf
	WORD $0x54ffe300
	WORD $0xb4000f30
	WORD $0x6f00e400
	WORD $0x927df1b0
	WORD $0x8b0b01c4
	WORD $0x8b0801c5
	WORD $0x8b0a01c6
	WORD $0x8b1001d1
	WORD $0x8b040004
	WORD $0x8b050005
	WORD $0x8b060006
	WORD $0x8b0e000e
	WORD $0x927df1a7
	WORD $0xd10020e7
	WORD $0xfc2f69c0
	WORD $0x910021ce
	WORD $0xfc2f68c0
	WORD $0xeb0701ff
	WORD $0x910020c6
	WORD $0xfc2f68a0
	WORD $0x910020a5
	WORD $0xfc2f6880
	WORD $0x91002084
	WORD $0x54fffec1
	WORD $0xeb1001bf
	WORD $0xaa1103ee
	WORD $0x54ffcd41
	WORD $0x17fffefd
	WORD $0x6f00e400
	WORD $0x8b0001cf
	WORD $0x927d05b0
	WORD $0x910041f1
	WORD $0x927be9af
	WORD $0x927be9a5
	WORD $0x8b0a0224
	WORD $0x8b090226
	WORD $0xad3f8220
	WORD $0xf10080a5
	WORD $0xad3f8080
	WORD $0x91008084
	WORD $0xad3f80c0
	WORD $0x8b0b0226
	WORD $0x91008231
	WORD $0xad3f80c0
	WORD $0x54fffee1
	WORD $0xeb0f01bf
	WORD $0x54ffdd40
	WORD $0xb40009b0
	WORD $0x6f00e400
	WORD $0x927df1b0
	WORD $0x8b0b01c4
	WORD $0x8b0901c5
	WORD $0x8b0a01c6
	WORD $0x8b1001d1
	WORD $0x8b040004
	WORD $0x8b050005
	WORD $0x8b060006
	WORD $0x8b0e000e
	WORD $0x927df1a7
	WORD $0xd10020e7
	WORD $0xfc2f69c0
	WORD $0x910021ce
	WORD $0xfc2f68c0
	WORD $0xeb0701ff
	WORD $0x910020c6
	WORD $0xfc2f68a0
	WORD $0x910020a5
	WORD $0xfc2f6880
	WORD $0x91002084
	WORD $0x54fffec1
	WORD $0xeb1001bf
	WORD $0xaa1103ee
	WORD $0x54ffc8e1
	WORD $0x17fffecf
	WORD $0x6f00e400
	WORD $0x927d05b0
	WORD $0x927be9af
	WORD $0x8b0e0011
	WORD $0x927be9a4
	WORD $0x8b0a0225
	WORD $0xac810220
	WORD $0xf1008084
	WORD $0xad0000a0
	WORD $0x8b0a00a5
	WORD $0xad0000a0
	WORD $0x8b0a00a5
	WORD $0xad0000a0
	WORD $0x8b0a00a5
	WORD $0xad0000a0
	WORD $0x54fffec1
	WORD $0xeb0f01bf
	WORD $0x54ffd7a0
	WORD $0xb4000450
	WORD $0x6f00e400
	WORD $0x927df1b0
	WORD $0x8b0e01f1
	WORD $0x8b1001ce
	WORD $0xcb1001ef
	WORD $0x8b110011
	WORD $0x8b0a0224
	WORD $0xfc008620
	WORD $0xb10021ef
	WORD $0xfd000080
	WORD $0x8b0a0084
	WORD $0xfd000080
	WORD $0x8b0a0084
	WORD $0xfd000080
	WORD $0xfc2a6880
	WORD $0x54fffee1
	WORD $0xeb1001bf
	WORD $0x54ffc581
	WORD $0x17fffea9
	WORD $0x8b0f01ce
	WORD $0x17fffea0
	WORD $0x8b0f01ce
	WORD $0x17fffdf1
	WORD $0x8b0f01ce
	WORD $0x17fffdfa
	WORD $0x8b0f01ce
	WORD $0x17fffe03
	WORD $0x8b0f01ce
	WORD $0x17fffde0
	WORD $0x8b0f01ce
	WORD $0x17fffe09
	WORD $0x8b0f01ce
	WORD $0x17fffe12
	WORD $0x8b0f01ce
	WORD $0x17fffe1b
	WORD $0x00000000
	WORD $0x00000000
	WORD $0x00000000
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x00000002
	WORD $0x00000000
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x00000006
	WORD $0x00000000
	WORD $0x00000003
	WORD $0x00000000
	WORD $0x00000006
	WORD $0x00000000
	WORD $0xcfcfcfcf
	WORD $0xcfcfcfcf
	WORD $0x33333333
	WORD $0x33333333
	WORD $0x67676767
	WORD $0x67676767
	WORD $0x19191919
	WORD $0x19191919
	WORD $0xffffff00
	WORD $0xffffff01
	WORD $0xffffff02
	WORD $0xffffff03
	WORD $0xffffff04
	WORD $0xffffff05
	WORD $0xffffff06
	WORD $0xffffff07
	WORD $0xffffff08
	WORD $0xffffff09
	WORD $0xffffff0a
	WORD $0xffffff0b
	WORD $0xffffff0c
	WORD $0xffffff0d
	WORD $0xffffff0e
	WORD $0xffffff0f
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0xffffff00
	WORD $0xffffff01
	WORD $0xffffff02
	WORD $0xffffff03
	WORD $0xffffff04
	WORD $0xffffff05
	WORD $0xffffff06
	WORD $0xffffff07
	WORD $0xffffff08
	WORD $0xffffff09
	WORD $0xffffff0a
	WORD $0xffffff0b
	WORD $0xffffff0c
	WORD $0xffffff0d
	WORD $0xffffff0e
	WORD $0xffffff0f
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x33323130
	WORD $0x37363534
	WORD $0x62613938
	WORD $0x66656463
	RET

// func jsonValidTokensNEON(b []byte, masks []uint64, stk []uint64) int
TEXT ·jsonValidTokensNEON(SB), NOSPLIT|NOFRAME, $0-80
	MOVD $ret+72(FP), R0
	MOVD b_base+0(FP), R1
	MOVD masks_base+24(FP), R2
	MOVD b_len+8(FP), R3
	MOVD stk_base+48(FP), R4
	MOVD stk_len+56(FP), R5
	WORD $0xa9bb7bfd
	WORD $0x9100fc69
	WORD $0x9280000c
	WORD $0xf940004f
	WORD $0x9346fd29
	WORD $0x9ac32190
	WORD $0xf101006d
	WORD $0xa9044ff4
	WORD $0xaa1f03f3
	WORD $0xaa1f03ea
	WORD $0x8b090c4b
	WORD $0xda90a186
	WORD $0x528d8c30
	WORD $0xa9025ff8
	WORD $0xaa1f03f8
	WORD $0x2a1f03f7
	WORD $0xf9400171
	WORD $0xaa1f03e8
	WORD $0x528005ae
	WORD $0x72acae70
	WORD $0x52800027
	WORD $0xa90167fa
	WORD $0xaa1101f1
	WORD $0xb203e7ef
	WORD $0xa90357f6
	WORD $0x8a3100d4
	WORD $0x528eadd1
	WORD $0x528e4e86
	WORD $0xb204c9ef
	WORD $0x72ad8d91
	WORD $0x72acaea6
	WORD $0x14000005
	WORD $0x2a1f03f7
	WORD $0x7100eb5f
	WORD $0xaa1503f8
	WORD $0x54002201
	WORD $0xd37ae676
	WORD $0x2a1703f9
	WORD $0xaa1803f5
	WORD $0xb5000214
	WORD $0x91000673
	WORD $0xcb1601b7
	WORD $0xeb09027f
	WORD $0x5400214a
	WORD $0xf8737974
	WORD $0xf8737858
	WORD $0x9ad7219a
	WORD $0xf10102f7
	WORD $0x91000673
	WORD $0x910102d6
	WORD $0xaa140314
	WORD $0xda9aa198
	WORD $0xea340314
	WORD $0x54fffea0
	WORD $0xd1000673
	WORD $0xdac00297
	WORD $0xd1000698
	WORD $0x7100133f
	WORD $0x8a140314
	WORD $0xdac012fd
	WORD $0xaa1603be
	WORD $0x8b1e0037
	WORD $0x394002fa
	WORD $0x54fffc20
	WORD $0x121f0738
	WORD $0x71000b1f
	WORD $0x540001a0
	WORD $0x7100173f
	WORD $0x540002c1
	WORD $0x7101f75f
	WORD $0x54000440
	WORD $0x7101775f
	WORD $0x540003a0
	WORD $0x7100b35f
	WORD $0x54001d21
	WORD $0xb4001d15
	WORD $0x531f0157
	WORD $0xaa1503f8
	WORD $0x17ffffd6
	WORD $0x71008b5f
	WORD $0x52800097
	WORD $0xaa1503f8
	WORD $0x54fffa40
	WORD $0x7101f75f
	WORD $0xaa1f03f6
	WORD $0x54001c61
	WORD $0x71000f3f
	WORD $0x54001c21
	WORD $0xb5000255
	WORD $0x140000df
	WORD $0x7101975f
	WORD $0x5400034c
	WORD $0x71016b5f
	WORD $0x540004ac
	WORD $0x71008b5f
	WORD $0x528000b7
	WORD $0xaa1503f8
	WORD $0x54fff860
	WORD $0x7100b75f
	WORD $0x54000ae0
	WORD $0x14000053
	WORD $0xb40019d5
	WORD $0x360000aa
	WORD $0x140000cc
	WORD $0xaa1f03f6
	WORD $0xb40019f5
	WORD $0x360019ca
	WORD $0xd10006b8
	WORD $0xd341fd4a
	WORD $0x528000b7
	WORD $0xf240171f
	WORD $0x54fff6a1
	WORD $0xf1000abf
	WORD $0x54fff66b
	WORD $0xd1000508
	WORD $0xf868788a
	WORD $0x17ffffb0
	WORD $0x7101cf5f
	WORD $0x540002ac
	WORD $0x71019b5f
	WORD $0x54000480
	WORD $0x7101bb5f
	WORD $0x540007a1
	WORD $0x910013d6
	WORD $0xeb0302df
	WORD $0x540016cc
	WORD $0xb94002f7
	WORD $0x6b1102ff
	WORD $0x54001300
	WORD $0x140000b2
	WORD $0x71016f5f
	WORD $0x54000400
	WORD $0x7101775f
	WORD $0x54000641
	WORD $0x7100073f
	WORD $0xaa1f03f6
	WORD $0x54001601
	WORD $0xb5fffbb5
	WORD $0x140000ae
	WORD $0x7101d35f
	WORD $0x54000480
	WORD $0x7101ef5f
	WORD $0x54000521
	WORD $0xf10006bf
	WORD $0x5400010b
	WORD $0x924016b6
	WORD $0xb50000d6
	WORD $0xeb05011f
	WORD $0x540015aa
	WORD $0x91000516
	WORD $0xf828788a
	WORD $0xaa1603e8
	WORD $0xaa0a04ea
	WORD $0x910006b8
	WORD $0x52800077
	WORD $0x17ffff89
	WORD $0x910017d6
	WORD $0xeb0302df
	WORD $0x540012ac
	WORD $0xb84012f7
	WORD $0x6b1002ff
	WORD $0x54000ee0
	WORD $0x14000091
	WORD $0xf10006bf
	WORD $0x5400010b
	WORD $0x924016b6
	WORD $0xb50000d6
	WORD $0xeb05011f
	WORD $0x5400132a
	WORD $0x91000516
	WORD $0xf828788a
	WORD $0xaa1603e8
	WORD $0xd37ff94a
	WORD $0x910006b8
	WORD $0x52800037
	WORD $0x17ffff75
	WORD $0x910013d6
	WORD $0xeb0302df
	WORD $0x5400102c
	WORD $0xb94002f7
	WORD $0x6b0602ff
	WORD $0x54000c60
	WORD $0x1400007d
	WORD $0x5100c357
	WORD $0x710026ff
	WORD $0x54000f48
	WORD $0xeb0303df
	WORD $0x7a4eb340
	WORD $0x9a9e17d8
	WORD $0x1a9f17f7
	WORD $0xeb03031f
	WORD $0x54000e8a
	WORD $0x38786839
	WORD $0x7100c33f
	WORD $0x540000a1
	WORD $0x91000716
	WORD $0xeb0302df
	WORD $0x5400024b
	WORD $0x14000034
	WORD $0x5100c738
	WORD $0x7100231f
	WORD $0x54000d48
	WORD $0x8b1603b6
	WORD $0x8b1702d6
	WORD $0x910006d6
	WORD $0xeb16007f
	WORD $0x54000900
	WORD $0x38766837
	WORD $0x910006d6
	WORD $0x5100c2f7
	WORD $0x71002aff
	WORD $0x54ffff43
	WORD $0xd10006d6
	WORD $0xeb0302df
	WORD $0x5400048a
	WORD $0x38766837
	WORD $0x7100baff
	WORD $0x54000421
	WORD $0x910006d6
	WORD $0xeb0302df
	WORD $0x54000aea
	WORD $0x38766837
	WORD $0x5100eaf7
	WORD $0x31002aff
	WORD $0x54000a63
	WORD $0x910022d7
	WORD $0xeb0302ff
	WORD $0x540001ac
	WORD $0xf8766836
	WORD $0xd204c6d8
	WORD $0x8b0f0318
	WORD $0xaa160316
	WORD $0xf201c2d8
	WORD $0xaa1703f6
	WORD $0x54fffee0
	WORD $0xdac00316
	WORD $0xdac012d6
	WORD $0x8b560ef6
	WORD $0xd10022d6
	WORD $0x1400000b
	WORD $0xeb0302df
	WORD $0x5400012a
	WORD $0x38766837
	WORD $0x5100c2f7
	WORD $0x710026ff
	WORD $0x540000a8
	WORD $0x910006d6
	WORD $0xeb16007f
	WORD $0x54ffff41
	WORD $0x1400001d
	WORD $0xeb0302df
	WORD $0x5400038a
	WORD $0x38766837
	WORD $0x321b02f7
	WORD $0x710196ff
	WORD $0x54000301
	WORD $0x910006d7
	WORD $0xeb0302ff
	WORD $0x540000ea
	WORD $0x38776838
	WORD $0x7100b71f
	WORD $0x54000060
	WORD $0x7100af1f
	WORD $0x54000041
	WORD $0x91000ad7
	WORD $0xeb0302ff
	WORD $0x5400052a
	WORD $0x38776836
	WORD $0x5100ead6
	WORD $0x31002adf
	WORD $0x540004a3
	WORD $0x38776836
	WORD $0x5100c2d6
	WORD $0x710026df
	WORD $0x540003e8
	WORD $0x910006f7
	WORD $0xeb17007f
	WORD $0x54ffff41
	WORD $0xaa0303f6
	WORD $0xb7f80396
	WORD $0xeb0302df
	WORD $0x540004ca
	WORD $0x9346fed9
	WORD $0xeb13033f
	WORD $0x540000c1
	WORD $0x9ad62694
	WORD $0x528000b7
	WORD $0xaa1503f8
	WORD $0x9ad62294
	WORD $0x17ffff03
	WORD $0x927ae6d3
	WORD $0xf8797857
	WORD $0xaa1503f8
	WORD $0xcb130073
	WORD $0x9ad32194
	WORD $0xf101027f
	WORD $0xda94a193
	WORD $0xf8797974
	WORD $0x9ad62673
	WORD $0xaa1402f4
	WORD $0x528000b7
	WORD $0x9ad62273
	WORD $0x8a340274
	WORD $0xaa1903f3
	WORD $0x17fffef4
	WORD $0xaa1703f6
	WORD $0xb6fffcd7
	WORD $0xaa1f03f6
	WORD $0x14000004
	WORD $0x7100173f
	WORD $0xfa400aa0
	WORD $0x1a9f17f6
	WORD $0xf9000016
	WORD $0xa9444ff4
	WORD $0xa94357f6
	WORD $0xa9425ff8
	WORD $0xa94167fa
	WORD $0xa8c57bfd
	WORD $0xd65f03c0
	WORD $0xf10002bf
	WORD $0x17fffff7
	WORD $0x92800016
	WORD $0x17fffff6
	RET

// func jsonValidNEON(b []byte, stk []uint64) int
TEXT ·jsonValidNEON(SB), NOSPLIT|NOFRAME, $0-56
	MOVD $ret+48(FP), R0
	MOVD b_base+0(FP), R1
	MOVD b_len+8(FP), R2
	MOVD stk_base+24(FP), R3
	MOVD stk_len+32(FP), R4
	// Constant pool appended below the body; the displacements
	// above were patched to reach it and the aligned loads made
	// unaligned, since nothing promises alignment inside a TEXT.
	WORD $0xf100045f
	WORD $0x54004fcb
	WORD $0xd10283ff
	WORD $0x9100fc4b
	WORD $0x4f01e440
	WORD $0x4f02e782
	WORD $0x4f01e403
	WORD $0x4f07e6e4
	WORD $0xa9094ff4
	WORD $0x4f00e445
	WORD $0x4f00e5a6
	WORD $0xd346fd73
	WORD $0x10004f8b
	WORD $0xf90003e4
	WORD $0xd2840024
	WORD $0x3dc05561
	WORD $0x52820227
	WORD $0xa9057bfd
	WORD $0xa90667fa
	WORD $0xaa1f03fe
	WORD $0xaa1f03ed
	WORD $0xa9075ff8
	WORD $0xaa1f03f4
	WORD $0xaa1f03e9
	WORD $0xa90857f6
	WORD $0xaa1f03ec
	WORD $0x2a1f03f6
	WORD $0xaa1f03ea
	WORD $0xaa1f03f7
	WORD $0x9280001a
	WORD $0x52800026
	WORD $0xf2e08004
	WORD $0x72a000a7
	WORD $0x92800008
	WORD $0xfd000bee
	WORD $0x6d0233ed
	WORD $0x6d032beb
	WORD $0x6d0423e9
	WORD $0xd37ae555
	WORD $0xaa1703f9
	WORD $0xf90007e8
	WORD $0xf9000fed
	WORD $0xcb15004f
	WORD $0xf10101ff
	WORD $0x5400120b
	WORD $0x8b15002b
	WORD $0xad404967
	WORD $0xad414570
	WORD $0x6e208e55
	WORD $0x6e208cf6
	WORD $0x6e228e5e
	WORD $0x6e208e14
	WORD $0x6e208e33
	WORD $0x6e228e1b
	WORD $0x6e238e3f
	WORD $0x6e238e08
	WORD $0x6e238ce9
	WORD $0x4e211eb5
	WORD $0x4e211ed6
	WORD $0x4e24860a
	WORD $0x4e211e97
	WORD $0x4e211e78
	WORD $0x4e24864b
	WORD $0x4e2484ec
	WORD $0x6e268e4d
	WORD $0x6e268e0e
	WORD $0x6e1542bc
	WORD $0x6e1642dd
	WORD $0x6e228e34
	WORD $0x6e1742f9
	WORD $0x6e18431a
	WORD $0x6e228cf3
	WORD $0x4eae1d08
	WORD $0x6e303470
	WORD $0x4e211e94
	WORD $0x4e1c3abc
	WORD $0x4e1d3add
	WORD $0x4e211f75
	WORD $0x4e193af7
	WORD $0x4e248639
	WORD $0x4e1a3b18
	WORD $0x6e238e5a
	WORD $0x4e211fd6
	WORD $0x6e2b34be
	WORD $0x6e323472
	WORD $0x4e211e73
	WORD $0x4e211e10
	WORD $0x4e71bb9b
	WORD $0x4e71bbbc
	WORD $0x6e268e3d
	WORD $0x6e3934b9
	WORD $0x4ead1f5a
	WORD $0x6e313471
	WORD $0x4e71baf7
	WORD $0x4e71bb18
	WORD $0x4e211e52
	WORD $0x4ebd1ffd
	WORD $0x6e268cff
	WORD $0x1e260390
	WORD $0x6e2a34bc
	WORD $0x6e2c34aa
	WORD $0x6e273467
	WORD $0x4ebe1f5a
	WORD $0x4e211e31
	WORD $0x1e2602ee
	WORD $0x4eb91fb9
	WORD $0x4ebf1d3d
	WORD $0x6e1542b7
	WORD $0x4ebc1d1c
	WORD $0x1e26030f
	WORD $0x6e144298
	WORD $0x1e260371
	WORD $0x6e1642db
	WORD $0x4e211ce7
	WORD $0x4eaa1fbd
	WORD $0x4e211f39
	WORD $0x4e211f5a
	WORD $0x33103dee
	WORD $0x4e211f9c
	WORD $0x6e13427e
	WORD $0x33103e30
	WORD $0x6e11423f
	WORD $0x6e104208
	WORD $0x4e211fbd
	WORD $0x6e124249
	WORD $0x6e0740ea
	WORD $0xaa0e8218
	WORD $0x92800010
	WORD $0x6e19432b
	WORD $0x6e1c438c
	WORD $0x6e1a434d
	WORD $0x4e173ab5
	WORD $0x6e1d43b7
	WORD $0x4e183a94
	WORD $0x4e1b3ad6
	WORD $0x4e1e3a73
	WORD $0x4e1f3a31
	WORD $0x4e083a10
	WORD $0x4e093a52
	WORD $0x4e0a38e7
	WORD $0x4e0b3b38
	WORD $0x4e0c3b99
	WORD $0x4e0d3b5a
	WORD $0x4e71bab5
	WORD $0x4e173bb7
	WORD $0x4e71ba94
	WORD $0x4e71bad6
	WORD $0x4e71ba73
	WORD $0x4e71ba31
	WORD $0x4e71ba10
	WORD $0x1e2602ad
	WORD $0x4e71b8e7
	WORD $0x1e26028b
	WORD $0x1e2602cf
	WORD $0x1e260271
	WORD $0x1e260228
	WORD $0x1e260204
	WORD $0x4e71ba50
	WORD $0x4e71bb11
	WORD $0x4e71bb32
	WORD $0x33103d6d
	WORD $0x4e71bb53
	WORD $0x4e71baf4
	WORD $0x33103df1
	WORD $0x33103d04
	WORD $0x1e2600ef
	WORD $0x1e26020b
	WORD $0x1e260228
	WORD $0x1e260247
	WORD $0x1e260265
	WORD $0x1e260297
	WORD $0x33103d6f
	WORD $0x33103d07
	WORD $0x33103cb7
	WORD $0xaa0d8225
	WORD $0xaa0481ee
	WORD $0xaa0782f1
	WORD $0xb40007c5
	WORD $0x8a3900ab
	WORD $0xaa0b072d
	WORD $0x8a2d016f
	WORD $0x9201f1ef
	WORD $0xab0b01eb
	WORD $0xd37ff96b
	WORD $0x1a9f37f7
	WORD $0xd200f16b
	WORD $0x8a0d0179
	WORD $0x14000035
	WORD $0x9acf234b
	WORD $0xf10005ff
	WORD $0xaa2b03f0
	WORD $0x5400344b
	WORD $0xaa0a03fa
	WORD $0xaa1303ea
	WORD $0xaa1403f3
	WORD $0xaa0303f4
	WORD $0xaa0003e8
	WORD $0xaa1f03f8
	WORD $0xaa1f03e5
	WORD $0xaa1f03f7
	WORD $0xaa1f03ee
	WORD $0xaa1f03f1
	WORD $0x8b15002d
	WORD $0x1400000e
	WORD $0x7100341f
	WORD $0x1a9f17e0
	WORD $0x9ad720e3
	WORD $0x9ad72084
	WORD $0x9ad7216b
	WORD $0x9ad72000
	WORD $0x910006f7
	WORD $0xeb1701ff
	WORD $0xaa180078
	WORD $0xaa050085
	WORD $0xaa0e016e
	WORD $0xaa110011
	WORD $0x54000200
	WORD $0x387769a0
	WORD $0x7100881f
	WORD $0x1a9f17e7
	WORD $0x7101701f
	WORD $0x1a9f17e4
	WORD $0x7100801f
	WORD $0x1a9f27eb
	WORD $0x54fffd88
	WORD $0x9ac020c3
	WORD $0xd280c01d
	WORD $0xf2c0003d
	WORD $0xea1d007f
	WORD $0x54fffce0
	WORD $0x52800020
	WORD $0x17ffffe7
	WORD $0xaa0803e0
	WORD $0xaa1403e3
	WORD $0xaa1303f4
	WORD $0xaa0a03f3
	WORD $0xaa1a03ea
	WORD $0x9280001a
	WORD $0xb5fff885
	WORD $0xaa1f03f7
	WORD $0xd2840024
	WORD $0x52820227
	WORD $0xf2e08004
	WORD $0x72a000a7
	WORD $0x8a39030b
	WORD $0xca0b056b
	WORD $0xca0b096b
	WORD $0xca0b116b
	WORD $0xca0b216b
	WORD $0xca0b416b
	WORD $0xca0b83cd
	WORD $0xca0b01b8
	WORD $0xea0e031f
	WORD $0x54002ea1
	WORD $0xaa3903eb
	WORD $0xf9400fe8
	WORD $0x8a0b00ab
	WORD $0x8a180179
	WORD $0xaa19050e
	WORD $0xea3001df
	WORD $0x54002dc1
	WORD $0xb500022e
	WORD $0xf94007e8
	WORD $0xaa11030b
	WORD $0x8a2b010b
	WORD $0xea10017d
	WORD $0x54002920
	WORD $0x910006be
	WORD $0x14000051
	WORD $0x510089ab
	WORD $0x9acb20cd
	WORD $0x7100e97f
	WORD $0x8a0401ab
	WORD $0xfa409964
	WORD $0x54002c00
	WORD $0xd10005cb
	WORD $0xea0e016e
	WORD $0x54fffe20
	WORD $0xdac001cb
	WORD $0xdac0116b
	WORD $0xaa15016b
	WORD $0x8b0b003d
	WORD $0x394003ad
	WORD $0x510189af
	WORD $0x71004dff
	WORD $0x54fffe08
	WORD $0x1acf20c8
	WORD $0x6a07011f
	WORD $0x54fffe61
	WORD $0x71004dff
	WORD $0x54fffd61
	WORD $0x9100116b
	WORD $0xeb02017f
	WORD $0x540029aa
	WORD $0x394007ab
	WORD $0x5100c16d
	WORD $0x12001dad
	WORD $0x710029bf
	WORD $0x54000123
	WORD $0x12001d6b
	WORD $0xaa1f03e5
	WORD $0x5101056b
	WORD $0x7100957f
	WORD $0x54002888
	WORD $0x9acb20cb
	WORD $0xf200157f
	WORD $0x54002820
	WORD $0x39400bab
	WORD $0x5100c16d
	WORD $0x12001dad
	WORD $0x710029bf
	WORD $0x54000123
	WORD $0x12001d6b
	WORD $0xaa1f03e5
	WORD $0x5101056b
	WORD $0x7100957f
	WORD $0x540026e8
	WORD $0x9acb20cb
	WORD $0xf200157f
	WORD $0x54002680
	WORD $0x39400fab
	WORD $0x5100c16d
	WORD $0x12001dad
	WORD $0x710029bf
	WORD $0x54000123
	WORD $0x12001d6b
	WORD $0xaa1f03e5
	WORD $0x5101056b
	WORD $0x7100957f
	WORD $0x54002548
	WORD $0x9acb20cb
	WORD $0xf200157f
	WORD $0x540024e0
	WORD $0x394013ab
	WORD $0x5100c16d
	WORD $0x12001dad
	WORD $0x710029bf
	WORD $0x54fff843
	WORD $0x12001d6b
	WORD $0xaa1f03e5
	WORD $0x5101056b
	WORD $0x7100957f
	WORD $0x540023a8
	WORD $0x9acb20cb
	WORD $0xf200157f
	WORD $0x54fff741
	WORD $0x14000119
	WORD $0xaa0b03ec
	WORD $0xb4001fdd
	WORD $0xdac003ab
	WORD $0x710012df
	WORD $0xdac0116e
	WORD $0xd10007ab
	WORD $0xaa1501d0
	WORD $0x8a1d017d
	WORD $0x8b10002d
	WORD $0x394001af
	WORD $0x540001e0
	WORD $0x121f7acb
	WORD $0x7100097f
	WORD $0x54000200
	WORD $0x710016df
	WORD $0x54000241
	WORD $0x7101f5ff
	WORD $0x540003c0
	WORD $0x710175ff
	WORD $0x540006a0
	WORD $0x7100b1ff
	WORD $0x54002041
	WORD $0xb400202c
	WORD $0x531f0136
	WORD $0x17ffffe9
	WORD $0x7100e9ff
	WORD $0x54001fa1
	WORD $0x2a1f03f6
	WORD $0x17ffffe5
	WORD $0x710089ff
	WORD $0x54000181
	WORD $0x52800096
	WORD $0x17ffffe1
	WORD $0x710195ff
	WORD $0x5400022c
	WORD $0x710169ff
	WORD $0x540003cc
	WORD $0x710089ff
	WORD $0x54000820
	WORD $0x7100b5ff
	WORD $0x54000c80
	WORD $0x14000060
	WORD $0x7101f5ff
	WORD $0xaa1f03e5
	WORD $0x54001d81
	WORD $0x71000edf
	WORD $0x54001d41
	WORD $0xaa1f03e5
	WORD $0xb4001d0c
	WORD $0x37000349
	WORD $0x140000e6
	WORD $0x7101cdff
	WORD $0x5400042c
	WORD $0x710199ff
	WORD $0x54000660
	WORD $0x7101b9ff
	WORD $0x54000a21
	WORD $0x9100120e
	WORD $0xeb0201df
	WORD $0x54001b8c
	WORD $0xb94001ab
	WORD $0x528eadc8
	WORD $0x72ad8d88
	WORD $0x6b08017f
	WORD $0x54001580
	WORD $0x140000d6
	WORD $0x71016dff
	WORD $0x540005e0
	WORD $0x710175ff
	WORD $0x54000881
	WORD $0x710006df
	WORD $0x54001a01
	WORD $0xaa1f03e5
	WORD $0xb40019ec
	WORD $0x370019c9
	WORD $0xd100058b
	WORD $0xd341fd29
	WORD $0x528000b6
	WORD $0xf240157f
	WORD $0x54fff621
	WORD $0xf100099f
	WORD $0x54fff5eb
	WORD $0xd1000694
	WORD $0xf8747869
	WORD $0x17ffffac
	WORD $0x7101d1ff
	WORD $0x54000540
	WORD $0x7101edff
	WORD $0x54000621
	WORD $0xf100059f
	WORD $0x5400012b
	WORD $0x9240158b
	WORD $0xb50000eb
	WORD $0xf94003e8
	WORD $0xeb08029f
	WORD $0x5400194a
	WORD $0x9100068b
	WORD $0xf8347869
	WORD $0xaa0b03f4
	WORD $0xaa0904c9
	WORD $0x9100058c
	WORD $0x52800076
	WORD $0x17ffff9b
	WORD $0x528000b6
	WORD $0x17ffff99
	WORD $0x9100160e
	WORD $0xeb0201df
	WORD $0x5400158c
	WORD $0xb84011ab
	WORD $0x528d8c28
	WORD $0x72acae68
	WORD $0x6b08017f
	WORD $0x54000f80
	WORD $0x140000a6
	WORD $0xf100059f
	WORD $0x5400012b
	WORD $0x9240158b
	WORD $0xb50000eb
	WORD $0xf94003e8
	WORD $0xeb08029f
	WORD $0x5400162a
	WORD $0x9100068b
	WORD $0xf8347869
	WORD $0xaa0b03f4
	WORD $0xd37ff929
	WORD $0x9100058c
	WORD $0x52800036
	WORD $0x17ffff82
	WORD $0x9100120e
	WORD $0xeb0201df
	WORD $0x540012ac
	WORD $0xb94001ab
	WORD $0x528e4e88
	WORD $0x72acaea8
	WORD $0x6b08017f
	WORD $0x54000ca0
	WORD $0x1400008f
	WORD $0x5100c1eb
	WORD $0x7100257f
	WORD $0x54001188
	WORD $0xeb02021f
	WORD $0x528005a8
	WORD $0x7a48b1e0
	WORD $0x9a90160d
	WORD $0x1a9f17eb
	WORD $0xeb0201bf
	WORD $0x540010aa
	WORD $0x386d682f
	WORD $0x7100c1ff
	WORD $0x540000a1
	WORD $0x910005ae
	WORD $0xeb0201df
	WORD $0x5400022b
	WORD $0x14000035
	WORD $0x5100c5ed
	WORD $0x710021bf
	WORD $0x54000f68
	WORD $0x8b0e03cd
	WORD $0x8b0b01ab
	WORD $0xeb0b005f
	WORD $0x54000940
	WORD $0x386b682d
	WORD $0x9100056b
	WORD $0x5100c1ad
	WORD $0x710029bf
	WORD $0x54ffff43
	WORD $0xd100056e
	WORD $0xeb0201df
	WORD $0x540004ca
	WORD $0x386e682b
	WORD $0x7100b97f
	WORD $0x54000461
	WORD $0x910005ce
	WORD $0xeb0201df
	WORD $0x54000d2a
	WORD $0x386e682b
	WORD $0x5100e96b
	WORD $0x3100297f
	WORD $0x54000ca3
	WORD $0x910021cb
	WORD $0xeb02017f
	WORD $0x540001ec
	WORD $0xf86e682d
	WORD $0xb203e7e8
	WORD $0xb204c908
	WORD $0xd204c5ae
	WORD $0x8b0801ce
	WORD $0xaa0d01cd
	WORD $0xaa0b03ee
	WORD $0xf201c1ad
	WORD $0x54fffea0
	WORD $0xdac001ad
	WORD $0xdac011ad
	WORD $0x8b4d0d6b
	WORD $0xd100216e
	WORD $0x1400000b
	WORD $0xeb0201df
	WORD $0x5400012a
	WORD $0x386e682b
	WORD $0x5100c16b
	WORD $0x7100257f
	WORD $0x540000a8
	WORD $0x910005ce
	WORD $0xeb0e005f
	WORD $0x54ffff41
	WORD $0x1400001d
	WORD $0xeb0201df
	WORD $0x5400038a
	WORD $0x386e682b
	WORD $0x321b016b
	WORD $0x7101957f
	WORD $0x54000301
	WORD $0x910005cd
	WORD $0xeb0201bf
	WORD $0x540000ea
	WORD $0x386d682b
	WORD $0x7100b57f
	WORD $0x54000060
	WORD $0x7100ad7f
	WORD $0x54000041
	WORD $0x910009cd
	WORD $0xeb0201bf
	WORD $0x5400072a
	WORD $0x386d682b
	WORD $0x5100e96b
	WORD $0x3100297f
	WORD $0x540006a3
	WORD $0x386d682b
	WORD $0x5100c16b
	WORD $0x7100257f
	WORD $0x540001e8
	WORD $0x910005ad
	WORD $0xeb0d005f
	WORD $0x54ffff41
	WORD $0xaa0203ee
	WORD $0xb7f8058e
	WORD $0xeb0201df
	WORD $0x5400072a
	WORD $0x9346fdcb
	WORD $0x9ace2348
	WORD $0xeb0a017f
	WORD $0x54000361
	WORD $0x8a1d011d
	WORD $0x528000b6
	WORD $0x17ffff0d
	WORD $0xaa0d03ee
	WORD $0xb6fffecd
	WORD $0x14000020
	WORD $0x92800008
	WORD $0x9100054a
	WORD $0x937fff1e
	WORD $0xd37fff2d
	WORD $0xeb13015f
	WORD $0x54ffb9cb
	WORD $0x14000015
	WORD $0x92800008
	WORD $0x9100054a
	WORD $0x937fff1e
	WORD $0xd37fff2d
	WORD $0xeb13015f
	WORD $0x54ffb8eb
	WORD $0x1400000e
	WORD $0xaa1f03f8
	WORD $0xaa1f03e5
	WORD $0xaa1f03ee
	WORD $0xaa1f03f1
	WORD $0xaa1f03f7
	WORD $0x17fffe8e
	WORD $0xd100056a
	WORD $0x528000b6
	WORD $0x9100054a
	WORD $0x937fff1e
	WORD $0xd37fff2d
	WORD $0xeb13015f
	WORD $0x54ffb72b
	WORD $0xb7f80098
	WORD $0x710016df
	WORD $0xfa400980
	WORD $0x14000011
	WORD $0xaa1f03e5
	WORD $0xa9494ff4
	WORD $0xfd400bee
	WORD $0xa94857f6
	WORD $0xa9475ff8
	WORD $0xa94667fa
	WORD $0xa9457bfd
	WORD $0x6d4423e9
	WORD $0x6d432beb
	WORD $0x6d4233ed
	WORD $0x910283ff
	WORD $0xf9000005
	WORD $0xd65f03c0
	WORD $0xf900001f
	WORD $0xd65f03c0
	WORD $0xf100019f
	WORD $0x1a9f17e5
	WORD $0x17fffff0
	WORD $0x92800005
	WORD $0x17ffffee
	WORD $0x00000000
	WORD $0x00000000
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x00000002
	WORD $0x00000000
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x00000006
	WORD $0x00000000
	WORD $0x00000003
	WORD $0x00000000
	WORD $0x00000006
	WORD $0x00000000
	WORD $0xcfcfcfcf
	WORD $0xcfcfcfcf
	WORD $0x33333333
	WORD $0x33333333
	WORD $0x67676767
	WORD $0x67676767
	WORD $0x19191919
	WORD $0x19191919
	WORD $0xffffff00
	WORD $0xffffff01
	WORD $0xffffff02
	WORD $0xffffff03
	WORD $0xffffff04
	WORD $0xffffff05
	WORD $0xffffff06
	WORD $0xffffff07
	WORD $0xffffff08
	WORD $0xffffff09
	WORD $0xffffff0a
	WORD $0xffffff0b
	WORD $0xffffff0c
	WORD $0xffffff0d
	WORD $0xffffff0e
	WORD $0xffffff0f
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0xffffff00
	WORD $0xffffff01
	WORD $0xffffff02
	WORD $0xffffff03
	WORD $0xffffff04
	WORD $0xffffff05
	WORD $0xffffff06
	WORD $0xffffff07
	WORD $0xffffff08
	WORD $0xffffff09
	WORD $0xffffff0a
	WORD $0xffffff0b
	WORD $0xffffff0c
	WORD $0xffffff0d
	WORD $0xffffff0e
	WORD $0xffffff0f
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x33323130
	WORD $0x37363534
	WORD $0x62613938
	WORD $0x66656463
	RET

// func jsonStage1NEON(out []uint64, masks []byte, nw int, carr []uint64, res []int64)
TEXT ·jsonStage1NEON(SB), NOSPLIT|NOFRAME, $0-104
	MOVD out_base+0(FP), R0
	MOVD masks_base+24(FP), R1
	MOVD nw+48(FP), R2
	MOVD carr_base+56(FP), R3
	MOVD res_base+80(FP), R4
	WORD $0xd101c3ff
	WORD $0x8b020448
	WORD $0xa940146a
	WORD $0xd37df04b
	WORD $0xd37cec4d
	WORD $0xd37be84c
	WORD $0xd37df109
	WORD $0xf9400868
	WORD $0xf100085f
	WORD $0xa9027bfd
	WORD $0xa90367fa
	WORD $0xa9045ff8
	WORD $0xa90557f6
	WORD $0xa9064ff4
	WORD $0xa90127eb
	WORD $0xf90007ec
	WORD $0x540009aa
	WORD $0x6f00e400
	WORD $0xaa1f03f8
	WORD $0x92800009
	WORD $0xeb18004f
	WORD $0x5400188d
	WORD $0xd37df307
	WORD $0xf9400beb
	WORD $0xd37ae711
	WORD $0xaa1f03f0
	WORD $0x8b0b00f3
	WORD $0xf94007eb
	WORD $0x8b0701a6
	WORD $0x8b07002c
	WORD $0x8b06000d
	WORD $0x8b13000e
	WORD $0x8b070162
	WORD $0xf9400feb
	WORD $0x8b060026
	WORD $0x8b020022
	WORD $0x8b070000
	WORD $0x8b130021
	WORD $0x8b0b018b
	WORD $0x14000032
	WORD $0x8a2a0267
	WORD $0xaa07054a
	WORD $0x8a2a00f4
	WORD $0x9201f294
	WORD $0xab070287
	WORD $0xd37ff8e7
	WORD $0xd200f0f4
	WORD $0x1a9f37e7
	WORD $0x8a0a028a
	WORD $0xf8707994
	WORD $0xf8707855
	WORD $0xf100013f
	WORD $0xfc7078c1
	WORD $0x8a2a0294
	WORD $0x8a2a026a
	WORD $0xca140694
	WORD $0x4e181ea1
	WORD $0xca140a94
	WORD $0xca141294
	WORD $0xca142294
	WORD $0xca144294
	WORD $0xca1480a5
	WORD $0xca3400b6
	WORD $0xca1400b4
	WORD $0xf8307814
	WORD $0x4e080ec2
	WORD $0xf8707976
	WORD $0x8a14014a
	WORD $0xaa0a0508
	WORD $0x937ffe85
	WORD $0xf83079d5
	WORD $0x8a160296
	WORD $0xf83079a8
	WORD $0x91000610
	WORD $0x4e221c21
	WORD $0xdac002d3
	WORD $0xfa404ac4
	WORD $0xd37ffd48
	WORD $0xaa0703ea
	WORD $0xdac01273
	WORD $0x4e205821
	WORD $0x8b110273
	WORD $0x91010231
	WORD $0x9a891269
	WORD $0xeb1001ff
	WORD $0x6e202821
	WORD $0x6e602821
	WORD $0x6ea06820
	WORD $0x54001040
	WORD $0xf8707833
	WORD $0xb5fff9d3
	WORD $0xaa1f03e7
	WORD $0x17ffffd5
	WORD $0x6f00e400
	WORD $0x8b090026
	WORD $0x8b010169
	WORD $0xaa1f03ef
	WORD $0xaa1f03f0
	WORD $0xaa1f03f3
	WORD $0x8b0d0031
	WORD $0x8b0c0027
	WORD $0x8b0b0014
	WORD $0x8b0d0015
	WORD $0x91002136
	WORD $0x92800009
	WORD $0xaa0503f9
	WORD $0xaa0a03f7
	WORD $0x14000051
	WORD $0xaa1f03ea
	WORD $0x8b0f003d
	WORD $0x3cef68e3
	WORD $0xf100013f
	WORD $0xa94077be
	WORD $0x8b0f000c
	WORD $0x8a3703de
	WORD $0x8a3803bd
	WORD $0x9e6703c1
	WORD $0x4e181fa1
	WORD $0x4ee18422
	WORD $0x6e211c41
	WORD $0x4f425422
	WORD $0x6e211c41
	WORD $0x4f445422
	WORD $0x6e211c41
	WORD $0x4f485422
	WORD $0x6e211c41
	WORD $0x4f505422
	WORD $0x6e211c41
	WORD $0x4f605422
	WORD $0x6e211c41
	WORD $0x3cef6a22
	WORD $0x4ec33844
	WORD $0x4ec37842
	WORD $0x9e66003d
	WORD $0xca3903be
	WORD $0xca1903b9
	WORD $0x4e080fc5
	WORD $0x4e183c3e
	WORD $0x4e251c81
	WORD $0xcab9ffdd
	WORD $0xca99ffde
	WORD $0x4e205821
	WORD $0x4e080fa4
	WORD $0x8b0f00dd
	WORD $0xa94077ab
	WORD $0xa9007999
	WORD $0x8a3700ac
	WORD $0x8a380345
	WORD $0x8b0f02b8
	WORD $0x4e241c42
	WORD $0x8a19018c
	WORD $0x8a1e00b7
	WORD $0x6e202821
	WORD $0x8a0b032b
	WORD $0xaa0c0508
	WORD $0xdac0016e
	WORD $0xfa404964
	WORD $0x93ccfeec
	WORD $0x4e205842
	WORD $0x937fffc5
	WORD $0x3caf6a83
	WORD $0x6e602821
	WORD $0xdac011cb
	WORD $0x8a1d03ce
	WORD $0xdac001dd
	WORD $0x8b10016b
	WORD $0xa9003308
	WORD $0x9a891169
	WORD $0xd37ffee8
	WORD $0x91000a78
	WORD $0x6ea06820
	WORD $0x6e202841
	WORD $0xdac013ab
	WORD $0xf100013f
	WORD $0x8b10016b
	WORD $0x91020210
	WORD $0xfa4049c4
	WORD $0x9101016b
	WORD $0x910041ef
	WORD $0x6e602821
	WORD $0x9a891169
	WORD $0x9100126b
	WORD $0xeb02017f
	WORD $0xaa1803f3
	WORD $0xaa0503f9
	WORD $0xaa0a03f7
	WORD $0x6ea06820
	WORD $0x54ffeb2c
	WORD $0x8b0f02ca
	WORD $0xf85f8145
	WORD $0xb40001a5
	WORD $0x8a3700b8
	WORD $0xaa1806f7
	WORD $0x8a37031a
	WORD $0x9201f35a
	WORD $0xab180358
	WORD $0xd37ffb18
	WORD $0xd200f31a
	WORD $0x1a9f37f8
	WORD $0x8a170357
	WORD $0xf940015a
	WORD $0xb50000ba
	WORD $0x17ffffa2
	WORD $0xaa1f03f8
	WORD $0xf940015a
	WORD $0xb4fff3fa
	WORD $0x8a38034a
	WORD $0xd37ff95d
	WORD $0xb37ff958
	WORD $0x8a3d035d
	WORD $0x9201f3bd
	WORD $0xab0a03bd
	WORD $0xd37ffbbd
	WORD $0x1a9f37ea
	WORD $0xd200f3bd
	WORD $0x8a1803b8
	WORD $0x17ffff95
	WORD $0xaa0a03e7
	WORD $0x3dc00081
	WORD $0xa9001467
	WORD $0xf9000868
	WORD $0x4ee08420
	WORD $0x3d800080
	WORD $0xb7f80089
	WORD $0xf9400888
	WORD $0xb6f80048
	WORD $0xf9000889
	WORD $0xa9464ff4
	WORD $0xa94557f6
	WORD $0xa9445ff8
	WORD $0xa94367fa
	WORD $0xa9427bfd
	WORD $0x9101c3ff
	WORD $0xd65f03c0
	RET

// func jsonCopyValidNEON(dst []byte, b []byte, html byte) int
TEXT ·jsonCopyValidNEON(SB), NOSPLIT|NOFRAME, $0-64
	MOVD $ret+56(FP), R0
	MOVD dst_base+0(FP), R1
	MOVD b_base+24(FP), R2
	MOVD b_len+32(FP), R3
	MOVBU html+48(FP), R4
	WORD $0xd10343ff
	WORD $0xf100807f
	WORD $0x6d043bef
	WORD $0x6d0533ed
	WORD $0x6d062beb
	WORD $0x6d0723e9
	WORD $0xa9087bfd
	WORD $0xa90967fa
	WORD $0xa90a5ff8
	WORD $0xa90b57f6
	WORD $0xa90c4ff4
	WORD $0xb9000fe4
	WORD $0xa9018be3
	WORD $0xf9000be1
	WORD $0x540000aa
	WORD $0xaa1f03e9
	WORD $0x52800028
	WORD $0x35002608
	WORD $0x140001af
	WORD $0x6f00e408
	WORD $0x4f01e400
	WORD $0xaa1f03e8
	WORD $0x4f01e441
	WORD $0x4f02e782
	WORD $0xf90003e0
	WORD $0x4f06e406
	WORD $0x4f07e614
	WORD $0x4f04e615
	WORD $0x4f07e696
	WORD $0x4f04e5f7
	WORD $0x4f07e7b9
	WORD $0x4f01e79a
	WORD $0x4f01e4dc
	WORD $0x4f07e45d
	WORD $0x6f00e409
	WORD $0x6f00e418
	WORD $0x6f00e41b
	WORD $0x1400000c
	WORD $0xf9400be9
	WORD $0x9101010a
	WORD $0x4ebf1fe8
	WORD $0x4ebe1fc9
	WORD $0x8b080129
	WORD $0xad00793f
	WORD $0x91008109
	WORD $0xf9400fe8
	WORD $0xeb08015f
	WORD $0xaa0903e8
	WORD $0x5400212c
	WORD $0xf94013e9
	WORD $0xb9400fee
	WORD $0x8b080129
	WORD $0x72001ddf
	WORD $0xad40793f
	WORD $0x6e218fca
	WORD $0x6e228fcb
	WORD $0x6e3e340c
	WORD $0x6e218fed
	WORD $0x6e228fee
	WORD $0x4eab1d4a
	WORD $0x6e3f340b
	WORD $0x4eae1dad
	WORD $0x4eac1d4a
	WORD $0x4eab1dab
	WORD $0x0e133d49
	WORD $0x0e1d3d4a
	WORD $0x0e1f3d4f
	WORD $0x0e133d6b
	WORD $0x0e1b3d46
	WORD $0x0e193d53
	WORD $0x0e173d55
	WORD $0x0e153d57
	WORD $0x0e0d3d59
	WORD $0x0e0b3d5d
	WORD $0x0e073d44
	WORD $0x0e053d42
	WORD $0x290727eb
	WORD $0x0e113d49
	WORD $0x0e013d4c
	WORD $0x0e113d6b
	WORD $0x0e1f3d71
	WORD $0x0e1d3d70
	WORD $0x0e1b3d65
	WORD $0x0e193d67
	WORD $0x0e173d74
	WORD $0x0e153d76
	WORD $0x0e0f3d78
	WORD $0x290627eb
	WORD $0x0e0f3d49
	WORD $0x0e0d3d7a
	WORD $0x0e0b3d7e
	WORD $0x0e093d60
	WORD $0x0e073d61
	WORD $0x0e053d63
	WORD $0x0e033d6b
	WORD $0x0e013d6d
	WORD $0x290527ea
	WORD $0x0e093d49
	WORD $0x0e033d4a
	WORD $0x54000b00
	WORD $0x1e2701aa
	WORD $0x1e27018b
	WORD $0x4e391fec
	WORD $0x6e3c8fed
	WORD $0x6e3d8fee
	WORD $0x4e391fcf
	WORD $0x6e3c8fc3
	WORD $0x4e031d6a
	WORD $0x4e031d4b
	WORD $0x6e3a8d8c
	WORD $0x4ead1dcd
	WORD $0x6e3d8fce
	WORD $0x4e051c6a
	WORD $0x4e051c4b
	WORD $0x4eac1dac
	WORD $0x4ea31dc3
	WORD $0x6e3a8dee
	WORD $0x4e071c2a
	WORD $0x4e071c8b
	WORD $0x4eae1c63
	WORD $0x4e091c0a
	WORD $0x4e091d2b
	WORD $0x2945a7ea
	WORD $0x4e0b1fca
	WORD $0x4e0b1fab
	WORD $0x4e0d1f4a
	WORD $0x4e0d1f2b
	WORD $0x4e0f1f0a
	WORD $0x4e0f1d4b
	WORD $0x4e111d2a
	WORD $0x2946a7ea
	WORD $0x4e111d4b
	WORD $0x4e131d2a
	WORD $0xb9403fe9
	WORD $0x4e131d2b
	WORD $0x4e151eca
	WORD $0xb9402be9
	WORD $0x4e151eeb
	WORD $0x4e171e8a
	WORD $0x4e171eab
	WORD $0x4e191cea
	WORD $0x4e191e6b
	WORD $0x4e1b1caa
	WORD $0x4e1b1ccb
	WORD $0x4e1d1e0a
	WORD $0x4e1d1d2b
	WORD $0x4e1f1e2a
	WORD $0x4e1f1deb
	WORD $0x4eac1d4a
	WORD $0x0e113d49
	WORD $0x0e133d4a
	WORD $0x0e1f3d4e
	WORD $0x4ea31d63
	WORD $0x0e1d3d50
	WORD $0x0e1b3d45
	WORD $0x0e193d47
	WORD $0x0e173d54
	WORD $0x0e153d56
	WORD $0x0e0f3d58
	WORD $0x0e0d3d5a
	WORD $0x0e0b3d5e
	WORD $0xb90033e9
	WORD $0x0e133c69
	WORD $0x0e093d40
	WORD $0x0e073d41
	WORD $0x0e053d43
	WORD $0x0e033d4b
	WORD $0x0e013d4d
	WORD $0x0e1f3c6f
	WORD $0x0e1d3c71
	WORD $0x0e1b3c66
	WORD $0x0e193c73
	WORD $0x0e173c75
	WORD $0x290727ea
	WORD $0x0e113c69
	WORD $0x0e153c77
	WORD $0x0e0d3c79
	WORD $0x0e0b3c7d
	WORD $0x0e073c64
	WORD $0x0e053c62
	WORD $0x0e033c6a
	WORD $0x0e013c6c
	WORD $0xb90037e9
	WORD $0x0e0f3c69
	WORD $0xb9002fe9
	WORD $0x0e093c69
	WORD $0x14000003
	WORD $0x2a1103ee
	WORD $0xb9402bf1
	WORD $0x1e2701aa
	WORD $0x1e27018b
	WORD $0x4e031d6a
	WORD $0x4e031d4b
	WORD $0x4e051c6a
	WORD $0x4e051c4b
	WORD $0x4e071c2a
	WORD $0x4e071c8b
	WORD $0x4e091c0a
	WORD $0x4e091d2b
	WORD $0x2945a7ea
	WORD $0x4e0b1fca
	WORD $0x4e0b1fab
	WORD $0x4e0d1f4a
	WORD $0x4e0d1f2b
	WORD $0x4e0f1f0a
	WORD $0x4e0f1d4b
	WORD $0x4e111d2a
	WORD $0x2946a7ea
	WORD $0x4e111d4b
	WORD $0x4e131d2a
	WORD $0xb9403fe9
	WORD $0x4e131d2b
	WORD $0x4e151eca
	WORD $0x4e151eeb
	WORD $0x4e171e8a
	WORD $0x4e171eab
	WORD $0x4e191cea
	WORD $0x4e191e6b
	WORD $0x4e1b1caa
	WORD $0x4e1b1ccb
	WORD $0x4e1d1e0a
	WORD $0x4e1d1e2b
	WORD $0x4e1f1dca
	WORD $0x4e1f1deb
	WORD $0x4eab1d43
	WORD $0x4f0f5463
	WORD $0x4e20a863
	WORD $0x6e03406a
	WORD $0x0eaa1c63
	WORD $0x9e660069
	WORD $0xb5000a49
	WORD $0x4ea81fe3
	WORD $0x4ea91fc8
	WORD $0x4ea81c63
	WORD $0x6e034068
	WORD $0x0ea81c63
	WORD $0x9e660069
	WORD $0xf201c13f
	WORD $0x54ffe700
	WORD $0x6e1f7123
	WORD $0x6e1e73eb
	WORD $0x4f06e7e4
	WORD $0x6e1e6bec
	WORD $0x6e1f692d
	WORD $0x6e1f7928
	WORD $0x6e1e7bea
	WORD $0x4f07e5a7
	WORD $0x4f05e413
	WORD $0x4f07e412
	WORD $0x6e3737d0
	WORD $0x6e3737f1
	WORD $0x6e243463
	WORD $0x6e243569
	WORD $0x4f07e5e4
	WORD $0x6e278d05
	WORD $0x6e278d47
	WORD $0x6e2435ab
	WORD $0x6e24358c
	WORD $0x4f05e7e4
	WORD $0x4eab1c63
	WORD $0x4eac1d29
	WORD $0x6e3e366b
	WORD $0x6e24354d
	WORD $0x6e24350e
	WORD $0x4f04e7e4
	WORD $0x6e328d4c
	WORD $0x6e328d12
	WORD $0x4ea31dc3
	WORD $0x6e3e36ae
	WORD $0x4ea91da9
	WORD $0x6e2437cf
	WORD $0x6e2437e4
	WORD $0x4e2c1d6b
	WORD $0x6e348d0c
	WORD $0x6e3f36ad
	WORD $0x4e251c84
	WORD $0x6e348d45
	WORD $0x4e271de7
	WORD $0x6e368d4f
	WORD $0x4e2c1dac
	WORD $0x6e368d0d
	WORD $0x4e251dc5
	WORD $0x6e3f366e
	WORD $0x4f07e7d3
	WORD $0x4e2f1e10
	WORD $0x4e2d1e31
	WORD $0x4ea41d84
	WORD $0x4e321dd2
	WORD $0x4ea71ca5
	WORD $0x4e331d4f
	WORD $0x6e36354a
	WORD $0x4e331d0d
	WORD $0x6e363508
	WORD $0x4eb01d47
	WORD $0x4e3e34d0
	WORD $0x6e268dea
	WORD $0x4eb11d11
	WORD $0x4e3f34c8
	WORD $0x6e268dac
	WORD $0x6e291e10
	WORD $0x4eaa1d69
	WORD $0x4ea51ce5
	WORD $0x6e231d03
	WORD $0x4eac1e47
	WORD $0x4ea41e24
	WORD $0x4ea91ca5
	WORD $0x4ea71c84
	WORD $0x4ebb1e07
	WORD $0x4eb81c63
	WORD $0x4ea71cbb
	WORD $0x4ea31c98
	WORD $0x17fffeef
	WORD $0xaa0803e9
	WORD $0x4ebb1f00
	WORD $0xf94003e0
	WORD $0x6e004001
	WORD $0x0ea11c00
	WORD $0x9e660008
	WORD $0xf100011f
	WORD $0x1a9f17e8
	WORD $0x34001028
	WORD $0xb4000289
	WORD $0xf1000528
	WORD $0x5400026d
	WORD $0xf94013ea
	WORD $0x38e8694a
	WORD $0x3101015f
	WORD $0x540001ea
	WORD $0xf94013ea
	WORD $0xd1000928
	WORD $0x38e8694a
	WORD $0x3101055f
	WORD $0x5400014c
	WORD $0xf100112a
	WORD $0xd1000d28
	WORD $0x540000eb
	WORD $0xf94013eb
	WORD $0x38e8696b
	WORD $0x3101017f
	WORD $0x9a88b148
	WORD $0x14000002
	WORD $0xaa1f03e8
	WORD $0xf9400fea
	WORD $0xeb0a013f
	WORD $0x5400062a
	WORD $0xb9400fea
	WORD $0x72001d5f
	WORD $0x54000400
	WORD $0xd280022b
	WORD $0x5280002a
	WORD $0xf2a2800b
	WORD $0xf2e0800b
	WORD $0xf94013ec
	WORD $0x3869698c
	WORD $0x12001d8d
	WORD $0x710081bf
	WORD $0x540004a3
	WORD $0x510089ae
	WORD $0x7100e9df
	WORD $0x54000088
	WORD $0x9ace214e
	WORD $0xea0b01df
	WORD $0x540003e1
	WORD $0x710389bf
	WORD $0x540003a0
	WORD $0xf9400bed
	WORD $0x382969ac
	WORD $0xf9400fec
	WORD $0x91000529
	WORD $0xeb09019f
	WORD $0x54fffdc1
	WORD $0xf9400fe9
	WORD $0x14000015
	WORD $0xf9400beb
	WORD $0x3829696a
	WORD $0xf9400fea
	WORD $0x91000529
	WORD $0xeb09015f
	WORD $0x54000160
	WORD $0xf94013ea
	WORD $0x3869694a
	WORD $0x12001d4b
	WORD $0x7100817f
	WORD $0x54000143
	WORD $0x7100897f
	WORD $0x54000100
	WORD $0x7101717f
	WORD $0x54fffe41
	WORD $0x14000005
	WORD $0xf9400fe9
	WORD $0x14000003
	WORD $0x8b0c0108
	WORD $0x91000508
	WORD $0xeb09011f
	WORD $0x5400070a
	WORD $0xf94013ea
	WORD $0x8b08014b
	WORD $0x39c0016a
	WORD $0x36ffff4a
	WORD $0x12001d4a
	WORD $0x5103094d
	WORD $0x710079bf
	WORD $0x54000082
	WORD $0x2a1f03ee
	WORD $0x5280002c
	WORD $0x1400000c
	WORD $0x121c0d4c
	WORD $0x7103819f
	WORD $0x54000081
	WORD $0x5280002e
	WORD $0x5280004c
	WORD $0x14000006
	WORD $0x5103c14c
	WORD $0x7100119f
	WORD $0x54000468
	WORD $0x2a1f03ee
	WORD $0x5280006c
	WORD $0x8b08018f
	WORD $0xeb0901ff
	WORD $0x540003ca
	WORD $0x39c0056f
	WORD $0x310101ff
	WORD $0x5400036a
	WORD $0x710079bf
	WORD $0x54000103
	WORD $0x39c0096d
	WORD $0x310105bf
	WORD $0x540002cc
	WORD $0x3700008e
	WORD $0x39c00d6d
	WORD $0x310101bf
	WORD $0x5400024a
	WORD $0x3940056b
	WORD $0x7103815f
	WORD $0x54000061
	WORD $0x7102817f
	WORD $0x540001a3
	WORD $0x7103b55f
	WORD $0x54000061
	WORD $0x71027d7f
	WORD $0x54000128
	WORD $0x7103c15f
	WORD $0x54000061
	WORD $0x7102417f
	WORD $0x540000a3
	WORD $0x7103d15f
	WORD $0x54fff921
	WORD $0x71023d7f
	WORD $0x54fff8e9
	WORD $0x92800009
	WORD $0xa94c4ff4
	WORD $0xf9000009
	WORD $0xa94b57f6
	WORD $0xa94a5ff8
	WORD $0xa94967fa
	WORD $0xa9487bfd
	WORD $0x6d4723e9
	WORD $0x6d462beb
	WORD $0x6d4533ed
	WORD $0x6d443bef
	WORD $0x910343ff
	WORD $0xd65f03c0
	RET

// func jsonQuoteNEON(dst []byte, b []byte, html byte) int
TEXT ·jsonQuoteNEON(SB), NOSPLIT|NOFRAME, $0-64
	MOVD $ret+56(FP), R0
	MOVD dst_base+0(FP), R1
	MOVD b_base+24(FP), R2
	MOVD b_len+32(FP), R3
	MOVBU html+48(FP), R4
	// Constant pool appended below the body; the displacements
	// above were patched to reach it and the aligned loads made
	// unaligned, since nothing promises alignment inside a TEXT.
	WORD $0xd10283ff
	WORD $0xf100047f
	WORD $0xa9057bfd
	WORD $0xa90667fa
	WORD $0xa9075ff8
	WORD $0xa90857f6
	WORD $0xa9094ff4
	WORD $0xb90027e4
	WORD $0xf90003e0
	WORD $0x540029eb
	WORD $0x10002b48
	WORD $0x6f00e400
	WORD $0x4f01e406
	WORD $0x3dc00101
	WORD $0x52800028
	WORD $0x4f01e447
	WORD $0x4e080d02
	WORD $0x100035e8
	WORD $0x4f02e790
	WORD $0xfd400503
	WORD $0x10002a08
	WORD $0x4f07e7b1
	WORD $0x3dc00904
	WORD $0x10003528
	WORD $0x4f01e792
	WORD $0x4f01e4d3
	WORD $0x4f07e454
	WORD $0xfd400105
	WORD $0x10002908
	WORD $0xaa1f03f3
	WORD $0xaa1f03e7
	WORD $0x3dc00515
	WORD $0xa9010fe2
	WORD $0xf90007e1
	WORD $0x1400000c
	WORD $0xa9408be1
	WORD $0x910080e8
	WORD $0xfd4017e0
	WORD $0xf9400fe3
	WORD $0x4e181d00
	WORD $0x8b070028
	WORD $0xad005d16
	WORD $0x4e183c07
	WORD $0x9e660013
	WORD $0xeb03027f
	WORD $0x5400258a
	WORD $0x91008268
	WORD $0xeb03011f
	WORD $0x540001ad
	WORD $0x38736848
	WORD $0x12001d09
	WORD $0x7100813f
	WORD $0x54001903
	WORD $0x7100893f
	WORD $0x54001b40
	WORD $0x7101713f
	WORD $0x54001b81
	WORD $0x4ee18400
	WORD $0x528b8b88
	WORD $0x78276828
	WORD $0x17ffffee
	WORD $0x91008268
	WORD $0xb94027fe
	WORD $0xf90017e8
	WORD $0x8b130048
	WORD $0xad405d16
	WORD $0x72001fdf
	WORD $0x6e278ef8
	WORD $0x6e308ef9
	WORD $0x6e3734da
	WORD $0x6e278edb
	WORD $0x6e308edc
	WORD $0x4eb91f18
	WORD $0x6e3634d9
	WORD $0x4ebc1f7b
	WORD $0x4eba1f18
	WORD $0x4eb91f79
	WORD $0x0e133f08
	WORD $0x0e1d3f0a
	WORD $0x0e1f3f0b
	WORD $0x0e1b3f14
	WORD $0x0e193f11
	WORD $0x0e173f06
	WORD $0x0e153f09
	WORD $0x0e0b3f00
	WORD $0x0e093f04
	WORD $0x0e073f02
	WORD $0x0e053f16
	WORD $0x0e033f18
	WORD $0xb9004be8
	WORD $0x0e113f08
	WORD $0x0e013f1a
	WORD $0x0e1f3f2d
	WORD $0x0e1d3f2c
	WORD $0x0e1b3f2f
	WORD $0x0e193f30
	WORD $0x0e173f25
	WORD $0x0e0d3f2e
	WORD $0x0e0b3f35
	WORD $0x0e093f21
	WORD $0x0e073f23
	WORD $0xb90043e8
	WORD $0x0e0f3f08
	WORD $0x0e053f37
	WORD $0x0e033f39
	WORD $0x0e013f3d
	WORD $0x2906a3ea
	WORD $0x0e153f28
	WORD $0x0e0d3f0a
	WORD $0xb9004fe8
	WORD $0x0e133f28
	WORD $0xb90047e8
	WORD $0x0e113f28
	WORD $0xb9003fe8
	WORD $0x0e0f3f28
	WORD $0x54000ae0
	WORD $0x1e2703b8
	WORD $0x1e270359
	WORD $0x6e338edb
	WORD $0x6e348edc
	WORD $0x4e311eda
	WORD $0x4e311efd
	WORD $0x6e338efe
	WORD $0x4e031f38
	WORD $0x4e031f19
	WORD $0x4ebb1f9b
	WORD $0x6e348efc
	WORD $0x6e328f5a
	WORD $0x6e328fbd
	WORD $0x4e051ef8
	WORD $0x4e051ed9
	WORD $0x4ebe1f9c
	WORD $0x4eba1f7a
	WORD $0x4ebd1f9b
	WORD $0x4e071c78
	WORD $0x4e071c59
	WORD $0x4e091c38
	WORD $0x4e091c99
	WORD $0x4e0b1eb8
	WORD $0x4e0b1c19
	WORD $0x4e0d1dd8
	WORD $0x4e0d1d59
	WORD $0x4e0f1d18
	WORD $0x294723ea
	WORD $0x4e0f1d59
	WORD $0x4e111d18
	WORD $0x294823ea
	WORD $0x4e111d59
	WORD $0x4e131d18
	WORD $0x294923ea
	WORD $0x4e131d59
	WORD $0x4e151d18
	WORD $0xb94037e8
	WORD $0x4e151d39
	WORD $0x4e171cb8
	WORD $0x4e171cd9
	WORD $0x4e191e18
	WORD $0x4e191e39
	WORD $0x4e1b1df8
	WORD $0x4e1b1e99
	WORD $0x4e1d1d98
	WORD $0x4e1d1d19
	WORD $0x4e1f1db8
	WORD $0x4e1f1d79
	WORD $0x4eba1f18
	WORD $0x0e153f08
	WORD $0x0e1f3f1e
	WORD $0x0e1d3f0c
	WORD $0x4ebb1f39
	WORD $0x0e1b3f0f
	WORD $0x0e193f10
	WORD $0x0e173f05
	WORD $0x0e0d3f0e
	WORD $0x0e0b3f15
	WORD $0x0e093f01
	WORD $0x0e073f03
	WORD $0x0e053f17
	WORD $0x0e133f2a
	WORD $0x290923ea
	WORD $0x0e133f08
	WORD $0x0e033f19
	WORD $0x0e013f1d
	WORD $0x0e1f3f2b
	WORD $0x0e1d3f2d
	WORD $0x0e1b3f34
	WORD $0x0e193f31
	WORD $0x0e173f26
	WORD $0x0e153f29
	WORD $0x0e0b3f20
	WORD $0x0e113f2a
	WORD $0x0e093f24
	WORD $0x290823ea
	WORD $0x0e113f08
	WORD $0x0e073f22
	WORD $0x0e053f36
	WORD $0x0e033f38
	WORD $0x0e013f3a
	WORD $0x0e0f3f2a
	WORD $0x290723ea
	WORD $0x0e0f3f08
	WORD $0x0e0d3f2a
	WORD $0x14000003
	WORD $0x2a0d03fe
	WORD $0xb94037ed
	WORD $0x1e2703b8
	WORD $0x1e270359
	WORD $0x4e031f38
	WORD $0x4e031f19
	WORD $0x4e051ef8
	WORD $0x4e051ed9
	WORD $0x4e071c78
	WORD $0x4e071c59
	WORD $0x4e091c38
	WORD $0x4e091c99
	WORD $0x4e0b1eb8
	WORD $0x4e0b1c19
	WORD $0x4e0d1dd8
	WORD $0x4e0d1d59
	WORD $0x4e0f1d18
	WORD $0x294723ea
	WORD $0x4e0f1d59
	WORD $0x4e111d18
	WORD $0x294823ea
	WORD $0x4e111d59
	WORD $0x4e131d18
	WORD $0x294923ea
	WORD $0x4e131d59
	WORD $0x4e151d18
	WORD $0x4e151d39
	WORD $0x4e171cb8
	WORD $0x4e171cd9
	WORD $0x4e191e18
	WORD $0x4e191e39
	WORD $0x4e1b1df8
	WORD $0x4e1b1e99
	WORD $0x4e1d1d98
	WORD $0x4e1d1db9
	WORD $0x4e1f1fd8
	WORD $0x4e1f1d79
	WORD $0x4eb91f18
	WORD $0x4f0f5718
	WORD $0x4e20ab18
	WORD $0x6e184319
	WORD $0x0eb91f18
	WORD $0x9e660308
	WORD $0xb4ffe5c8
	WORD $0x0e013ec8
	WORD $0xa9410fe2
	WORD $0xf94007e1
	WORD $0x12001d09
	WORD $0x7100813f
	WORD $0x54ffe742
	WORD $0x7100253f
	WORD $0x5400016d
	WORD $0x7100293f
	WORD $0x540005c0
	WORD $0x7100313f
	WORD $0x54000600
	WORD $0x7100353f
	WORD $0x540003c1
	WORD $0x4ee18400
	WORD $0x528e4b88
	WORD $0x78276828
	WORD $0x17ffff23
	WORD $0x7100213f
	WORD $0x54000580
	WORD $0x7100253f
	WORD $0x540002c1
	WORD $0x4ee18400
	WORD $0x528e8b88
	WORD $0x78276828
	WORD $0x17ffff1b
	WORD $0x4ee18400
	WORD $0x52844b88
	WORD $0x78276828
	WORD $0x17ffff17
	WORD $0xb94027ea
	WORD $0x72001d5f
	WORD $0x54000740
	WORD $0x12001d0a
	WORD $0x7100f95f
	WORD $0x54000408
	WORD $0x92401d0b
	WORD $0x5280002c
	WORD $0x9acb218b
	WORD $0xd2c0080c
	WORD $0xf2ea000c
	WORD $0xea0c017f
	WORD $0x54000320
	WORD $0xd344fd28
	WORD $0x92400d29
	WORD $0x1000082b
	WORD $0x9105816b
	WORD $0x4ef58400
	WORD $0x8b07002a
	WORD $0x38686968
	WORD $0x38696969
	WORD $0xbd000145
	WORD $0x39001148
	WORD $0x39001549
	WORD $0x17fffefe
	WORD $0x4ee18400
	WORD $0x528dcb88
	WORD $0x78276828
	WORD $0x17fffefa
	WORD $0x4ee18400
	WORD $0x528ccb88
	WORD $0x78276828
	WORD $0x17fffef6
	WORD $0x4ee18400
	WORD $0x528c4b88
	WORD $0x78276828
	WORD $0x17fffef2
	WORD $0x7103895f
	WORD $0x540002c1
	WORD $0x91000a69
	WORD $0xeb03013f
	WORD $0x540002ca
	WORD $0x8b13004a
	WORD $0x3940054a
	WORD $0x7102015f
	WORD $0x54000241
	WORD $0x38696849
	WORD $0x121f192a
	WORD $0x7102a15f
	WORD $0x540001c1
	WORD $0x8b070028
	WORD $0x7102a13f
	WORD $0x52800649
	WORD $0x39001109
	WORD $0x52800709
	WORD $0x4ee48400
	WORD $0x1a890529
	WORD $0xbd000103
	WORD $0x39001509
	WORD $0x17fffedb
	WORD $0x4ee28400
	WORD $0x38276828
	WORD $0x17fffed8
	WORD $0x52800029
	WORD $0x38276828
	WORD $0x4e080d36
	WORD $0x4ef68400
	WORD $0x17fffed3
	WORD $0xaa1f03e7
	WORD $0xf94003e8
	WORD $0xa9494ff4
	WORD $0xa94857f6
	WORD $0xa9475ff8
	WORD $0xf9000107
	WORD $0xa94667fa
	WORD $0xa9457bfd
	WORD $0x910283ff
	WORD $0xd65f03c0
	WORD $0x00000000
	WORD $0x00000000
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x00000002
	WORD $0x00000000
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x00000006
	WORD $0x00000000
	WORD $0x00000003
	WORD $0x00000000
	WORD $0x00000006
	WORD $0x00000000
	WORD $0xcfcfcfcf
	WORD $0xcfcfcfcf
	WORD $0x33333333
	WORD $0x33333333
	WORD $0x67676767
	WORD $0x67676767
	WORD $0x19191919
	WORD $0x19191919
	WORD $0xffffff00
	WORD $0xffffff01
	WORD $0xffffff02
	WORD $0xffffff03
	WORD $0xffffff04
	WORD $0xffffff05
	WORD $0xffffff06
	WORD $0xffffff07
	WORD $0xffffff08
	WORD $0xffffff09
	WORD $0xffffff0a
	WORD $0xffffff0b
	WORD $0xffffff0c
	WORD $0xffffff0d
	WORD $0xffffff0e
	WORD $0xffffff0f
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0xffffff00
	WORD $0xffffff01
	WORD $0xffffff02
	WORD $0xffffff03
	WORD $0xffffff04
	WORD $0xffffff05
	WORD $0xffffff06
	WORD $0xffffff07
	WORD $0xffffff08
	WORD $0xffffff09
	WORD $0xffffff0a
	WORD $0xffffff0b
	WORD $0xffffff0c
	WORD $0xffffff0d
	WORD $0xffffff0e
	WORD $0xffffff0f
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x33323130
	WORD $0x37363534
	WORD $0x62613938
	WORD $0x66656463
	WORD $0x3030755c
	WORD $0x00000000
	WORD $0x3032755c
	WORD $0x00000000
	WORD $0x00190067
	WORD $0x00190019
	WORD $0x003300cf
	WORD $0x00330033
	RET

// func jsonCopyRunNEON(dst []byte, b []byte, html byte) int
TEXT ·jsonCopyRunNEON(SB), NOSPLIT|NOFRAME, $0-64
	MOVD $ret+56(FP), R0
	MOVD dst_base+0(FP), R1
	MOVD b_base+24(FP), R2
	MOVD b_len+32(FP), R3
	MOVBU html+48(FP), R4
	WORD $0xf100807f
	WORD $0x540000aa
	WORD $0xaa1f03e8
	WORD $0xeb0303ff
	WORD $0x540023eb
	WORD $0x1400014a
	WORD $0x4f01e400
	WORD $0x4f01e441
	WORD $0x72001c9f
	WORD $0x4f02e782
	WORD $0x540005a0
	WORD $0x4f07e7a3
	WORD $0x4f01e784
	WORD $0xaa1f03e9
	WORD $0x4f01e4c5
	WORD $0x4f07e446
	WORD $0x8b090048
	WORD $0xad404107
	WORD $0x6e218cf1
	WORD $0x6e218e12
	WORD $0x6e228e13
	WORD $0x6e228cf4
	WORD $0x6e258cf5
	WORD $0x6e258e16
	WORD $0x6e268e17
	WORD $0x6e268cf8
	WORD $0x4e231e19
	WORD $0x4e231cfa
	WORD $0x4eb31e52
	WORD $0x4eb41e31
	WORD $0x6e273414
	WORD $0x4eb51f13
	WORD $0x4eb61ef5
	WORD $0x6e303416
	WORD $0x6e248f57
	WORD $0x6e248f38
	WORD $0x4eb41e31
	WORD $0x4eb61e52
	WORD $0x4eb71e73
	WORD $0x4eb81eb4
	WORD $0x4eb41e52
	WORD $0x4eb31e31
	WORD $0x4eb21e31
	WORD $0x6e114232
	WORD $0x0eb21e31
	WORD $0x9e660228
	WORD $0xb5002348
	WORD $0x9101012b
	WORD $0x91008128
	WORD $0x8b09002a
	WORD $0xeb03017f
	WORD $0xaa0803e9
	WORD $0xad004147
	WORD $0x54fffb6d
	WORD $0x1400001a
	WORD $0xaa1f03e9
	WORD $0x8b090048
	WORD $0xad401103
	WORD $0x6e218c85
	WORD $0x6e218c66
	WORD $0x6e228c67
	WORD $0x6e228c90
	WORD $0x6e233411
	WORD $0x4ea71cc6
	WORD $0x6e243407
	WORD $0x4eb01ca5
	WORD $0x4eb11cc6
	WORD $0x4ea71ca5
	WORD $0x4ea51cc5
	WORD $0x6e0540a6
	WORD $0x0ea61ca5
	WORD $0x9e6600a8
	WORD $0xb5002088
	WORD $0x9101012b
	WORD $0x91008128
	WORD $0x8b09002a
	WORD $0xeb03017f
	WORD $0xaa0803e9
	WORD $0xad001143
	WORD $0x54fffd2d
	WORD $0xeb03011f
	WORD $0x54001a0a
	WORD $0xd10203ff
	WORD $0x4f01e442
	WORD $0x4f02e783
	WORD $0x8b030049
	WORD $0xad7f0520
	WORD $0x4f01e404
	WORD $0xa9037bfd
	WORD $0x72001c9f
	WORD $0xa90467fa
	WORD $0x6e228c25
	WORD $0x6e238c26
	WORD $0x6e228c02
	WORD $0x6e238c03
	WORD $0x6e213487
	WORD $0x6e203484
	WORD $0xa9055ff8
	WORD $0x4ea61ca5
	WORD $0xa90657f6
	WORD $0x4ea31c42
	WORD $0xa9074ff4
	WORD $0x4ea71ca3
	WORD $0x4ea41c42
	WORD $0x0e1f3c69
	WORD $0x0e153c66
	WORD $0x0e133c73
	WORD $0x0e1f3c4a
	WORD $0x0e113c75
	WORD $0x0e0f3c77
	WORD $0x0e0d3c79
	WORD $0x0e0b3c7d
	WORD $0x0e073c6b
	WORD $0x0e053c6d
	WORD $0x0e033c6f
	WORD $0x0e013c71
	WORD $0x290527ea
	WORD $0x0e1d3c69
	WORD $0x0e153c47
	WORD $0x0e1d3c4a
	WORD $0x0e133c54
	WORD $0x0e113c56
	WORD $0x0e0f3c58
	WORD $0x0e0d3c5a
	WORD $0x0e0b3c5e
	WORD $0x0e073c4c
	WORD $0x0e053c4e
	WORD $0x290427ea
	WORD $0x0e1b3c69
	WORD $0x0e033c50
	WORD $0x0e1b3c4a
	WORD $0x0e013c45
	WORD $0x290327ea
	WORD $0x0e193c69
	WORD $0x0e193c4a
	WORD $0x290227ea
	WORD $0x0e173c69
	WORD $0x0e173c4a
	WORD $0x290127ea
	WORD $0x0e093c69
	WORD $0x0e093c4a
	WORD $0x54000c40
	WORD $0x1e2700a2
	WORD $0x1e270223
	WORD $0x4f01e4c4
	WORD $0x4f07e445
	WORD $0x4f07e7a6
	WORD $0x4f01e787
	WORD $0x4e031e02
	WORD $0x4e031de3
	WORD $0x6e248c10
	WORD $0x6e248c24
	WORD $0x6e258c11
	WORD $0x4e261c12
	WORD $0x6e258c25
	WORD $0x4e261c26
	WORD $0x4e051dc2
	WORD $0x4e051da3
	WORD $0x4eb01e30
	WORD $0x6e278e51
	WORD $0x4ea41ca4
	WORD $0x6e278cc6
	WORD $0x4e071d82
	WORD $0x4e071d63
	WORD $0x4eb11e05
	WORD $0x4ea61c84
	WORD $0x4e091d42
	WORD $0x4e091d23
	WORD $0x294127ea
	WORD $0x4e0b1fc2
	WORD $0x4e0b1fa3
	WORD $0x4e0d1f42
	WORD $0x4e0d1f23
	WORD $0x4e0f1f02
	WORD $0x4e0f1ee3
	WORD $0x4e111ec2
	WORD $0x4e111ea3
	WORD $0x4e131e82
	WORD $0x4e131e63
	WORD $0x4e151ce2
	WORD $0x4e151cc3
	WORD $0x4e171d42
	WORD $0x4e171d23
	WORD $0x294227ea
	WORD $0x4e191d42
	WORD $0x4e191d23
	WORD $0x294327ea
	WORD $0x4e1b1d42
	WORD $0x4e1b1d23
	WORD $0x294427ea
	WORD $0x4e1d1d42
	WORD $0x4e1d1d23
	WORD $0x294527ea
	WORD $0x4e1f1d42
	WORD $0x4e1f1d23
	WORD $0x4ea51c42
	WORD $0x4ea41c63
	WORD $0x0e1f3c49
	WORD $0x0e153c47
	WORD $0x0e133c54
	WORD $0x0e113c56
	WORD $0x0e0f3c58
	WORD $0x0e0d3c5a
	WORD $0x0e0b3c5e
	WORD $0x0e093c4a
	WORD $0x0e073c4c
	WORD $0x0e053c4e
	WORD $0x0e033c50
	WORD $0x0e013c45
	WORD $0xb9002be9
	WORD $0x0e1d3c49
	WORD $0x0e153c66
	WORD $0x0e133c73
	WORD $0x0e113c75
	WORD $0x0e0f3c77
	WORD $0x0e0d3c79
	WORD $0x0e0b3c7d
	WORD $0x0e073c6b
	WORD $0x0e053c6d
	WORD $0x0e033c6f
	WORD $0x0e013c71
	WORD $0xb90023e9
	WORD $0x0e1b3c49
	WORD $0xb9001be9
	WORD $0x0e193c49
	WORD $0xb90013e9
	WORD $0x0e173c49
	WORD $0xb9000be9
	WORD $0x0e1f3c69
	WORD $0xb9002fe9
	WORD $0x0e1d3c69
	WORD $0xb90027e9
	WORD $0x0e1b3c69
	WORD $0xb9001fe9
	WORD $0x0e193c69
	WORD $0xb90017e9
	WORD $0x0e173c69
	WORD $0xb9000fe9
	WORD $0x0e093c69
	WORD $0x1e2700a2
	WORD $0x1e270223
	WORD $0x4e031e02
	WORD $0x4e031de3
	WORD $0x4e051dc2
	WORD $0x4e051da3
	WORD $0x4e071d82
	WORD $0x4e071d63
	WORD $0x4e091d42
	WORD $0x4e091d23
	WORD $0x294127ea
	WORD $0x4e0b1fc2
	WORD $0x4e0b1fa3
	WORD $0xa9437bfd
	WORD $0x4e0d1f42
	WORD $0x4e0d1f23
	WORD $0xa94467fa
	WORD $0x4e0f1f02
	WORD $0x4e0f1ee3
	WORD $0xa9455ff8
	WORD $0x4e111ec2
	WORD $0x4e111ea3
	WORD $0xa94657f6
	WORD $0x4e131e82
	WORD $0x4e131e63
	WORD $0xa9474ff4
	WORD $0x4e151ce2
	WORD $0x4e151cc3
	WORD $0x4e171d42
	WORD $0x4e171d23
	WORD $0x294227ea
	WORD $0x4e191d42
	WORD $0x4e191d23
	WORD $0x294327ea
	WORD $0x4e1b1d42
	WORD $0x4e1b1d23
	WORD $0x294427ea
	WORD $0x4e1d1d42
	WORD $0x4e1d1d23
	WORD $0x294527ea
	WORD $0x4e1f1d42
	WORD $0x4e1f1d23
	WORD $0x4ea31c42
	WORD $0x4f0f5442
	WORD $0x4e20a842
	WORD $0x6e024043
	WORD $0x0ea31c42
	WORD $0x9e660049
	WORD $0x910203ff
	WORD $0xb4000629
	WORD $0xeb03011f
	WORD $0x540005aa
	WORD $0x72001c9f
	WORD $0x540003e0
	WORD $0xd280022a
	WORD $0x52800029
	WORD $0xf2a2800a
	WORD $0xf2e0800a
	WORD $0x3868684b
	WORD $0x12001d6c
	WORD $0x7100819f
	WORD $0x54000463
	WORD $0x5100898d
	WORD $0x7100e9bf
	WORD $0x54000088
	WORD $0x9acd212d
	WORD $0xea0a01bf
	WORD $0x540003a1
	WORD $0x7103899f
	WORD $0x54000360
	WORD $0x3828682b
	WORD $0x91000508
	WORD $0xeb08007f
	WORD $0x54fffe21
	WORD $0xf9000003
	WORD $0xd65f03c0
	WORD $0x7100895f
	WORD $0x54000260
	WORD $0x7101715f
	WORD $0x54000220
	WORD $0x38286829
	WORD $0x91000508
	WORD $0xeb08007f
	WORD $0x54fffee0
	WORD $0x38686849
	WORD $0x12001d2a
	WORD $0x7100815f
	WORD $0x54fffea2
	WORD $0x14000008
	WORD $0xaa0903e8
	WORD $0xeb03013f
	WORD $0x540000aa
	WORD $0x17ffffd8
	WORD $0xaa0903e8
	WORD $0xeb03013f
	WORD $0x54fffaab
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x8b030028
	WORD $0xf9000003
	WORD $0xad3f0500
	WORD $0xd65f03c0
	RET

// func b64EncodeNEON(dst []byte, b []byte) int
TEXT ·b64EncodeNEON(SB), NOSPLIT|NOFRAME, $0-56
	MOVD $ret+48(FP), R0
	MOVD dst_base+0(FP), R1
	MOVD b_base+24(FP), R2
	MOVD dst_len+8(FP), R3
	MOVD b_len+32(FP), R4
	// Constant pool appended below the body; the displacements
	// above were patched to reach it and the aligned loads made
	// unaligned, since nothing promises alignment inside a TEXT.
	WORD $0xb200f3e8
	WORD $0xf28aaac8
	WORD $0x9b487c88
	WORD $0x8b48fd0a
	WORD $0x8b0a054b
	WORD $0xeb0b0089
	WORD $0x9a8a0548
	WORD $0xd37ef508
	WORD $0xeb08007f
	WORD $0x5400008a
	WORD $0x92800008
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0xf1000c9f
	WORD $0x540000aa
	WORD $0xeb0b009f
	WORD $0x54001921
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0xfc1e0fea
	WORD $0xf100215f
	WORD $0x6d0123e9
	WORD $0x54000062
	WORD $0xaa1f03ec
	WORD $0x1400008f
	WORD $0xf100415f
	WORD $0x54000862
	WORD $0xaa1f03ec
	WORD $0xaa0c03ee
	WORD $0x0f01e600
	WORD $0x0f01e781
	WORD $0x0f01e7e2
	WORD $0x4f01e7e3
	WORD $0x8b0c058f
	WORD $0x4f01e7c4
	WORD $0x4f02e425
	WORD $0x10001d8d
	WORD $0x4f02e4e6
	WORD $0x4f07e787
	WORD $0x10001d30
	WORD $0x4f01e570
	WORD $0x4f01e5f1
	WORD $0x927de94c
	WORD $0x4f01e672
	WORD $0x4f00e733
	WORD $0x3dc00db4
	WORD $0x8b0e082d
	WORD $0x3dc01215
	WORD $0xcb0c01ce
	WORD $0x8b0f004f
	WORD $0x0cdf41f6
	WORD $0xb10021ce
	WORD $0x0f0a56fa
	WORD $0x0f0c56d9
	WORD $0x0e221f1c
	WORD $0x2f0e06dd
	WORD $0x0e211f5a
	WORD $0x0e201f39
	WORD $0x1e60435b
	WORD $0x2f0c16f9
	WORD $0x6e18079a
	WORD $0x6e18073d
	WORD $0x2f0a171b
	WORD $0x6e180736
	WORD $0x6e18079b
	WORD $0x6e3536de
	WORD $0x6e3436d6
	WORD $0x6e323758
	WORD $0x6e33377c
	WORD $0x6e651cde
	WORD $0x6e248f79
	WORD $0x4ebc1f97
	WORD $0x6e7e1cf6
	WORD $0x6e651cd7
	WORD $0x4e3d86d6
	WORD $0x6eb81cf7
	WORD $0x6e248fb8
	WORD $0x6eb81e16
	WORD $0x6e238fb8
	WORD $0x4e3b86f7
	WORD $0x4eb81f1a
	WORD $0x6eb91e17
	WORD $0x6e238f79
	WORD $0x6e761e3a
	WORD $0x4eb91f3c
	WORD $0x6e771e3c
	WORD $0x6e1a435b
	WORD $0x6e1c439d
	WORD $0x0c9f01ba
	WORD $0x54fffb21
	WORD $0xeb0c015f
	WORD $0x54000981
	WORD $0x14000079
	WORD $0x4f03e4e0
	WORD $0x4f02e421
	WORD $0x927ce54c
	WORD $0x4f02e4e2
	WORD $0x4f06e5e3
	WORD $0xaa0203ed
	WORD $0x4f07e784
	WORD $0x4f01e605
	WORD $0xaa0103ee
	WORD $0x4f00e726
	WORD $0x4f01e667
	WORD $0x927ce54f
	WORD $0x4f01e790
	WORD $0x4f01e7f1
	WORD $0x4f01e7d2
	WORD $0x4f01e573
	WORD $0x4f01e5f4
	WORD $0x4cdf41b5
	WORD $0xf10041ef
	WORD $0x4f0a56d9
	WORD $0x4f0c56b8
	WORD $0x6e2036bc
	WORD $0x4e311efb
	WORD $0x6e2336a8
	WORD $0x4e301f39
	WORD $0x4e251f18
	WORD $0x6e611c5c
	WORD $0x6e26377e
	WORD $0x6e27376a
	WORD $0x4eb91f3a
	WORD $0x6f0c16d8
	WORD $0x6e273739
	WORD $0x6ea81c9c
	WORD $0x6f0e06a8
	WORD $0x6e611c5e
	WORD $0x6f0a16fa
	WORD $0x6e26371d
	WORD $0x6e273709
	WORD $0x6f0e16bc
	WORD $0x6e328d15
	WORD $0x6eaa1c9e
	WORD $0x6e26375f
	WORD $0x6e611c5d
	WORD $0x6e328f17
	WORD $0x6e7c1e75
	WORD $0x6e328f5c
	WORD $0x6e611c5f
	WORD $0x4e3b87de
	WORD $0x6ea91c9d
	WORD $0x6e7f1c99
	WORD $0x6e318d1f
	WORD $0x4e3887b6
	WORD $0x6e328f7d
	WORD $0x6e318f7b
	WORD $0x4e3a8739
	WORD $0x6eb71e76
	WORD $0x6e318f17
	WORD $0x4ebd1fb8
	WORD $0x6ebc1e79
	WORD $0x4ebf1ffc
	WORD $0x6e7e1e78
	WORD $0x6e751e9c
	WORD $0x6e318f55
	WORD $0x4eb71efd
	WORD $0x6e761e9d
	WORD $0x4ebb1f7f
	WORD $0x6e781e9f
	WORD $0x4eb51ebe
	WORD $0x6e791e9e
	WORD $0x4c9f01dc
	WORD $0x54fff961
	WORD $0xeb0c015f
	WORD $0x54000600
	WORD $0x371feeca
	WORD $0x8b0c058d
	WORD $0x0f028420
	WORD $0x0f0284e1
	WORD $0x0f078782
	WORD $0x0f0187c3
	WORD $0x1000180f
	WORD $0x0f0187e4
	WORD $0x0f018565
	WORD $0x8b0201ae
	WORD $0x0f0185e6
	WORD $0x1000176d
	WORD $0xfd400df0
	WORD $0xfd4009a7
	WORD $0x8b0c082d
	WORD $0xcb0c014c
	WORD $0x910009ce
	WORD $0x385ff1cf
	WORD $0x385fe1d0
	WORD $0xf100058c
	WORD $0x384035c3
	WORD $0x53047df1
	WORD $0x1e270211
	WORD $0x53067c65
	WORD $0x331c0611
	WORD $0x331e0de5
	WORD $0x531e0def
	WORD $0x4e061e31
	WORD $0x12001471
	WORD $0x4e0a1cb1
	WORD $0x4e0e1e31
	WORD $0x4eb11e32
	WORD $0x2e673633
	WORD $0x4e0a1df2
	WORD $0x2e601c33
	WORD $0x53027e0f
	WORD $0x4e021df1
	WORD $0x2e703652
	WORD $0x2e731c52
	WORD $0x2e638e33
	WORD $0x0e718652
	WORD $0x2e648e31
	WORD $0x2eb31cb2
	WORD $0x2e721cd1
	WORD $0x0e001a31
	WORD $0x0d9f81b1
	WORD $0x54fffc61
	WORD $0x6d4123e9
	WORD $0xfc4207ea
	WORD $0xeb0b009f
	WORD $0x54ffe720
	WORD $0x8b0b004b
	WORD $0xf100093f
	WORD $0x3940016d
	WORD $0x54000061
	WORD $0x3940056b
	WORD $0x14000002
	WORD $0x2a1f03eb
	WORD $0x71019dbf
	WORD $0x5280082c
	WORD $0x528008ef
	WORD $0x53027dae
	WORD $0x1a8c81f0
	WORD $0x71033dbf
	WORD $0x12800071
	WORD $0x53047d62
	WORD $0x8b0a082a
	WORD $0x1a908230
	WORD $0x7100f9df
	WORD $0x0b0e0210
	WORD $0x331c05a2
	WORD $0x5280056d
	WORD $0x1a9001b0
	WORD $0x7100fddf
	WORD $0x528005ee
	WORD $0x1a9001d0
	WORD $0x7100645f
	WORD $0x1a8c81ef
	WORD $0x7100cc5f
	WORD $0x39000150
	WORD $0x1a8f822f
	WORD $0x7100f85f
	WORD $0x0b0201ef
	WORD $0x1a8f01ad
	WORD $0x7100fc5f
	WORD $0x1a8d01cd
	WORD $0xf100093f
	WORD $0x528007a9
	WORD $0x3900054d
	WORD $0x528007ad
	WORD $0x54000121
	WORD $0x531e0d6d
	WORD $0x331e0d6c
	WORD $0x11011dab
	WORD $0x710065bf
	WORD $0x1a8c816b
	WORD $0x1103f1ac
	WORD $0x7100cdbf
	WORD $0x1a8b818d
	WORD $0x3900094d
	WORD $0x39000d49
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x00000000
	WORD $0x00000000
	WORD $0x00000000
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x00000002
	WORD $0x00000000
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x00000006
	WORD $0x00000000
	WORD $0x00000003
	WORD $0x00000000
	WORD $0x00000006
	WORD $0x00000000
	WORD $0xcfcfcfcf
	WORD $0xcfcfcfcf
	WORD $0x33333333
	WORD $0x33333333
	WORD $0x67676767
	WORD $0x67676767
	WORD $0x19191919
	WORD $0x19191919
	WORD $0xffffff00
	WORD $0xffffff01
	WORD $0xffffff02
	WORD $0xffffff03
	WORD $0xffffff04
	WORD $0xffffff05
	WORD $0xffffff06
	WORD $0xffffff07
	WORD $0xffffff08
	WORD $0xffffff09
	WORD $0xffffff0a
	WORD $0xffffff0b
	WORD $0xffffff0c
	WORD $0xffffff0d
	WORD $0xffffff0e
	WORD $0xffffff0f
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0xffffff00
	WORD $0xffffff01
	WORD $0xffffff02
	WORD $0xffffff03
	WORD $0xffffff04
	WORD $0xffffff05
	WORD $0xffffff06
	WORD $0xffffff07
	WORD $0xffffff08
	WORD $0xffffff09
	WORD $0xffffff0a
	WORD $0xffffff0b
	WORD $0xffffff0c
	WORD $0xffffff0d
	WORD $0xffffff0e
	WORD $0xffffff0f
	WORD $0x0c080400
	WORD $0x1c181410
	WORD $0x2c282420
	WORD $0x3c383430
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x08040201
	WORD $0x80402010
	WORD $0x33323130
	WORD $0x37363534
	WORD $0x62613938
	WORD $0x66656463
	WORD $0x3030755c
	WORD $0x00000000
	WORD $0x3032755c
	WORD $0x00000000
	WORD $0x00190067
	WORD $0x00190019
	WORD $0x003300cf
	WORD $0x00330033
	RET

// func b64DecodeNEON(dst []byte, b []byte) int
TEXT ·b64DecodeNEON(SB), NOSPLIT|NOFRAME, $0-56
	MOVD $ret+48(FP), R0
	MOVD dst_base+0(FP), R1
	MOVD b_base+24(FP), R2
	MOVD dst_len+8(FP), R3
	MOVD b_len+32(FP), R4
	WORD $0xf240049f
	WORD $0x54000080
	WORD $0x92800008
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0xb4000404
	WORD $0x8b040048
	WORD $0x385ff10a
	WORD $0x7100f55f
	WORD $0x1a9f17e9
	WORD $0xf100089f
	WORD $0x540000eb
	WORD $0x385fe108
	WORD $0x7100f55f
	WORD $0x5280002a
	WORD $0x9a8a154a
	WORD $0x7100f51f
	WORD $0x9a890149
	WORD $0x9342fc8b
	WORD $0x8b840568
	WORD $0xcb090108
	WORD $0xeb08007f
	WORD $0x54fffd8b
	WORD $0xf100097f
	WORD $0xd100056a
	WORD $0x5400260b
	WORD $0xd10583ff
	WORD $0xf100857f
	WORD $0x6d113bef
	WORD $0x6d1233ed
	WORD $0x6d132beb
	WORD $0x6d1423e9
	WORD $0xf900abfd
	WORD $0x540000c2
	WORD $0x2a1f03ed
	WORD $0xaa1f03ec
	WORD $0x140000e4
	WORD $0xf900001f
	WORD $0xd65f03c0
	WORD $0x6f00e400
	WORD $0x6f00e401
	WORD $0x927be94c
	WORD $0x4f05e7ea
	WORD $0x4f04e7ef
	WORD $0xaa0203ed
	WORD $0x4f05e729
	WORD $0x4f06e60d
	WORD $0xaa0103ef
	WORD $0x4f02e40e
	WORD $0x4f00e54b
	WORD $0x927be94e
	WORD $0xad0787e0
	WORD $0xaa0d03f0
	WORD $0x4f00e485
	WORD $0x4cdf021c
	WORD $0x4f00e743
	WORD $0x4c400216
	WORD $0x4f00e493
	WORD $0x4f01e5f2
	WORD $0x4f00e495
	WORD $0x910181f0
	WORD $0xf10081ce
	WORD $0x910201ad
	WORD $0x4e258785
	WORD $0x4e2a8782
	WORD $0x4e2f8781
	WORD $0x4e2986c6
	WORD $0x4e2a86da
	WORD $0x4e2a87b0
	WORD $0x4e2f86c0
	WORD $0x4e298787
	WORD $0x4e2d879b
	WORD $0x6e223464
	WORD $0x6e213461
	WORD $0x4e2a86ec
	WORD $0xad049be5
	WORD $0x4f01e565
	WORD $0x4f01e566
	WORD $0x4e2f87b1
	WORD $0x6ee41dc2
	WORD $0x4e2d87a4
	WORD $0x6e2c3468
	WORD $0x6e258f85
	WORD $0x6e313471
	WORD $0x3d8037e5
	WORD $0x4f01e5e5
	WORD $0x6e258f85
	WORD $0x3d803be5
	WORD $0x4e2987a5
	WORD $0xad0313e5
	WORD $0x4e3387a4
	WORD $0x6e268fa5
	WORD $0x4ea11c33
	WORD $0x4e2d87c1
	WORD $0x4e2a8706
	WORD $0x3d8023e4
	WORD $0x6e3a3464
	WORD $0x3d802fe5
	WORD $0x6e328fa5
	WORD $0x6e303472
	WORD $0x6e621cf3
	WORD $0x4e2987c2
	WORD $0x3d8003e1
	WORD $0x4ea41c94
	WORD $0x4e3587c1
	WORD $0x4f00e484
	WORD $0x6ef21dd0
	WORD $0x6e203472
	WORD $0x4f01e560
	WORD $0x6e3b3575
	WORD $0x4e2f87fb
	WORD $0x3d8033e5
	WORD $0x6e6e1f54
	WORD $0x4f01e57a
	WORD $0xad010be1
	WORD $0x4e2d87e2
	WORD $0x4e2487e4
	WORD $0x4e2a87c5
	WORD $0x6e208fc1
	WORD $0x4f01e5e0
	WORD $0x4e2f87c7
	WORD $0x6e3a8ffa
	WORD $0x6e223562
	WORD $0x6e273467
	WORD $0x6e208fc0
	WORD $0x3d8007fa
	WORD $0x4f01e5fa
	WORD $0xad0203e1
	WORD $0x4e2a87e0
	WORD $0x4e2987e1
	WORD $0x6e3a8ffc
	WORD $0x3dc02bff
	WORD $0x6e25347e
	WORD $0x6e26347d
	WORD $0x4e2d86fa
	WORD $0x6e741ff2
	WORD $0x3dc027f4
	WORD $0x4e2d86df
	WORD $0x6efe1dc5
	WORD $0x4e2986fe
	WORD $0x6eb51e93
	WORD $0x3dc01bf5
	WORD $0x4e2f86f4
	WORD $0x6efd1dc6
	WORD $0x4e2f871d
	WORD $0x6e3a357a
	WORD $0x6eb11eb0
	WORD $0x4ea81d11
	WORD $0x6e203475
	WORD $0x6e343474
	WORD $0x4e298708
	WORD $0x6e3d347d
	WORD $0x6e6e1d91
	WORD $0x6ef51dc0
	WORD $0x3dc01ff5
	WORD $0x4e2a872c
	WORD $0x6e353575
	WORD $0x6ebd1d06
	WORD $0x6eb41fd1
	WORD $0xad4153fd
	WORD $0x4e29873e
	WORD $0x6ea71e85
	WORD $0x3dc023f4
	WORD $0x6e3b3467
	WORD $0x6e2c347b
	WORD $0x6eb51e90
	WORD $0x3dc003f5
	WORD $0x6e3f3574
	WORD $0x4f00e49f
	WORD $0x6ea71c20
	WORD $0x4e2f8727
	WORD $0x6e353575
	WORD $0x4e2d8701
	WORD $0x6e6e1d9b
	WORD $0x6e273467
	WORD $0x4e2d8723
	WORD $0x6eb51fa5
	WORD $0x4e3f86d5
	WORD $0x6ea21c80
	WORD $0x3dc013e4
	WORD $0x4e3f86fd
	WORD $0x6e213561
	WORD $0x4e3f8702
	WORD $0x6e7b1fc7
	WORD $0x4f01e57b
	WORD $0x6eb41eb2
	WORD $0x4f01e7d5
	WORD $0x6eba1fb1
	WORD $0x3dc007fa
	WORD $0x6e661c41
	WORD $0x6e233562
	WORD $0x4e3f8723
	WORD $0x6ea41ea5
	WORD $0x6eba1ea0
	WORD $0x4f01e7fa
	WORD $0x6e671c62
	WORD $0xad458fe4
	WORD $0x6ea41eb0
	WORD $0x4f01e564
	WORD $0x6ebc1f40
	WORD $0x6e248ed4
	WORD $0x6e248ee6
	WORD $0x6e248f04
	WORD $0x6e701f43
	WORD $0x6eb41eb2
	WORD $0x3dc037f4
	WORD $0x6ea41ea1
	WORD $0x6e3b8f24
	WORD $0x4f01e5fb
	WORD $0x6e711ea6
	WORD $0x6eb41eb3
	WORD $0x3dc017f4
	WORD $0x6eb41f45
	WORD $0x4f01e5f4
	WORD $0x6ea41ea2
	WORD $0x6e3b8f35
	WORD $0x6e348ec7
	WORD $0x6e348ef0
	WORD $0x6e348f11
	WORD $0x3dc03bf4
	WORD $0x6f0e04a4
	WORD $0x6eb51f42
	WORD $0xad47dbf5
	WORD $0x6eb41f53
	WORD $0x6e721f47
	WORD $0x4f0e54b2
	WORD $0x4f0c5474
	WORD $0x6eb01f46
	WORD $0x6eb11f41
	WORD $0x4ea01e52
	WORD $0x4eb41c91
	WORD $0x6f0c0464
	WORD $0x4f0a5674
	WORD $0x4eb51cf5
	WORD $0x4eb61e73
	WORD $0x4ea11cd6
	WORD $0x4ea51c63
	WORD $0x4f0e5425
	WORD $0x6f0e0421
	WORD $0x4eb41c90
	WORD $0x4f0c54c4
	WORD $0x6f0c04c6
	WORD $0x4f0a54e7
	WORD $0x4eb61eb4
	WORD $0x4ea31e63
	WORD $0x4ea21cb7
	WORD $0x4ea41c36
	WORD $0x4c9f41f0
	WORD $0x4ea71cd5
	WORD $0x4ea01c61
	WORD $0x4ea21e80
	WORD $0x4c0041f5
	WORD $0xaa1003ef
	WORD $0x54ffe6c1
	WORD $0x4ea01c20
	WORD $0xeb0c015f
	WORD $0x6e004001
	WORD $0x0ea11c00
	WORD $0x9e66000d
	WORD $0xaa4d81ad
	WORD $0xaa4d41ad
	WORD $0xd348fdae
	WORD $0x2a0e01ad
	WORD $0x54000760
	WORD $0x8b0c058e
	WORD $0x0f0587e0
	WORD $0x0f008741
	WORD $0x0f028402
	WORD $0x0f0487e3
	WORD $0xaa2c03ef
	WORD $0x0f058724
	WORD $0x0f068605
	WORD $0x8b0101ce
	WORD $0x0f008546
	WORD $0x0f008487
	WORD $0x8b0c084c
	WORD $0x0f018570
	WORD $0x0f0185f1
	WORD $0x8b0b01eb
	WORD $0x0f0187d2
	WORD $0x0f0187f3
	WORD $0x910009ce
	WORD $0xbc404594
	WORD $0xf100056b
	WORD $0x2e341015
	WORD $0x2e341077
	WORD $0x2e3410b8
	WORD $0x1e6042b6
	WORD $0x2f00b437
	WORD $0x2f00b438
	WORD $0x2e773437
	WORD $0x2f00b436
	WORD $0x2e763436
	WORD $0x2ef61c55
	WORD $0x2e341096
	WORD $0x2eb71ed5
	WORD $0x2e3410f6
	WORD $0x2e7834d7
	WORD $0x2f08a694
	WORD $0x2eb71ed5
	WORD $0x2e708e96
	WORD $0x2e718e94
	WORD $0x2eb61e55
	WORD $0x2e751e74
	WORD $0x0e063e8f
	WORD $0x0e023e91
	WORD $0x0e0a3e90
	WORD $0x9e660285
	WORD $0x0e0e3e83
	WORD $0x53041de4
	WORD $0x53021e06
	WORD $0x2a101870
	WORD $0x2a110891
	WORD $0xaa4580a4
	WORD $0x2a0f10cf
	WORD $0x381fe1d1
	WORD $0xd350fc91
	WORD $0x2a0d008d
	WORD $0x381ff1cf
	WORD $0x2a1101ad
	WORD $0x380035d0
	WORD $0x54fffb21
	WORD $0x6d5423e9
	WORD $0xf940abfd
	WORD $0x6d532beb
	WORD $0x6d5233ed
	WORD $0x6d513bef
	WORD $0x910583ff
	WORD $0x3737d74d
	WORD $0x8b0a084b
	WORD $0x0f0507e1
	WORD $0x0f0407e2
	WORD $0x0f000744
	WORD $0x0f060605
	WORD $0xf100053f
	WORD $0x7d400160
	WORD $0x0f020406
	WORD $0x0f050727
	WORD $0x2f08a400
	WORD $0x2e601021
	WORD $0x2e601042
	WORD $0x2e6010a5
	WORD $0x1e604023
	WORD $0x2f003422
	WORD $0x2f003425
	WORD $0x2ea23482
	WORD $0x2f003423
	WORD $0x2ea33483
	WORD $0x0f000544
	WORD $0x2ee31cc1
	WORD $0x2e6010e3
	WORD $0x0f000486
	WORD $0x2ea21c61
	WORD $0x2ea53482
	WORD $0x2e6010c3
	WORD $0x0f010564
	WORD $0x2f10a400
	WORD $0x2ea21c61
	WORD $0x0f0107c2
	WORD $0x2ea48c03
	WORD $0x0f0105e4
	WORD $0x2ea31c41
	WORD $0x0f0107e2
	WORD $0x2ea48c00
	WORD $0x2e611c40
	WORD $0x54000308
	WORD $0x3940096c
	WORD $0x5280080d
	WORD $0x5101058e
	WORD $0x5101858f
	WORD $0x51011d90
	WORD $0x710069df
	WORD $0x1a8d31ce
	WORD $0x710069ff
	WORD $0x5100c18f
	WORD $0x1a8e320e
	WORD $0x11001190
	WORD $0x710029ff
	WORD $0x1a8e320f
	WORD $0x7100ad9f
	WORD $0x528007ce
	WORD $0x1a8f01d0
	WORD $0x7100bd9f
	WORD $0x528007ef
	WORD $0x1a9001ec
	WORD $0xf100053f
	WORD $0x540001a1
	WORD $0x2a1f03eb
	WORD $0x14000003
	WORD $0x2a1f03eb
	WORD $0x2a1f03ec
	WORD $0x2a1f03ed
	WORD $0x1e26000f
	WORD $0x0e0c3c0e
	WORD $0x2a0c01ef
	WORD $0x2a0f03ef
	WORD $0x2a0e01ef
	WORD $0x3737ceaf
	WORD $0x14000019
	WORD $0x39400d6b
	WORD $0x51010570
	WORD $0x51018571
	WORD $0x71006a1f
	WORD $0x1a8d320d
	WORD $0x51011d70
	WORD $0x71006a3f
	WORD $0x5100c171
	WORD $0x1a8d320d
	WORD $0x11001170
	WORD $0x71002a3f
	WORD $0x1a8d320d
	WORD $0x7100ad7f
	WORD $0x1a8d01cd
	WORD $0x7100bd7f
	WORD $0x1a8d01eb
	WORD $0x12001d6d
	WORD $0x5280002b
	WORD $0x1e26000f
	WORD $0x0e0c3c0e
	WORD $0x2a0c01ef
	WORD $0x2a0f01af
	WORD $0x2a0e01ef
	WORD $0x3737cb8f
	WORD $0x0e0c3c0f
	WORD $0x1e260010
	WORD $0x8b0a054a
	WORD $0xf100053f
	WORD $0x12001d89
	WORD $0x8b0a002a
	WORD $0x53041def
	WORD $0x2a1009ef
	WORD $0x3900014f
	WORD $0x540000e8
	WORD $0x53027d2c
	WORD $0x2a0e118c
	WORD $0x3900054c
	WORD $0x3500008b
	WORD $0xf9000008
	WORD $0xd65f03c0
	WORD $0x34ffffcb
	WORD $0x2a0919a9
	WORD $0x39000949
	WORD $0xf9000008
	WORD $0xd65f03c0
	RET

// func parseIntsNEON(dst []int64, src []byte, idx []int32) (count int, ok bool)
TEXT ·parseIntsNEON(SB), NOSPLIT|NOFRAME, $0-81
	MOVD $count+72(FP), R0
	MOVD $ok+80(FP), R1
	MOVD dst_base+0(FP), R2
	MOVD src_base+24(FP), R3
	MOVD idx_base+48(FP), R4
	MOVD idx_len+56(FP), R5
	// Constant pool appended below the body; the displacements
	// above were patched to reach it and the aligned loads made
	// unaligned, since nothing promises alignment inside a TEXT.
	WORD $0xf10004bf
	WORD $0x54002a0b
	WORD $0xd102c3ff
	WORD $0x0f06e600
	WORD $0x0f00e521
	WORD $0xa90767fa
	WORD $0x0f00e422
	WORD $0x4f06e603
	WORD $0xaa1f03ed
	WORD $0x4f00e524
	WORD $0x4f00e425
	WORD $0xaa1f03e8
	WORD $0x10002989
	WORD $0x91000129
	WORD $0x92f00019
	WORD $0xfd001bec
	WORD $0x6d042beb
	WORD $0x6d0523e9
	WORD $0xa9067bfd
	WORD $0xa9085ff8
	WORD $0xa90957f6
	WORD $0xa90a4ff4
	WORD $0xa90007e0
	WORD $0xa90217e4
	WORD $0xa9010fe2
	WORD $0xb8a8788b
	WORD $0xeb0b01bf
	WORD $0x540001ca
	WORD $0x386d686c
	WORD $0x7100b59f
	WORD $0x54000060
	WORD $0x7100ad9f
	WORD $0x54000121
	WORD $0x7100b59f
	WORD $0x910005ad
	WORD $0x1a9f07fa
	WORD $0xcb0d016e
	WORD $0xd10051cf
	WORD $0xb1004dff
	WORD $0x540000e2
	WORD $0x1400011a
	WORD $0x5280003a
	WORD $0xcb0d016e
	WORD $0xd10051cf
	WORD $0xb1004dff
	WORD $0x540022a3
	WORD $0xf10021df
	WORD $0x540000a2
	WORD $0xaa1f03ef
	WORD $0x2a1f03f1
	WORD $0xaa1f03f0
	WORD $0x140000f1
	WORD $0xf10041df
	WORD $0x54000b42
	WORD $0xaa1f03ef
	WORD $0x2a1f03f1
	WORD $0xaa1f03f0
	WORD $0x6f00e406
	WORD $0x6f00e407
	WORD $0xd37df16a
	WORD $0x6f00e410
	WORD $0x6f00e411
	WORD $0xaa0f03ec
	WORD $0xcb0d0d4a
	WORD $0x6f00e412
	WORD $0x8b0f01a0
	WORD $0x8b000066
	WORD $0x4e011e26
	WORD $0x4e081e07
	WORD $0xcb0f0d4a
	WORD $0x927d05cf
	WORD $0x8b0a012a
	WORD $0xcb0f0190
	WORD $0xd1010151
	WORD $0xfc4084d3
	WORD $0x91002210
	WORD $0xad415a37
	WORD $0x0e208673
	WORD $0xacfe663a
	WORD $0x6e1642d6
	WORD $0x6e1742f7
	WORD $0x2f08a674
	WORD $0x2e213673
	WORD $0x6e194339
	WORD $0x6e1a435a
	WORD $0x2f10a695
	WORD $0x6f10a694
	WORD $0x9e6602ca
	WORD $0x4e183ed7
	WORD $0x4e183ef5
	WORD $0x0e221e73
	WORD $0x4e183f56
	WORD $0x2f20a6b8
	WORD $0x6f20a6b5
	WORD $0x2f20a69b
	WORD $0x6f20a694
	WORD $0x0eb31cc6
	WORD $0x9e66030c
	WORD $0x9e6602a0
	WORD $0x9e660361
	WORD $0x4e183f14
	WORD $0x9e660293
	WORD $0x4e183ea7
	WORD $0x9b0c7d4a
	WORD $0x9e6602ec
	WORD $0x9b147ef8
	WORD $0x4e183f37
	WORD $0x4e183f74
	WORD $0x9b007d8c
	WORD $0x9e660320
	WORD $0x9b077ea7
	WORD $0x9b017c00
	WORD $0x9e660341
	WORD $0x9e670195
	WORD $0x9b147ef4
	WORD $0x9b137c21
	WORD $0x4e183e93
	WORD $0x9e670154
	WORD $0x9e670017
	WORD $0x4e181cf5
	WORD $0x4e181f14
	WORD $0x9b137ed3
	WORD $0x4e181e97
	WORD $0x9e670036
	WORD $0x4ef186b1
	WORD $0x4ee78687
	WORD $0x4ef086f0
	WORD $0x4e181e76
	WORD $0x4ef286d2
	WORD $0xb5fff930
	WORD $0x4ef084e7
	WORD $0x4ef28630
	WORD $0xeb0f01df
	WORD $0x9e6600ca
	WORD $0x4ef084e6
	WORD $0xaa4a814a
	WORD $0x5ef1b8c6
	WORD $0xaa4a414a
	WORD $0xd348fd4c
	WORD $0x9e6600d0
	WORD $0x2a0c0151
	WORD $0x540012e1
	WORD $0x140000a5
	WORD $0xcb0d0d2f
	WORD $0x6f00e406
	WORD $0x6f00e411
	WORD $0x6f00e407
	WORD $0x6f00e412
	WORD $0x8b0d0070
	WORD $0x6f00e410
	WORD $0x6f00e415
	WORD $0x8b0b0df1
	WORD $0x6f00e413
	WORD $0x6f00e416
	WORD $0x927c01cf
	WORD $0x6f00e414
	WORD $0xd1020231
	WORD $0x927c01c6
	WORD $0xb9003ffa
	WORD $0x3cc10617
	WORD $0xd10040c6
	WORD $0xad43663c
	WORD $0x4e2386f7
	WORD $0xad412a2b
	WORD $0x6e19433d
	WORD $0xad42663f
	WORD $0x2f08a6f8
	WORD $0x6e1c439c
	WORD $0x6f08a6fa
	WORD $0x6e0a414a
	WORD $0x6e2436f7
	WORD $0x2f10a71b
	WORD $0x6f10a708
	WORD $0x9e6603a7
	WORD $0x6e194338
	WORD $0x2f10a749
	WORD $0x4e183fb6
	WORD $0x4e183f99
	WORD $0x4e183d5d
	WORD $0x6e0b417d
	WORD $0x2f20a77e
	WORD $0x6f20a77b
	WORD $0x2f20a519
	WORD $0x2f20a52c
	WORD $0x6f10a75a
	WORD $0x4e251ef7
	WORD $0x4e183f03
	WORD $0x9e6603d3
	WORD $0x9e660374
	WORD $0x9e660335
	WORD $0x4e183fd8
	WORD $0x4e183f7a
	WORD $0x6e1f43fb
	WORD $0x9e660197
	WORD $0x4e183d9e
	WORD $0x6f20a53f
	WORD $0x9b137ce7
	WORD $0x9e660393
	WORD $0x6f20a51c
	WORD $0xacfc7a28
	WORD $0x2f20a749
	WORD $0x9e6603e1
	WORD $0x9e6603a0
	WORD $0x9b147e73
	WORD $0x9e660314
	WORD $0x4e183f8a
	WORD $0x6e1e43de
	WORD $0x6f20a75a
	WORD $0x4e183f25
	WORD $0x9b017c00
	WORD $0x9e6700f8
	WORD $0x4eb71cc6
	WORD $0x9b157e94
	WORD $0x9e660155
	WORD $0x4e183f4c
	WORD $0x9e670279
	WORD $0x9b177eb7
	WORD $0x9b187ed5
	WORD $0x4e183f78
	WORD $0x9b1a7f36
	WORD $0x4e183fba
	WORD $0x9b1e7fb9
	WORD $0x4e183ffd
	WORD $0x9e66037e
	WORD $0x6e08411b
	WORD $0x9e67001f
	WORD $0x4e181eb8
	WORD $0x9b0a7f0a
	WORD $0x9e660398
	WORD $0x4e181ed9
	WORD $0x9b1d7f5a
	WORD $0x4e183d3d
	WORD $0x9e660364
	WORD $0x4e183f62
	WORD $0x9e67029b
	WORD $0x9b187fd8
	WORD $0x4e183fde
	WORD $0x4ef18711
	WORD $0x4ee78727
	WORD $0x9b0c7c4c
	WORD $0x4e181f5f
	WORD $0x9b1d7fc1
	WORD $0x9e66013d
	WORD $0x9e6603de
	WORD $0x9e67031c
	WORD $0x9b057c62
	WORD $0x4ef387f3
	WORD $0x9b1d7fdd
	WORD $0x9e66035e
	WORD $0x9e6702fa
	WORD $0x4e181d5c
	WORD $0x9b1e7c84
	WORD $0x4e181f3a
	WORD $0x4e181c5b
	WORD $0x9e6703bd
	WORD $0x4ef08790
	WORD $0x4ef58755
	WORD $0x4ef28772
	WORD $0x9e67009e
	WORD $0x4e181c3d
	WORD $0x4e181d9e
	WORD $0x4ef687b6
	WORD $0x4ef487d4
	WORD $0xb5fff306
	WORD $0x6e0640d7
	WORD $0x4ef58631
	WORD $0xeb0f01df
	WORD $0x4ef68652
	WORD $0x4ef384e7
	WORD $0x4ef48610
	WORD $0x0eb71cc6
	WORD $0x4ef28631
	WORD $0x4ef084e7
	WORD $0x9e6600ca
	WORD $0x4ee78626
	WORD $0xaa4a814a
	WORD $0x5ef1b8c6
	WORD $0xaa4a414a
	WORD $0xd348fd4c
	WORD $0x9e6600d0
	WORD $0x2a0c0151
	WORD $0x540000c1
	WORD $0xa94217e4
	WORD $0x92f00019
	WORD $0xa9410fe2
	WORD $0xb9403ffa
	WORD $0x14000015
	WORD $0xa94217e4
	WORD $0x92f00019
	WORD $0xa9410fe2
	WORD $0xb9403ffa
	WORD $0x371fe2ce
	WORD $0xcb0f016a
	WORD $0x8b0d01ec
	WORD $0xcb0d014d
	WORD $0x8b0c006e
	WORD $0x384015ca
	WORD $0x8b0d0d2c
	WORD $0x5100c14a
	WORD $0xf85f818c
	WORD $0x92401d4f
	WORD $0x7100255f
	WORD $0x9b0f4190
	WORD $0x1a9f97ea
	WORD $0xf10005ad
	WORD $0x2a0a0231
	WORD $0x54fffec1
	WORD $0x12001e2a
	WORD $0x7100035f
	WORD $0xda99132c
	WORD $0x7100015f
	WORD $0xfa4c0202
	WORD $0x54000148
	WORD $0x7100035f
	WORD $0x9100056d
	WORD $0xda90160a
	WORD $0xf828784a
	WORD $0x91000508
	WORD $0xeb05011f
	WORD $0x54ffdb41
	WORD $0x52800029
	WORD $0x14000003
	WORD $0x2a1f03e9
	WORD $0xaa0803e5
	WORD $0xa94007e0
	WORD $0xfd401bec
	WORD $0xa94a4ff4
	WORD $0xa94957f6
	WORD $0xa9485ff8
	WORD $0xa94767fa
	WORD $0xa9467bfd
	WORD $0x6d4523e9
	WORD $0x6d442beb
	WORD $0x9102c3ff
	WORD $0xf9000005
	WORD $0x39000029
	WORD $0xd65f03c0
	WORD $0x52800029
	WORD $0xf9000005
	WORD $0x39000029
	WORD $0xd65f03c0
	WORD $0x00000000
	WORD $0x00000000
	WORD $0x00000000
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x0000000a
	WORD $0x00000000
	WORD $0x00000064
	WORD $0x00000000
	WORD $0x000003e8
	WORD $0x00000000
	WORD $0x00002710
	WORD $0x00000000
	WORD $0x000186a0
	WORD $0x00000000
	WORD $0x000f4240
	WORD $0x00000000
	WORD $0x00989680
	WORD $0x00000000
	WORD $0x05f5e100
	WORD $0x00000000
	WORD $0x3b9aca00
	WORD $0x00000000
	WORD $0x540be400
	WORD $0x00000002
	WORD $0x4876e800
	WORD $0x00000017
	WORD $0xd4a51000
	WORD $0x000000e8
	WORD $0x4e72a000
	WORD $0x00000918
	WORD $0x107a4000
	WORD $0x00005af3
	WORD $0xa4c68000
	WORD $0x00038d7e
	WORD $0x6fc10000
	WORD $0x002386f2
	WORD $0x5d8a0000
	WORD $0x01634578
	WORD $0xa7640000
	WORD $0x0de0b6b3
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x0000000a
	WORD $0x00000000
	WORD $0x00000064
	WORD $0x00000000
	WORD $0x000003e8
	WORD $0x00000000
	WORD $0x00002710
	WORD $0x00000000
	WORD $0x000186a0
	WORD $0x00000000
	WORD $0x000f4240
	WORD $0x00000000
	WORD $0x00989680
	WORD $0x00000000
	WORD $0x05f5e100
	WORD $0x00000000
	WORD $0x3b9aca00
	WORD $0x00000000
	WORD $0x540be400
	WORD $0x00000002
	WORD $0x4876e800
	WORD $0x00000017
	WORD $0xd4a51000
	WORD $0x000000e8
	WORD $0x4e72a000
	WORD $0x00000918
	WORD $0x107a4000
	WORD $0x00005af3
	WORD $0xa4c68000
	WORD $0x00038d7e
	WORD $0x6fc10000
	WORD $0x002386f2
	WORD $0x5d8a0000
	WORD $0x01634578
	WORD $0xa7640000
	WORD $0x0de0b6b3
	WORD $0x89e80000
	WORD $0x8ac72304
	WORD $0x31303030
	WORD $0x33303230
	WORD $0x35303430
	WORD $0x37303630
	WORD $0x39303830
	WORD $0x31313031
	WORD $0x33313231
	WORD $0x35313431
	WORD $0x37313631
	WORD $0x39313831
	WORD $0x31323032
	WORD $0x33323232
	WORD $0x35323432
	WORD $0x37323632
	WORD $0x39323832
	WORD $0x31333033
	WORD $0x33333233
	WORD $0x35333433
	WORD $0x37333633
	WORD $0x39333833
	WORD $0x31343034
	WORD $0x33343234
	WORD $0x35343434
	WORD $0x37343634
	WORD $0x39343834
	WORD $0x31353035
	WORD $0x33353235
	WORD $0x35353435
	WORD $0x37353635
	WORD $0x39353835
	WORD $0x31363036
	WORD $0x33363236
	WORD $0x35363436
	WORD $0x37363636
	WORD $0x39363836
	WORD $0x31373037
	WORD $0x33373237
	WORD $0x35373437
	WORD $0x37373637
	WORD $0x39373837
	WORD $0x31383038
	WORD $0x33383238
	WORD $0x35383438
	WORD $0x37383638
	WORD $0x39383838
	WORD $0x31393039
	WORD $0x33393239
	WORD $0x35393439
	WORD $0x37393639
	WORD $0x39393839
	WORD $0x00000000
	WORD $0x00000000
	RET

// func parseUintsNEON(dst []uint64, src []byte, idx []int32) (count int, ok bool)
TEXT ·parseUintsNEON(SB), NOSPLIT|NOFRAME, $0-81
	MOVD $count+72(FP), R0
	MOVD $ok+80(FP), R1
	MOVD dst_base+0(FP), R2
	MOVD src_base+24(FP), R3
	MOVD idx_base+48(FP), R4
	MOVD idx_len+56(FP), R5
	// Constant pool appended below the body; the displacements
	// above were patched to reach it and the aligned loads made
	// unaligned, since nothing promises alignment inside a TEXT.
	WORD $0xf10004bf
	WORD $0x54002cab
	WORD $0xd10303ff
	WORD $0x0f06e600
	WORD $0x0f00e521
	WORD $0xa9077bfd
	WORD $0x0f00e422
	WORD $0x4f06e603
	WORD $0xd2b13d1d
	WORD $0x4f00e524
	WORD $0x4f00e425
	WORD $0xf2c4609d
	WORD $0xa90867fa
	WORD $0xaa1f03ef
	WORD $0xaa1f03e8
	WORD $0x10002d3a
	WORD $0x9102635a
	WORD $0xf2f158fd
	WORD $0xfd0023ec
	WORD $0x6d052beb
	WORD $0x6d0623e9
	WORD $0xa9095ff8
	WORD $0xa90a57f6
	WORD $0xa90b4ff4
	WORD $0xa90107e0
	WORD $0xa90317e4
	WORD $0xa9020fe2
	WORD $0x14000007
	WORD $0xaa1f03f3
	WORD $0xf8287853
	WORD $0x91000508
	WORD $0x9100058f
	WORD $0xeb05011f
	WORD $0x54002920
	WORD $0xb8a8788c
	WORD $0xcb0f018d
	WORD $0xd10055ae
	WORD $0xb10051df
	WORD $0x540027a3
	WORD $0xf10051bf
	WORD $0x54000141
	WORD $0x386f686e
	WORD $0x5100c1ce
	WORD $0x710025df
	WORD $0x540026e8
	WORD $0x92401dde
	WORD $0x52800026
	WORD $0xeb0d00df
	WORD $0x54fffd82
	WORD $0x14000005
	WORD $0xaa1f03e6
	WORD $0xaa1f03fe
	WORD $0xeb0d03ff
	WORD $0x54fffce2
	WORD $0x8b0601f3
	WORD $0xd37df189
	WORD $0xcb130191
	WORD $0xf100223f
	WORD $0x54000082
	WORD $0x2a1f03f4
	WORD $0xaa1f03f3
	WORD $0x14000103
	WORD $0xf100423f
	WORD $0x54000be2
	WORD $0xaa1f03ee
	WORD $0x2a1f03f4
	WORD $0xaa1f03f3
	WORD $0x6f00e406
	WORD $0x6f00e407
	WORD $0xaa0903fd
	WORD $0xcb0f0d29
	WORD $0x8b0601ca
	WORD $0x6f00e410
	WORD $0x6f00e411
	WORD $0x6f00e412
	WORD $0x8b0f006b
	WORD $0xcb0a0d29
	WORD $0x4e011e86
	WORD $0x4e081e67
	WORD $0x927df233
	WORD $0x8b090349
	WORD $0xaa1300c6
	WORD $0x8b0a0174
	WORD $0xd1010135
	WORD $0xcb1301c7
	WORD $0xfc408693
	WORD $0xb10020e7
	WORD $0xad415ab7
	WORD $0x0e208673
	WORD $0xacfe66ba
	WORD $0x6e1642d6
	WORD $0x6e1742f7
	WORD $0x2f08a674
	WORD $0x2e213673
	WORD $0x6e194339
	WORD $0x6e1a435a
	WORD $0x2f10a695
	WORD $0x6f10a694
	WORD $0x9e6602c9
	WORD $0x4e183ed0
	WORD $0x4e183ef9
	WORD $0x0e221e73
	WORD $0x2f20a6b8
	WORD $0x6f20a6b5
	WORD $0x2f20a69b
	WORD $0x6f20a694
	WORD $0x0eb31cc6
	WORD $0x9e66030a
	WORD $0x9e6602ab
	WORD $0x9e66036e
	WORD $0x4e183f00
	WORD $0x9e660281
	WORD $0x4e183eb8
	WORD $0x4e183e97
	WORD $0x4e183f76
	WORD $0x9b0a7d29
	WORD $0x9e6602ea
	WORD $0x9b007e10
	WORD $0x4e183f20
	WORD $0x9b0b7d4a
	WORD $0x9e66032b
	WORD $0x9e670134
	WORD $0x9b187f38
	WORD $0x9b0e7d6b
	WORD $0x9e66034e
	WORD $0x4e181e14
	WORD $0x9e670155
	WORD $0x9b167c00
	WORD $0x9b017dce
	WORD $0x4e183f41
	WORD $0x4e181f15
	WORD $0x9e670177
	WORD $0x4ee78687
	WORD $0x9b177c21
	WORD $0x4e181c17
	WORD $0x4ef186b1
	WORD $0x9e6701d6
	WORD $0x4ef086f0
	WORD $0x4e181c36
	WORD $0x4ef286d2
	WORD $0x54fff921
	WORD $0x4ef084e7
	WORD $0x4ef28630
	WORD $0xeb13023f
	WORD $0x9e6600c9
	WORD $0x4ef084e6
	WORD $0xaa498129
	WORD $0xaa494129
	WORD $0x5ef1b8c6
	WORD $0xd348fd2a
	WORD $0x2a0a0134
	WORD $0x9e6600d3
	WORD $0xaa1d03e9
	WORD $0xd2b13d1d
	WORD $0xf2c4609d
	WORD $0xf2f158fd
	WORD $0x54001481
	WORD $0x140000b3
	WORD $0xcb0f0d27
	WORD $0x6f00e406
	WORD $0x6f00e411
	WORD $0x6f00e407
	WORD $0x6f00e412
	WORD $0x8b130073
	WORD $0x6f00e410
	WORD $0x6f00e414
	WORD $0xcb060cf4
	WORD $0x6f00e413
	WORD $0x6f00e416
	WORD $0x927cee35
	WORD $0x6f00e415
	WORD $0x8b140354
	WORD $0xf90027fe
	WORD $0xd1020294
	WORD $0xf90007e9
	WORD $0x3cc10677
	WORD $0xf10042b5
	WORD $0xad43669c
	WORD $0x4e2386f7
	WORD $0xad412a8b
	WORD $0x6e19433d
	WORD $0xad42669f
	WORD $0x2f08a6f8
	WORD $0x6e1c439c
	WORD $0x6f08a6fa
	WORD $0x6e0a414a
	WORD $0x6e2436f7
	WORD $0x2f10a71b
	WORD $0x6f10a708
	WORD $0x9e6603b6
	WORD $0x6e194338
	WORD $0x2f10a749
	WORD $0x4e183fba
	WORD $0x4e183f8a
	WORD $0x6e0b417d
	WORD $0x4e183d49
	WORD $0x2f20a77e
	WORD $0x6f20a77b
	WORD $0x2f20a519
	WORD $0x2f20a52c
	WORD $0x6f10a75a
	WORD $0x4e251ef7
	WORD $0x4e183f05
	WORD $0x9e6603d7
	WORD $0x9e660378
	WORD $0x9e660339
	WORD $0x4e183fde
	WORD $0x4e183f6b
	WORD $0x6e1f43fb
	WORD $0x9e66019d
	WORD $0x4e183d81
	WORD $0x6f20a53f
	WORD $0x9b177ed6
	WORD $0x9e660397
	WORD $0x6f20a51c
	WORD $0xacfc7a88
	WORD $0x2f20a749
	WORD $0x9e6603f0
	WORD $0x9e6603a2
	WORD $0x9b187ef7
	WORD $0x9e660318
	WORD $0x4e183f80
	WORD $0x6e1e43de
	WORD $0x6f20a75a
	WORD $0x4e183f2e
	WORD $0x9b017d29
	WORD $0x9e660381
	WORD $0x9e6702d8
	WORD $0x4eb71cc6
	WORD $0x9b197f18
	WORD $0x9e660159
	WORD $0x4e183f44
	WORD $0x9e6702f9
	WORD $0x9b107c50
	WORD $0x9e660122
	WORD $0x9b1d7f3d
	WORD $0x9b1e7f59
	WORD $0x4e183f7e
	WORD $0x9b0b7d5a
	WORD $0x4e183faa
	WORD $0x4e183feb
	WORD $0x9e67021f
	WORD $0x9b007fc0
	WORD $0x9e66037e
	WORD $0x6e08411b
	WORD $0x4e181f38
	WORD $0x9b0b7d4a
	WORD $0x4e183d2b
	WORD $0x4e181f59
	WORD $0x9b017fc1
	WORD $0x4e183fde
	WORD $0x9e660367
	WORD $0x4e183f63
	WORD $0x9e67031b
	WORD $0x4ef18711
	WORD $0x9b0e7cae
	WORD $0x4e181d5f
	WORD $0x4ee78727
	WORD $0x9b0b7fcb
	WORD $0x9e6603de
	WORD $0x9e67003c
	WORD $0x9b047c63
	WORD $0x4e181ddb
	WORD $0x4ef387f3
	WORD $0x9b027fc2
	WORD $0x9e66035e
	WORD $0x9e6703ba
	WORD $0x4e181c1c
	WORD $0x9b1e7ce7
	WORD $0x4e181d3a
	WORD $0x4ef28772
	WORD $0x9e67005d
	WORD $0x4ef08790
	WORD $0x4ef48754
	WORD $0x9e6700fe
	WORD $0x4e181d7d
	WORD $0x4e181c7e
	WORD $0x4ef687b6
	WORD $0x4ef587d5
	WORD $0x54fff301
	WORD $0x6e0640d7
	WORD $0x4ef48631
	WORD $0x927cee2e
	WORD $0x4ef68652
	WORD $0x4ef384e7
	WORD $0xeb0e023f
	WORD $0x4ef58610
	WORD $0x0eb71cc6
	WORD $0x4ef28631
	WORD $0x4ef084e7
	WORD $0x9e6600c9
	WORD $0x4ee78626
	WORD $0xaa498129
	WORD $0x5ef1b8c6
	WORD $0xaa494129
	WORD $0xd348fd2a
	WORD $0x9e6600d3
	WORD $0x2a0a0134
	WORD $0x54000141
	WORD $0xa94317e4
	WORD $0xd2b13d1d
	WORD $0xa9420fe2
	WORD $0xf2c4609d
	WORD $0xf94027fe
	WORD $0x1000091a
	WORD $0x9102635a
	WORD $0xf2f158fd
	WORD $0x1400001d
	WORD $0xa94317e4
	WORD $0xd2b13d1d
	WORD $0xf94013e2
	WORD $0xf2c4609d
	WORD $0xf94027fe
	WORD $0xf2f158fd
	WORD $0x37180491
	WORD $0xf94017e3
	WORD $0xf94007e9
	WORD $0xaa0e00c6
	WORD $0x1000075a
	WORD $0x9102635a
	WORD $0xcb060d29
	WORD $0xcb0f0d29
	WORD $0x8b0f006f
	WORD $0x8b090349
	WORD $0xd1002130
	WORD $0x386669e9
	WORD $0xf85f860a
	WORD $0x910004c6
	WORD $0x5100c129
	WORD $0x92401d2b
	WORD $0x7100253f
	WORD $0x9b0b4d53
	WORD $0x1a9f97e9
	WORD $0xeb0d00df
	WORD $0x2a090294
	WORD $0x54fffecb
	WORD $0x72001e9f
	WORD $0x54000241
	WORD $0xf10051bf
	WORD $0x54ffd941
	WORD $0xf10007df
	WORD $0x540001c8
	WORD $0x540000c1
	WORD $0x92b13d09
	WORD $0xf2db9f69
	WORD $0xf2eea709
	WORD $0xeb09027f
	WORD $0x54000108
	WORD $0x9b1d4fd3
	WORD $0x17fffec0
	WORD $0xf94017e3
	WORD $0xf94007e9
	WORD $0x1000031a
	WORD $0x9102635a
	WORD $0x17fffee1
	WORD $0x2a1f03e9
	WORD $0xaa0803e5
	WORD $0x14000006
	WORD $0x52800029
	WORD $0xf9000005
	WORD $0x39000029
	WORD $0xd65f03c0
	WORD $0x52800029
	WORD $0xa94107e0
	WORD $0xfd4023ec
	WORD $0xa94b4ff4
	WORD $0xa94a57f6
	WORD $0xa9495ff8
	WORD $0xa94867fa
	WORD $0xa9477bfd
	WORD $0x6d4623e9
	WORD $0x6d452beb
	WORD $0x910303ff
	WORD $0xf9000005
	WORD $0x39000029
	WORD $0xd65f03c0
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x0000000a
	WORD $0x00000000
	WORD $0x00000064
	WORD $0x00000000
	WORD $0x000003e8
	WORD $0x00000000
	WORD $0x00002710
	WORD $0x00000000
	WORD $0x000186a0
	WORD $0x00000000
	WORD $0x000f4240
	WORD $0x00000000
	WORD $0x00989680
	WORD $0x00000000
	WORD $0x05f5e100
	WORD $0x00000000
	WORD $0x3b9aca00
	WORD $0x00000000
	WORD $0x540be400
	WORD $0x00000002
	WORD $0x4876e800
	WORD $0x00000017
	WORD $0xd4a51000
	WORD $0x000000e8
	WORD $0x4e72a000
	WORD $0x00000918
	WORD $0x107a4000
	WORD $0x00005af3
	WORD $0xa4c68000
	WORD $0x00038d7e
	WORD $0x6fc10000
	WORD $0x002386f2
	WORD $0x5d8a0000
	WORD $0x01634578
	WORD $0xa7640000
	WORD $0x0de0b6b3
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x0000000a
	WORD $0x00000000
	WORD $0x00000064
	WORD $0x00000000
	WORD $0x000003e8
	WORD $0x00000000
	WORD $0x00002710
	WORD $0x00000000
	WORD $0x000186a0
	WORD $0x00000000
	WORD $0x000f4240
	WORD $0x00000000
	WORD $0x00989680
	WORD $0x00000000
	WORD $0x05f5e100
	WORD $0x00000000
	WORD $0x3b9aca00
	WORD $0x00000000
	WORD $0x540be400
	WORD $0x00000002
	WORD $0x4876e800
	WORD $0x00000017
	WORD $0xd4a51000
	WORD $0x000000e8
	WORD $0x4e72a000
	WORD $0x00000918
	WORD $0x107a4000
	WORD $0x00005af3
	WORD $0xa4c68000
	WORD $0x00038d7e
	WORD $0x6fc10000
	WORD $0x002386f2
	WORD $0x5d8a0000
	WORD $0x01634578
	WORD $0xa7640000
	WORD $0x0de0b6b3
	WORD $0x89e80000
	WORD $0x8ac72304
	WORD $0x31303030
	WORD $0x33303230
	WORD $0x35303430
	WORD $0x37303630
	WORD $0x39303830
	WORD $0x31313031
	WORD $0x33313231
	WORD $0x35313431
	WORD $0x37313631
	WORD $0x39313831
	WORD $0x31323032
	WORD $0x33323232
	WORD $0x35323432
	WORD $0x37323632
	WORD $0x39323832
	WORD $0x31333033
	WORD $0x33333233
	WORD $0x35333433
	WORD $0x37333633
	WORD $0x39333833
	WORD $0x31343034
	WORD $0x33343234
	WORD $0x35343434
	WORD $0x37343634
	WORD $0x39343834
	WORD $0x31353035
	WORD $0x33353235
	WORD $0x35353435
	WORD $0x37353635
	WORD $0x39353835
	WORD $0x31363036
	WORD $0x33363236
	WORD $0x35363436
	WORD $0x37363636
	WORD $0x39363836
	WORD $0x31373037
	WORD $0x33373237
	WORD $0x35373437
	WORD $0x37373637
	WORD $0x39373837
	WORD $0x31383038
	WORD $0x33383238
	WORD $0x35383438
	WORD $0x37383638
	WORD $0x39383838
	WORD $0x31393039
	WORD $0x33393239
	WORD $0x35393439
	WORD $0x37393639
	WORD $0x39393839
	WORD $0x00000000
	WORD $0x00000000
	RET

// func formatIntsNEON(dst []byte, vals []int64, sep byte) int
TEXT ·formatIntsNEON(SB), NOSPLIT|NOFRAME, $0-64
	MOVD $ret+56(FP), R0
	MOVD dst_base+0(FP), R1
	MOVD vals_base+24(FP), R2
	MOVD vals_len+32(FP), R3
	MOVD dst_len+8(FP), R4
	MOVBU sep+48(FP), R5
	// Constant pool appended below the body; the displacements
	// above were patched to reach it and the aligned loads made
	// unaligned, since nothing promises alignment inside a TEXT.
	WORD $0xf1000468
	WORD $0x54000dab
	WORD $0xd10143ff
	WORD $0xd29eb86f
	WORD $0x910033ea
	WORD $0xaa1f03e6
	WORD $0xf2ab850f
	WORD $0xaa1f03e9
	WORD $0x9100402b
	WORD $0xf2d851ef
	WORD $0xd100414c
	WORD $0xd100054d
	WORD $0x528005ae
	WORD $0xf2e51eaf
	WORD $0x52800c90
	WORD $0x10000d31
	WORD $0x9104e231
	WORD $0xf90013f7
	WORD $0xa90357f6
	WORD $0xa9044ff4
	WORD $0x14000005
	WORD $0x91000529
	WORD $0xaa0703e6
	WORD $0xeb03013f
	WORD $0x54000b20
	WORD $0xcb060087
	WORD $0xf10054ff
	WORD $0x54000aab
	WORD $0xf8697847
	WORD $0xb7f80407
	WORD $0xf10190ff
	WORD $0x54000483
	WORD $0x2a1f03f3
	WORD $0x91000554
	WORD $0xaa0703f5
	WORD $0xd342fce7
	WORD $0x11000a73
	WORD $0x9bcf7ce7
	WORD $0xd342fce7
	WORD $0x9b10d4f6
	WORD $0xd344feb5
	WORD $0xf109c2bf
	WORD $0x8b160636
	WORD $0x394006d7
	WORD $0x394002d6
	WORD $0x381ff297
	WORD $0x38002696
	WORD $0x54fffe68
	WORD $0xf10028ff
	WORD $0x540002a3
	WORD $0x8b070627
	WORD $0x8b334154
	WORD $0x394004f5
	WORD $0x394000f6
	WORD $0x11000a67
	WORD $0x39000295
	WORD $0x39000696
	WORD $0x710004ff
	WORD $0x5400022a
	WORD $0xaa0603e7
	WORD $0x1400002c
	WORD $0x910004d3
	WORD $0x3826682e
	WORD $0xcb0703e7
	WORD $0xaa1303e6
	WORD $0xf10190ff
	WORD $0x54fffbc2
	WORD $0x2a1f03f3
	WORD $0xf10028ff
	WORD $0x54fffda2
	WORD $0x321c04f4
	WORD $0x32000267
	WORD $0x38334954
	WORD $0x710004ff
	WORD $0x54fffe2b
	WORD $0x710080ff
	WORD $0x2a0703f4
	WORD $0x54000082
	WORD $0xaa1403f3
	WORD $0xaa0603e7
	WORD $0x14000013
	WORD $0x927b6695
	WORD $0x92401293
	WORD $0x8b140196
	WORD $0x8b1500c7
	WORD $0x8b060166
	WORD $0x927b6697
	WORD $0xad7f82c1
	WORD $0xd10082f7
	WORD $0xd10082d6
	WORD $0x4e200800
	WORD $0x4e200821
	WORD $0x6e004000
	WORD $0x6e014021
	WORD $0xad3f84c0
	WORD $0x910080c6
	WORD $0xb5fffef7
	WORD $0xeb1402bf
	WORD $0x540000c0
	WORD $0x387369a6
	WORD $0xf1000673
	WORD $0x38276826
	WORD $0x910004e7
	WORD $0x54ffff88
	WORD $0xeb08013f
	WORD $0x54fff580
	WORD $0x910004e6
	WORD $0x38276825
	WORD $0xaa0603e7
	WORD $0x17ffffa8
	WORD $0xf900001f
	WORD $0xd65f03c0
	WORD $0x92800007
	WORD $0xa9444ff4
	WORD $0xf94013f7
	WORD $0xa94357f6
	WORD $0x910143ff
	WORD $0xf9000007
	WORD $0xd65f03c0
	WORD $0x00000000
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x0000000a
	WORD $0x00000000
	WORD $0x00000064
	WORD $0x00000000
	WORD $0x000003e8
	WORD $0x00000000
	WORD $0x00002710
	WORD $0x00000000
	WORD $0x000186a0
	WORD $0x00000000
	WORD $0x000f4240
	WORD $0x00000000
	WORD $0x00989680
	WORD $0x00000000
	WORD $0x05f5e100
	WORD $0x00000000
	WORD $0x3b9aca00
	WORD $0x00000000
	WORD $0x540be400
	WORD $0x00000002
	WORD $0x4876e800
	WORD $0x00000017
	WORD $0xd4a51000
	WORD $0x000000e8
	WORD $0x4e72a000
	WORD $0x00000918
	WORD $0x107a4000
	WORD $0x00005af3
	WORD $0xa4c68000
	WORD $0x00038d7e
	WORD $0x6fc10000
	WORD $0x002386f2
	WORD $0x5d8a0000
	WORD $0x01634578
	WORD $0xa7640000
	WORD $0x0de0b6b3
	WORD $0x00000001
	WORD $0x00000000
	WORD $0x0000000a
	WORD $0x00000000
	WORD $0x00000064
	WORD $0x00000000
	WORD $0x000003e8
	WORD $0x00000000
	WORD $0x00002710
	WORD $0x00000000
	WORD $0x000186a0
	WORD $0x00000000
	WORD $0x000f4240
	WORD $0x00000000
	WORD $0x00989680
	WORD $0x00000000
	WORD $0x05f5e100
	WORD $0x00000000
	WORD $0x3b9aca00
	WORD $0x00000000
	WORD $0x540be400
	WORD $0x00000002
	WORD $0x4876e800
	WORD $0x00000017
	WORD $0xd4a51000
	WORD $0x000000e8
	WORD $0x4e72a000
	WORD $0x00000918
	WORD $0x107a4000
	WORD $0x00005af3
	WORD $0xa4c68000
	WORD $0x00038d7e
	WORD $0x6fc10000
	WORD $0x002386f2
	WORD $0x5d8a0000
	WORD $0x01634578
	WORD $0xa7640000
	WORD $0x0de0b6b3
	WORD $0x89e80000
	WORD $0x8ac72304
	WORD $0x31303030
	WORD $0x33303230
	WORD $0x35303430
	WORD $0x37303630
	WORD $0x39303830
	WORD $0x31313031
	WORD $0x33313231
	WORD $0x35313431
	WORD $0x37313631
	WORD $0x39313831
	WORD $0x31323032
	WORD $0x33323232
	WORD $0x35323432
	WORD $0x37323632
	WORD $0x39323832
	WORD $0x31333033
	WORD $0x33333233
	WORD $0x35333433
	WORD $0x37333633
	WORD $0x39333833
	WORD $0x31343034
	WORD $0x33343234
	WORD $0x35343434
	WORD $0x37343634
	WORD $0x39343834
	WORD $0x31353035
	WORD $0x33353235
	WORD $0x35353435
	WORD $0x37353635
	WORD $0x39353835
	WORD $0x31363036
	WORD $0x33363236
	WORD $0x35363436
	WORD $0x37363636
	WORD $0x39363836
	WORD $0x31373037
	WORD $0x33373237
	WORD $0x35373437
	WORD $0x37373637
	WORD $0x39373837
	WORD $0x31383038
	WORD $0x33383238
	WORD $0x35383438
	WORD $0x37383638
	WORD $0x39383838
	WORD $0x31393039
	WORD $0x33393239
	WORD $0x35393439
	WORD $0x37393639
	WORD $0x39393839
	WORD $0x00000000
	WORD $0x00000000
	RET

// func hexDecodeNEON(dst []byte, src []byte) (n int, ok bool)
TEXT ·hexDecodeNEON(SB), NOSPLIT|NOFRAME, $0-57
	MOVD $n+48(FP), R0
	MOVD $ok+56(FP), R1
	MOVD dst_base+0(FP), R2
	MOVD src_base+24(FP), R3
	MOVD dst_len+8(FP), R4
	MOVD src_len+32(FP), R5
	WORD $0x8b45fca8
	WORD $0x9341fd08
	WORD $0xeb08009f
	WORD $0x9a88b088
	WORD $0xf100811f
	WORD $0x540000aa
	WORD $0xaa1f03ec
	WORD $0xeb0803ff
	WORD $0x54000aab
	WORD $0x14000079
	WORD $0x4f01e400
	WORD $0x4f06e4c1
	WORD $0xaa1f03e9
	WORD $0x4f07e6c2
	WORD $0x4f04e723
	WORD $0x9100806a
	WORD $0x4f07e744
	WORD $0x4f06e605
	WORD $0x9100404b
	WORD $0x4f00e546
	WORD $0x4f05e527
	WORD $0x4f04e610
	WORD $0xad7f4951
	WORD $0xad405153
	WORD $0x4ea01e35
	WORD $0x4ea01e56
	WORD $0x4e218631
	WORD $0x4ea01e77
	WORD $0x4ea01e98
	WORD $0x4e218652
	WORD $0x4e218673
	WORD $0x4e218694
	WORD $0x4e2386b5
	WORD $0x4e2386d6
	WORD $0x6e313451
	WORD $0x4e2386f7
	WORD $0x4e238718
	WORD $0x6e323452
	WORD $0x6e333453
	WORD $0x6e343454
	WORD $0x6e353495
	WORD $0x6e363496
	WORD $0x6e373497
	WORD $0x6e383498
	WORD $0x4e351e31
	WORD $0x4e361e52
	WORD $0x4e371e73
	WORD $0x4e381e94
	WORD $0x4eb31e31
	WORD $0x4eb41e52
	WORD $0x4eb11e51
	WORD $0x6e30aa31
	WORD $0x1e26022c
	WORD $0x370004ac
	WORD $0xd100814c
	WORD $0x4c408153
	WORD $0x9101012e
	WORD $0x8b09016d
	WORD $0xeb0801df
	WORD $0x9101014a
	WORD $0x4c408191
	WORD $0x9100812c
	WORD $0xaa0c03e9
	WORD $0x4e258675
	WORD $0x4ea01e99
	WORD $0x4f0c5673
	WORD $0x4e258636
	WORD $0x4e258657
	WORD $0x4ea01e58
	WORD $0x4f0c5631
	WORD $0x4e258692
	WORD $0x6e3534d4
	WORD $0x4e278739
	WORD $0x4e30867c
	WORD $0x6e3634d5
	WORD $0x6e3734d6
	WORD $0x4e278718
	WORD $0x4e30863a
	WORD $0x6e3234db
	WORD $0x6ef41f93
	WORD $0x6e781ef6
	WORD $0x6ef51f51
	WORD $0x6efb1f32
	WORD $0x4eb11ed1
	WORD $0x4eb31e52
	WORD $0xad3fc9b1
	WORD $0x54fff80d
	WORD $0xeb08019f
	WORD $0x540000ab
	WORD $0x14000029
	WORD $0xaa0903ec
	WORD $0xeb08013f
	WORD $0x540004ca
	WORD $0x8b0c0469
	WORD $0x91000529
	WORD $0x1400000f
	WORD $0x12001daf
	WORD $0x531c6d4a
	WORD $0x12001d6b
	WORD $0x51015dce
	WORD $0x710029ff
	WORD $0x1a8e31ad
	WORD $0x5101c14e
	WORD $0x7100297f
	WORD $0x1a8e314a
	WORD $0x2a0a01aa
	WORD $0x382c684a
	WORD $0x9100058c
	WORD $0xeb0c011f
	WORD $0x540002a0
	WORD $0x385ff12a
	WORD $0x321b014d
	WORD $0x5100c14b
	WORD $0x510185ad
	WORD $0x7100297f
	WORD $0x12001dad
	WORD $0x7a4529a0
	WORD $0x54000148
	WORD $0x3840252e
	WORD $0x5100c1cd
	WORD $0x321b01ce
	WORD $0x710029bf
	WORD $0x54fffcc3
	WORD $0x510185cf
	WORD $0x12001def
	WORD $0x710015ff
	WORD $0x54fffc49
	WORD $0xf900000c
	WORD $0x3900003f
	WORD $0xd65f03c0
	WORD $0x52800029
	WORD $0xf9000008
	WORD $0x39000029
	WORD $0xd65f03c0
	RET

// func hexEncodeNEON(dst []byte, b []byte) int
TEXT ·hexEncodeNEON(SB), NOSPLIT|NOFRAME, $0-56
	MOVD $ret+48(FP), R0
	MOVD dst_base+0(FP), R1
	MOVD b_base+24(FP), R2
	MOVD dst_len+8(FP), R3
	MOVD b_len+32(FP), R4
	WORD $0x8b43fc68
	WORD $0x9341fd08
	WORD $0xeb04011f
	WORD $0x9a84b108
	WORD $0xd37ff909
	WORD $0xf100051f
	WORD $0xf9000009
	WORD $0x54000ccb
	WORD $0xf100211f
	WORD $0x54000062
	WORD $0xaa1f03e9
	WORD $0x1400004e
	WORD $0xf100811f
	WORD $0x54000062
	WORD $0xaa1f03e9
	WORD $0x1400002e
	WORD $0x4f00e5e0
	WORD $0x4f05e401
	WORD $0x927d050a
	WORD $0x4f01e602
	WORD $0x4f02e6e3
	WORD $0x927be509
	WORD $0x4f00e544
	WORD $0x9100802b
	WORD $0x9100404c
	WORD $0x927be50d
	WORD $0xad7fc187
	WORD $0x4f01e605
	WORD $0x4f02e6e6
	WORD $0x4f01e612
	WORD $0x4f02e6f3
	WORD $0xd100816e
	WORD $0xf10081ad
	WORD $0x9100818c
	WORD $0x4e201cf1
	WORD $0x6e273434
	WORD $0x4e201e16
	WORD $0x6f0c14e5
	WORD $0x6f0c14e6
	WORD $0x6f0c1612
	WORD $0x6f0c1613
	WORD $0x6e313487
	WORD $0x6e303430
	WORD $0x4ea21e35
	WORD $0x4e238631
	WORD $0x6ef41cc5
	WORD $0x4e2386d4
	WORD $0x4ea71ce6
	WORD $0x6e363487
	WORD $0x6ef01e72
	WORD $0x6e711ea6
	WORD $0x4ea21ed1
	WORD $0x4ea71cf3
	WORD $0x6e741e33
	WORD $0x4c0081c5
	WORD $0x4c008172
	WORD $0x9101016b
	WORD $0x54fffc21
	WORD $0xeb09011f
	WORD $0x54000640
	WORD $0xb40003aa
	WORD $0x0f00e5e0
	WORD $0x0f05e401
	WORD $0xaa0903ec
	WORD $0x0f01e602
	WORD $0x0f02e6e3
	WORD $0x927ded09
	WORD $0x0f00e544
	WORD $0x8b0c042a
	WORD $0xcb09018b
	WORD $0x8b0c004c
	WORD $0xfc408585
	WORD $0x0f01e606
	WORD $0x0f02e6e7
	WORD $0xb100216b
	WORD $0x0e201cb0
	WORD $0x2e253431
	WORD $0x2f0c14a6
	WORD $0x2f0c14a7
	WORD $0x2e303485
	WORD $0x0ea21e12
	WORD $0x0e238610
	WORD $0x2ef11ce6
	WORD $0x0ea51ca7
	WORD $0x2e701e47
	WORD $0x0c9f8146
	WORD $0x54fffe21
	WORD $0xeb09011f
	WORD $0x540002a0
	WORD $0x8b09042a
	WORD $0xcb090108
	WORD $0x8b090049
	WORD $0x9100054a
	WORD $0x3840152b
	WORD $0x5280060e
	WORD $0x53047d6c
	WORD $0x12000d6d
	WORD $0x7102817f
	WORD $0x33000d6e
	WORD $0x321c058b
	WORD $0x11015d8c
	WORD $0x1a8c316b
	WORD $0x11015dac
	WORD $0x710029bf
	WORD $0x381ff14b
	WORD $0x1a8c31cb
	WORD $0xf1000508
	WORD $0x3800254b
	WORD $0x54fffe21
	WORD $0xd65f03c0
	RET

// func indexNEON(haystack []byte, needle []byte) int
TEXT ·indexNEON(SB), NOSPLIT|NOFRAME, $0-56
	MOVD $ret+48(FP), R0
	MOVD haystack_base+0(FP), R1
	MOVD needle_base+24(FP), R2
	MOVD haystack_len+8(FP), R3
	MOVD needle_len+32(FP), R4
	WORD $0xb40000c4
	WORD $0xeb04006a
	WORD $0x540000ca
	WORD $0x92800010
	WORD $0xf9000010
	WORD $0xd65f03c0
	WORD $0xf900001f
	WORD $0xd65f03c0
	WORD $0xa9bf4ff4
	WORD $0xd100048b
	WORD $0x39400048
	WORD $0xf100fd5f
	WORD $0x386b6849
	WORD $0x5400010a
	WORD $0xaa1f03ec
	WORD $0xeb0a019f
	WORD $0x5400096d
	WORD $0x92800010
	WORD $0xa8c14ff4
	WORD $0xf9000010
	WORD $0xd65f03c0
	WORD $0x4e010d20
	WORD $0x4e010d01
	WORD $0xf100088d
	WORD $0x54000c2d
	WORD $0xaa1f03ef
	WORD $0x9100042e
	WORD $0x52800810
	WORD $0x8b0f0031
	WORD $0x8b0f002c
	WORD $0x3cc20e26
	WORD $0x8b040183
	WORD $0xad400d82
	WORD $0x8b040231
	WORD $0x3cdff064
	WORD $0x3cc0f065
	WORD $0x3dc00d87
	WORD $0x3cdff230
	WORD $0x3cc0f231
	WORD $0x6e218cc6
	WORD $0x6e218c42
	WORD $0x6e218c63
	WORD $0x6e208c84
	WORD $0x6e208ca5
	WORD $0x6e218ce7
	WORD $0x6e208e10
	WORD $0x6e208e31
	WORD $0xaa1003ec
	WORD $0x4e241c42
	WORD $0x4e251c63
	WORD $0x4e301cc4
	WORD $0x4e311ce5
	WORD $0x4ea41c42
	WORD $0x4ea51c63
	WORD $0x4ea21c62
	WORD $0x6e30a842
	WORD $0x1e260051
	WORD $0x36000371
	WORD $0xaa1f03f1
	WORD $0xaa0e03e3
	WORD $0x14000005
	WORD $0x91000631
	WORD $0x91000463
	WORD $0xf101023f
	WORD $0x54000280
	WORD $0xaa0f0230
	WORD $0x8b100025
	WORD $0x394000a6
	WORD $0x6b0800df
	WORD $0x54ffff01
	WORD $0x8b0400a5
	WORD $0x91000446
	WORD $0xaa0d03e7
	WORD $0x385ff0a5
	WORD $0x6b0900bf
	WORD $0xaa0303e5
	WORD $0x54fffe21
	WORD $0x384014b3
	WORD $0x384014d4
	WORD $0x6b14027f
	WORD $0x54fffda1
	WORD $0xf10004e7
	WORD $0x54ffff61
	WORD $0x17ffffbf
	WORD $0xb240158f
	WORD $0x91010190
	WORD $0x910101ce
	WORD $0xeb0a01ff
	WORD $0xaa0c03ef
	WORD $0x54fff86d
	WORD $0x17ffffb5
	WORD $0xf100088d
	WORD $0x9100054a
	WORD $0x54000aad
	WORD $0x9100044b
	WORD $0x14000005
	WORD $0x9100058c
	WORD $0x92800010
	WORD $0xeb0a019f
	WORD $0x54fff5e0
	WORD $0x8b0c002e
	WORD $0x394001cf
	WORD $0x6b0801ff
	WORD $0x54ffff21
	WORD $0x8b0401ce
	WORD $0xaa0b03ef
	WORD $0xaa0d03f0
	WORD $0x385ff1ce
	WORD $0x6b0901df
	WORD $0x9100042e
	WORD $0x54fffe41
	WORD $0x386c69d1
	WORD $0x384015e2
	WORD $0x6b02023f
	WORD $0x54fffdc1
	WORD $0xf1000610
	WORD $0x910005ce
	WORD $0x54ffff41
	WORD $0xa8c14ff4
	WORD $0xf900000c
	WORD $0xd65f03c0
	WORD $0xaa1f03ed
	WORD $0x5280080e
	WORD $0x14000006
	WORD $0xb240158e
	WORD $0x910101ad
	WORD $0xeb0a01df
	WORD $0x9101018e
	WORD $0x54fff1ec
	WORD $0x8b0d002f
	WORD $0x8b0d002c
	WORD $0x3cc20de6
	WORD $0x8b040190
	WORD $0xad400d82
	WORD $0x8b0401ef
	WORD $0x3cdff204
	WORD $0x3cc0f205
	WORD $0x3dc00d87
	WORD $0x3cdff1f0
	WORD $0x3cc0f1f1
	WORD $0x6e218cc6
	WORD $0x6e218c42
	WORD $0x6e218c63
	WORD $0x6e208c84
	WORD $0x6e208ca5
	WORD $0x6e218ce7
	WORD $0x6e208e10
	WORD $0x6e208e31
	WORD $0xaa0e03ec
	WORD $0x4e241c42
	WORD $0x4e251c63
	WORD $0x4e301cc4
	WORD $0x4e311ce5
	WORD $0x4ea41c42
	WORD $0x4ea51c63
	WORD $0x4ea21c62
	WORD $0x6e30a842
	WORD $0x1e26004f
	WORD $0x3607fbcf
	WORD $0xaa1f03ee
	WORD $0xaa0103ef
	WORD $0x8b0b0030
	WORD $0x14000006
	WORD $0xd10005ce
	WORD $0x910005ef
	WORD $0x91000610
	WORD $0xb10101df
	WORD $0x54fffaa0
	WORD $0x386d69f1
	WORD $0x6b08023f
	WORD $0x54ffff21
	WORD $0x386d6a11
	WORD $0x6b09023f
	WORD $0x54fffec1
	WORD $0xcb0e01b0
	WORD $0xa8c14ff4
	WORD $0xf9000010
	WORD $0xd65f03c0
	WORD $0x8b0b002b
	WORD $0x92800010
	WORD $0x14000004
	WORD $0x9100058c
	WORD $0xeb0c015f
	WORD $0x54ffeb60
	WORD $0x386c682d
	WORD $0x6b0801bf
	WORD $0x54ffff61
	WORD $0x386c696d
	WORD $0x6b0901bf
	WORD $0x54ffff01
	WORD $0xa8c14ff4
	WORD $0xf900000c
	WORD $0xd65f03c0
	RET

// func lastIndexNEON(haystack []byte, needle []byte) int
TEXT ·lastIndexNEON(SB), NOSPLIT|NOFRAME, $0-56
	MOVD $ret+48(FP), R0
	MOVD haystack_base+0(FP), R1
	MOVD needle_base+24(FP), R2
	MOVD haystack_len+8(FP), R3
	MOVD needle_len+32(FP), R4
	WORD $0xb4000084
	WORD $0xeb04006b
	WORD $0x5400008a
	WORD $0x92800003
	WORD $0xf9000003
	WORD $0xd65f03c0
	WORD $0xf81f0ff3
	WORD $0xd100048a
	WORD $0x39400048
	WORD $0xf100fd7f
	WORD $0x386a6849
	WORD $0x54000f2b
	WORD $0x4e010d20
	WORD $0x4e010d01
	WORD $0xf100088c
	WORD $0x540008cd
	WORD $0x8b01016d
	WORD $0x910005ad
	WORD $0xd100fd6e
	WORD $0x8b0e0030
	WORD $0x8b0e002f
	WORD $0x3cc20e05
	WORD $0x3dc00de4
	WORD $0xad400de2
	WORD $0x8b0401ef
	WORD $0x8b040210
	WORD $0x3cdff1e6
	WORD $0x3cc0f1e7
	WORD $0x3cdff210
	WORD $0x3cc0f211
	WORD $0x6e218ca5
	WORD $0x6e218c42
	WORD $0x6e218c63
	WORD $0x6e208cc6
	WORD $0x6e218c84
	WORD $0x6e208ce7
	WORD $0x6e208e10
	WORD $0x6e208e31
	WORD $0x4e261c42
	WORD $0x4e271c63
	WORD $0x4e301ca5
	WORD $0x4e311c84
	WORD $0x4ea51c42
	WORD $0x4ea41c63
	WORD $0x4ea21c62
	WORD $0x6e30a842
	WORD $0x1e26004f
	WORD $0x3600036f
	WORD $0xaa0d03ef
	WORD $0x528007f1
	WORD $0x14000004
	WORD $0xd1000611
	WORD $0xd10005ef
	WORD $0xb40002b0
	WORD $0x8b0e0223
	WORD $0xaa1103f0
	WORD $0x8b030031
	WORD $0x39400225
	WORD $0x6b0800bf
	WORD $0x54ffff01
	WORD $0x8b040231
	WORD $0x91000445
	WORD $0xaa0c03e6
	WORD $0x385ff231
	WORD $0x6b09023f
	WORD $0xaa0f03f1
	WORD $0x54fffe21
	WORD $0x38401627
	WORD $0x384014b3
	WORD $0x6b1300ff
	WORD $0x54fffda1
	WORD $0xf10004c6
	WORD $0x54ffff61
	WORD $0x14000038
	WORD $0xd101016e
	WORD $0xf101f97f
	WORD $0xd10101ad
	WORD $0xaa0e03eb
	WORD $0x54fff88c
	WORD $0x14000036
	WORD $0xd101016e
	WORD $0xf101f97f
	WORD $0xd1010063
	WORD $0xaa0e03eb
	WORD $0x5400062d
	WORD $0x8b01016c
	WORD $0x3cdc1d82
	WORD $0x3dc00583
	WORD $0x8b04018d
	WORD $0x3dc00d84
	WORD $0x3cc20d86
	WORD $0x3cdff1a5
	WORD $0x8b04018c
	WORD $0x3cc0f1a7
	WORD $0x6e218c42
	WORD $0x3cdff190
	WORD $0x3cc0f191
	WORD $0x6e218c63
	WORD $0x6e208ca5
	WORD $0x6e208ce7
	WORD $0x6e218cc6
	WORD $0x6e218c84
	WORD $0x6e208e10
	WORD $0x6e208e31
	WORD $0x4e251c42
	WORD $0x4e271c63
	WORD $0x4e301cc5
	WORD $0x4e311c84
	WORD $0x4ea51c42
	WORD $0x4ea41c63
	WORD $0x4ea21c62
	WORD $0x6e30a842
	WORD $0x1e26004c
	WORD $0x3607fbec
	WORD $0xaa1f03ec
	WORD $0xaa0103ed
	WORD $0x14000005
	WORD $0x9100058c
	WORD $0xd10005ad
	WORD $0xf101019f
	WORD $0x54fffb00
	WORD $0x386b69ae
	WORD $0x6b0801df
	WORD $0x54ffff41
	WORD $0x8b0301ae
	WORD $0x385ff1ce
	WORD $0x6b0901df
	WORD $0x54fffec1
	WORD $0xcb0c0163
	WORD $0xf84107f3
	WORD $0xf9000003
	WORD $0xd65f03c0
	WORD $0xaa0b03ee
	WORD $0xb7f8058e
	WORD $0xf100088b
	WORD $0x5400038d
	WORD $0x9100044a
	WORD $0x92800003
	WORD $0x14000004
	WORD $0xf10001df
	WORD $0xd10005ce
	WORD $0x54fffe8d
	WORD $0x8b0e002c
	WORD $0x3940018d
	WORD $0x6b0801bf
	WORD $0x54ffff41
	WORD $0x8b04018c
	WORD $0xaa0a03ed
	WORD $0xaa0b03ef
	WORD $0x385ff18c
	WORD $0x6b09019f
	WORD $0x9100042c
	WORD $0x54fffe61
	WORD $0x386e6990
	WORD $0x384015b1
	WORD $0x6b11021f
	WORD $0x54fffde1
	WORD $0xf10005ef
	WORD $0x9100058c
	WORD $0x54ffff41
	WORD $0xf84107f3
	WORD $0xf900000e
	WORD $0xd65f03c0
	WORD $0x8b0a002a
	WORD $0x14000004
	WORD $0xf10001df
	WORD $0xd10005ce
	WORD $0x5400014d
	WORD $0x386e682b
	WORD $0x6b08017f
	WORD $0x54ffff61
	WORD $0x386e694b
	WORD $0x6b09017f
	WORD $0x54ffff01
	WORD $0xf84107f3
	WORD $0xf900000e
	WORD $0xd65f03c0
	WORD $0x92800003
	WORD $0xf84107f3
	WORD $0xf9000003
	WORD $0xd65f03c0
	RET

// func countSeqNEON(haystack []byte, needle []byte) int
TEXT ·countSeqNEON(SB), NOSPLIT|NOFRAME, $0-56
	MOVD $ret+48(FP), R0
	MOVD haystack_base+0(FP), R1
	MOVD needle_base+24(FP), R2
	MOVD haystack_len+8(FP), R3
	MOVD needle_len+32(FP), R4
	WORD $0xaa1f03e8
	WORD $0xb4001164
	WORD $0xeb040069
	WORD $0x5400112b
	WORD $0x8b02008b
	WORD $0x3840144a
	WORD $0x385ff16b
	WORD $0xaa1f03e8
	WORD $0xaa1f03ed
	WORD $0x4e010d41
	WORD $0x4e010d60
	WORD $0xeb0901bf
	WORD $0x9100fdaf
	WORD $0x910101ac
	WORD $0x9a89c1b0
	WORD $0xaa0d03ee
	WORD $0x14000005
	WORD $0xeb1001df
	WORD $0x910005ce
	WORD $0x9100058c
	WORD $0x54000f00
	WORD $0xeb0901ff
	WORD $0x8b0e002d
	WORD $0x540003ed
	WORD $0x394001ad
	WORD $0x6b0a01bf
	WORD $0x54fffee1
	WORD $0x8b0401cd
	WORD $0x8b0d0031
	WORD $0x385ff231
	WORD $0x6b0b023f
	WORD $0x54fffe41
	WORD $0xf1000c9f
	WORD $0x9100042c
	WORD $0xaa0203ef
	WORD $0xd1000890
	WORD $0x540000ca
	WORD $0x5280002c
	WORD $0x8b0c0108
	WORD $0xeb0901bf
	WORD $0x54fffc6d
	WORD $0x14000063
	WORD $0x386e6991
	WORD $0x384015e3
	WORD $0x6b03023f
	WORD $0x54000b61
	WORD $0xf1000610
	WORD $0x9100058c
	WORD $0x54ffff41
	WORD $0x5280002c
	WORD $0x8b0c0108
	WORD $0xeb0901bf
	WORD $0x54fffaed
	WORD $0x14000057
	WORD $0x8b0401af
	WORD $0xad400da2
	WORD $0x3cdff1e4
	WORD $0x3cc0f1e5
	WORD $0x8b0e002f
	WORD $0x3cc20de6
	WORD $0x3dc00da7
	WORD $0x8b0401ef
	WORD $0x6e218c42
	WORD $0x6e218c63
	WORD $0x3cdff1f0
	WORD $0x3cc0f1f1
	WORD $0x6e208c84
	WORD $0x6e208ca5
	WORD $0x6e218cc6
	WORD $0x6e218ce7
	WORD $0x6e208e10
	WORD $0x6e208e31
	WORD $0x910101cf
	WORD $0x4e241c42
	WORD $0x4e251c63
	WORD $0x4e301cc4
	WORD $0x4e311ce5
	WORD $0x4ea41c42
	WORD $0x4ea51c63
	WORD $0x4ea21c62
	WORD $0x6e30a842
	WORD $0x1e26004d
	WORD $0x3600064d
	WORD $0xaa0e03f0
	WORD $0x14000004
	WORD $0x91000610
	WORD $0xeb0f021f
	WORD $0x540004ea
	WORD $0x3870682d
	WORD $0x6b0a01bf
	WORD $0x54ffff61
	WORD $0x8b04020d
	WORD $0x8b0d0031
	WORD $0x385ff231
	WORD $0x6b0b023f
	WORD $0x54fffec1
	WORD $0xf1000c9f
	WORD $0x9100042e
	WORD $0xaa0203f1
	WORD $0xd1000883
	WORD $0x540000ea
	WORD $0x5280002e
	WORD $0xeb0f01bf
	WORD $0x8b0e0108
	WORD $0xaa0d03ee
	WORD $0x54fffd4b
	WORD $0x1400001b
	WORD $0x387069c5
	WORD $0x38401626
	WORD $0x6b0600bf
	WORD $0x54000141
	WORD $0xf1000463
	WORD $0x910005ce
	WORD $0x54ffff41
	WORD $0x5280002e
	WORD $0xeb0f01bf
	WORD $0x8b0e0108
	WORD $0xaa0d03ee
	WORD $0x54fffbab
	WORD $0x1400000e
	WORD $0x9100060d
	WORD $0xeb0f01bf
	WORD $0x8b1f0108
	WORD $0xaa0d03ee
	WORD $0x54fffaeb
	WORD $0x14000008
	WORD $0x910005cd
	WORD $0xeb0d019f
	WORD $0x9a8ec58d
	WORD $0xeb0901bf
	WORD $0x54fff12d
	WORD $0x14000009
	WORD $0xaa0f03ed
	WORD $0xeb0901bf
	WORD $0x54fff0ad
	WORD $0x14000005
	WORD $0x910005cd
	WORD $0x8b1f0108
	WORD $0xeb0901bf
	WORD $0x54fff00d
	WORD $0xf9000008
	WORD $0xd65f03c0
	RET

// func toUpperASCIINEON(dst []byte, b []byte)
TEXT ·toUpperASCIINEON(SB), NOSPLIT|NOFRAME, $0-48
	MOVD dst_base+0(FP), R0
	MOVD b_base+24(FP), R1
	MOVD dst_len+8(FP), R2
	WORD $0xf100045f
	WORD $0x540007eb
	WORD $0xf100205f
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x14000030
	WORD $0xf100805f
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x1400001a
	WORD $0x4f04e7e0
	WORD $0x4f00e741
	WORD $0x927d0449
	WORD $0x4f07e402
	WORD $0x927be448
	WORD $0x9100400a
	WORD $0x9100402b
	WORD $0x927be44c
	WORD $0xad7f9163
	WORD $0xf100818c
	WORD $0x9100816b
	WORD $0x4e208465
	WORD $0x4e208486
	WORD $0x6e253425
	WORD $0x6e263426
	WORD $0x4e221ca5
	WORD $0x4e221cc6
	WORD $0x4e258463
	WORD $0x4e268484
	WORD $0xad3f9143
	WORD $0x9100814a
	WORD $0x54fffe61
	WORD $0xeb08005f
	WORD $0x540003e0
	WORD $0xb4000269
	WORD $0x0f04e7e0
	WORD $0x0f00e741
	WORD $0xaa0803eb
	WORD $0x0f07e402
	WORD $0x927dec48
	WORD $0x8b0b000a
	WORD $0xcb080169
	WORD $0x8b0b002b
	WORD $0xfc408563
	WORD $0xb1002129
	WORD $0x0e208464
	WORD $0x2e243424
	WORD $0x0e221c84
	WORD $0x0e248463
	WORD $0xfc008543
	WORD $0x54ffff21
	WORD $0xeb08005f
	WORD $0x54000180
	WORD $0xcb080049
	WORD $0x8b08000a
	WORD $0x8b080028
	WORD $0x3840150b
	WORD $0x5101856c
	WORD $0x5100816d
	WORD $0x7100699f
	WORD $0x1a8b31ab
	WORD $0xf1000529
	WORD $0x3800154b
	WORD $0x54ffff21
	WORD $0xd65f03c0
	RET

// func toLowerASCIINEON(dst []byte, b []byte)
TEXT ·toLowerASCIINEON(SB), NOSPLIT|NOFRAME, $0-48
	MOVD dst_base+0(FP), R0
	MOVD b_base+24(FP), R1
	MOVD dst_len+8(FP), R2
	WORD $0xf100045f
	WORD $0x540007eb
	WORD $0xf100205f
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x14000030
	WORD $0xf100805f
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x1400001a
	WORD $0x4f05e7e0
	WORD $0x4f00e741
	WORD $0x927d0449
	WORD $0x4f01e402
	WORD $0x927be448
	WORD $0x9100400a
	WORD $0x9100402b
	WORD $0x927be44c
	WORD $0xad7f9163
	WORD $0xf100818c
	WORD $0x9100816b
	WORD $0x4e208465
	WORD $0x4e208486
	WORD $0x6e253425
	WORD $0x6e263426
	WORD $0x4e221ca5
	WORD $0x4e221cc6
	WORD $0x4e258463
	WORD $0x4e268484
	WORD $0xad3f9143
	WORD $0x9100814a
	WORD $0x54fffe61
	WORD $0xeb08005f
	WORD $0x540003e0
	WORD $0xb4000269
	WORD $0x0f05e7e0
	WORD $0x0f00e741
	WORD $0xaa0803eb
	WORD $0x0f01e402
	WORD $0x927dec48
	WORD $0x8b0b000a
	WORD $0xcb080169
	WORD $0x8b0b002b
	WORD $0xfc408563
	WORD $0xb1002129
	WORD $0x0e208464
	WORD $0x2e243424
	WORD $0x0e221c84
	WORD $0x0e248463
	WORD $0xfc008543
	WORD $0x54ffff21
	WORD $0xeb08005f
	WORD $0x54000180
	WORD $0xcb080049
	WORD $0x8b08000a
	WORD $0x8b080028
	WORD $0x3840150b
	WORD $0x5101056c
	WORD $0x1100816d
	WORD $0x7100699f
	WORD $0x1a8b31ab
	WORD $0xf1000529
	WORD $0x3800154b
	WORD $0x54ffff21
	WORD $0xd65f03c0
	RET

// func replaceByteNEON(dst []byte, b []byte, old byte, with byte)
TEXT ·replaceByteNEON(SB), NOSPLIT|NOFRAME, $0-50
	MOVD dst_base+0(FP), R0
	MOVD b_base+24(FP), R1
	MOVBU old+48(FP), R2
	MOVBU with+49(FP), R3
	MOVD dst_len+8(FP), R4
	WORD $0xf100049f
	WORD $0x540006ab
	WORD $0xf100209f
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x14000028
	WORD $0xf100809f
	WORD $0x54000062
	WORD $0xaa1f03e8
	WORD $0x14000015
	WORD $0x4e010c40
	WORD $0x4e010c61
	WORD $0x927d0489
	WORD $0x927be488
	WORD $0x9100400a
	WORD $0x9100402b
	WORD $0x927be48c
	WORD $0xad7f8d62
	WORD $0xf100818c
	WORD $0x9100816b
	WORD $0x6e208c44
	WORD $0x6e208c65
	WORD $0x6ea41c22
	WORD $0x6ea51c23
	WORD $0xad3f8d42
	WORD $0x9100814a
	WORD $0x54fffee1
	WORD $0xeb08009f
	WORD $0x54000340
	WORD $0xb4000209
	WORD $0x0e010c40
	WORD $0x0e010c61
	WORD $0xaa0803eb
	WORD $0x927dec88
	WORD $0x8b0b000a
	WORD $0xcb080169
	WORD $0x8b0b002b
	WORD $0xfc408562
	WORD $0xb1002129
	WORD $0x2e208c43
	WORD $0x2ea31c22
	WORD $0xfc008542
	WORD $0x54ffff61
	WORD $0xeb08009f
	WORD $0x54000140
	WORD $0xcb080089
	WORD $0x8b08000a
	WORD $0x8b080028
	WORD $0x3840150b
	WORD $0x6b22017f
	WORD $0x1a8b006b
	WORD $0xf1000529
	WORD $0x3800154b
	WORD $0x54ffff61
	WORD $0xd65f03c0
	RET

