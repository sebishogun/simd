// Code generated by simdgen. DO NOT EDIT.
//
// Source:  csrc/reduce.c
// Target:  s390x/vx
// Clang:   clang version 22.1.8
// Command: clang --target=s390x-linux-gnu -march=z14 -O3 -ffreestanding -fno-builtin -fno-builtin-memset -fno-builtin-memcpy -fno-jump-tables -fno-math-errno -fno-asynchronous-unwind-tables -fno-exceptions -fno-rtti -fomit-frame-pointer -fno-stack-protector -mllvm -inline-threshold=1000 -ffp-contract=off -c ../csrc/reduce.c -o ../tools/build/tmp/reduce_s390x_vx.o
//
// The instruction bytes below are the compiled function verbatim. Branches
// inside a function are PC-relative and remain correct because the whole
// body moves as a unit.

//go:build s390x && !purego

#include "textflag.h"
#include "funcdata.h"

// func minrFloat32VX(a []float32) float32
TEXT ·minrFloat32VX(SB), NOSPLIT, $160-28
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·minrFloat32VXBody(SB)
	RET

// minrFloat32VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·minrFloat32VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024e700
	WORD $0x30002005
	WORD $0xa7190000
	WORD $0xe7200000
	WORD $0x0056e710
	WORD $0x00000056
	WORD $0xec44014b
	WORD $0x087cec04
	WORD $0xfff800d9
	WORD $0xeb100003
	WORD $0x000cec18
	WORD $0x00d7007c
	WORD $0x41001001
	WORD $0xe7300000
	WORD $0x0044ec50
	WORD $0x02be0059
	WORD $0xa7190000
	WORD $0xb90400e3
	WORD $0xe7200000
	WORD $0x0056e710
	WORD $0x00000056
	WORD $0xe750e000
	WORD $0x0006e740
	WORD $0xe0100006
	WORD $0xe7153000
	WORD $0x28eae723
	WORD $0x500028eb
	WORD $0xe7623000
	WORD $0x20eae773
	WORD $0x200020eb
	WORD $0xe7121000
	WORD $0x0e6ae724
	WORD $0x300028ea
	WORD $0xe7334000
	WORD $0x28ebe773
	WORD $0x400028fb
	WORD $0xe7835000
	WORD $0x28fbe767
	WORD $0x6000006a
	WORD $0xe7713000
	WORD $0x20eae703
	WORD $0x100028eb
	WORD $0xe7425000
	WORD $0x28ebe765
	WORD $0x200028eb
	WORD $0xa75bfffe
	WORD $0x41101010
	WORD $0xe7232000
	WORD $0x0e6ae734
	WORD $0x100028eb
	WORD $0xe7514000
	WORD $0x28ebe785
	WORD $0x2000898d
	WORD $0xe7741000
	WORD $0x798de770
	WORD $0x7000046a
	WORD $0xe7017000
	WORD $0x3b8de732
	WORD $0x80006b8d
	WORD $0xe7141000
	WORD $0x0e6fe725
	WORD $0x20000e6f
	WORD $0xe7553000
	WORD $0x138de744
	WORD $0x0000238d
	WORD $0xe7252000
	WORD $0x608de750
	WORD $0xe0200006
	WORD $0xe7141000
	WORD $0x708de740
	WORD $0xe0300006
	WORD $0x41e0e040
	WORD $0xe7153000
	WORD $0x28eae723
	WORD $0x500028eb
	WORD $0xe7623000
	WORD $0x20eae773
	WORD $0x200020eb
	WORD $0xe7121000
	WORD $0x0e6ae724
	WORD $0x300028ea
	WORD $0xe7434000
	WORD $0x28ebe773
	WORD $0x400028fb
	WORD $0xe7835000
	WORD $0x28fbe701
	WORD $0x300028ea
	WORD $0xe7331000
	WORD $0x28ebe724
	WORD $0x20000e6a
	WORD $0xe7425000
	WORD $0x28ebe751
	WORD $0x400028eb
	WORD $0xe7652000
	WORD $0x28ebe767
	WORD $0x6000006a
	WORD $0xe7741000
	WORD $0x20ebe785
	WORD $0x2000898d
	WORD $0xe7741000
	WORD $0x798de703
	WORD $0x00000e6a
	WORD $0xe7717000
	WORD $0x728de732
	WORD $0x80006b8d
	WORD $0xe7252000
	WORD $0x0e6fe714
	WORD $0x10000e6f
	WORD $0xe7553000
	WORD $0x138de744
	WORD $0x7000218d
	WORD $0xe7141000
	WORD $0x018de725
	WORD $0x2000608d
	WORD $0xa774ff46
	WORD $0xa7010001
	WORD $0xa774000c
	WORD $0xa7f4006a
	WORD $0xa7190000
	WORD $0xe7200000
	WORD $0x0056e710
	WORD $0x00000056
	WORD $0xe7500000
	WORD $0x0044eb11
	WORD $0x0002000d
	WORD $0xe7413000
	WORD $0x0006e762
	WORD $0x500020ea
	WORD $0xe7752000
	WORD $0x20ebe731
	WORD $0x30100006
	WORD $0xe7676000
	WORD $0x006ae771
	WORD $0x500020ea
	WORD $0xe7051000
	WORD $0x28ebe770
	WORD $0x7000046a
	WORD $0xe7045000
	WORD $0x28eae715
	WORD $0x400028eb
	WORD $0xe7010000
	WORD $0x0e6ae713
	WORD $0x500028ea
	WORD $0xe7253000
	WORD $0x28ebe765
	WORD $0x300028fb
	WORD $0xe7554000
	WORD $0x20fbe712
	WORD $0x10000e6a
	WORD $0xe7224000
	WORD $0x28ebe731
	WORD $0x300028eb
	WORD $0xe7442000
	WORD $0x28ebe753
	WORD $0x100028eb
	WORD $0xe7542000
	WORD $0x508de763
	WORD $0x1000698d
	WORD $0xe7516000
	WORD $0x5b8de752
	WORD $0x5000418d
	WORD $0xe7131000
	WORD $0x0e6fe702
	WORD $0x00000e6f
	WORD $0xe7445000
	WORD $0x018de733
	WORD $0x5000138d
	WORD $0xe7131000
	WORD $0x708de724
	WORD $0x2000608d
	WORD $0xec1401bc
	WORD $0x0059ec14
	WORD $0x0013a064
	WORD $0xeb510002
	WORD $0x000de730
	WORD $0x00000056
	WORD $0xe7353000
	WORD $0x00034100
	WORD $0x1001ec04
	WORD $0x000d4064
	WORD $0xa7f40010
	WORD $0xe7300000
	WORD $0x00564100
	WORD $0x1001ec04
	WORD $0x0009a064
	WORD $0xeb500002
	WORD $0x000de735
	WORD $0x30001003
	WORD $0x41001002
	WORD $0xec040030
	WORD $0xa064eb50
	WORD $0x0002000d
	WORD $0xe7353000
	WORD $0x20034100
	WORD $0x1003ec04
	WORD $0x002a4064
	WORD $0x41001004
	WORD $0xec040030
	WORD $0xa064eb50
	WORD $0x0002000d
	WORD $0xe7053000
	WORD $0x00034100
	WORD $0x1005ec04
	WORD $0x002a4064
	WORD $0x41001006
	WORD $0xec040030
	WORD $0xa064eb50
	WORD $0x0002000d
	WORD $0xe7053000
	WORD $0x20034100
	WORD $0x1007ec04
	WORD $0x002a4064
	WORD $0xa7f4002d
	WORD $0x41001003
	WORD $0xec04ffdc
	WORD $0xa064eb50
	WORD $0x0002000d
	WORD $0xe7353000
	WORD $0x30034100
	WORD $0x1004ec04
	WORD $0xffd64064
	WORD $0x41001005
	WORD $0xec04ffdc
	WORD $0xa064eb50
	WORD $0x0002000d
	WORD $0xe7053000
	WORD $0x10034100
	WORD $0x1006ec04
	WORD $0xffd64064
	WORD $0x41001007
	WORD $0xec040009
	WORD $0xa064eb10
	WORD $0x0002000d
	WORD $0xe7013000
	WORD $0x3003e740
	WORD $0x00000044
	WORD $0xe7524000
	WORD $0x20eae764
	WORD $0x200020eb
	WORD $0xe7165000
	WORD $0x086ae753
	WORD $0x400020ea
	WORD $0xe7643000
	WORD $0x20ebe754
	WORD $0x300028fb
	WORD $0xe7265000
	WORD $0x086ae732
	WORD $0x300028eb
	WORD $0xe7432000
	WORD $0x28ebe753
	WORD $0x2000598d
	WORD $0xe7425000
	WORD $0x4b8de723
	WORD $0x20000e6f
	WORD $0xe7334000
	WORD $0x238de733
	WORD $0x2000118d
	WORD $0xe7504000
	WORD $0x20eae764
	WORD $0x000020eb
	WORD $0xe7040000
	WORD $0x28fbb309
	WORD $0x0033e756
	WORD $0x5000006a
	WORD $0xe7610000
	WORD $0x20ebe770
	WORD $0x100020eb
	WORD $0xe7001000
	WORD $0x098da714
	WORD $0x0016e723
	WORD $0x0001204d
	WORD $0xb3090022
	WORD $0xa7140010
	WORD $0xb3090023
	WORD $0xa744000c
	WORD $0xb3090032
	WORD $0xa7440007
	WORD $0xed200555
	WORD $0x0010a744
	WORD $0x00032823
	WORD $0xb3090022
	WORD $0xe7114000
	WORD $0x28eae744
	WORD $0x100020eb
	WORD $0xe7710000
	WORD $0x728de756
	WORD $0x5000006f
	WORD $0xa7140018
	WORD $0xe7630002
	WORD $0x204db309
	WORD $0x0066a714
	WORD $0x0010b309
	WORD $0x0062a744
	WORD $0x000cb309
	WORD $0x0026a744
	WORD $0x0009ed60
	WORD $0x05550010
	WORD $0xa7440003
	WORD $0x28622826
	WORD $0xb3090022
	WORD $0xe7441000
	WORD $0x026ae700
	WORD $0x7000508d
	WORD $0xa7140018
	WORD $0xe7330003
	WORD $0x204db309
	WORD $0x0033a714
	WORD $0x0010b309
	WORD $0x0032a744
	WORD $0x000cb309
	WORD $0x0023a744
	WORD $0x0009ed30
	WORD $0x05550010
	WORD $0xa7440003
	WORD $0x28322823
	WORD $0xb3090022
	WORD $0xe7001000
	WORD $0x408da714
	WORD $0x0019b309
	WORD $0x0000a714
	WORD $0x0014b309
	WORD $0x0002a744
	WORD $0x0010b309
	WORD $0x0020a744
	WORD $0x000ded00
	WORD $0x05550010
	WORD $0x2810a744
	WORD $0x00032812
	WORD $0x2821a7f4
	WORD $0x00032820
	WORD $0xb3090022
	WORD $0xa7140018
	WORD $0xe7100001
	WORD $0x204db309
	WORD $0x0011a714
	WORD $0x0010b309
	WORD $0x0012a744
	WORD $0x000cb309
	WORD $0x0021a744
	WORD $0x0009ed10
	WORD $0x05550010
	WORD $0xa7440003
	WORD $0x28122821
	WORD $0xb3090022
	WORD $0xa7140018
	WORD $0xe7100002
	WORD $0x204db309
	WORD $0x0011a714
	WORD $0x0010b309
	WORD $0x0012a744
	WORD $0x000cb309
	WORD $0x0021a744
	WORD $0x0009ed10
	WORD $0x05550010
	WORD $0xa7440003
	WORD $0x28122821
	WORD $0xb3090022
	WORD $0xa7140018
	WORD $0xe7000003
	WORD $0x204db309
	WORD $0x0000a714
	WORD $0x0010b309
	WORD $0x0002a744
	WORD $0x000cb309
	WORD $0x0020a744
	WORD $0x0009ed00
	WORD $0x05550010
	WORD $0xa7440003
	WORD $0x28022820
	WORD $0x70202000
	WORD $0xebeff070
	WORD $0x000407fe
	RET

// func maxrFloat32VX(a []float32) float32
TEXT ·maxrFloat32VX(SB), NOSPLIT, $160-28
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·maxrFloat32VXBody(SB)
	RET

// maxrFloat32VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·maxrFloat32VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024e700
	WORD $0x30002005
	WORD $0xa7190000
	WORD $0xe7200000
	WORD $0x0056e710
	WORD $0x00000056
	WORD $0xec44014b
	WORD $0x087cec04
	WORD $0xfff800d9
	WORD $0xeb100003
	WORD $0x000cec18
	WORD $0x00d7007c
	WORD $0x41001001
	WORD $0xe7300000
	WORD $0x0044ec50
	WORD $0x02be0059
	WORD $0xa7190000
	WORD $0xb90400e3
	WORD $0xe7200000
	WORD $0x0056e710
	WORD $0x00000056
	WORD $0xe750e000
	WORD $0x0006e740
	WORD $0xe0100006
	WORD $0xe7153000
	WORD $0x28eae723
	WORD $0x500028eb
	WORD $0xe7121000
	WORD $0x0e6ae724
	WORD $0x300028ea
	WORD $0xe7334000
	WORD $0x28ebe723
	WORD $0x20000e6a
	WORD $0xe7325000
	WORD $0x28ebe773
	WORD $0x500028fb
	WORD $0xe7623000
	WORD $0x20eae773
	WORD $0x200020eb
	WORD $0xe7452000
	WORD $0x28ebe761
	WORD $0x400028eb
	WORD $0xe7337000
	WORD $0x0e6ae773
	WORD $0x400028fb
	WORD $0xa75bfffe
	WORD $0x41101010
	WORD $0xe7676000
	WORD $0x006ae771
	WORD $0x300020ea
	WORD $0xe7031000
	WORD $0x28ebe754
	WORD $0x100028eb
	WORD $0xe7667000
	WORD $0x0e6ae714
	WORD $0x10000e6f
	WORD $0xe7325000
	WORD $0x398de770
	WORD $0x7000046a
	WORD $0xe7014000
	WORD $0x698de725
	WORD $0x20000e6f
	WORD $0xe7553000
	WORD $0x138de744
	WORD $0x0000238d
	WORD $0xe7252000
	WORD $0x608de750
	WORD $0xe0200006
	WORD $0xe7141000
	WORD $0x708de740
	WORD $0xe0300006
	WORD $0x41e0e040
	WORD $0xe7623000
	WORD $0x20eae773
	WORD $0x200020eb
	WORD $0xe7153000
	WORD $0x28eae723
	WORD $0x500028eb
	WORD $0xe7676000
	WORD $0x006ae772
	WORD $0x500020eb
	WORD $0xe7735000
	WORD $0x28fbe712
	WORD $0x10000e6a
	WORD $0xe7243000
	WORD $0x28eae743
	WORD $0x400028eb
	WORD $0xe7614000
	WORD $0x28ebe777
	WORD $0x7000026a
	WORD $0xe7734000
	WORD $0x28fbe701
	WORD $0x300028ea
	WORD $0xe7331000
	WORD $0x28ebe724
	WORD $0x20000e6a
	WORD $0xe7452000
	WORD $0x28ebe754
	WORD $0x100028eb
	WORD $0xe7667000
	WORD $0x0e6ae703
	WORD $0x00000e6a
	WORD $0xe7314000
	WORD $0x698de772
	WORD $0x5000708d
	WORD $0xe7252000
	WORD $0x0e6fe714
	WORD $0x10000e6f
	WORD $0xe7557000
	WORD $0x118de744
	WORD $0x3000238d
	WORD $0xe7141000
	WORD $0x018de725
	WORD $0x2000608d
	WORD $0xa774ff46
	WORD $0xa7010001
	WORD $0xa774000c
	WORD $0xa7f4006a
	WORD $0xa7190000
	WORD $0xe7200000
	WORD $0x0056e710
	WORD $0x00000056
	WORD $0xe7500000
	WORD $0x0044eb11
	WORD $0x0002000d
	WORD $0xe7413000
	WORD $0x0006e762
	WORD $0x500020ea
	WORD $0xe7752000
	WORD $0x20ebe731
	WORD $0x30100006
	WORD $0xe7676000
	WORD $0x006ae771
	WORD $0x500020ea
	WORD $0xe7051000
	WORD $0x28ebe770
	WORD $0x7000046a
	WORD $0xe7045000
	WORD $0x28eae715
	WORD $0x400028eb
	WORD $0xe7010000
	WORD $0x0e6ae713
	WORD $0x500028ea
	WORD $0xe7253000
	WORD $0x28ebe741
	WORD $0x300028eb
	WORD $0xe7524000
	WORD $0x28ebe765
	WORD $0x300028fb
	WORD $0xe7554000
	WORD $0x20fbe712
	WORD $0x10000e6a
	WORD $0xe7242000
	WORD $0x28ebe733
	WORD $0x100028eb
	WORD $0xe7555000
	WORD $0x046ae744
	WORD $0x60000e6a
	WORD $0xe7413000
	WORD $0x498de752
	WORD $0x4000508d
	WORD $0xe7131000
	WORD $0x0e6fe702
	WORD $0x00000e6f
	WORD $0xe7445000
	WORD $0x018de733
	WORD $0x4000138d
	WORD $0xe7131000
	WORD $0x708de724
	WORD $0x2000608d
	WORD $0xec1401bc
	WORD $0x0059ec14
	WORD $0x0013a064
	WORD $0xeb510002
	WORD $0x000de730
	WORD $0x00000056
	WORD $0xe7353000
	WORD $0x00034100
	WORD $0x1001ec04
	WORD $0x000d4064
	WORD $0xa7f40010
	WORD $0xe7300000
	WORD $0x00564100
	WORD $0x1001ec04
	WORD $0x0009a064
	WORD $0xeb500002
	WORD $0x000de735
	WORD $0x30001003
	WORD $0x41001002
	WORD $0xec040030
	WORD $0xa064eb50
	WORD $0x0002000d
	WORD $0xe7353000
	WORD $0x20034100
	WORD $0x1003ec04
	WORD $0x002a4064
	WORD $0x41001004
	WORD $0xec040030
	WORD $0xa064eb50
	WORD $0x0002000d
	WORD $0xe7053000
	WORD $0x00034100
	WORD $0x1005ec04
	WORD $0x002a4064
	WORD $0x41001006
	WORD $0xec040030
	WORD $0xa064eb50
	WORD $0x0002000d
	WORD $0xe7053000
	WORD $0x20034100
	WORD $0x1007ec04
	WORD $0x002a4064
	WORD $0xa7f4002d
	WORD $0x41001003
	WORD $0xec04ffdc
	WORD $0xa064eb50
	WORD $0x0002000d
	WORD $0xe7353000
	WORD $0x30034100
	WORD $0x1004ec04
	WORD $0xffd64064
	WORD $0x41001005
	WORD $0xec04ffdc
	WORD $0xa064eb50
	WORD $0x0002000d
	WORD $0xe7053000
	WORD $0x10034100
	WORD $0x1006ec04
	WORD $0xffd64064
	WORD $0x41001007
	WORD $0xec040009
	WORD $0xa064eb10
	WORD $0x0002000d
	WORD $0xe7013000
	WORD $0x3003e740
	WORD $0x00000044
	WORD $0xe7524000
	WORD $0x20eae764
	WORD $0x200020eb
	WORD $0xe7065000
	WORD $0x086ae753
	WORD $0x400020ea
	WORD $0xe7643000
	WORD $0x20ebe732
	WORD $0x300028eb
	WORD $0xe7543000
	WORD $0x28fbe716
	WORD $0x5000086a
	WORD $0xe7232000
	WORD $0x28ebe733
	WORD $0x50000e6a
	WORD $0xe7121000
	WORD $0x0e6fe732
	WORD $0x3000398d
	WORD $0xe7333000
	WORD $0x138de733
	WORD $0x2000018d
	WORD $0xe7504000
	WORD $0x20eae764
	WORD $0x000020eb
	WORD $0xe7710000
	WORD $0x20ebe744
	WORD $0x000028fb
	WORD $0xb3090033
	WORD $0xe7565000
	WORD $0x006ae760
	WORD $0x100020eb
	WORD $0xe7774000
	WORD $0x026aa714
	WORD $0x0011e723
	WORD $0x0001204d
	WORD $0xb3090022
	WORD $0xa714001b
	WORD $0xb3090023
	WORD $0xa7240017
	WORD $0xb3090032
	WORD $0xa7d40005
	WORD $0x2823a7f4
	WORD $0x0010e703
	WORD $0x00000856
	WORD $0xed200555
	WORD $0x0010a744
	WORD $0x0005e702
	WORD $0x00000856
	WORD $0xe7200000
	WORD $0x0456b309
	WORD $0x0022e701
	WORD $0x400028ea
	WORD $0xe7441000
	WORD $0x20ebe771
	WORD $0x0000708d
	WORD $0xe7565000
	WORD $0x006fa714
	WORD $0x0017e763
	WORD $0x0002204d
	WORD $0xb3090066
	WORD $0xa714000f
	WORD $0xb3090062
	WORD $0xa724000b
	WORD $0xb3090026
	WORD $0xa7240008
	WORD $0xed600555
	WORD $0x0010a744
	WORD $0x00032826
	WORD $0xb3090022
	WORD $0xe7440000
	WORD $0x026ae700
	WORD $0x7000508d
	WORD $0xa7140017
	WORD $0xe7330003
	WORD $0x204db309
	WORD $0x0033a714
	WORD $0x000fb309
	WORD $0x0032a724
	WORD $0x000bb309
	WORD $0x0023a724
	WORD $0x0008ed30
	WORD $0x05550010
	WORD $0xa7440003
	WORD $0x2823b309
	WORD $0x0022e700
	WORD $0x1000408d
	WORD $0xa7140014
	WORD $0xb3090000
	WORD $0xa714000f
	WORD $0xb3090002
	WORD $0xa724000b
	WORD $0xb3090020
	WORD $0xa7240008
	WORD $0xed000555
	WORD $0x0010a744
	WORD $0x00032820
	WORD $0xb3090022
	WORD $0xa7140017
	WORD $0xe7100001
	WORD $0x204db309
	WORD $0x0011a714
	WORD $0x000fb309
	WORD $0x0012a724
	WORD $0x000bb309
	WORD $0x0021a724
	WORD $0x0008ed10
	WORD $0x05550010
	WORD $0xa7440003
	WORD $0x2821b309
	WORD $0x0022a714
	WORD $0x0017e710
	WORD $0x0002204d
	WORD $0xb3090011
	WORD $0xa714000f
	WORD $0xb3090012
	WORD $0xa724000b
	WORD $0xb3090021
	WORD $0xa7240008
	WORD $0xed100555
	WORD $0x0010a744
	WORD $0x00032821
	WORD $0xb3090022
	WORD $0xa7140017
	WORD $0xe7000003
	WORD $0x204db309
	WORD $0x0000a714
	WORD $0x000fb309
	WORD $0x0002a724
	WORD $0x000bb309
	WORD $0x0020a724
	WORD $0x0008ed00
	WORD $0x05550010
	WORD $0xa7440003
	WORD $0x28207020
	WORD $0x2000ebef
	WORD $0xf0700004
	BYTE $0x07
	BYTE $0xfe
	RET

// func sumsqFloat32VX(a []float32) float32
TEXT ·sumsqFloat32VX(SB), NOSPLIT, $160-28
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·sumsqFloat32VXBody(SB)
	RET

// sumsqFloat32VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumsqFloat32VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024e700
	WORD $0x00000044
	WORD $0xec4a0019
	WORD $0x107ce710
	WORD $0x00000044
	WORD $0xe7300000
	WORD $0x0044e720
	WORD $0x00000044
	WORD $0xe7400000
	WORD $0x0044a719
	WORD $0x0000ec14
	WORD $0x010c4064
	WORD $0xe7500000
	WORD $0x0044a7f4
	WORD $0x0117ec54
	WORD $0xfff000d9
	WORD $0xeb150004
	WORD $0x000c4110
	WORD $0x1001ec01
	WORD $0x3ebf0059
	WORD $0xec5a0015
	WORD $0x307de710
	WORD $0x00000044
	WORD $0xa7590000
	WORD $0xa7190010
	WORD $0xe7300000
	WORD $0x0044e720
	WORD $0x00000044
	WORD $0xe7400000
	WORD $0x0044a7f4
	WORD $0x00b3e710
	WORD $0x00000044
	WORD $0xe7300000
	WORD $0x0044e720
	WORD $0x00000044
	WORD $0xe7400000
	WORD $0x0044ec51
	WORD $0x03bd0059
	WORD $0xa7190010
	WORD $0xb90400e3
	WORD $0xe750e030
	WORD $0x0006e760
	WORD $0xe0000006
	WORD $0xe770e010
	WORD $0x0006e700
	WORD $0xe0200806
	WORD $0xe7777000
	WORD $0x20e7e766
	WORD $0x600020e7
	WORD $0xe7555000
	WORD $0x20e7e700
	WORD $0x00002ee7
	WORD $0xe7445000
	WORD $0x20e3e711
	WORD $0x600020e3
	WORD $0xe750e060
	WORD $0x0006e760
	WORD $0xe0500006
	WORD $0xe710e070
	WORD $0x08064110
	WORD $0x1040a75b
	WORD $0xfffce733
	WORD $0x700020e3
	WORD $0xe770e040
	WORD $0x0006e722
	WORD $0x000022e3
	WORD $0xe7777000
	WORD $0x20e7e766
	WORD $0x600020e7
	WORD $0xe7555000
	WORD $0x20e7e701
	WORD $0x10002ee7
	WORD $0xe710e0a0
	WORD $0x0806e722
	WORD $0x500020e3
	WORD $0xe750e0b0
	WORD $0x0006e733
	WORD $0x600020e3
	WORD $0xe760e080
	WORD $0x0006e711
	WORD $0x700020e3
	WORD $0xe770e090
	WORD $0x0006e744
	WORD $0x000022e3
	WORD $0xe7777000
	WORD $0x20e7e766
	WORD $0x600020e7
	WORD $0xe7555000
	WORD $0x20e7e701
	WORD $0x10002ee7
	WORD $0xe710e0f0
	WORD $0x0806e744
	WORD $0x500020e3
	WORD $0xe750e0e0
	WORD $0x0006e711
	WORD $0x600020e3
	WORD $0xe760e0d0
	WORD $0x0006e733
	WORD $0x700020e3
	WORD $0xe770e0c0
	WORD $0x000641e0
	WORD $0xe100e722
	WORD $0x000022e3
	WORD $0xe7011000
	WORD $0x2ee7e777
	WORD $0x700020e7
	WORD $0xe7666000
	WORD $0x20e7e755
	WORD $0x500020e7
	WORD $0xe7225000
	WORD $0x20e3e733
	WORD $0x600020e3
	WORD $0xe7117000
	WORD $0x20e3e744
	WORD $0x000022e3
	WORD $0xa774ff6a
	WORD $0xec080033
	WORD $0x007cec51
	WORD $0xfff000d9
	WORD $0xebe50002
	WORD $0x000de75e
	WORD $0x30300006
	WORD $0xe76e3020
	WORD $0x0006e77e
	WORD $0x30100006
	WORD $0xe70e3000
	WORD $0x0806e700
	WORD $0x00002ee7
	WORD $0xe7777000
	WORD $0x20e7e766
	WORD $0x600020e7
	WORD $0xe7555000
	WORD $0x20e7e744
	WORD $0x500020e3
	WORD $0xe7226000
	WORD $0x20e3b904
	WORD $0x00514110
	WORD $0x1010e733
	WORD $0x700020e3
	WORD $0xe7110000
	WORD $0x22e3a707
	WORD $0xffd5ec14
	WORD $0x01bb0059
	WORD $0xec14fefa
	WORD $0xa064eb51
	WORD $0x0002000d
	WORD $0xed053000
	WORD $0x0024b317
	WORD $0x0000e750
	WORD $0x00000044
	WORD $0xe7005000
	WORD $0x2061e700
	WORD $0x50003061
	WORD $0xe7600000
	WORD $0x0044e770
	WORD $0x00000044
	WORD $0x41001001
	WORD $0xec040080
	WORD $0x40644100
	WORD $0x1002ec04
	WORD $0x008f4064
	WORD $0x41001003
	WORD $0xec04009e
	WORD $0x40644100
	WORD $0x1004ec04
	WORD $0x00ad4064
	WORD $0x41001005
	WORD $0xec0400bc
	WORD $0x40644100
	WORD $0x1006ec04
	WORD $0x00cb4064
	WORD $0x41001007
	WORD $0xec0400da
	WORD $0x40644100
	WORD $0x1008ec04
	WORD $0x00e94064
	WORD $0x41001009
	WORD $0xec0400f8
	WORD $0x40644100
	WORD $0x100aec04
	WORD $0x01074064
	WORD $0x4100100b
	WORD $0xec040116
	WORD $0x40644100
	WORD $0x100cec04
	WORD $0x01254064
	WORD $0x4100100d
	WORD $0xec040134
	WORD $0x40644100
	WORD $0x100eec04
	WORD $0x01434064
	WORD $0x4100100f
	WORD $0xec040012
	WORD $0xa064eb10
	WORD $0x0002000d
	WORD $0xe7013000
	WORD $0x2805e700
	WORD $0x00082ee7
	WORD $0xe7000000
	WORD $0x2421e770
	WORD $0x00032022
	WORD $0xe7335000
	WORD $0x20e3e744
	WORD $0x700020e3
	WORD $0xe7010000
	WORD $0x20e3e712
	WORD $0x600020e3
	WORD $0xe7010000
	WORD $0x20e3e714
	WORD $0x300020e3
	WORD $0xe7010000
	WORD $0x20e3e710
	WORD $0x00003060
	WORD $0xe7010000
	WORD $0x20e3e710
	WORD $0x0001204d
	WORD $0xe7010000
	WORD $0x20e3e700
	WORD $0x2000000b
	WORD $0xebeff070
	WORD $0x000407fe
	WORD $0xeb500002
	WORD $0x000de705
	WORD $0x30002805
	WORD $0xe7000008
	WORD $0x2ee7e700
	WORD $0x00002421
	WORD $0xe7000001
	WORD $0x20224100
	WORD $0x1002ec04
	WORD $0xff77a064
	WORD $0xeb500002
	WORD $0x000de705
	WORD $0x30002805
	WORD $0xe7000008
	WORD $0x2ee7e700
	WORD $0x00002421
	WORD $0xe7000002
	WORD $0x20224100
	WORD $0x1003ec04
	WORD $0xff68a064
	WORD $0xeb500002
	WORD $0x000de705
	WORD $0x30002805
	WORD $0xe7000008
	WORD $0x2ee7e700
	WORD $0x00002421
	WORD $0xe7000003
	WORD $0x20224100
	WORD $0x1004ec04
	WORD $0xff59a064
	WORD $0xeb500002
	WORD $0x000de705
	WORD $0x30002805
	WORD $0xe7000008
	WORD $0x2ee7e700
	WORD $0x00002421
	WORD $0xe7500000
	WORD $0x20224100
	WORD $0x1005ec04
	WORD $0xff4aa064
	WORD $0xeb500002
	WORD $0x000de705
	WORD $0x30002805
	WORD $0xe7000008
	WORD $0x2ee7e700
	WORD $0x00002421
	WORD $0xe7500001
	WORD $0x20224100
	WORD $0x1006ec04
	WORD $0xff3ba064
	WORD $0xeb500002
	WORD $0x000de705
	WORD $0x30002805
	WORD $0xe7000008
	WORD $0x2ee7e700
	WORD $0x00002421
	WORD $0xe7500002
	WORD $0x20224100
	WORD $0x1007ec04
	WORD $0xff2ca064
	WORD $0xeb500002
	WORD $0x000de705
	WORD $0x30002805
	WORD $0xe7000008
	WORD $0x2ee7e700
	WORD $0x00002421
	WORD $0xe7500003
	WORD $0x20224100
	WORD $0x1008ec04
	WORD $0xff1da064
	WORD $0xeb500002
	WORD $0x000de705
	WORD $0x30002805
	WORD $0xe7000008
	WORD $0x2ee7e700
	WORD $0x00002421
	WORD $0xe7600000
	WORD $0x20224100
	WORD $0x1009ec04
	WORD $0xff0ea064
	WORD $0xeb500002
	WORD $0x000de705
	WORD $0x30002805
	WORD $0xe7000008
	WORD $0x2ee7e700
	WORD $0x00002421
	WORD $0xe7600001
	WORD $0x20224100
	WORD $0x100aec04
	WORD $0xfeffa064
	WORD $0xeb500002
	WORD $0x000de705
	WORD $0x30002805
	WORD $0xe7000008
	WORD $0x2ee7e700
	WORD $0x00002421
	WORD $0xe7600002
	WORD $0x20224100
	WORD $0x100bec04
	WORD $0xfef0a064
	WORD $0xeb500002
	WORD $0x000de705
	WORD $0x30002805
	WORD $0xe7000008
	WORD $0x2ee7e700
	WORD $0x00002421
	WORD $0xe7600003
	WORD $0x20224100
	WORD $0x100cec04
	WORD $0xfee1a064
	WORD $0xeb500002
	WORD $0x000de705
	WORD $0x30002805
	WORD $0xe7000008
	WORD $0x2ee7e700
	WORD $0x00002421
	WORD $0xe7700000
	WORD $0x20224100
	WORD $0x100dec04
	WORD $0xfed2a064
	WORD $0xeb500002
	WORD $0x000de705
	WORD $0x30002805
	WORD $0xe7000008
	WORD $0x2ee7e700
	WORD $0x00002421
	WORD $0xe7700001
	WORD $0x20224100
	WORD $0x100eec04
	WORD $0xfec3a064
	WORD $0xeb500002
	WORD $0x000de705
	WORD $0x30002805
	WORD $0xe7000008
	WORD $0x2ee7e700
	WORD $0x00002421
	WORD $0xe7700002
	WORD $0x20224100
	WORD $0x100fec04
	WORD $0xfeb44064
	WORD $0xa7f4fec0
	RET

// func sumsqdevFloat32VX(a []float32, c float32) float32
TEXT ·sumsqdevFloat32VX(SB), NOSPLIT, $160-36
	NO_LOCAL_POINTERS
	MOVD $ret+32(FP), R2
	MOVD a_base+0(FP), R3
	FMOVS c+24(FP), F0
	MOVD a_len+8(FP), R4
	BL ·sumsqdevFloat32VXBody(SB)
	RET

// sumsqdevFloat32VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumsqdevFloat32VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024e710
	WORD $0x00000044
	WORD $0xec4a0019
	WORD $0x107ce720
	WORD $0x00000044
	WORD $0xe7300000
	WORD $0x0044e740
	WORD $0x00000044
	WORD $0xe7500000
	WORD $0x0044a719
	WORD $0x0000ec14
	WORD $0x014e4064
	WORD $0xe7600000
	WORD $0x0044a7f4
	WORD $0x015bec54
	WORD $0xfff000d9
	WORD $0xeb150004
	WORD $0x000c4110
	WORD $0x1001ec01
	WORD $0x3ebf0059
	WORD $0xec5a0015
	WORD $0x307de720
	WORD $0x00000044
	WORD $0xa7590000
	WORD $0xa7190010
	WORD $0xe7300000
	WORD $0x0044e740
	WORD $0x00000044
	WORD $0xe7500000
	WORD $0x0044a7f4
	WORD $0x00e6e760
	WORD $0x0000204d
	WORD $0xe7200000
	WORD $0x0044e730
	WORD $0x00000044
	WORD $0xe7400000
	WORD $0x0044e750
	WORD $0x00000044
	WORD $0xec5103bd
	WORD $0x0059a719
	WORD $0x0010b904
	WORD $0x00e3e700
	WORD $0xe0000806
	WORD $0xe7006000
	WORD $0x2ce2e770
	WORD $0xe0100006
	WORD $0xe710e020
	WORD $0x0806e720
	WORD $0xe0300806
	WORD $0xe7226000
	WORD $0x2ce2e711
	WORD $0x60002ce2
	WORD $0xe7776000
	WORD $0x20e2e700
	WORD $0x00002ee7
	WORD $0xe730e040
	WORD $0x0806e777
	WORD $0x700020e7
	WORD $0xe740e050
	WORD $0x0806e711
	WORD $0x10002ee7
	WORD $0x41101040
	WORD $0xa75bfffc
	WORD $0xe7222000
	WORD $0x2ee7e722
	WORD $0x000022e3
	WORD $0xe7036000
	WORD $0x2ce2e730
	WORD $0xe0800806
	WORD $0xe7552000
	WORD $0x22e3e720
	WORD $0xe0600806
	WORD $0xe7441000
	WORD $0x22e3e710
	WORD $0xe0700806
	WORD $0xe7446000
	WORD $0x2ce2e733
	WORD $0x700020e3
	WORD $0xe7700000
	WORD $0x26e7e722
	WORD $0x60002ce2
	WORD $0xe7116000
	WORD $0x2ce2e704
	WORD $0x40002ee7
	WORD $0xe740e0b0
	WORD $0x0806e722
	WORD $0x700020e3
	WORD $0xe770e0a0
	WORD $0x0006e711
	WORD $0x10002ee7
	WORD $0xe7222000
	WORD $0x2ee7e744
	WORD $0x60002ce2
	WORD $0xe7776000
	WORD $0x20e2e733
	WORD $0x60002ce2
	WORD $0xe7330000
	WORD $0x22e3e700
	WORD $0xe0900806
	WORD $0xe7442000
	WORD $0x22e3e723
	WORD $0x30002ee7
	WORD $0xe7777000
	WORD $0x20e7e734
	WORD $0x40002ee7
	WORD $0xe7551000
	WORD $0x22e3e710
	WORD $0xe0e00806
	WORD $0xe7006000
	WORD $0x2ce2e755
	WORD $0x300022e3
	WORD $0xe730e0c0
	WORD $0x0806e744
	WORD $0x700020e3
	WORD $0xe770e0f0
	WORD $0x0006e722
	WORD $0x200022e3
	WORD $0xe720e0d0
	WORD $0x080641e0
	WORD $0xe100e700
	WORD $0x00002ee7
	WORD $0xe7226000
	WORD $0x2ce2e733
	WORD $0x60002ce2
	WORD $0xe7116000
	WORD $0x2ce2e777
	WORD $0x600020e2
	WORD $0xe7330000
	WORD $0x22e3e777
	WORD $0x700020e7
	WORD $0xe7011000
	WORD $0x2ee7e713
	WORD $0x30002ee7
	WORD $0xe7222000
	WORD $0x2ee7e733
	WORD $0x200022e3
	WORD $0xe7221000
	WORD $0x22e3e744
	WORD $0x000022e3
	WORD $0xe7557000
	WORD $0x20e3a774
	WORD $0xff3aec08
	WORD $0x0042007c
	WORD $0xec51fff0
	WORD $0x00d9e760
	WORD $0x0000204d
	WORD $0xebe50002
	WORD $0x000de77e
	WORD $0x30300006
	WORD $0xe70e3000
	WORD $0x0806e71e
	WORD $0x30100806
	WORD $0xe72e3020
	WORD $0x0806e722
	WORD $0x60002ce2
	WORD $0xe7116000
	WORD $0x2ce2e700
	WORD $0x60002ce2
	WORD $0xe7776000
	WORD $0x20e2e777
	WORD $0x700020e7
	WORD $0xe7000000
	WORD $0x2ee7b904
	WORD $0x00514110
	WORD $0x1010e711
	WORD $0x10002ee7
	WORD $0xe7222000
	WORD $0x2ee7e744
	WORD $0x200022e3
	WORD $0xe7331000
	WORD $0x22e3e722
	WORD $0x000022e3
	WORD $0xe7557000
	WORD $0x20e3a707
	WORD $0xffc9ec14
	WORD $0x01bb0059
	WORD $0xec14feb8
	WORD $0xa064eb51
	WORD $0x0002000d
	WORD $0xed153000
	WORD $0x0024b30b
	WORD $0x0010b317
	WORD $0x0011e760
	WORD $0x00000044
	WORD $0xe7116000
	WORD $0x2061e711
	WORD $0x60003061
	WORD $0xe7700000
	WORD $0x0044e700
	WORD $0x00000844
	WORD $0x41001001
	WORD $0xec040082
	WORD $0x40644100
	WORD $0x1002ec04
	WORD $0x00944064
	WORD $0x41001003
	WORD $0xec0400a6
	WORD $0x40644100
	WORD $0x1004ec04
	WORD $0x00b84064
	WORD $0x41001005
	WORD $0xec0400ca
	WORD $0x40644100
	WORD $0x1006ec04
	WORD $0x00dc4064
	WORD $0x41001007
	WORD $0xec0400ee
	WORD $0x40644100
	WORD $0x1008ec04
	WORD $0x01004064
	WORD $0x41001009
	WORD $0xec040112
	WORD $0x40644100
	WORD $0x100aec04
	WORD $0x01244064
	WORD $0x4100100b
	WORD $0xec040136
	WORD $0x40644100
	WORD $0x100cec04
	WORD $0x01484064
	WORD $0x4100100d
	WORD $0xec04015a
	WORD $0x40644100
	WORD $0x100eec04
	WORD $0x016c4064
	WORD $0x4100100f
	WORD $0xec040014
	WORD $0xa064eb10
	WORD $0x0002000d
	WORD $0xe7113000
	WORD $0x2805e701
	WORD $0x000824e2
	WORD $0xb3170000
	WORD $0xe7000000
	WORD $0x2021e700
	WORD $0x00032822
	WORD $0xe7036000
	WORD $0x20e3e735
	WORD $0x000022e3
	WORD $0xe7121000
	WORD $0x20e3e724
	WORD $0x700020e3
	WORD $0xe7121000
	WORD $0x20e3e703
	WORD $0x000020e3
	WORD $0xe7001000
	WORD $0x20e3e710
	WORD $0x00003060
	WORD $0xe7010000
	WORD $0x20e3e710
	WORD $0x0001204d
	WORD $0xe7010000
	WORD $0x20e3e700
	WORD $0x2000000b
	WORD $0xebeff070
	WORD $0x000407fe
	WORD $0xeb500002
	WORD $0x000de715
	WORD $0x30002805
	WORD $0xe7110008
	WORD $0x2ce2e711
	WORD $0x10082ee7
	WORD $0xe7010000
	WORD $0x2421e710
	WORD $0x00012022
	WORD $0x41001002
	WORD $0xec04ff72
	WORD $0xa064eb50
	WORD $0x0002000d
	WORD $0xe7153000
	WORD $0x2805e711
	WORD $0x00082ce2
	WORD $0xe7111008
	WORD $0x2ee7e701
	WORD $0x00002421
	WORD $0xe7100002
	WORD $0x20224100
	WORD $0x1003ec04
	WORD $0xff60a064
	WORD $0xeb500002
	WORD $0x000de715
	WORD $0x30002805
	WORD $0xe7110008
	WORD $0x2ce2e711
	WORD $0x10082ee7
	WORD $0xe7010000
	WORD $0x2421e710
	WORD $0x00032022
	WORD $0x41001004
	WORD $0xec04ff4e
	WORD $0xa064eb50
	WORD $0x0002000d
	WORD $0xe7153000
	WORD $0x2805e711
	WORD $0x00082ce2
	WORD $0xe7111008
	WORD $0x2ee7e701
	WORD $0x00002421
	WORD $0xe7600000
	WORD $0x20224100
	WORD $0x1005ec04
	WORD $0xff3ca064
	WORD $0xeb500002
	WORD $0x000de715
	WORD $0x30002805
	WORD $0xe7110008
	WORD $0x2ce2e711
	WORD $0x10082ee7
	WORD $0xe7010000
	WORD $0x2421e760
	WORD $0x00012022
	WORD $0x41001006
	WORD $0xec04ff2a
	WORD $0xa064eb50
	WORD $0x0002000d
	WORD $0xe7153000
	WORD $0x2805e711
	WORD $0x00082ce2
	WORD $0xe7111008
	WORD $0x2ee7e701
	WORD $0x00002421
	WORD $0xe7600002
	WORD $0x20224100
	WORD $0x1007ec04
	WORD $0xff18a064
	WORD $0xeb500002
	WORD $0x000de715
	WORD $0x30002805
	WORD $0xe7110008
	WORD $0x2ce2e711
	WORD $0x10082ee7
	WORD $0xe7010000
	WORD $0x2421e760
	WORD $0x00032022
	WORD $0x41001008
	WORD $0xec04ff06
	WORD $0xa064eb50
	WORD $0x0002000d
	WORD $0xe7153000
	WORD $0x2805e711
	WORD $0x00082ce2
	WORD $0xe7111008
	WORD $0x2ee7e701
	WORD $0x00002421
	WORD $0xe7700000
	WORD $0x20224100
	WORD $0x1009ec04
	WORD $0xfef4a064
	WORD $0xeb500002
	WORD $0x000de715
	WORD $0x30002805
	WORD $0xe7110008
	WORD $0x2ce2e711
	WORD $0x10082ee7
	WORD $0xe7010000
	WORD $0x2421e770
	WORD $0x00012022
	WORD $0x4100100a
	WORD $0xec04fee2
	WORD $0xa064eb50
	WORD $0x0002000d
	WORD $0xe7153000
	WORD $0x2805e711
	WORD $0x00082ce2
	WORD $0xe7111008
	WORD $0x2ee7e701
	WORD $0x00002421
	WORD $0xe7700002
	WORD $0x20224100
	WORD $0x100bec04
	WORD $0xfed0a064
	WORD $0xeb500002
	WORD $0x000de715
	WORD $0x30002805
	WORD $0xe7110008
	WORD $0x2ce2e711
	WORD $0x10082ee7
	WORD $0xe7010000
	WORD $0x2421e770
	WORD $0x00032022
	WORD $0x4100100c
	WORD $0xec04febe
	WORD $0xa064eb50
	WORD $0x0002000d
	WORD $0xe7153000
	WORD $0x2805e711
	WORD $0x00082ce2
	WORD $0xe7111008
	WORD $0x2ee7e701
	WORD $0x00002421
	WORD $0xe7000000
	WORD $0x28224100
	WORD $0x100dec04
	WORD $0xfeaca064
	WORD $0xeb500002
	WORD $0x000de715
	WORD $0x30002805
	WORD $0xe7110008
	WORD $0x2ce2e711
	WORD $0x10082ee7
	WORD $0xe7010000
	WORD $0x2421e700
	WORD $0x00012822
	WORD $0x4100100e
	WORD $0xec04fe9a
	WORD $0xa064eb50
	WORD $0x0002000d
	WORD $0xe7153000
	WORD $0x2805e711
	WORD $0x00082ce2
	WORD $0xe7111008
	WORD $0x2ee7e701
	WORD $0x00002421
	WORD $0xe7000002
	WORD $0x28224100
	WORD $0x100fec04
	WORD $0xfe884064
	WORD $0xa7f4fe96
	RET

// func minrFloat64VX(a []float64) float64
TEXT ·minrFloat64VX(SB), NOSPLIT, $160-32
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·minrFloat64VXBody(SB)
	RET

// minrFloat64VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·minrFloat64VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xa7fbffd0
	WORD $0x6080f028
	WORD $0x6090f020
	WORD $0x60a0f018
	WORD $0x60b0f010
	WORD $0x60c0f008
	WORD $0x60d0f000
	WORD $0xe7103000
	WORD $0x3005a719
	WORD $0x0000e741
	WORD $0x00000056
	WORD $0xe7310000
	WORD $0x0056e721
	WORD $0x00000056
	WORD $0xe7010000
	WORD $0x0056ec44
	WORD $0x00d4087c
	WORD $0xe7500000
	WORD $0x0044a719
	WORD $0x0008b904
	WORD $0x0053e741
	WORD $0x00000056
	WORD $0xe7310000
	WORD $0x0056e721
	WORD $0x00000056
	WORD $0xe7010000
	WORD $0x0056e760
	WORD $0x50300006
	WORD $0xe7105000
	WORD $0x0806e724
	WORD $0x500038ea
	WORD $0xe7354000
	WORD $0x38ebe743
	WORD $0x500038ea
	WORD $0xe7553000
	WORD $0x38ebe770
	WORD $0x50200006
	WORD $0xe7005010
	WORD $0x0806e723
	WORD $0x20000e6a
	WORD $0xe7354000
	WORD $0x0e6ae742
	WORD $0x500038ea
	WORD $0xe7552000
	WORD $0x38ebe760
	WORD $0x500038ea
	WORD $0xe7750000
	WORD $0x38ebe745
	WORD $0x40000e6a
	WORD $0xe7576000
	WORD $0x0e6ae761
	WORD $0x50003cea
	WORD $0xe7751000
	WORD $0x3aebe7b6
	WORD $0x500038ea
	WORD $0x41101008
	WORD $0x41505040
	WORD $0xe7c56000
	WORD $0x38ebe780
	WORD $0x50003cea
	WORD $0xe7950000
	WORD $0x3aebe767
	WORD $0x60000e6a
	WORD $0xe7775000
	WORD $0x38eae7a5
	WORD $0x700038eb
	WORD $0xe7b56000
	WORD $0x30fbe7c5
	WORD $0x700030fb
	WORD $0xe7d50000
	WORD $0x32fbe7bc
	WORD $0xb0000e6a
	WORD $0xe7c51000
	WORD $0x3afbe7d4
	WORD $0x10003aeb
	WORD $0xe7898000
	WORD $0x0e6ae793
	WORD $0x00003aeb
	WORD $0xe7e27000
	WORD $0x38ebe7f0
	WORD $0x600038eb
	WORD $0xe7814000
	WORD $0x34ebe790
	WORD $0x300034eb
	WORD $0xe77a7000
	WORD $0x0e6ae7a7
	WORD $0x200038eb
	WORD $0xe7a60000
	WORD $0x30ebe7c1
	WORD $0x4000cd8d
	WORD $0xe7d03000
	WORD $0xd48de7c7
	WORD $0x2000c08d
	WORD $0xe7b60000
	WORD $0xb08de7a0
	WORD $0xb000a08d
	WORD $0xe7a2c000
	WORD $0xa98de793
	WORD $0xd000908d
	WORD $0xe7c4c000
	WORD $0x8a8de7bf
	WORD $0xb0000e6f
	WORD $0xe77e7000
	WORD $0x0e6fe789
	WORD $0x80000e6f
	WORD $0xe76d6000
	WORD $0x0e6fe711
	WORD $0xc0006f8d
	WORD $0xe7009000
	WORD $0x8d8de777
	WORD $0xa000738d
	WORD $0xe766a000
	WORD $0xb18de706
	WORD $0x0000518d
	WORD $0xe7272000
	WORD $0x418de730
	WORD $0x3000358d
	WORD $0xe7414000
	WORD $0x258dec14
	WORD $0xff48c064
	WORD $0xec1401bc
	WORD $0x0059ec14
	WORD $0x0013a064
	WORD $0xeb510003
	WORD $0x000de761
	WORD $0x00000056
	WORD $0xe7653000
	WORD $0x00024100
	WORD $0x1001ec04
	WORD $0x000d4064
	WORD $0xa7f40010
	WORD $0xe7610000
	WORD $0x00564100
	WORD $0x1001ec04
	WORD $0x0009a064
	WORD $0xeb500003
	WORD $0x000de765
	WORD $0x30001002
	WORD $0x41001002
	WORD $0xec040013
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7710000
	WORD $0x0056e775
	WORD $0x30000002
	WORD $0x41001003
	WORD $0xec04000d
	WORD $0x4064a7f4
	WORD $0x0010e771
	WORD $0x00000056
	WORD $0x41001003
	WORD $0xec040009
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7753000
	WORD $0x10024100
	WORD $0x1004ec04
	WORD $0x0013a064
	WORD $0xeb500003
	WORD $0x000de751
	WORD $0x00000056
	WORD $0xe7553000
	WORD $0x00024100
	WORD $0x1005ec04
	WORD $0x000d4064
	WORD $0xa7f40010
	WORD $0xe7510000
	WORD $0x00564100
	WORD $0x1005ec04
	WORD $0x0009a064
	WORD $0xeb500003
	WORD $0x000de755
	WORD $0x30001002
	WORD $0x41001006
	WORD $0xec040010
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7153000
	WORD $0x00024100
	WORD $0x1007ec04
	WORD $0x000a4064
	WORD $0xa7f4000d
	WORD $0x41001007
	WORD $0xec040009
	WORD $0xa064eb10
	WORD $0x0003000d
	WORD $0xe7113000
	WORD $0x1002e700
	WORD $0x00000844
	WORD $0xe7140000
	WORD $0x3aeae720
	WORD $0x40003ceb
	WORD $0xe7621000
	WORD $0x0e6ae713
	WORD $0x00003aea
	WORD $0xe7203000
	WORD $0x3cebe712
	WORD $0x10000e6a
	WORD $0xe7260000
	WORD $0x3aeae730
	WORD $0x60003ceb
	WORD $0xe7e06000
	WORD $0x3cfbe773
	WORD $0x20000e6a
	WORD $0xe7946000
	WORD $0x38ebe7b6
	WORD $0x400038eb
	WORD $0xe7e64000
	WORD $0xe98de725
	WORD $0x00003aea
	WORD $0xe7305000
	WORD $0x3cebe779
	WORD $0x70000e6f
	WORD $0xe7b4e000
	WORD $0xbb8de723
	WORD $0x20000e6a
	WORD $0xe7370000
	WORD $0x3aeae740
	WORD $0x70003ceb
	WORD $0xe7d07000
	WORD $0x3cfbe776
	WORD $0xb0007b8d
	WORD $0xe7843000
	WORD $0x0e6ae7a3
	WORD $0x700038eb
	WORD $0xe7c73000
	WORD $0x38ebe750
	WORD $0x50003cfb
	WORD $0xe7d73000
	WORD $0xd98de747
	WORD $0x4000658d
	WORD $0xe7c3d000
	WORD $0xcb8de78a
	WORD $0x80000e6f
	WORD $0xb3190044
	WORD $0xe7325000
	WORD $0x38ebe745
	WORD $0x200038eb
	WORD $0xe7552000
	WORD $0x598de767
	WORD $0xc000838d
	WORD $0xa7140018
	WORD $0xe7740001
	WORD $0x304db319
	WORD $0x0077a714
	WORD $0x0010b319
	WORD $0x0074a744
	WORD $0x000cb319
	WORD $0x0047a744
	WORD $0x0009ed70
	WORD $0x05550011
	WORD $0xa7440003
	WORD $0x28742847
	WORD $0xb3190044
	WORD $0xe7810000
	WORD $0x3aeae790
	WORD $0x10003ceb
	WORD $0xe7601000
	WORD $0x3cfbe772
	WORD $0x000032ea
	WORD $0xe7702000
	WORD $0x3cebe742
	WORD $0x50004b8d
	WORD $0xe7332000
	WORD $0x0e6fe736
	WORD $0x3000118d
	WORD $0xa7140019
	WORD $0xb3190033
	WORD $0xa7140014
	WORD $0xb3190034
	WORD $0xa7440010
	WORD $0xb3190043
	WORD $0xa744000d
	WORD $0xed300555
	WORD $0x00112863
	WORD $0xa7440003
	WORD $0x28642846
	WORD $0xa7f40003
	WORD $0x2843b319
	WORD $0x0044e769
	WORD $0x8000066a
	WORD $0xe7101000
	WORD $0x38ebe721
	WORD $0x000038eb
	WORD $0xe7510000
	WORD $0x698de777
	WORD $0x7000046a
	WORD $0xe7554000
	WORD $0x338da714
	WORD $0x0018e733
	WORD $0x0001304d
	WORD $0xb3190033
	WORD $0xa7140010
	WORD $0xb3190034
	WORD $0xa744000c
	WORD $0xb3190043
	WORD $0xa7440009
	WORD $0xed300555
	WORD $0x0011a744
	WORD $0x00032834
	WORD $0x2843b319
	WORD $0x0044e730
	WORD $0x000032ea
	WORD $0xe7000000
	WORD $0x3cebe720
	WORD $0x50002b8d
	WORD $0xe7616000
	WORD $0x046fe725
	WORD $0x2000708d
	WORD $0xa7140019
	WORD $0xb3190022
	WORD $0xa7140014
	WORD $0xb3190024
	WORD $0xa7440010
	WORD $0xb3190042
	WORD $0xa744000d
	WORD $0xed200555
	WORD $0x00112852
	WORD $0xa7440003
	WORD $0x28542845
	WORD $0xa7f40003
	WORD $0x2842b319
	WORD $0x0044e730
	WORD $0x3000046a
	WORD $0xe7112000
	WORD $0x628da714
	WORD $0x0018e722
	WORD $0x0001304d
	WORD $0xb3190022
	WORD $0xa7140010
	WORD $0xb3190024
	WORD $0xa744000c
	WORD $0xb3190042
	WORD $0xa7440009
	WORD $0xed200555
	WORD $0x0011a744
	WORD $0x00032824
	WORD $0x2842b319
	WORD $0x0044e701
	WORD $0x0000308d
	WORD $0xa7140019
	WORD $0xb3190000
	WORD $0xa7140014
	WORD $0xb3190004
	WORD $0xa7440010
	WORD $0xb3190040
	WORD $0xa744000d
	WORD $0xed000555
	WORD $0x00112810
	WORD $0xa7440003
	WORD $0x28142841
	WORD $0xa7f40003
	WORD $0x2840b319
	WORD $0x0044a714
	WORD $0x0018e700
	WORD $0x0001304d
	WORD $0xb3190000
	WORD $0xa7140010
	WORD $0xb3190004
	WORD $0xa744000c
	WORD $0xb3190040
	WORD $0xa7440009
	WORD $0xed000555
	WORD $0x0011a744
	WORD $0x00032804
	WORD $0x28406880
	WORD $0xf0286890
	WORD $0xf02068a0
	WORD $0xf01868b0
	WORD $0xf01068c0
	WORD $0xf00868d0
	WORD $0xf0006040
	WORD $0x2000a7fb
	WORD $0x003007fe
	RET

// func maxrFloat64VX(a []float64) float64
TEXT ·maxrFloat64VX(SB), NOSPLIT, $160-32
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·maxrFloat64VXBody(SB)
	RET

// maxrFloat64VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·maxrFloat64VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xa7fbffd0
	WORD $0x6080f028
	WORD $0x6090f020
	WORD $0x60a0f018
	WORD $0x60b0f010
	WORD $0x60c0f008
	WORD $0x60d0f000
	WORD $0xe7103000
	WORD $0x3005a719
	WORD $0x0000e741
	WORD $0x00000056
	WORD $0xe7310000
	WORD $0x0056e721
	WORD $0x00000056
	WORD $0xe7010000
	WORD $0x0056ec44
	WORD $0x00d4087c
	WORD $0xe7500000
	WORD $0x0044a719
	WORD $0x0008b904
	WORD $0x0053e741
	WORD $0x00000056
	WORD $0xe7310000
	WORD $0x0056e721
	WORD $0x00000056
	WORD $0xe7010000
	WORD $0x0056e710
	WORD $0x50300806
	WORD $0xe7205000
	WORD $0x38eae735
	WORD $0x000038eb
	WORD $0xe7445000
	WORD $0x38eae755
	WORD $0x400038eb
	WORD $0xe7605020
	WORD $0x0006e770
	WORD $0x50100006
	WORD $0xe7232000
	WORD $0x0e6ae735
	WORD $0x40000e6a
	WORD $0xe7435000
	WORD $0x38eae755
	WORD $0x300038eb
	WORD $0xe7625000
	WORD $0x38eae775
	WORD $0x200038eb
	WORD $0xe7005000
	WORD $0x0806e745
	WORD $0x40000e6a
	WORD $0xe7576000
	WORD $0x0e6ae761
	WORD $0x50003cea
	WORD $0xe7751000
	WORD $0x3aebe767
	WORD $0x60000e6a
	WORD $0x41101008
	WORD $0x41505040
	WORD $0xe7775000
	WORD $0x38eae7a5
	WORD $0x700038eb
	WORD $0xe7b65000
	WORD $0x38eae7c5
	WORD $0x600038eb
	WORD $0xe7805000
	WORD $0x3ceae795
	WORD $0x00003aeb
	WORD $0xe7826000
	WORD $0x30ebe793
	WORD $0x700030eb
	WORD $0xe77a7000
	WORD $0x0e6ae7a4
	WORD $0x00003aeb
	WORD $0xe7a01000
	WORD $0x32ebe7b5
	WORD $0x600030fb
	WORD $0xe7c57000
	WORD $0x30fbe7d5
	WORD $0x000032fb
	WORD $0xe7bcb000
	WORD $0x0e6ae7c5
	WORD $0x10003afb
	WORD $0xe7d10000
	WORD $0x3cebe789
	WORD $0x80000e6a
	WORD $0xe7904000
	WORD $0x3cebe7e7
	WORD $0x300038eb
	WORD $0xe7f62000
	WORD $0x38ebe7ca
	WORD $0xc0000a6a
	WORD $0xe7aad000
	WORD $0x0c6ae799
	WORD $0xc000006a
	WORD $0xe788b000
	WORD $0x006ae782
	WORD $0x6000808d
	WORD $0xe7937000
	WORD $0x908de7a4
	WORD $0x0000ab8d
	WORD $0xe7c01000
	WORD $0xcb8de7bf
	WORD $0xb0000e6f
	WORD $0xe77e7000
	WORD $0x0e6fe789
	WORD $0x80000e6f
	WORD $0xe76d6000
	WORD $0x0e6fe711
	WORD $0xc0006f8d
	WORD $0xe700a000
	WORD $0x8f8de777
	WORD $0x9000718d
	WORD $0xe7668000
	WORD $0xb18de726
	WORD $0x2000518d
	WORD $0xe7373000
	WORD $0x418de740
	WORD $0x4000358d
	WORD $0xe7010000
	WORD $0x258dec14
	WORD $0xff48c064
	WORD $0xec1401bc
	WORD $0x0059ec14
	WORD $0x0013a064
	WORD $0xeb510003
	WORD $0x000de761
	WORD $0x00000056
	WORD $0xe7653000
	WORD $0x00024100
	WORD $0x1001ec04
	WORD $0x000d4064
	WORD $0xa7f40010
	WORD $0xe7610000
	WORD $0x00564100
	WORD $0x1001ec04
	WORD $0x0009a064
	WORD $0xeb500003
	WORD $0x000de765
	WORD $0x30001002
	WORD $0x41001002
	WORD $0xec040013
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7710000
	WORD $0x0056e775
	WORD $0x30000002
	WORD $0x41001003
	WORD $0xec04000d
	WORD $0x4064a7f4
	WORD $0x0010e771
	WORD $0x00000056
	WORD $0x41001003
	WORD $0xec040009
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7753000
	WORD $0x10024100
	WORD $0x1004ec04
	WORD $0x0013a064
	WORD $0xeb500003
	WORD $0x000de751
	WORD $0x00000056
	WORD $0xe7553000
	WORD $0x00024100
	WORD $0x1005ec04
	WORD $0x000d4064
	WORD $0xa7f40010
	WORD $0xe7510000
	WORD $0x00564100
	WORD $0x1005ec04
	WORD $0x0009a064
	WORD $0xeb500003
	WORD $0x000de755
	WORD $0x30001002
	WORD $0x41001006
	WORD $0xec040170
	WORD $0x40644100
	WORD $0x1007ec04
	WORD $0x0009a064
	WORD $0xeb100003
	WORD $0x000de711
	WORD $0x30001002
	WORD $0xe7000000
	WORD $0x0844e714
	WORD $0x00003aea
	WORD $0xe7204000
	WORD $0x3cebe752
	WORD $0x10000e6a
	WORD $0xe7130000
	WORD $0x3aeae720
	WORD $0x30003ceb
	WORD $0xe7121000
	WORD $0x0e6ae726
	WORD $0x00003aea
	WORD $0xe7306000
	WORD $0x3cebe7b4
	WORD $0x600038eb
	WORD $0xe7e06000
	WORD $0x3cfbe763
	WORD $0x20000e6a
	WORD $0xe7864000
	WORD $0x38ebe7bb
	WORD $0xe0000e6a
	WORD $0xe7250000
	WORD $0x3aeae730
	WORD $0x50003ceb
	WORD $0xe7b46000
	WORD $0xb98de768
	WORD $0x60000e6f
	WORD $0xe7232000
	WORD $0x0e6ae737
	WORD $0x00003aea
	WORD $0xe7407000
	WORD $0x3cebe7a3
	WORD $0x700038eb
	WORD $0xe7c07000
	WORD $0x3cfbe766
	WORD $0xb0006b8d
	WORD $0xe7743000
	WORD $0x0e6ae797
	WORD $0x300038eb
	WORD $0xe7aac000
	WORD $0x0e6ae742
	WORD $0x500038eb
	WORD $0xe7464000
	WORD $0x558de7d0
	WORD $0x50003cfb
	WORD $0xe7a37000
	WORD $0xa98de779
	WORD $0x70000e6f
	WORD $0xb3190044
	WORD $0xe7352000
	WORD $0x38ebe744
	WORD $0xd0000e6a
	WORD $0xe767a000
	WORD $0x738da714
	WORD $0x0017e774
	WORD $0x0001304d
	WORD $0xb3190077
	WORD $0xa714000f
	WORD $0xb3190074
	WORD $0xa724000b
	WORD $0xb3190047
	WORD $0xa7240008
	WORD $0xed700555
	WORD $0x0011a744
	WORD $0x00032847
	WORD $0xb3190044
	WORD $0xe7710000
	WORD $0x32eae770
	WORD $0x10003ceb
	WORD $0xe7501000
	WORD $0x38ebe780
	WORD $0x10003cfb
	WORD $0xe7620000
	WORD $0x3aeae790
	WORD $0x20003ceb
	WORD $0xe7425000
	WORD $0x498de733
	WORD $0x20000e6f
	WORD $0xe7363000
	WORD $0x118da714
	WORD $0x0014b319
	WORD $0x0033a714
	WORD $0x000fb319
	WORD $0x0034a724
	WORD $0x000bb319
	WORD $0x0043a724
	WORD $0x0008ed30
	WORD $0x05550011
	WORD $0xa7440003
	WORD $0x2843b319
	WORD $0x0044e767
	WORD $0x7000046a
	WORD $0xe7710000
	WORD $0x30ebe725
	WORD $0x80000e6a
	WORD $0xe7196000
	WORD $0x0e6ae755
	WORD $0x4000338d
	WORD $0xa7140017
	WORD $0xe7330001
	WORD $0x304db319
	WORD $0x0033a714
	WORD $0x000fb319
	WORD $0x0034a724
	WORD $0x000bb319
	WORD $0x0043a724
	WORD $0x0008ed30
	WORD $0x05550011
	WORD $0xa7440003
	WORD $0x2843b319
	WORD $0x0044e730
	WORD $0x000032ea
	WORD $0xe7000000
	WORD $0x3cebe720
	WORD $0x1000298d
	WORD $0xe7676000
	WORD $0x006fe725
	WORD $0x2000118d
	WORD $0xa7140014
	WORD $0xb3190022
	WORD $0xa714000f
	WORD $0xb3190024
	WORD $0xa724000b
	WORD $0xb3190042
	WORD $0xa7240008
	WORD $0xed200555
	WORD $0x0011a744
	WORD $0x00032842
	WORD $0xb3190044
	WORD $0xe7303000
	WORD $0x046ae711
	WORD $0x2000628d
	WORD $0xa7140017
	WORD $0xe7220001
	WORD $0x304db319
	WORD $0x0022a714
	WORD $0x000fb319
	WORD $0x0024a724
	WORD $0x000bb319
	WORD $0x0042a724
	WORD $0x0008ed20
	WORD $0x05550011
	WORD $0xa7440003
	WORD $0x2842b319
	WORD $0x0044e701
	WORD $0x0000308d
	WORD $0xa7140014
	WORD $0xb3190000
	WORD $0xa714000f
	WORD $0xb3190004
	WORD $0xa724000b
	WORD $0xb3190040
	WORD $0xa7240008
	WORD $0xed000555
	WORD $0x0011a744
	WORD $0x00032840
	WORD $0xb3190044
	WORD $0xa7140017
	WORD $0xe7000001
	WORD $0x304db319
	WORD $0x0000a714
	WORD $0x000fb319
	WORD $0x0004a724
	WORD $0x000bb319
	WORD $0x0040a724
	WORD $0x0008ed00
	WORD $0x05550011
	WORD $0xa7440003
	WORD $0x28406880
	WORD $0xf0286890
	WORD $0xf02068a0
	WORD $0xf01868b0
	WORD $0xf01068c0
	WORD $0xf00868d0
	WORD $0xf0006040
	WORD $0x2000a7fb
	WORD $0x003007fe
	WORD $0xeb500003
	WORD $0x000de715
	WORD $0x30000002
	WORD $0x41001007
	WORD $0xec04fe90
	WORD $0x4064a7f4
	BYTE $0xfe
	BYTE $0x93
	RET

// func sumsqFloat64VX(a []float64) float64
TEXT ·sumsqFloat64VX(SB), NOSPLIT, $160-32
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·sumsqFloat64VXBody(SB)
	RET

// sumsqFloat64VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumsqFloat64VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024e700
	WORD $0x00000044
	WORD $0xec4a0025
	WORD $0x107ce720
	WORD $0x00000044
	WORD $0xe7300000
	WORD $0x0044e710
	WORD $0x00000044
	WORD $0xe7400000
	WORD $0x0044e750
	WORD $0x00000044
	WORD $0xe7600000
	WORD $0x0044e770
	WORD $0x00000044
	WORD $0xe7000000
	WORD $0x0844a719
	WORD $0x0000ec14
	WORD $0x01d84064
	WORD $0xe7100000
	WORD $0x0844a7f4
	WORD $0x01dfec54
	WORD $0xfff000d9
	WORD $0xeb150004
	WORD $0x000c4110
	WORD $0x1001ec01
	WORD $0x3ebf0059
	WORD $0xec5a0021
	WORD $0x307de720
	WORD $0x00000044
	WORD $0xa7590000
	WORD $0xa7190010
	WORD $0xe7300000
	WORD $0x0044e710
	WORD $0x00000044
	WORD $0xe7400000
	WORD $0x0044e750
	WORD $0x00000044
	WORD $0xe7600000
	WORD $0x0044e770
	WORD $0x00000044
	WORD $0xe7000000
	WORD $0x0844a7f4
	WORD $0x014fe720
	WORD $0x00000044
	WORD $0xe7300000
	WORD $0x0044e710
	WORD $0x00000044
	WORD $0xe7400000
	WORD $0x0044e750
	WORD $0x00000044
	WORD $0xe7600000
	WORD $0x0044e770
	WORD $0x00000044
	WORD $0xe7000000
	WORD $0x0844ec51
	WORD $0x03bd0059
	WORD $0xa7190010
	WORD $0xb90400e3
	WORD $0xe710e070
	WORD $0x3806e720
	WORD $0xe0603806
	WORD $0xe730e050
	WORD $0x3806e740
	WORD $0xe0403806
	WORD $0xe750e030
	WORD $0x3806e760
	WORD $0xe0003806
	WORD $0xe770e010
	WORD $0x3806e780
	WORD $0xe0203806
	WORD $0xe7555000
	WORD $0x3ee7e744
	WORD $0x40003ee7
	WORD $0xe7333000
	WORD $0x3ee7e722
	WORD $0x20003ee7
	WORD $0xe7111000
	WORD $0x3ee7e788
	WORD $0x80003ee7
	WORD $0xe7777000
	WORD $0x3ee7e766
	WORD $0x60003ee7
	WORD $0xe7001000
	WORD $0x3ee3e777
	WORD $0x200032e3
	WORD $0xe7663000
	WORD $0x32e3e755
	WORD $0x400032e3
	WORD $0xe710e0a0
	WORD $0x3806e744
	WORD $0x500032e3
	WORD $0xe720e090
	WORD $0x3806e730
	WORD $0xe0803806
	WORD $0xe740e0b0
	WORD $0x3806e750
	WORD $0xe0c03806
	WORD $0xe790e0d0
	WORD $0x3806e722
	WORD $0x600032e3
	WORD $0xe760e0e0
	WORD $0x3806e7a0
	WORD $0xe0f03806
	WORD $0xe7337000
	WORD $0x32e3e755
	WORD $0x50003ee7
	WORD $0xe7444000
	WORD $0x3ee7e733
	WORD $0x30003ee7
	WORD $0xe770e100
	WORD $0x38064110
	WORD $0x1040e722
	WORD $0x20003ee7
	WORD $0xa75bfffc
	WORD $0xe7111000
	WORD $0x3ee7e711
	WORD $0x800032e3
	WORD $0xe780e110
	WORD $0x3806e7aa
	WORD $0xa0003ee7
	WORD $0xe7666000
	WORD $0x3ee7e799
	WORD $0x90003ee7
	WORD $0xe7111000
	WORD $0x32e3e710
	WORD $0xe1703806
	WORD $0xe7332000
	WORD $0x32e3e720
	WORD $0xe1603806
	WORD $0xe7223000
	WORD $0x32e3e730
	WORD $0xe1503806
	WORD $0xe7444000
	WORD $0x32e3e740
	WORD $0xe1403806
	WORD $0xe7555000
	WORD $0x32e3e750
	WORD $0xe1303806
	WORD $0xe7669000
	WORD $0x32e3e790
	WORD $0xe1203806
	WORD $0xe7777000
	WORD $0x3ee7e777
	WORD $0x600032e3
	WORD $0xe760e1d0
	WORD $0x3806e755
	WORD $0x50003ee7
	WORD $0xe7444000
	WORD $0x3ee7e733
	WORD $0x30003ee7
	WORD $0xe7222000
	WORD $0x3ee7e711
	WORD $0x10003ee7
	WORD $0xe700a000
	WORD $0x3ee3e7a0
	WORD $0xe1f03806
	WORD $0xe7999000
	WORD $0x3ee7e788
	WORD $0x80003ee7
	WORD $0xe7001000
	WORD $0x3ee3e710
	WORD $0xe1a03806
	WORD $0xe7772000
	WORD $0x32e3e720
	WORD $0xe1903806
	WORD $0xe7663000
	WORD $0x32e3e730
	WORD $0xe1803806
	WORD $0xe7554000
	WORD $0x32e3e740
	WORD $0xe1b03806
	WORD $0xe7445000
	WORD $0x32e3e750
	WORD $0xe1c03806
	WORD $0xe7227000
	WORD $0x32e3e770
	WORD $0xe1e03806
	WORD $0x41e0e200
	WORD $0xe7aaa000
	WORD $0x3ee7e777
	WORD $0x70003ee7
	WORD $0xe7666000
	WORD $0x3ee7e733
	WORD $0x800032e3
	WORD $0xe7555000
	WORD $0x3ee7e744
	WORD $0x40003ee7
	WORD $0xe7333000
	WORD $0x3ee7e722
	WORD $0x20003ee7
	WORD $0xe7111000
	WORD $0x3ee7e711
	WORD $0x900032e3
	WORD $0xe7111000
	WORD $0x32e3e733
	WORD $0x200032e3
	WORD $0xe7223000
	WORD $0x32e3e744
	WORD $0x400032e3
	WORD $0xe7555000
	WORD $0x32e3e766
	WORD $0x600032e3
	WORD $0xe7777000
	WORD $0x32e3e700
	WORD $0xa0003ee3
	WORD $0xa774feda
	WORD $0xec080057
	WORD $0x007cec51
	WORD $0xfff000d9
	WORD $0xebe50003
	WORD $0x000de71e
	WORD $0x30303806
	WORD $0xe72e3020
	WORD $0x3806e73e
	WORD $0x30103806
	WORD $0xe74e3000
	WORD $0x3806e75e
	WORD $0x30403806
	WORD $0xe76e3050
	WORD $0x3806e77e
	WORD $0x30603806
	WORD $0xe78e3070
	WORD $0x3806e788
	WORD $0x80003ee7
	WORD $0xe7777000
	WORD $0x3ee7e766
	WORD $0x60003ee7
	WORD $0xe7555000
	WORD $0x3ee7e744
	WORD $0x40003ee7
	WORD $0xe7333000
	WORD $0x3ee7e722
	WORD $0x20003ee7
	WORD $0xe7111000
	WORD $0x3ee7e744
	WORD $0x100032e3
	WORD $0xe7112000
	WORD $0x32e3e733
	WORD $0x300032e3
	WORD $0xe7224000
	WORD $0x32e3e755
	WORD $0x500032e3
	WORD $0xb9040051
	WORD $0x41101010
	WORD $0xe7666000
	WORD $0x32e3e777
	WORD $0x700032e3
	WORD $0xe7008000
	WORD $0x3ee3a707
	WORD $0xffb1ec14
	WORD $0x01bb0059
	WORD $0xec14fe2e
	WORD $0xa064eb51
	WORD $0x0003000d
	WORD $0x68053000
	WORD $0xb31c0000
	WORD $0xe7100000
	WORD $0x0844e700
	WORD $0x10003261
	WORD $0xe7300000
	WORD $0x0844e720
	WORD $0x00000844
	WORD $0xe7700000
	WORD $0x0844e760
	WORD $0x00000844
	WORD $0xe7500000
	WORD $0x0844e740
	WORD $0x00000844
	WORD $0x41001001
	WORD $0xec04008f
	WORD $0x40644100
	WORD $0x1002ec04
	WORD $0x009b4064
	WORD $0x41001003
	WORD $0xec0400a7
	WORD $0x40644100
	WORD $0x1004ec04
	WORD $0x00b34064
	WORD $0x41001005
	WORD $0xec0400bf
	WORD $0x40644100
	WORD $0x1006ec04
	WORD $0x00cb4064
	WORD $0x41001007
	WORD $0xec0400d7
	WORD $0x40644100
	WORD $0x1008ec04
	WORD $0x00e34064
	WORD $0x41001009
	WORD $0xec0400ef
	WORD $0x40644100
	WORD $0x100aec04
	WORD $0x00fb4064
	WORD $0x4100100b
	WORD $0xec040107
	WORD $0x40644100
	WORD $0x100cec04
	WORD $0x01134064
	WORD $0x4100100d
	WORD $0xec04011f
	WORD $0x40644100
	WORD $0x100eec04
	WORD $0x012b4064
	WORD $0x4100100f
	WORD $0xec04000f
	WORD $0xa064eb10
	WORD $0x0003000d
	WORD $0xe7813000
	WORD $0x3805e788
	WORD $0x80083ee7
	WORD $0xe7448000
	WORD $0x0e84e702
	WORD $0x000030e3
	WORD $0xe7266000
	WORD $0x32e3e733
	WORD $0x100032e3
	WORD $0xe7223000
	WORD $0x30e3e737
	WORD $0x500032e3
	WORD $0xe7113000
	WORD $0x32e3e755
	WORD $0x700032e3
	WORD $0xe7131000
	WORD $0x30e3e730
	WORD $0x400036e3
	WORD $0xe7442000
	WORD $0x32e3e705
	WORD $0x000030e3
	WORD $0xe7334000
	WORD $0x30e3e701
	WORD $0x000030e3
	WORD $0xe7132000
	WORD $0x30e3e701
	WORD $0x000030e3
	WORD $0xe7100001
	WORD $0x304de701
	WORD $0x000030e3
	WORD $0xe7002000
	WORD $0x000aebef
	WORD $0xf0700004
	WORD $0x07feeb50
	WORD $0x0003000d
	WORD $0xe7853000
	WORD $0x3805e788
	WORD $0x80083ee7
	WORD $0xe7008000
	WORD $0x02844100
	WORD $0x1002ec04
	WORD $0xff6ba064
	WORD $0xeb500003
	WORD $0x000de785
	WORD $0x30003805
	WORD $0xe7888008
	WORD $0x3ee7e718
	WORD $0x10001e84
	WORD $0x41001003
	WORD $0xec04ff5f
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7853000
	WORD $0x3805e788
	WORD $0x80083ee7
	WORD $0xe7118000
	WORD $0x0e844100
	WORD $0x1004ec04
	WORD $0xff53a064
	WORD $0xeb500003
	WORD $0x000de785
	WORD $0x30003805
	WORD $0xe7888008
	WORD $0x3ee7e738
	WORD $0x30001e84
	WORD $0x41001005
	WORD $0xec04ff47
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7853000
	WORD $0x3805e788
	WORD $0x80083ee7
	WORD $0xe7338000
	WORD $0x0e844100
	WORD $0x1006ec04
	WORD $0xff3ba064
	WORD $0xeb500003
	WORD $0x000de785
	WORD $0x30003805
	WORD $0xe7888008
	WORD $0x3ee7e728
	WORD $0x20001e84
	WORD $0x41001007
	WORD $0xec04ff2f
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7853000
	WORD $0x3805e788
	WORD $0x80083ee7
	WORD $0xe7228000
	WORD $0x0e844100
	WORD $0x1008ec04
	WORD $0xff23a064
	WORD $0xeb500003
	WORD $0x000de785
	WORD $0x30003805
	WORD $0xe7888008
	WORD $0x3ee7e778
	WORD $0x70001e84
	WORD $0x41001009
	WORD $0xec04ff17
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7853000
	WORD $0x3805e788
	WORD $0x80083ee7
	WORD $0xe7778000
	WORD $0x0e844100
	WORD $0x100aec04
	WORD $0xff0ba064
	WORD $0xeb500003
	WORD $0x000de785
	WORD $0x30003805
	WORD $0xe7888008
	WORD $0x3ee7e768
	WORD $0x60001e84
	WORD $0x4100100b
	WORD $0xec04feff
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7853000
	WORD $0x3805e788
	WORD $0x80083ee7
	WORD $0xe7668000
	WORD $0x0e844100
	WORD $0x100cec04
	WORD $0xfef3a064
	WORD $0xeb500003
	WORD $0x000de785
	WORD $0x30003805
	WORD $0xe7888008
	WORD $0x3ee7e758
	WORD $0x50001e84
	WORD $0x4100100d
	WORD $0xec04fee7
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7853000
	WORD $0x3805e788
	WORD $0x80083ee7
	WORD $0xe7558000
	WORD $0x0e844100
	WORD $0x100eec04
	WORD $0xfedba064
	WORD $0xeb500003
	WORD $0x000de785
	WORD $0x30003805
	WORD $0xe7888008
	WORD $0x3ee7e748
	WORD $0x40001e84
	WORD $0x4100100f
	WORD $0xec04fecf
	WORD $0x4064a7f4
	BYTE $0xfe
	BYTE $0xd8
	RET

// func sumsqdevFloat64VX(a []float64, c float64) float64
TEXT ·sumsqdevFloat64VX(SB), NOSPLIT, $160-40
	NO_LOCAL_POINTERS
	MOVD $ret+32(FP), R2
	MOVD a_base+0(FP), R3
	FMOVD c+24(FP), F0
	MOVD a_len+8(FP), R4
	BL ·sumsqdevFloat64VXBody(SB)
	RET

// sumsqdevFloat64VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumsqdevFloat64VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024e710
	WORD $0x00000044
	WORD $0xec4a0025
	WORD $0x107ce720
	WORD $0x00000044
	WORD $0xe7300000
	WORD $0x0044e740
	WORD $0x00000044
	WORD $0xe7500000
	WORD $0x0044e760
	WORD $0x00000044
	WORD $0xe7700000
	WORD $0x0044e700
	WORD $0x00000844
	WORD $0xe7100000
	WORD $0x0844a719
	WORD $0x0000ec14
	WORD $0x02564064
	WORD $0xe7200000
	WORD $0x0844a7f4
	WORD $0x025fec54
	WORD $0xfff000d9
	WORD $0xeb150004
	WORD $0x000c4110
	WORD $0x1001ec01
	WORD $0x3ebf0059
	WORD $0xec5a0021
	WORD $0x307de720
	WORD $0x00000044
	WORD $0xa7590000
	WORD $0xa7190010
	WORD $0xe7300000
	WORD $0x0044e740
	WORD $0x00000044
	WORD $0xe7500000
	WORD $0x0044e760
	WORD $0x00000044
	WORD $0xe7700000
	WORD $0x0044e700
	WORD $0x00000844
	WORD $0xe7100000
	WORD $0x0844a7f4
	WORD $0x01b2e720
	WORD $0x0000384d
	WORD $0xe7200000
	WORD $0x0044e730
	WORD $0x00000044
	WORD $0xe7400000
	WORD $0x0044e750
	WORD $0x00000044
	WORD $0xe7600000
	WORD $0x0044e770
	WORD $0x00000044
	WORD $0xe7000000
	WORD $0x0844e710
	WORD $0x00000844
	WORD $0xec5103bd
	WORD $0x0059a719
	WORD $0x0010b904
	WORD $0x00e3e7a0
	WORD $0xe0703806
	WORD $0xe750e020
	WORD $0x3806e770
	WORD $0xe0403806
	WORD $0xe7aa2000
	WORD $0x3ee2e777
	WORD $0x20003ee2
	WORD $0xe7552000
	WORD $0x3ee2e760
	WORD $0xe0303806
	WORD $0xe780e050
	WORD $0x3806e790
	WORD $0xe0603806
	WORD $0xe7aaa000
	WORD $0x3ee7e799
	WORD $0x20003ee2
	WORD $0xe7882000
	WORD $0x3ee2e766
	WORD $0x20003ee2
	WORD $0xe7555000
	WORD $0x3ee7e777
	WORD $0x70003ee7
	WORD $0xe730e010
	WORD $0x3806e740
	WORD $0xe0003806
	WORD $0xe711a000
	WORD $0x3ee3e7a0
	WORD $0xe0c03806
	WORD $0xe7b0e0b0
	WORD $0x3806e7c0
	WORD $0xe0a03806
	WORD $0xe7442000
	WORD $0x3ee2e733
	WORD $0x20003ee2
	WORD $0xe7666000
	WORD $0x3ee7e788
	WORD $0x80003ee7
	WORD $0xe7999000
	WORD $0x3ee7e766
	WORD $0x700032e3
	WORD $0xe770e0d0
	WORD $0x3806e7cc
	WORD $0x20003ee2
	WORD $0xe7d0e080
	WORD $0x3806e7bb
	WORD $0x20003ee2
	WORD $0xe7e0e190
	WORD $0x38064110
	WORD $0x1040e744
	WORD $0x500032e3
	WORD $0xa75bfffc
	WORD $0xe75a2000
	WORD $0x3ee2e777
	WORD $0x20003ee2
	WORD $0xe7333000
	WORD $0x3ee7e744
	WORD $0x40003ee7
	WORD $0xe7009000
	WORD $0x3ee3e790
	WORD $0xe0e03806
	WORD $0xe7778000
	WORD $0x32e3e780
	WORD $0xe0f03806
	WORD $0xe7556000
	WORD $0x32e3e760
	WORD $0xe0903806
	WORD $0xe7dd2000
	WORD $0x3ee2e755
	WORD $0x50003ee7
	WORD $0xe7abb000
	WORD $0x3ee7e7bc
	WORD $0xc0003ee7
	WORD $0xe7c0e160
	WORD $0x3806e766
	WORD $0x20003ee2
	WORD $0xe7992000
	WORD $0x3ee2e788
	WORD $0x20003ee2
	WORD $0xe7777000
	WORD $0x3ee7e742
	WORD $0x40003ae3
	WORD $0xe7233000
	WORD $0x32e3e73d
	WORD $0xd00036e7
	WORD $0xe7d0e170
	WORD $0x3806e744
	WORD $0xb00032e3
	WORD $0xe7b0e140
	WORD $0x3806e755
	WORD $0xa00032e3
	WORD $0xe7a0e130
	WORD $0x3806e766
	WORD $0x500032e3
	WORD $0xe750e150
	WORD $0x3806e788
	WORD $0x80003ee7
	WORD $0xe7999000
	WORD $0x3ee7e736
	WORD $0x60003ee7
	WORD $0xe760e120
	WORD $0x3806e7dd
	WORD $0x20003ee2
	WORD $0xe7cc2000
	WORD $0x3ee2e777
	WORD $0x700032e3
	WORD $0xe7552000
	WORD $0x3ee2e77b
	WORD $0x20003ee2
	WORD $0xe7b0e1c0
	WORD $0x3806e7aa
	WORD $0x20003ee2
	WORD $0xe7662000
	WORD $0x3ee2e722
	WORD $0x300032e3
	WORD $0xe730e110
	WORD $0x3806e734
	WORD $0x300034e3
	WORD $0xe740e100
	WORD $0x3806e700
	WORD $0x90003ee3
	WORD $0xe79aa000
	WORD $0x3ee7e711
	WORD $0x80003ee3
	WORD $0xe7777000
	WORD $0x3ee7e755
	WORD $0x50003ee7
	WORD $0xe78cc000
	WORD $0x3ee7e7c0
	WORD $0xe1a03806
	WORD $0xe7add000
	WORD $0x3ee7e7d0
	WORD $0xe1803806
	WORD $0xe7442000
	WORD $0x3ee2e733
	WORD $0x20003ee2
	WORD $0xe7666000
	WORD $0x3ee7e711
	WORD $0xa0003ee3
	WORD $0xe7a0e1d0
	WORD $0x3806e700
	WORD $0x80003ee3
	WORD $0xe780e1e0
	WORD $0x3806e777
	WORD $0x500032e3
	WORD $0xe750e1f0
	WORD $0x3806e766
	WORD $0x700032e3
	WORD $0xe770e1b0
	WORD $0x380641e0
	WORD $0xe200e733
	WORD $0x30003ee7
	WORD $0xe7444000
	WORD $0x3ee7e7ee
	WORD $0x20003ee2
	WORD $0xe7559000
	WORD $0x32e3e79d
	WORD $0x20003ee2
	WORD $0xe7cc2000
	WORD $0x3ee2e777
	WORD $0x20003ee2
	WORD $0xe7bb2000
	WORD $0x3ee2e7aa
	WORD $0x20003ee2
	WORD $0xe7446000
	WORD $0x32e3e768
	WORD $0x20003ee2
	WORD $0xe7552000
	WORD $0x3ee2e755
	WORD $0x50003ee7
	WORD $0xe7666000
	WORD $0x3ee7e78a
	WORD $0xa0003ee7
	WORD $0xe7434000
	WORD $0x3ae3e7ab
	WORD $0xb0003ee7
	WORD $0xe7777000
	WORD $0x3ee7e7bc
	WORD $0xc0003ee7
	WORD $0xe7999000
	WORD $0x3ee7e73e
	WORD $0xe00036e7
	WORD $0xe7223000
	WORD $0x32e3e732
	WORD $0x300030e3
	WORD $0xe7249000
	WORD $0x36e3e744
	WORD $0xb00032e3
	WORD $0xe7557000
	WORD $0x32e3e766
	WORD $0xa00032e3
	WORD $0xe7778000
	WORD $0x32e3e700
	WORD $0x60003ee3
	WORD $0xe7115000
	WORD $0x3ee3a774
	WORD $0xfe7aec08
	WORD $0x0072007c
	WORD $0xec51fff0
	WORD $0x00d9e720
	WORD $0x0000384d
	WORD $0xebe50003
	WORD $0x000de73e
	WORD $0x30703806
	WORD $0xe74e3060
	WORD $0x3806e75e
	WORD $0x30503806
	WORD $0xe76e3040
	WORD $0x3806e77e
	WORD $0x30303806
	WORD $0xe78e3000
	WORD $0x3806e79e
	WORD $0x30103806
	WORD $0xe7ae3020
	WORD $0x3806e7aa
	WORD $0x20003ee2
	WORD $0xe7992000
	WORD $0x3ee2e788
	WORD $0x20003ee2
	WORD $0xe7772000
	WORD $0x3ee2e766
	WORD $0x20003ee2
	WORD $0xe7552000
	WORD $0x3ee2e744
	WORD $0x20003ee2
	WORD $0xe7332000
	WORD $0x3ee2e733
	WORD $0x30003ee7
	WORD $0xe7444000
	WORD $0x3ee7e755
	WORD $0x50003ee7
	WORD $0xe7666000
	WORD $0x3ee7e777
	WORD $0x70003ee7
	WORD $0xb9040051
	WORD $0x41101010
	WORD $0xe7888000
	WORD $0x3ee7e799
	WORD $0x90003ee7
	WORD $0xe7aaa000
	WORD $0x3ee7e744
	WORD $0xa00032e3
	WORD $0xe7339000
	WORD $0x32e3e722
	WORD $0x800032e3
	WORD $0xe7557000
	WORD $0x32e3e766
	WORD $0x600032e3
	WORD $0xe7775000
	WORD $0x32e3e700
	WORD $0x40003ee3
	WORD $0xe7113000
	WORD $0x3ee3a707
	WORD $0xff99ec14
	WORD $0x01bb0059
	WORD $0xec14fdb0
	WORD $0xa064eb51
	WORD $0x0003000d
	WORD $0x68153000
	WORD $0xb31b0010
	WORD $0xb31c0011
	WORD $0xe7200000
	WORD $0x0844e711
	WORD $0x20003261
	WORD $0xe7400000
	WORD $0x0844e730
	WORD $0x00000844
	WORD $0xe7800000
	WORD $0x0844e770
	WORD $0x00000844
	WORD $0xe7600000
	WORD $0x0844e750
	WORD $0x00000844
	WORD $0x41001001
	WORD $0xec040091
	WORD $0x40644100
	WORD $0x1002ec04
	WORD $0x00a04064
	WORD $0x41001003
	WORD $0xec0400af
	WORD $0x40644100
	WORD $0x1004ec04
	WORD $0x00be4064
	WORD $0x41001005
	WORD $0xec0400cd
	WORD $0x40644100
	WORD $0x1006ec04
	WORD $0x00dc4064
	WORD $0x41001007
	WORD $0xec0400eb
	WORD $0x40644100
	WORD $0x1008ec04
	WORD $0x00fa4064
	WORD $0x41001009
	WORD $0xec040109
	WORD $0x40644100
	WORD $0x100aec04
	WORD $0x01184064
	WORD $0x4100100b
	WORD $0xec040127
	WORD $0x40644100
	WORD $0x100cec04
	WORD $0x01364064
	WORD $0x4100100d
	WORD $0xec040145
	WORD $0x40644100
	WORD $0x100eec04
	WORD $0x01544064
	WORD $0x4100100f
	WORD $0xec040011
	WORD $0xa064eb10
	WORD $0x0003000d
	WORD $0xe7913000
	WORD $0x3805e709
	WORD $0x000834e2
	WORD $0xb31c0000
	WORD $0xe7550000
	WORD $0x0c84e706
	WORD $0x800032e3
	WORD $0xe7121000
	WORD $0x30e3e700
	WORD $0x100030e3
	WORD $0xe7177000
	WORD $0x32e3e723
	WORD $0x200032e3
	WORD $0xe7112000
	WORD $0x30e3e720
	WORD $0x600036e3
	WORD $0xe7344000
	WORD $0x32e3e722
	WORD $0x300030e3
	WORD $0xe7315000
	WORD $0x36e3e745
	WORD $0x300032e3
	WORD $0xe7334000
	WORD $0x30e3e702
	WORD $0x000030e3
	WORD $0xe7131000
	WORD $0x30e3e701
	WORD $0x000030e3
	WORD $0xe7100001
	WORD $0x304de701
	WORD $0x000030e3
	WORD $0xe7002000
	WORD $0x000aebef
	WORD $0xf0700004
	WORD $0x07feeb50
	WORD $0x0003000d
	WORD $0xe7953000
	WORD $0x3805e799
	WORD $0x00083ce2
	WORD $0xe7999008
	WORD $0x3ee7e711
	WORD $0x90000284
	WORD $0x41001002
	WORD $0xec04ff66
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7953000
	WORD $0x3805e799
	WORD $0x00083ce2
	WORD $0xe7999008
	WORD $0x3ee7e729
	WORD $0x20001e84
	WORD $0x41001003
	WORD $0xec04ff57
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7953000
	WORD $0x3805e799
	WORD $0x00083ce2
	WORD $0xe7999008
	WORD $0x3ee7e722
	WORD $0x90000e84
	WORD $0x41001004
	WORD $0xec04ff48
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7953000
	WORD $0x3805e799
	WORD $0x00083ce2
	WORD $0xe7999008
	WORD $0x3ee7e749
	WORD $0x40001e84
	WORD $0x41001005
	WORD $0xec04ff39
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7953000
	WORD $0x3805e799
	WORD $0x00083ce2
	WORD $0xe7999008
	WORD $0x3ee7e744
	WORD $0x90000e84
	WORD $0x41001006
	WORD $0xec04ff2a
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7953000
	WORD $0x3805e799
	WORD $0x00083ce2
	WORD $0xe7999008
	WORD $0x3ee7e739
	WORD $0x30001e84
	WORD $0x41001007
	WORD $0xec04ff1b
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7953000
	WORD $0x3805e799
	WORD $0x00083ce2
	WORD $0xe7999008
	WORD $0x3ee7e733
	WORD $0x90000e84
	WORD $0x41001008
	WORD $0xec04ff0c
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7953000
	WORD $0x3805e799
	WORD $0x00083ce2
	WORD $0xe7999008
	WORD $0x3ee7e789
	WORD $0x80001e84
	WORD $0x41001009
	WORD $0xec04fefd
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7953000
	WORD $0x3805e799
	WORD $0x00083ce2
	WORD $0xe7999008
	WORD $0x3ee7e788
	WORD $0x90000e84
	WORD $0x4100100a
	WORD $0xec04feee
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7953000
	WORD $0x3805e799
	WORD $0x00083ce2
	WORD $0xe7999008
	WORD $0x3ee7e779
	WORD $0x70001e84
	WORD $0x4100100b
	WORD $0xec04fedf
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7953000
	WORD $0x3805e799
	WORD $0x00083ce2
	WORD $0xe7999008
	WORD $0x3ee7e777
	WORD $0x90000e84
	WORD $0x4100100c
	WORD $0xec04fed0
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7953000
	WORD $0x3805e799
	WORD $0x00083ce2
	WORD $0xe7999008
	WORD $0x3ee7e769
	WORD $0x60001e84
	WORD $0x4100100d
	WORD $0xec04fec1
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7953000
	WORD $0x3805e799
	WORD $0x00083ce2
	WORD $0xe7999008
	WORD $0x3ee7e766
	WORD $0x90000e84
	WORD $0x4100100e
	WORD $0xec04feb2
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7953000
	WORD $0x3805e799
	WORD $0x00083ce2
	WORD $0xe7999008
	WORD $0x3ee7e759
	WORD $0x50001e84
	WORD $0x4100100f
	WORD $0xec04fea3
	WORD $0x4064a7f4
	BYTE $0xfe
	BYTE $0xae
	RET

// func sumsqInt32VX(a []int32) int32
TEXT ·sumsqInt32VX(SB), NOSPLIT, $160-28
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·sumsqInt32VXBody(SB)
	RET

// sumsqInt32VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumsqInt32VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xec4c000c
	WORD $0x007cec4a
	WORD $0x000e047d
	WORD $0xa7080000
	WORD $0xa7190000
	WORD $0xa7f40029
	WORD $0xa7080000
	WORD $0x50002000
	WORD $0x07feec14
	WORD $0x01bd0059
	WORD $0xe7000000
	WORD $0x0044b904
	WORD $0x0053b904
	WORD $0x0001e710
	WORD $0x50000006
	WORD $0xe7011200
	WORD $0x00aaa70b
	WORD $0xfffc4150
	WORD $0x5010a764
	WORD $0xfff6e710
	WORD $0x00000044
	WORD $0xe7001000
	WORD $0x2067e700
	WORD $0x00032021
	WORD $0xec410013
	WORD $0x8064b909
	WORD $0x0041eb11
	WORD $0x0002000d
	WORD $0xb9080031
	WORD $0x58103000
	WORD $0xb9fd1011
	WORD $0x1a014130
	WORD $0x3004a747
	WORD $0xfff95000
	WORD $0x200007fe
	RET

// func sumsqdevInt32VX(a []int32, c int32) int32
TEXT ·sumsqdevInt32VX(SB), NOSPLIT, $160-36
	NO_LOCAL_POINTERS
	MOVD $ret+32(FP), R2
	MOVD a_base+0(FP), R3
	MOVW c+24(FP), R4
	MOVD a_len+8(FP), R5
	BL ·sumsqdevInt32VXBody(SB)
	RET

// sumsqdevInt32VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumsqdevInt32VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024ec5c
	WORD $0x000c007c
	WORD $0xec5a0011
	WORD $0x047da719
	WORD $0x0000a708
	WORD $0x0000a7f4
	WORD $0x0035a708
	WORD $0x00005000
	WORD $0x2000ebef
	WORD $0xf0700004
	WORD $0x07fee704
	WORD $0x40000062
	WORD $0xec1501bd
	WORD $0x0059e710
	WORD $0x0001204d
	WORD $0xe7000000
	WORD $0x0044b904
	WORD $0x00e3b904
	WORD $0x0001e720
	WORD $0xe0000006
	WORD $0xe7221000
	WORD $0x20f7a70b
	WORD $0xfffce702
	WORD $0x220000aa
	WORD $0x41e0e010
	WORD $0xa764fff3
	WORD $0xe7100000
	WORD $0x0044e700
	WORD $0x10002067
	WORD $0xe7000003
	WORD $0x2021ec51
	WORD $0x00148064
	WORD $0xb9090051
	WORD $0xeb110002
	WORD $0x000db908
	WORD $0x00315810
	WORD $0x30001b14
	WORD $0xb9fd1011
	WORD $0x1a014130
	WORD $0x3004a757
	WORD $0xfff85000
	WORD $0x2000ebef
	WORD $0xf0700004
	BYTE $0x07
	BYTE $0xfe
	RET

// func sumsqdiffInt32VX(a []int32, b []int32) int32
TEXT ·sumsqdiffInt32VX(SB), NOSPLIT, $160-52
	NO_LOCAL_POINTERS
	MOVD $ret+48(FP), R2
	MOVD a_base+0(FP), R3
	MOVD b_base+24(FP), R4
	MOVD a_len+8(FP), R5
	BL ·sumsqdiffInt32VXBody(SB)
	RET

// sumsqdiffInt32VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumsqdiffInt32VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024ec5c
	WORD $0x000c007c
	WORD $0xec5a0011
	WORD $0x047da719
	WORD $0x0000a708
	WORD $0x0000a7f4
	WORD $0x0032a708
	WORD $0x00005000
	WORD $0x2000ebef
	WORD $0xf0700004
	WORD $0x07feec15
	WORD $0x01bd0059
	WORD $0xe7000000
	WORD $0x0044a7e9
	WORD $0x0000b904
	WORD $0x0001e71e
	WORD $0x30000006
	WORD $0xe72e4000
	WORD $0x0006e711
	WORD $0x200020f7
	WORD $0xe7011200
	WORD $0x00aaa70b
	WORD $0xfffc41e0
	WORD $0xe010a764
	WORD $0xfff0e710
	WORD $0x00000044
	WORD $0xe7001000
	WORD $0x2067e700
	WORD $0x00032021
	WORD $0xec510013
	WORD $0x8064b909
	WORD $0x0051eb11
	WORD $0x0002000d
	WORD $0x58e13000
	WORD $0x5be14000
	WORD $0xb9fde0ee
	WORD $0x1a0e4110
	WORD $0x1004a757
	WORD $0xfff75000
	WORD $0x2000ebef
	WORD $0xf0700004
	BYTE $0x07
	BYTE $0xfe
	RET

// func sumsqInt8VX(a []int8) int8
TEXT ·sumsqInt8VX(SB), NOSPLIT, $160-25
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·sumsqInt8VXBody(SB)
	RET

// sumsqInt8VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumsqInt8VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xec4c000c
	WORD $0x007cec4a
	WORD $0x000e107d
	WORD $0xa7080000
	WORD $0xa7190000
	WORD $0xa7f4002c
	WORD $0xa7080000
	WORD $0x42002000
	WORD $0x07feec14
	WORD $0x01bb0059
	WORD $0xe7000000
	WORD $0x0044b904
	WORD $0x0053b904
	WORD $0x0001e710
	WORD $0x50000006
	WORD $0xe7011000
	WORD $0x00aaa70b
	WORD $0xfff04150
	WORD $0x5010a764
	WORD $0xfff6e710
	WORD $0x00000044
	WORD $0xe7001000
	WORD $0x0064e700
	WORD $0x10002067
	WORD $0xe7000003
	WORD $0x2021ec41
	WORD $0x00118064
	WORD $0xb9090041
	WORD $0xb9080013
	WORD $0xe3301000
	WORD $0x0076b9fd
	WORD $0x30331a03
	WORD $0x41101001
	WORD $0xa747fff8
	WORD $0x42002000
	BYTE $0x07
	BYTE $0xfe
	RET

// func sumsqdevInt8VX(a []int8, c int8) int8
TEXT ·sumsqdevInt8VX(SB), NOSPLIT, $160-33
	NO_LOCAL_POINTERS
	MOVD $ret+32(FP), R2
	MOVD a_base+0(FP), R3
	MOVB c+24(FP), R4
	MOVD a_len+8(FP), R5
	BL ·sumsqdevInt8VXBody(SB)
	RET

// sumsqdevInt8VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumsqdevInt8VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024ec5c
	WORD $0x000c007c
	WORD $0xec5a0011
	WORD $0x107da719
	WORD $0x0000a708
	WORD $0x0000a7f4
	WORD $0x0038a708
	WORD $0x00004200
	WORD $0x2000ebef
	WORD $0xf0700004
	WORD $0x07fee704
	WORD $0x40000062
	WORD $0xec1501bb
	WORD $0x0059e710
	WORD $0x0007004d
	WORD $0xe7000000
	WORD $0x0044b904
	WORD $0x00e3b904
	WORD $0x0001e720
	WORD $0xe0000006
	WORD $0xe7221000
	WORD $0x00f7a70b
	WORD $0xfff0e702
	WORD $0x200000aa
	WORD $0x41e0e010
	WORD $0xa764fff3
	WORD $0xe7100000
	WORD $0x0044e700
	WORD $0x10000064
	WORD $0xe7001000
	WORD $0x2067e700
	WORD $0x00032021
	WORD $0xec510012
	WORD $0x8064b909
	WORD $0x0051b908
	WORD $0x0013e330
	WORD $0x10000076
	WORD $0x1b34b9fd
	WORD $0x30331a03
	WORD $0x41101001
	WORD $0xa757fff7
	WORD $0x42002000
	WORD $0xebeff070
	WORD $0x000407fe
	RET

// func sumsqInt16VX(a []int16) int16
TEXT ·sumsqInt16VX(SB), NOSPLIT, $160-26
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·sumsqInt16VXBody(SB)
	RET

// sumsqInt16VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumsqInt16VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xec4c000c
	WORD $0x007cec4a
	WORD $0x000e087d
	WORD $0xa7080000
	WORD $0xa7190000
	WORD $0xa7f4002c
	WORD $0xa7080000
	WORD $0x40002000
	WORD $0x07feec14
	WORD $0x01bc0059
	WORD $0xe7000000
	WORD $0x0044b904
	WORD $0x0053b904
	WORD $0x0001e710
	WORD $0x50000006
	WORD $0xe7011100
	WORD $0x00aaa70b
	WORD $0xfff84150
	WORD $0x5010a764
	WORD $0xfff6e710
	WORD $0x00000044
	WORD $0xe7001000
	WORD $0x1064e700
	WORD $0x10002067
	WORD $0xe7000003
	WORD $0x2021ec41
	WORD $0x00138064
	WORD $0xb9090041
	WORD $0xeb110001
	WORD $0x000db908
	WORD $0x00314810
	WORD $0x3000b9fd
	WORD $0x10111a01
	WORD $0x41303002
	WORD $0xa747fff9
	WORD $0x40002000
	BYTE $0x07
	BYTE $0xfe
	RET

// func sumsqdevInt16VX(a []int16, c int16) int16
TEXT ·sumsqdevInt16VX(SB), NOSPLIT, $160-34
	NO_LOCAL_POINTERS
	MOVD $ret+32(FP), R2
	MOVD a_base+0(FP), R3
	MOVH c+24(FP), R4
	MOVD a_len+8(FP), R5
	BL ·sumsqdevInt16VXBody(SB)
	RET

// sumsqdevInt16VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumsqdevInt16VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024ec5c
	WORD $0x000c007c
	WORD $0xec5a0011
	WORD $0x087da719
	WORD $0x0000a708
	WORD $0x0000a7f4
	WORD $0x0038a708
	WORD $0x00004000
	WORD $0x2000ebef
	WORD $0xf0700004
	WORD $0x07fee704
	WORD $0x40000062
	WORD $0xec1501bc
	WORD $0x0059e710
	WORD $0x0003104d
	WORD $0xe7000000
	WORD $0x0044b904
	WORD $0x00e3b904
	WORD $0x0001e720
	WORD $0xe0000006
	WORD $0xe7221000
	WORD $0x10f7a70b
	WORD $0xfff8e702
	WORD $0x210000aa
	WORD $0x41e0e010
	WORD $0xa764fff3
	WORD $0xe7100000
	WORD $0x0044e700
	WORD $0x10001064
	WORD $0xe7001000
	WORD $0x2067e700
	WORD $0x00032021
	WORD $0xec510014
	WORD $0x8064b909
	WORD $0x0051eb11
	WORD $0x0001000d
	WORD $0xb9080031
	WORD $0x48103000
	WORD $0x1b14b9fd
	WORD $0x10111a01
	WORD $0x41303002
	WORD $0xa757fff8
	WORD $0x40002000
	WORD $0xebeff070
	WORD $0x000407fe
	RET

// func sumsqdiffInt16VX(a []int16, b []int16) int16
TEXT ·sumsqdiffInt16VX(SB), NOSPLIT, $160-50
	NO_LOCAL_POINTERS
	MOVD $ret+48(FP), R2
	MOVD a_base+0(FP), R3
	MOVD b_base+24(FP), R4
	MOVD a_len+8(FP), R5
	BL ·sumsqdiffInt16VXBody(SB)
	RET

// sumsqdiffInt16VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumsqdiffInt16VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024ec5c
	WORD $0x000c007c
	WORD $0xec5a0011
	WORD $0x087da719
	WORD $0x0000a708
	WORD $0x0000a7f4
	WORD $0x0035a708
	WORD $0x00004000
	WORD $0x2000ebef
	WORD $0xf0700004
	WORD $0x07feec15
	WORD $0x01bc0059
	WORD $0xe7000000
	WORD $0x0044a7e9
	WORD $0x0000b904
	WORD $0x0001e71e
	WORD $0x30000006
	WORD $0xe72e4000
	WORD $0x0006e711
	WORD $0x200010f7
	WORD $0xe7011100
	WORD $0x00aaa70b
	WORD $0xfff841e0
	WORD $0xe010a764
	WORD $0xfff0e710
	WORD $0x00000044
	WORD $0xe7001000
	WORD $0x1064e700
	WORD $0x10002067
	WORD $0xe7000003
	WORD $0x2021ec51
	WORD $0x00138064
	WORD $0xb9090051
	WORD $0xeb110001
	WORD $0x000d48e1
	WORD $0x30004be1
	WORD $0x4000b9fd
	WORD $0xe0ee1a0e
	WORD $0x41101002
	WORD $0xa757fff7
	WORD $0x40002000
	WORD $0xebeff070
	WORD $0x000407fe
	RET

// func sumsqUint8VX(a []byte) byte
TEXT ·sumsqUint8VX(SB), NOSPLIT, $160-25
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·sumsqUint8VXBody(SB)
	RET

// sumsqUint8VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumsqUint8VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xec4c000c
	WORD $0x007cec4a
	WORD $0x000e107d
	WORD $0xa7080000
	WORD $0xa7190000
	WORD $0xa7f4002c
	WORD $0xa7080000
	WORD $0x42002000
	WORD $0x07feec14
	WORD $0x01bb0059
	WORD $0xe7000000
	WORD $0x0044b904
	WORD $0x0053b904
	WORD $0x0001e710
	WORD $0x50000006
	WORD $0xe7011000
	WORD $0x00aaa70b
	WORD $0xfff04150
	WORD $0x5010a764
	WORD $0xfff6e710
	WORD $0x00000044
	WORD $0xe7001000
	WORD $0x0064e700
	WORD $0x10002067
	WORD $0xe7000003
	WORD $0x2021ec41
	WORD $0x00118064
	WORD $0xb9090041
	WORD $0xb9080013
	WORD $0xe3301000
	WORD $0x0076b9fd
	WORD $0x30331a03
	WORD $0x41101001
	WORD $0xa747fff8
	WORD $0x42002000
	BYTE $0x07
	BYTE $0xfe
	RET

// func sumsqdevUint8VX(a []byte, c byte) byte
TEXT ·sumsqdevUint8VX(SB), NOSPLIT, $160-33
	NO_LOCAL_POINTERS
	MOVD $ret+32(FP), R2
	MOVD a_base+0(FP), R3
	MOVBZ c+24(FP), R4
	MOVD a_len+8(FP), R5
	BL ·sumsqdevUint8VXBody(SB)
	RET

// sumsqdevUint8VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumsqdevUint8VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024ec5c
	WORD $0x000c007c
	WORD $0xec5a0011
	WORD $0x107da719
	WORD $0x0000a708
	WORD $0x0000a7f4
	WORD $0x0038a708
	WORD $0x00004200
	WORD $0x2000ebef
	WORD $0xf0700004
	WORD $0x07fee704
	WORD $0x40000062
	WORD $0xec1501bb
	WORD $0x0059e710
	WORD $0x0007004d
	WORD $0xe7000000
	WORD $0x0044b904
	WORD $0x00e3b904
	WORD $0x0001e720
	WORD $0xe0000006
	WORD $0xe7221000
	WORD $0x00f7a70b
	WORD $0xfff0e702
	WORD $0x200000aa
	WORD $0x41e0e010
	WORD $0xa764fff3
	WORD $0xe7100000
	WORD $0x0044e700
	WORD $0x10000064
	WORD $0xe7001000
	WORD $0x2067e700
	WORD $0x00032021
	WORD $0xec510012
	WORD $0x8064b909
	WORD $0x0051b908
	WORD $0x0013e330
	WORD $0x10000076
	WORD $0x1b34b9fd
	WORD $0x30331a03
	WORD $0x41101001
	WORD $0xa757fff7
	WORD $0x42002000
	WORD $0xebeff070
	WORD $0x000407fe
	RET

// func sumsqUint16VX(a []uint16) uint16
TEXT ·sumsqUint16VX(SB), NOSPLIT, $160-26
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·sumsqUint16VXBody(SB)
	RET

// sumsqUint16VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumsqUint16VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xec4c000c
	WORD $0x007cec4a
	WORD $0x000e087d
	WORD $0xa7080000
	WORD $0xa7190000
	WORD $0xa7f4002c
	WORD $0xa7080000
	WORD $0x40002000
	WORD $0x07feec14
	WORD $0x01bc0059
	WORD $0xe7000000
	WORD $0x0044b904
	WORD $0x0053b904
	WORD $0x0001e710
	WORD $0x50000006
	WORD $0xe7011100
	WORD $0x00aaa70b
	WORD $0xfff84150
	WORD $0x5010a764
	WORD $0xfff6e710
	WORD $0x00000044
	WORD $0xe7001000
	WORD $0x1064e700
	WORD $0x10002067
	WORD $0xe7000003
	WORD $0x2021ec41
	WORD $0x00138064
	WORD $0xb9090041
	WORD $0xeb110001
	WORD $0x000db908
	WORD $0x00314810
	WORD $0x3000b9fd
	WORD $0x10111a01
	WORD $0x41303002
	WORD $0xa747fff9
	WORD $0x40002000
	BYTE $0x07
	BYTE $0xfe
	RET

// func sumsqdevUint16VX(a []uint16, c uint16) uint16
TEXT ·sumsqdevUint16VX(SB), NOSPLIT, $160-34
	NO_LOCAL_POINTERS
	MOVD $ret+32(FP), R2
	MOVD a_base+0(FP), R3
	MOVHZ c+24(FP), R4
	MOVD a_len+8(FP), R5
	BL ·sumsqdevUint16VXBody(SB)
	RET

// sumsqdevUint16VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumsqdevUint16VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024ec5c
	WORD $0x000c007c
	WORD $0xec5a0011
	WORD $0x087da719
	WORD $0x0000a708
	WORD $0x0000a7f4
	WORD $0x0038a708
	WORD $0x00004000
	WORD $0x2000ebef
	WORD $0xf0700004
	WORD $0x07fee704
	WORD $0x40000062
	WORD $0xec1501bc
	WORD $0x0059e710
	WORD $0x0003104d
	WORD $0xe7000000
	WORD $0x0044b904
	WORD $0x00e3b904
	WORD $0x0001e720
	WORD $0xe0000006
	WORD $0xe7221000
	WORD $0x10f7a70b
	WORD $0xfff8e702
	WORD $0x210000aa
	WORD $0x41e0e010
	WORD $0xa764fff3
	WORD $0xe7100000
	WORD $0x0044e700
	WORD $0x10001064
	WORD $0xe7001000
	WORD $0x2067e700
	WORD $0x00032021
	WORD $0xec510014
	WORD $0x8064b909
	WORD $0x0051eb11
	WORD $0x0001000d
	WORD $0xb9080031
	WORD $0x48103000
	WORD $0x1b14b9fd
	WORD $0x10111a01
	WORD $0x41303002
	WORD $0xa757fff8
	WORD $0x40002000
	WORD $0xebeff070
	WORD $0x000407fe
	RET

// func sumsqdiffUint16VX(a []uint16, b []uint16) uint16
TEXT ·sumsqdiffUint16VX(SB), NOSPLIT, $160-50
	NO_LOCAL_POINTERS
	MOVD $ret+48(FP), R2
	MOVD a_base+0(FP), R3
	MOVD b_base+24(FP), R4
	MOVD a_len+8(FP), R5
	BL ·sumsqdiffUint16VXBody(SB)
	RET

// sumsqdiffUint16VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumsqdiffUint16VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024ec5c
	WORD $0x000c007c
	WORD $0xec5a0011
	WORD $0x087da719
	WORD $0x0000a708
	WORD $0x0000a7f4
	WORD $0x0035a708
	WORD $0x00004000
	WORD $0x2000ebef
	WORD $0xf0700004
	WORD $0x07feec15
	WORD $0x01bc0059
	WORD $0xe7000000
	WORD $0x0044a7e9
	WORD $0x0000b904
	WORD $0x0001e71e
	WORD $0x30000006
	WORD $0xe72e4000
	WORD $0x0006e711
	WORD $0x200010f7
	WORD $0xe7011100
	WORD $0x00aaa70b
	WORD $0xfff841e0
	WORD $0xe010a764
	WORD $0xfff0e710
	WORD $0x00000044
	WORD $0xe7001000
	WORD $0x1064e700
	WORD $0x10002067
	WORD $0xe7000003
	WORD $0x2021ec51
	WORD $0x00138064
	WORD $0xb9090051
	WORD $0xeb110001
	WORD $0x000d48e1
	WORD $0x30004be1
	WORD $0x4000b9fd
	WORD $0xe0ee1a0e
	WORD $0x41101002
	WORD $0xa757fff7
	WORD $0x40002000
	WORD $0xebeff070
	WORD $0x000407fe
	RET

// func sumsqUint32VX(a []uint32) uint32
TEXT ·sumsqUint32VX(SB), NOSPLIT, $160-28
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·sumsqUint32VXBody(SB)
	RET

// sumsqUint32VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumsqUint32VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xec4c000c
	WORD $0x007cec4a
	WORD $0x000e047d
	WORD $0xa7080000
	WORD $0xa7190000
	WORD $0xa7f40029
	WORD $0xa7080000
	WORD $0x50002000
	WORD $0x07feec14
	WORD $0x01bd0059
	WORD $0xe7000000
	WORD $0x0044b904
	WORD $0x0053b904
	WORD $0x0001e710
	WORD $0x50000006
	WORD $0xe7011200
	WORD $0x00aaa70b
	WORD $0xfffc4150
	WORD $0x5010a764
	WORD $0xfff6e710
	WORD $0x00000044
	WORD $0xe7001000
	WORD $0x2067e700
	WORD $0x00032021
	WORD $0xec410013
	WORD $0x8064b909
	WORD $0x0041eb11
	WORD $0x0002000d
	WORD $0xb9080031
	WORD $0x58103000
	WORD $0xb9fd1011
	WORD $0x1a014130
	WORD $0x3004a747
	WORD $0xfff95000
	WORD $0x200007fe
	RET

// func sumsqdevUint32VX(a []uint32, c uint32) uint32
TEXT ·sumsqdevUint32VX(SB), NOSPLIT, $160-36
	NO_LOCAL_POINTERS
	MOVD $ret+32(FP), R2
	MOVD a_base+0(FP), R3
	MOVWZ c+24(FP), R4
	MOVD a_len+8(FP), R5
	BL ·sumsqdevUint32VXBody(SB)
	RET

// sumsqdevUint32VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumsqdevUint32VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024ec5c
	WORD $0x000c007c
	WORD $0xec5a0011
	WORD $0x047da719
	WORD $0x0000a708
	WORD $0x0000a7f4
	WORD $0x0035a708
	WORD $0x00005000
	WORD $0x2000ebef
	WORD $0xf0700004
	WORD $0x07fee704
	WORD $0x40000062
	WORD $0xec1501bd
	WORD $0x0059e710
	WORD $0x0001204d
	WORD $0xe7000000
	WORD $0x0044b904
	WORD $0x00e3b904
	WORD $0x0001e720
	WORD $0xe0000006
	WORD $0xe7221000
	WORD $0x20f7a70b
	WORD $0xfffce702
	WORD $0x220000aa
	WORD $0x41e0e010
	WORD $0xa764fff3
	WORD $0xe7100000
	WORD $0x0044e700
	WORD $0x10002067
	WORD $0xe7000003
	WORD $0x2021ec51
	WORD $0x00148064
	WORD $0xb9090051
	WORD $0xeb110002
	WORD $0x000db908
	WORD $0x00315810
	WORD $0x30001b14
	WORD $0xb9fd1011
	WORD $0x1a014130
	WORD $0x3004a757
	WORD $0xfff85000
	WORD $0x2000ebef
	WORD $0xf0700004
	BYTE $0x07
	BYTE $0xfe
	RET

// func sumsqdiffUint32VX(a []uint32, b []uint32) uint32
TEXT ·sumsqdiffUint32VX(SB), NOSPLIT, $160-52
	NO_LOCAL_POINTERS
	MOVD $ret+48(FP), R2
	MOVD a_base+0(FP), R3
	MOVD b_base+24(FP), R4
	MOVD a_len+8(FP), R5
	BL ·sumsqdiffUint32VXBody(SB)
	RET

// sumsqdiffUint32VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumsqdiffUint32VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024ec5c
	WORD $0x000c007c
	WORD $0xec5a0011
	WORD $0x047da719
	WORD $0x0000a708
	WORD $0x0000a7f4
	WORD $0x0032a708
	WORD $0x00005000
	WORD $0x2000ebef
	WORD $0xf0700004
	WORD $0x07feec15
	WORD $0x01bd0059
	WORD $0xe7000000
	WORD $0x0044a7e9
	WORD $0x0000b904
	WORD $0x0001e71e
	WORD $0x30000006
	WORD $0xe72e4000
	WORD $0x0006e711
	WORD $0x200020f7
	WORD $0xe7011200
	WORD $0x00aaa70b
	WORD $0xfffc41e0
	WORD $0xe010a764
	WORD $0xfff0e710
	WORD $0x00000044
	WORD $0xe7001000
	WORD $0x2067e700
	WORD $0x00032021
	WORD $0xec510013
	WORD $0x8064b909
	WORD $0x0051eb11
	WORD $0x0002000d
	WORD $0x58e13000
	WORD $0x5be14000
	WORD $0xb9fde0ee
	WORD $0x1a0e4110
	WORD $0x1004a757
	WORD $0xfff75000
	WORD $0x2000ebef
	WORD $0xf0700004
	BYTE $0x07
	BYTE $0xfe
	RET

// func sumFloat32VX(a []float32) float32
TEXT ·sumFloat32VX(SB), NOSPLIT, $160-28
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·sumFloat32VXBody(SB)
	RET

// sumFloat32VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumFloat32VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024e700
	WORD $0x00000044
	WORD $0xec4a0016
	WORD $0x107ce710
	WORD $0x00000044
	WORD $0xe7200000
	WORD $0x0044e730
	WORD $0x00000044
	WORD $0xe7400000
	WORD $0x0044a719
	WORD $0x0000ec14
	WORD $0x00cd4064
	WORD $0xa7f400d0
	WORD $0xec54fff0
	WORD $0x00d9eb15
	WORD $0x0004000c
	WORD $0x41101001
	WORD $0xec013ebf
	WORD $0x0059ec5a
	WORD $0x0015307d
	WORD $0xe7100000
	WORD $0x0044a759
	WORD $0x0000a719
	WORD $0x0010e720
	WORD $0x00000044
	WORD $0xe7300000
	WORD $0x0044e740
	WORD $0x00000044
	WORD $0xa7f40083
	WORD $0xe7100000
	WORD $0x0044e720
	WORD $0x00000044
	WORD $0xe7300000
	WORD $0x0044e740
	WORD $0x00000044
	WORD $0xec5103bd
	WORD $0x0059a719
	WORD $0x0010b904
	WORD $0x00e3e750
	WORD $0xe0100006
	WORD $0xe760e000
	WORD $0x0006e770
	WORD $0xe0200006
	WORD $0xe700e030
	WORD $0x0806e744
	WORD $0x000022e3
	WORD $0xe7337000
	WORD $0x20e3e711
	WORD $0x600020e3
	WORD $0xe7225000
	WORD $0x20e3e750
	WORD $0xe0700006
	WORD $0xe760e060
	WORD $0x0006e770
	WORD $0xe0400006
	WORD $0xe700e050
	WORD $0x0806e722
	WORD $0x000022e3
	WORD $0xe7117000
	WORD $0x20e3e770
	WORD $0xe0a00006
	WORD $0xe7336000
	WORD $0x20e3e760
	WORD $0xe0800006
	WORD $0xe700e0b0
	WORD $0x08064110
	WORD $0x1040a75b
	WORD $0xfffce744
	WORD $0x500020e3
	WORD $0xe750e090
	WORD $0x0006e744
	WORD $0x000022e3
	WORD $0xe700e0d0
	WORD $0x0806e733
	WORD $0x700020e3
	WORD $0xe770e0c0
	WORD $0x0006e711
	WORD $0x600020e3
	WORD $0xe760e0e0
	WORD $0x0006e722
	WORD $0x500020e3
	WORD $0xe750e0f0
	WORD $0x000641e0
	WORD $0xe100e722
	WORD $0x000022e3
	WORD $0xe7117000
	WORD $0x20e3e733
	WORD $0x600020e3
	WORD $0xe7445000
	WORD $0x20e3a774
	WORD $0xff9aec08
	WORD $0x0027007c
	WORD $0xec51fff0
	WORD $0x00d9ebe5
	WORD $0x0002000d
	WORD $0xe75e3030
	WORD $0x0006e76e
	WORD $0x30000006
	WORD $0xe77e3010
	WORD $0x0006e70e
	WORD $0x30200806
	WORD $0xe7330000
	WORD $0x22e3e722
	WORD $0x700020e3
	WORD $0xe7116000
	WORD $0x20e3e744
	WORD $0x500020e3
	WORD $0xb9040051
	WORD $0x41101010
	WORD $0xa707ffe1
	WORD $0xec1401bb
	WORD $0x0059ec14
	WORD $0x0009a064
	WORD $0xeb510002
	WORD $0x000de705
	WORD $0x30006004
	WORD $0xe7500000
	WORD $0x0044e760
	WORD $0x00000044
	WORD $0xe7700000
	WORD $0x00444100
	WORD $0x1001ec04
	WORD $0x00774064
	WORD $0x41001002
	WORD $0xec04007d
	WORD $0x40644100
	WORD $0x1003ec04
	WORD $0x00834064
	WORD $0x41001004
	WORD $0xec040089
	WORD $0x40644100
	WORD $0x1005ec04
	WORD $0x008f4064
	WORD $0x41001006
	WORD $0xec040095
	WORD $0x40644100
	WORD $0x1007ec04
	WORD $0x009b4064
	WORD $0x41001008
	WORD $0xec0400a1
	WORD $0x40644100
	WORD $0x1009ec04
	WORD $0x00a74064
	WORD $0x4100100a
	WORD $0xec0400ad
	WORD $0x40644100
	WORD $0x100bec04
	WORD $0x00b34064
	WORD $0x4100100c
	WORD $0xec0400b9
	WORD $0x40644100
	WORD $0x100dec04
	WORD $0x00bf4064
	WORD $0x4100100e
	WORD $0xec0400c5
	WORD $0x40644100
	WORD $0x100fec04
	WORD $0x0009a064
	WORD $0xeb100002
	WORD $0x000de771
	WORD $0x30003003
	WORD $0xe7225000
	WORD $0x20e3e744
	WORD $0x700020e3
	WORD $0xe7010000
	WORD $0x20e3e713
	WORD $0x600020e3
	WORD $0xe7010000
	WORD $0x20e3e714
	WORD $0x200020e3
	WORD $0xe7010000
	WORD $0x20e3e710
	WORD $0x00003060
	WORD $0xe7010000
	WORD $0x20e3e710
	WORD $0x0001204d
	WORD $0xe7010000
	WORD $0x20e3e700
	WORD $0x2000000b
	WORD $0xebeff070
	WORD $0x000407fe
	WORD $0xeb500002
	WORD $0x000de705
	WORD $0x30001003
	WORD $0x41001002
	WORD $0xec04ff89
	WORD $0xa064eb50
	WORD $0x0002000d
	WORD $0xe7053000
	WORD $0x20034100
	WORD $0x1003ec04
	WORD $0xff83a064
	WORD $0xeb500002
	WORD $0x000de705
	WORD $0x30003003
	WORD $0x41001004
	WORD $0xec04ff7d
	WORD $0xa064eb50
	WORD $0x0002000d
	WORD $0xe7553000
	WORD $0x00034100
	WORD $0x1005ec04
	WORD $0xff77a064
	WORD $0xeb500002
	WORD $0x000de755
	WORD $0x30001003
	WORD $0x41001006
	WORD $0xec04ff71
	WORD $0xa064eb50
	WORD $0x0002000d
	WORD $0xe7553000
	WORD $0x20034100
	WORD $0x1007ec04
	WORD $0xff6ba064
	WORD $0xeb500002
	WORD $0x000de755
	WORD $0x30003003
	WORD $0x41001008
	WORD $0xec04ff65
	WORD $0xa064eb50
	WORD $0x0002000d
	WORD $0xe7653000
	WORD $0x00034100
	WORD $0x1009ec04
	WORD $0xff5fa064
	WORD $0xeb500002
	WORD $0x000de765
	WORD $0x30001003
	WORD $0x4100100a
	WORD $0xec04ff59
	WORD $0xa064eb50
	WORD $0x0002000d
	WORD $0xe7653000
	WORD $0x20034100
	WORD $0x100bec04
	WORD $0xff53a064
	WORD $0xeb500002
	WORD $0x000de765
	WORD $0x30003003
	WORD $0x4100100c
	WORD $0xec04ff4d
	WORD $0xa064eb50
	WORD $0x0002000d
	WORD $0xe7753000
	WORD $0x00034100
	WORD $0x100dec04
	WORD $0xff47a064
	WORD $0xeb500002
	WORD $0x000de775
	WORD $0x30001003
	WORD $0x4100100e
	WORD $0xec04ff41
	WORD $0xa064eb50
	WORD $0x0002000d
	WORD $0xe7753000
	WORD $0x20034100
	WORD $0x100fec04
	WORD $0xff3b4064
	WORD $0xa7f4ff3e
	RET

// func l1normFloat32VX(a []float32) float32
TEXT ·l1normFloat32VX(SB), NOSPLIT, $160-28
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·l1normFloat32VXBody(SB)
	RET

// l1normFloat32VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·l1normFloat32VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024e700
	WORD $0x00000044
	WORD $0xec4a0019
	WORD $0x107ce710
	WORD $0x00000044
	WORD $0xe7300000
	WORD $0x0044e720
	WORD $0x00000044
	WORD $0xe7400000
	WORD $0x0044a719
	WORD $0x0000ec14
	WORD $0x010c4064
	WORD $0xe7500000
	WORD $0x0044a7f4
	WORD $0x0117ec54
	WORD $0xfff000d9
	WORD $0xeb150004
	WORD $0x000c4110
	WORD $0x1001ec01
	WORD $0x3ebf0059
	WORD $0xec5a0015
	WORD $0x307de710
	WORD $0x00000044
	WORD $0xa7590000
	WORD $0xa7190010
	WORD $0xe7300000
	WORD $0x0044e720
	WORD $0x00000044
	WORD $0xe7400000
	WORD $0x0044a7f4
	WORD $0x00b3e710
	WORD $0x00000044
	WORD $0xe7300000
	WORD $0x0044e720
	WORD $0x00000044
	WORD $0xe7400000
	WORD $0x0044ec51
	WORD $0x03bd0059
	WORD $0xa7190010
	WORD $0xb90400e3
	WORD $0xe750e030
	WORD $0x0006e760
	WORD $0xe0000006
	WORD $0xe770e010
	WORD $0x0006e700
	WORD $0xe0200806
	WORD $0xe7000020
	WORD $0x2ccce777
	WORD $0x002020cc
	WORD $0xe7660020
	WORD $0x20cce755
	WORD $0x002020cc
	WORD $0xe7445000
	WORD $0x20e3e711
	WORD $0x600020e3
	WORD $0xe7337000
	WORD $0x20e3e722
	WORD $0x000022e3
	WORD $0xe750e060
	WORD $0x0006e760
	WORD $0xe0500006
	WORD $0xe770e040
	WORD $0x0006e700
	WORD $0xe0700806
	WORD $0xe7000020
	WORD $0x2ccc4110
	WORD $0x1040e777
	WORD $0x002020cc
	WORD $0xa75bfffc
	WORD $0xe7660020
	WORD $0x20cce755
	WORD $0x002020cc
	WORD $0xe7225000
	WORD $0x20e3e750
	WORD $0xe0b00006
	WORD $0xe7550020
	WORD $0x20cce733
	WORD $0x600020e3
	WORD $0xe760e080
	WORD $0x0006e766
	WORD $0x002020cc
	WORD $0xe7117000
	WORD $0x20e3e770
	WORD $0xe0900006
	WORD $0xe7770020
	WORD $0x20cce744
	WORD $0x000022e3
	WORD $0xe700e0a0
	WORD $0x0806e700
	WORD $0x00202ccc
	WORD $0xe7445000
	WORD $0x20e3e750
	WORD $0xe0e00006
	WORD $0xe7550020
	WORD $0x20cce711
	WORD $0x600020e3
	WORD $0xe760e0d0
	WORD $0x0006e766
	WORD $0x002020cc
	WORD $0xe7337000
	WORD $0x20e3e770
	WORD $0xe0c00006
	WORD $0xe7770020
	WORD $0x20cce722
	WORD $0x000022e3
	WORD $0xe700e0f0
	WORD $0x080641e0
	WORD $0xe100e700
	WORD $0x00202ccc
	WORD $0xe7225000
	WORD $0x20e3e733
	WORD $0x600020e3
	WORD $0xe7117000
	WORD $0x20e3e744
	WORD $0x000022e3
	WORD $0xa774ff6a
	WORD $0xec080033
	WORD $0x007cec51
	WORD $0xfff000d9
	WORD $0xebe50002
	WORD $0x000de75e
	WORD $0x30300006
	WORD $0xe76e3020
	WORD $0x0006e77e
	WORD $0x30100006
	WORD $0xe70e3000
	WORD $0x0806e700
	WORD $0x00202ccc
	WORD $0xe7770020
	WORD $0x20cce766
	WORD $0x002020cc
	WORD $0xe7550020
	WORD $0x20cce744
	WORD $0x500020e3
	WORD $0xe7226000
	WORD $0x20e3e733
	WORD $0x700020e3
	WORD $0xe7110000
	WORD $0x22e3b904
	WORD $0x00514110
	WORD $0x1010a707
	WORD $0xffd5ec14
	WORD $0x01bb0059
	WORD $0xec14fefa
	WORD $0xa064eb51
	WORD $0x0002000d
	WORD $0xed053000
	WORD $0x0024e750
	WORD $0x00000044
	WORD $0xb3700000
	WORD $0xe7005000
	WORD $0x2061e700
	WORD $0x50003061
	WORD $0xe7600000
	WORD $0x0044e770
	WORD $0x00000044
	WORD $0x41001001
	WORD $0xec04007c
	WORD $0x40644100
	WORD $0x1002ec04
	WORD $0x00874064
	WORD $0x41001003
	WORD $0xec040092
	WORD $0x40644100
	WORD $0x1004ec04
	WORD $0x009d4064
	WORD $0x41001005
	WORD $0xec0400a8
	WORD $0x40644100
	WORD $0x1006ec04
	WORD $0x00b34064
	WORD $0x41001007
	WORD $0xec0400be
	WORD $0x40644100
	WORD $0x1008ec04
	WORD $0x00c94064
	WORD $0x41001009
	WORD $0xec0400d4
	WORD $0x40644100
	WORD $0x100aec04
	WORD $0x00df4064
	WORD $0x4100100b
	WORD $0xec0400ea
	WORD $0x40644100
	WORD $0x100cec04
	WORD $0x00f54064
	WORD $0x4100100d
	WORD $0xec040100
	WORD $0x40644100
	WORD $0x100eec04
	WORD $0x010b4064
	WORD $0x4100100f
	WORD $0xec04000e
	WORD $0xa064eb10
	WORD $0x0002000d
	WORD $0xc0097fff
	WORD $0xffff5401
	WORD $0x3000e770
	WORD $0x00032022
	WORD $0xe7335000
	WORD $0x20e3e744
	WORD $0x700020e3
	WORD $0xe7010000
	WORD $0x20e3e712
	WORD $0x600020e3
	WORD $0xe7010000
	WORD $0x20e3e714
	WORD $0x300020e3
	WORD $0xe7010000
	WORD $0x20e3e710
	WORD $0x00003060
	WORD $0xe7010000
	WORD $0x20e3e710
	WORD $0x0001204d
	WORD $0xe7010000
	WORD $0x20e3e700
	WORD $0x2000000b
	WORD $0xebeff070
	WORD $0x000407fe
	WORD $0xeb500002
	WORD $0x000dc009
	WORD $0x7fffffff
	WORD $0x54053000
	WORD $0xe7000001
	WORD $0x20224100
	WORD $0x1002ec04
	WORD $0xff7fa064
	WORD $0xeb500002
	WORD $0x000dc009
	WORD $0x7fffffff
	WORD $0x54053000
	WORD $0xe7000002
	WORD $0x20224100
	WORD $0x1003ec04
	WORD $0xff74a064
	WORD $0xeb500002
	WORD $0x000dc009
	WORD $0x7fffffff
	WORD $0x54053000
	WORD $0xe7000003
	WORD $0x20224100
	WORD $0x1004ec04
	WORD $0xff69a064
	WORD $0xeb500002
	WORD $0x000dc009
	WORD $0x7fffffff
	WORD $0x54053000
	WORD $0xe7500000
	WORD $0x20224100
	WORD $0x1005ec04
	WORD $0xff5ea064
	WORD $0xeb500002
	WORD $0x000dc009
	WORD $0x7fffffff
	WORD $0x54053000
	WORD $0xe7500001
	WORD $0x20224100
	WORD $0x1006ec04
	WORD $0xff53a064
	WORD $0xeb500002
	WORD $0x000dc009
	WORD $0x7fffffff
	WORD $0x54053000
	WORD $0xe7500002
	WORD $0x20224100
	WORD $0x1007ec04
	WORD $0xff48a064
	WORD $0xeb500002
	WORD $0x000dc009
	WORD $0x7fffffff
	WORD $0x54053000
	WORD $0xe7500003
	WORD $0x20224100
	WORD $0x1008ec04
	WORD $0xff3da064
	WORD $0xeb500002
	WORD $0x000dc009
	WORD $0x7fffffff
	WORD $0x54053000
	WORD $0xe7600000
	WORD $0x20224100
	WORD $0x1009ec04
	WORD $0xff32a064
	WORD $0xeb500002
	WORD $0x000dc009
	WORD $0x7fffffff
	WORD $0x54053000
	WORD $0xe7600001
	WORD $0x20224100
	WORD $0x100aec04
	WORD $0xff27a064
	WORD $0xeb500002
	WORD $0x000dc009
	WORD $0x7fffffff
	WORD $0x54053000
	WORD $0xe7600002
	WORD $0x20224100
	WORD $0x100bec04
	WORD $0xff1ca064
	WORD $0xeb500002
	WORD $0x000dc009
	WORD $0x7fffffff
	WORD $0x54053000
	WORD $0xe7600003
	WORD $0x20224100
	WORD $0x100cec04
	WORD $0xff11a064
	WORD $0xeb500002
	WORD $0x000dc009
	WORD $0x7fffffff
	WORD $0x54053000
	WORD $0xe7700000
	WORD $0x20224100
	WORD $0x100dec04
	WORD $0xff06a064
	WORD $0xeb500002
	WORD $0x000dc009
	WORD $0x7fffffff
	WORD $0x54053000
	WORD $0xe7700001
	WORD $0x20224100
	WORD $0x100eec04
	WORD $0xfefba064
	WORD $0xeb500002
	WORD $0x000dc009
	WORD $0x7fffffff
	WORD $0x54053000
	WORD $0xe7700002
	WORD $0x20224100
	WORD $0x100fec04
	WORD $0xfef04064
	WORD $0xa7f4fef8
	RET

// func sumLanesFloat32VX(dst []float32, a []float32)
TEXT ·sumLanesFloat32VX(SB), NOSPLIT, $160-48
	NO_LOCAL_POINTERS
	MOVD dst_base+0(FP), R2
	MOVD a_base+24(FP), R3
	MOVD a_len+32(FP), R4
	MOVD dst_len+8(FP), R5
	BL ·sumLanesFloat32VXBody(SB)
	RET

// sumLanesFloat32VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumLanesFloat32VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xec54e000
	WORD $0x10fce720
	WORD $0x20300006
	WORD $0xe7102020
	WORD $0x0006e700
	WORD $0x20100006
	WORD $0xe7302000
	WORD $0x0006ec44
	WORD $0x00ac107c
	WORD $0xa74bfff0
	WORD $0xeb140004
	WORD $0x000c4110
	WORD $0x1001ec01
	WORD $0x3ebf0059
	WORD $0xec4a0009
	WORD $0x307da749
	WORD $0x0000a719
	WORD $0x0010a7f4
	WORD $0x0077ec41
	WORD $0x03bd0059
	WORD $0xa7190010
	WORD $0xb9040053
	WORD $0xe7405010
	WORD $0x0006e750
	WORD $0x50000006
	WORD $0xe7605020
	WORD $0x0006e770
	WORD $0x50300006
	WORD $0xe7227000
	WORD $0x20e3e711
	WORD $0x600020e3
	WORD $0xe7335000
	WORD $0x20e3e700
	WORD $0x400020e3
	WORD $0xe7405070
	WORD $0x0006e750
	WORD $0x50600006
	WORD $0xe7605040
	WORD $0x0006e770
	WORD $0x50500006
	WORD $0xe7007000
	WORD $0x20e3e733
	WORD $0x600020e3
	WORD $0xe76050a0
	WORD $0x0006e711
	WORD $0x500020e3
	WORD $0xe7505080
	WORD $0x0006e770
	WORD $0x50b00006
	WORD $0x41101040
	WORD $0xa74bfffc
	WORD $0xe7224000
	WORD $0x20e3e740
	WORD $0x50900006
	WORD $0xe7227000
	WORD $0x20e3e770
	WORD $0x50d00006
	WORD $0xe7116000
	WORD $0x20e3e760
	WORD $0x50c00006
	WORD $0xe7335000
	WORD $0x20e3e750
	WORD $0x50e00006
	WORD $0xe7004000
	WORD $0x20e3e740
	WORD $0x50f00006
	WORD $0x41505100
	WORD $0xe7007000
	WORD $0x20e3e733
	WORD $0x600020e3
	WORD $0xe7115000
	WORD $0x20e3e722
	WORD $0x400020e3
	WORD $0xa774ff9a
	WORD $0xec080027
	WORD $0x007cec41
	WORD $0xfff000d9
	WORD $0xeb540002
	WORD $0x000de745
	WORD $0x30300006
	WORD $0xe7553000
	WORD $0x0006e765
	WORD $0x30100006
	WORD $0xe7753020
	WORD $0x0006e711
	WORD $0x700020e3
	WORD $0xe7006000
	WORD $0x20e3e733
	WORD $0x500020e3
	WORD $0xe7224000
	WORD $0x20e3b904
	WORD $0x00414110
	WORD $0x1010a707
	WORD $0xffe1e730
	WORD $0x2000000e
	WORD $0xe7002010
	WORD $0x000ee710
	WORD $0x2020000e
	WORD $0xe7202030
	WORD $0x000e07fe
	RET

// func sumFloat64VX(a []float64) float64
TEXT ·sumFloat64VX(SB), NOSPLIT, $160-32
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·sumFloat64VXBody(SB)
	RET

// sumFloat64VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumFloat64VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024e700
	WORD $0x00000044
	WORD $0xec4a0022
	WORD $0x107ce710
	WORD $0x00000044
	WORD $0xe7200000
	WORD $0x0044e730
	WORD $0x00000044
	WORD $0xe7400000
	WORD $0x0044e750
	WORD $0x00000044
	WORD $0xe7600000
	WORD $0x0044e770
	WORD $0x00000044
	WORD $0xe7000000
	WORD $0x0844a719
	WORD $0x0000ec14
	WORD $0x015d4064
	WORD $0xa7f40160
	WORD $0xec54fff0
	WORD $0x00d9eb15
	WORD $0x0004000c
	WORD $0x41101001
	WORD $0xec013ebf
	WORD $0x0059ec5a
	WORD $0x0021307d
	WORD $0xe7100000
	WORD $0x0044a759
	WORD $0x0000a719
	WORD $0x0010e720
	WORD $0x00000044
	WORD $0xe7300000
	WORD $0x0044e740
	WORD $0x00000044
	WORD $0xe7500000
	WORD $0x0044e760
	WORD $0x00000044
	WORD $0xe7700000
	WORD $0x0044e700
	WORD $0x00000844
	WORD $0xa7f400ef
	WORD $0xe7100000
	WORD $0x0044e720
	WORD $0x00000044
	WORD $0xe7300000
	WORD $0x0044e740
	WORD $0x00000044
	WORD $0xe7500000
	WORD $0x0044e760
	WORD $0x00000044
	WORD $0xe7700000
	WORD $0x0044e700
	WORD $0x00000844
	WORD $0xec5103bd
	WORD $0x0059a719
	WORD $0x0010b904
	WORD $0x00e3e710
	WORD $0xe0100806
	WORD $0xe720e000
	WORD $0x0806e730
	WORD $0xe0200806
	WORD $0xe740e030
	WORD $0x0806e750
	WORD $0xe0400806
	WORD $0xe760e050
	WORD $0x0806e770
	WORD $0xe0600806
	WORD $0xe780e070
	WORD $0x0806e700
	WORD $0x80003ee3
	WORD $0xe7777000
	WORD $0x32e3e766
	WORD $0x600032e3
	WORD $0xe7555000
	WORD $0x32e3e744
	WORD $0x400032e3
	WORD $0xe7333000
	WORD $0x32e3e711
	WORD $0x200032e3
	WORD $0xe7221000
	WORD $0x32e3e710
	WORD $0xe0f00806
	WORD $0xe720e0e0
	WORD $0x0806e730
	WORD $0xe0d00806
	WORD $0xe740e0c0
	WORD $0x0806e750
	WORD $0xe0b00806
	WORD $0xe760e0a0
	WORD $0x0806e770
	WORD $0xe0800806
	WORD $0xe780e090
	WORD $0x0806e722
	WORD $0x800032e3
	WORD $0xe7117000
	WORD $0x32e3e733
	WORD $0x600032e3
	WORD $0xe7445000
	WORD $0x32e3e755
	WORD $0x400032e3
	WORD $0xe7663000
	WORD $0x32e3e777
	WORD $0x200032e3
	WORD $0xe7001000
	WORD $0x3ee3e710
	WORD $0xe1100806
	WORD $0xe720e100
	WORD $0x0806e730
	WORD $0xe1200806
	WORD $0xe740e130
	WORD $0x0806e750
	WORD $0xe1400806
	WORD $0xe7555000
	WORD $0x32e3e744
	WORD $0x400032e3
	WORD $0xe760e150
	WORD $0x0806e766
	WORD $0x600032e3
	WORD $0xe7333000
	WORD $0x32e3e770
	WORD $0xe1600806
	WORD $0xe7777000
	WORD $0x32e3e711
	WORD $0x200032e3
	WORD $0xe780e170
	WORD $0x0806e720
	WORD $0xe1e00806
	WORD $0xe7008000
	WORD $0x3ee3e722
	WORD $0x100032e3
	WORD $0xe710e1f0
	WORD $0x0806e730
	WORD $0xe1d00806
	WORD $0xe7663000
	WORD $0x32e3e777
	WORD $0x200032e3
	WORD $0xe740e1c0
	WORD $0x0806e750
	WORD $0xe1b00806
	WORD $0xe7445000
	WORD $0x32e3e755
	WORD $0x400032e3
	WORD $0xe760e1a0
	WORD $0x0806e770
	WORD $0xe1800806
	WORD $0xe7117000
	WORD $0x32e3e733
	WORD $0x600032e3
	WORD $0xe780e190
	WORD $0x08064110
	WORD $0x1040a75b
	WORD $0xfffc41e0
	WORD $0xe200e722
	WORD $0x800032e3
	WORD $0xe7001000
	WORD $0x3ee3a774
	WORD $0xff3aec08
	WORD $0x003f007c
	WORD $0xec51fff0
	WORD $0x00d9ebe5
	WORD $0x0003000d
	WORD $0xe71e3070
	WORD $0x0806e72e
	WORD $0x30600806
	WORD $0xe73e3050
	WORD $0x0806e74e
	WORD $0x30400806
	WORD $0xe75e3030
	WORD $0x0806e76e
	WORD $0x30000806
	WORD $0xe77e3010
	WORD $0x0806e78e
	WORD $0x30200806
	WORD $0xe7338000
	WORD $0x32e3e722
	WORD $0x700032e3
	WORD $0xe7116000
	WORD $0x32e3e744
	WORD $0x500032e3
	WORD $0xe7554000
	WORD $0x32e3e766
	WORD $0x300032e3
	WORD $0xe7772000
	WORD $0x32e3e700
	WORD $0x10003ee3
	WORD $0xb9040051
	WORD $0x41101010
	WORD $0xa707ffc9
	WORD $0xec1401bb
	WORD $0x0059ec14
	WORD $0x0009a064
	WORD $0xeb510003
	WORD $0x000de705
	WORD $0x30003004
	WORD $0xe7100000
	WORD $0x0844e730
	WORD $0x00000844
	WORD $0xe7200000
	WORD $0x0844e770
	WORD $0x00000844
	WORD $0xe7600000
	WORD $0x0844e750
	WORD $0x00000844
	WORD $0xe7400000
	WORD $0x08444100
	WORD $0x1001ec04
	WORD $0x00894064
	WORD $0x41001002
	WORD $0xec04008f
	WORD $0x40644100
	WORD $0x1003ec04
	WORD $0x00954064
	WORD $0x41001004
	WORD $0xec04009b
	WORD $0x40644100
	WORD $0x1005ec04
	WORD $0x00a14064
	WORD $0x41001006
	WORD $0xec0400a7
	WORD $0x40644100
	WORD $0x1007ec04
	WORD $0x00ad4064
	WORD $0x41001008
	WORD $0xec0400b3
	WORD $0x40644100
	WORD $0x1009ec04
	WORD $0x00b94064
	WORD $0x4100100a
	WORD $0xec0400bf
	WORD $0x40644100
	WORD $0x100bec04
	WORD $0x00c54064
	WORD $0x4100100c
	WORD $0xec0400cb
	WORD $0x40644100
	WORD $0x100dec04
	WORD $0x00d14064
	WORD $0x4100100e
	WORD $0xec0400d7
	WORD $0x40644100
	WORD $0x100fec04
	WORD $0x0009a064
	WORD $0xeb100003
	WORD $0x000de741
	WORD $0x30001802
	WORD $0xe7010000
	WORD $0x30e3e716
	WORD $0x600032e3
	WORD $0xe7221000
	WORD $0x32e3e711
	WORD $0x200030e3
	WORD $0xe7275000
	WORD $0x32e3e733
	WORD $0x300032e3
	WORD $0xe7557000
	WORD $0x32e3e722
	WORD $0x300030e3
	WORD $0xe7304000
	WORD $0x36e3e744
	WORD $0x200032e3
	WORD $0xe7050000
	WORD $0x30e3e733
	WORD $0x400030e3
	WORD $0xe7020000
	WORD $0x30e3e713
	WORD $0x100030e3
	WORD $0xe7010000
	WORD $0x30e3e710
	WORD $0x0001304d
	WORD $0xe7010000
	WORD $0x30e3e700
	WORD $0x2000000a
	WORD $0xebeff070
	WORD $0x000407fe
	WORD $0xeb500003
	WORD $0x000de705
	WORD $0x30001002
	WORD $0x41001002
	WORD $0xec04ff77
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7153000
	WORD $0x08024100
	WORD $0x1003ec04
	WORD $0xff71a064
	WORD $0xeb500003
	WORD $0x000de715
	WORD $0x30001802
	WORD $0x41001004
	WORD $0xec04ff6b
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7353000
	WORD $0x08024100
	WORD $0x1005ec04
	WORD $0xff65a064
	WORD $0xeb500003
	WORD $0x000de735
	WORD $0x30001802
	WORD $0x41001006
	WORD $0xec04ff5f
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7253000
	WORD $0x08024100
	WORD $0x1007ec04
	WORD $0xff59a064
	WORD $0xeb500003
	WORD $0x000de725
	WORD $0x30001802
	WORD $0x41001008
	WORD $0xec04ff53
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7753000
	WORD $0x08024100
	WORD $0x1009ec04
	WORD $0xff4da064
	WORD $0xeb500003
	WORD $0x000de775
	WORD $0x30001802
	WORD $0x4100100a
	WORD $0xec04ff47
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7653000
	WORD $0x08024100
	WORD $0x100bec04
	WORD $0xff41a064
	WORD $0xeb500003
	WORD $0x000de765
	WORD $0x30001802
	WORD $0x4100100c
	WORD $0xec04ff3b
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7553000
	WORD $0x08024100
	WORD $0x100dec04
	WORD $0xff35a064
	WORD $0xeb500003
	WORD $0x000de755
	WORD $0x30001802
	WORD $0x4100100e
	WORD $0xec04ff2f
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7453000
	WORD $0x08024100
	WORD $0x100fec04
	WORD $0xff294064
	WORD $0xa7f4ff2c
	RET

// func l1normFloat64VX(a []float64) float64
TEXT ·l1normFloat64VX(SB), NOSPLIT, $160-32
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·l1normFloat64VXBody(SB)
	RET

// l1normFloat64VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·l1normFloat64VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024e700
	WORD $0x00000044
	WORD $0xec4a0025
	WORD $0x107ce710
	WORD $0x00000044
	WORD $0xe7300000
	WORD $0x0044e720
	WORD $0x00000044
	WORD $0xe7400000
	WORD $0x0044e750
	WORD $0x00000044
	WORD $0xe7600000
	WORD $0x0044e770
	WORD $0x00000044
	WORD $0xe7000000
	WORD $0x0844a719
	WORD $0x0000ec14
	WORD $0x00d44064
	WORD $0xe7100000
	WORD $0x0844a7f4
	WORD $0x00dbec04
	WORD $0xfff000d9
	WORD $0xeb100004
	WORD $0x000cec18
	WORD $0x0266007c
	WORD $0x41001001
	WORD $0xe7100000
	WORD $0x0044e730
	WORD $0x00000044
	WORD $0xe7200000
	WORD $0x0044e740
	WORD $0x00000044
	WORD $0xe7500000
	WORD $0x0044e760
	WORD $0x00000044
	WORD $0xe7700000
	WORD $0x0044e700
	WORD $0x00000844
	WORD $0xec5003be
	WORD $0x0059a719
	WORD $0x0000b904
	WORD $0x00e3e710
	WORD $0xe0703806
	WORD $0xe720e060
	WORD $0x3806e730
	WORD $0xe0503806
	WORD $0xe740e040
	WORD $0x3806e750
	WORD $0xe0303806
	WORD $0xe760e000
	WORD $0x3806e770
	WORD $0xe0103806
	WORD $0xe780e020
	WORD $0x3806e788
	WORD $0x00203ccc
	WORD $0xe7770020
	WORD $0x3ccce766
	WORD $0x00203ccc
	WORD $0xe7550020
	WORD $0x3ccce744
	WORD $0x00203ccc
	WORD $0xe7330020
	WORD $0x3ccce722
	WORD $0x00203ccc
	WORD $0xe7110020
	WORD $0x3ccce700
	WORD $0x10003ee3
	WORD $0xe7772000
	WORD $0x32e3e766
	WORD $0x300032e3
	WORD $0xe7554000
	WORD $0x32e3e744
	WORD $0x500032e3
	WORD $0xe7116000
	WORD $0x32e3e710
	WORD $0xe0a03806
	WORD $0xe720e090
	WORD $0x3806e733
	WORD $0x700032e3
	WORD $0xe730e080
	WORD $0x3806e740
	WORD $0xe0b03806
	WORD $0xe7228000
	WORD $0x32e3e750
	WORD $0xe0c03806
	WORD $0xe760e0d0
	WORD $0x3806e770
	WORD $0xe0e03806
	WORD $0xe780e0f0
	WORD $0x3806e788
	WORD $0x00203ccc
	WORD $0xe7770020
	WORD $0x3ccc41e0
	WORD $0xe100a75b
	WORD $0xfffee766
	WORD $0x00203ccc
	WORD $0x41101020
	WORD $0xe7550020
	WORD $0x3ccce744
	WORD $0x00203ccc
	WORD $0xe7330020
	WORD $0x3ccce722
	WORD $0x00203ccc
	WORD $0xe7110020
	WORD $0x3ccce722
	WORD $0x100032e3
	WORD $0xe7332000
	WORD $0x32e3e711
	WORD $0x300032e3
	WORD $0xe7444000
	WORD $0x32e3e755
	WORD $0x500032e3
	WORD $0xe7666000
	WORD $0x32e3e777
	WORD $0x700032e3
	WORD $0xe7008000
	WORD $0x3ee3a774
	WORD $0xff6aa701
	WORD $0x0001a774
	WORD $0x01c2ec14
	WORD $0x01bb0059
	WORD $0xec14ff32
	WORD $0xa064eb51
	WORD $0x0003000d
	WORD $0x68053000
	WORD $0xe7100000
	WORD $0x0844b370
	WORD $0x0000e700
	WORD $0x10003261
	WORD $0xe7300000
	WORD $0x0844e720
	WORD $0x00000844
	WORD $0xe7700000
	WORD $0x0844e760
	WORD $0x00000844
	WORD $0xe7500000
	WORD $0x0844e740
	WORD $0x00000844
	WORD $0x41001001
	WORD $0xec04008f
	WORD $0x40644100
	WORD $0x1002ec04
	WORD $0x009b4064
	WORD $0x41001003
	WORD $0xec0400a7
	WORD $0x40644100
	WORD $0x1004ec04
	WORD $0x00b34064
	WORD $0x41001005
	WORD $0xec0400bf
	WORD $0x40644100
	WORD $0x1006ec04
	WORD $0x00cb4064
	WORD $0x41001007
	WORD $0xec0400d7
	WORD $0x40644100
	WORD $0x1008ec04
	WORD $0x00e34064
	WORD $0x41001009
	WORD $0xec0400ef
	WORD $0x40644100
	WORD $0x100aec04
	WORD $0x00fb4064
	WORD $0x4100100b
	WORD $0xec040107
	WORD $0x40644100
	WORD $0x100cec04
	WORD $0x01134064
	WORD $0x4100100d
	WORD $0xec04011f
	WORD $0x40644100
	WORD $0x100eec04
	WORD $0x012b4064
	WORD $0x4100100f
	WORD $0xec04000f
	WORD $0xa064eb10
	WORD $0x0003000d
	WORD $0xe7813000
	WORD $0x3805e788
	WORD $0x00283ccc
	WORD $0xe7448000
	WORD $0x0e84e701
	WORD $0x000030e3
	WORD $0xe7166000
	WORD $0x32e3e733
	WORD $0x100032e3
	WORD $0xe7113000
	WORD $0x30e3e737
	WORD $0x500032e3
	WORD $0xe7223000
	WORD $0x32e3e755
	WORD $0x700032e3
	WORD $0xe7232000
	WORD $0x30e3e730
	WORD $0x400036e3
	WORD $0xe7442000
	WORD $0x32e3e705
	WORD $0x000030e3
	WORD $0xe7334000
	WORD $0x30e3e702
	WORD $0x000030e3
	WORD $0xe7131000
	WORD $0x30e3e701
	WORD $0x000030e3
	WORD $0xe7100001
	WORD $0x304de701
	WORD $0x000030e3
	WORD $0xe7002000
	WORD $0x000aebef
	WORD $0xf0700004
	WORD $0x07feeb50
	WORD $0x0003000d
	WORD $0xe7853000
	WORD $0x3805e788
	WORD $0x00283ccc
	WORD $0xe7008000
	WORD $0x02844100
	WORD $0x1002ec04
	WORD $0xff6ba064
	WORD $0xeb500003
	WORD $0x000de785
	WORD $0x30003805
	WORD $0xe7880028
	WORD $0x3ccce718
	WORD $0x10001e84
	WORD $0x41001003
	WORD $0xec04ff5f
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7853000
	WORD $0x3805e788
	WORD $0x00283ccc
	WORD $0xe7118000
	WORD $0x0e844100
	WORD $0x1004ec04
	WORD $0xff53a064
	WORD $0xeb500003
	WORD $0x000de785
	WORD $0x30003805
	WORD $0xe7880028
	WORD $0x3ccce738
	WORD $0x30001e84
	WORD $0x41001005
	WORD $0xec04ff47
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7853000
	WORD $0x3805e788
	WORD $0x00283ccc
	WORD $0xe7338000
	WORD $0x0e844100
	WORD $0x1006ec04
	WORD $0xff3ba064
	WORD $0xeb500003
	WORD $0x000de785
	WORD $0x30003805
	WORD $0xe7880028
	WORD $0x3ccce728
	WORD $0x20001e84
	WORD $0x41001007
	WORD $0xec04ff2f
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7853000
	WORD $0x3805e788
	WORD $0x00283ccc
	WORD $0xe7228000
	WORD $0x0e844100
	WORD $0x1008ec04
	WORD $0xff23a064
	WORD $0xeb500003
	WORD $0x000de785
	WORD $0x30003805
	WORD $0xe7880028
	WORD $0x3ccce778
	WORD $0x70001e84
	WORD $0x41001009
	WORD $0xec04ff17
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7853000
	WORD $0x3805e788
	WORD $0x00283ccc
	WORD $0xe7778000
	WORD $0x0e844100
	WORD $0x100aec04
	WORD $0xff0ba064
	WORD $0xeb500003
	WORD $0x000de785
	WORD $0x30003805
	WORD $0xe7880028
	WORD $0x3ccce768
	WORD $0x60001e84
	WORD $0x4100100b
	WORD $0xec04feff
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7853000
	WORD $0x3805e788
	WORD $0x00283ccc
	WORD $0xe7668000
	WORD $0x0e844100
	WORD $0x100cec04
	WORD $0xfef3a064
	WORD $0xeb500003
	WORD $0x000de785
	WORD $0x30003805
	WORD $0xe7880028
	WORD $0x3ccce758
	WORD $0x50001e84
	WORD $0x4100100d
	WORD $0xec04fee7
	WORD $0xa064eb50
	WORD $0x0003000d
	WORD $0xe7853000
	WORD $0x3805e788
	WORD $0x00283ccc
	WORD $0xe7558000
	WORD $0x0e844100
	WORD $0x100eec04
	WORD $0xfedba064
	WORD $0xeb500003
	WORD $0x000de785
	WORD $0x30003805
	WORD $0xe7880028
	WORD $0x3ccce748
	WORD $0x40001e84
	WORD $0x4100100f
	WORD $0xec04fecf
	WORD $0x4064a7f4
	WORD $0xfed8e710
	WORD $0x00000044
	WORD $0xe7300000
	WORD $0x0044e720
	WORD $0x00000044
	WORD $0xe7400000
	WORD $0x0044e750
	WORD $0x00000044
	WORD $0xe7600000
	WORD $0x0044e770
	WORD $0x00000044
	WORD $0xe7000000
	WORD $0x0844a719
	WORD $0x0000eb11
	WORD $0x0003000d
	WORD $0xe7113030
	WORD $0x3806e721
	WORD $0x30203806
	WORD $0xe7313010
	WORD $0x3806e741
	WORD $0x30003806
	WORD $0xe7513040
	WORD $0x3806e761
	WORD $0x30503806
	WORD $0xe7713060
	WORD $0x3806e781
	WORD $0x30703806
	WORD $0xe7880020
	WORD $0x3ccce777
	WORD $0x00203ccc
	WORD $0xe7660020
	WORD $0x3ccce755
	WORD $0x00203ccc
	WORD $0xe7440020
	WORD $0x3ccce733
	WORD $0x00203ccc
	WORD $0xe7220020
	WORD $0x3ccce711
	WORD $0x00203ccc
	WORD $0xe7441000
	WORD $0x32e3e722
	WORD $0x200032e3
	WORD $0xe7333000
	WORD $0x32e3e711
	WORD $0x400032e3
	WORD $0xe7555000
	WORD $0x32e3ec14
	WORD $0x01bb0059
	WORD $0xe7666000
	WORD $0x32e3e777
	WORD $0x700032e3
	WORD $0xe7008000
	WORD $0x3ee3ec14
	WORD $0xfd27a064
	WORD $0xa7f4fdf5
	RET

// func sumLanesFloat64VX(dst []float64, a []float64)
TEXT ·sumLanesFloat64VX(SB), NOSPLIT, $160-48
	NO_LOCAL_POINTERS
	MOVD dst_base+0(FP), R2
	MOVD a_base+24(FP), R3
	MOVD a_len+32(FP), R4
	MOVD dst_len+8(FP), R5
	BL ·sumLanesFloat64VXBody(SB)
	RET

// sumLanesFloat64VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumLanesFloat64VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xec54e000
	WORD $0x10fce760
	WORD $0x20700006
	WORD $0xe7502060
	WORD $0x0006e740
	WORD $0x20500006
	WORD $0xe7302040
	WORD $0x0006e720
	WORD $0x20300006
	WORD $0xe7102020
	WORD $0x0006e700
	WORD $0x20100006
	WORD $0xe7702000
	WORD $0x0006ec44
	WORD $0x0124107c
	WORD $0xa74bfff0
	WORD $0xeb140004
	WORD $0x000c4110
	WORD $0x1001ec01
	WORD $0x3ebf0059
	WORD $0xec4a0009
	WORD $0x307da749
	WORD $0x0000a719
	WORD $0x0010a7f4
	WORD $0x00d7ec41
	WORD $0x03bd0059
	WORD $0xa7190010
	WORD $0xb9040053
	WORD $0xe7005010
	WORD $0x0806e710
	WORD $0x50000806
	WORD $0xe7205020
	WORD $0x0806e730
	WORD $0x50300806
	WORD $0xe7405040
	WORD $0x0806e750
	WORD $0x50500806
	WORD $0xe7605060
	WORD $0x0806e770
	WORD $0x50700806
	WORD $0xe7667000
	WORD $0x32e3e755
	WORD $0x600032e3
	WORD $0xe7445000
	WORD $0x32e3e733
	WORD $0x400032e3
	WORD $0xe7223000
	WORD $0x32e3e711
	WORD $0x200032e3
	WORD $0xe7771000
	WORD $0x32e3e700
	WORD $0x000032e3
	WORD $0xe70050f0
	WORD $0x0806e710
	WORD $0x50e00806
	WORD $0xe72050d0
	WORD $0x0806e730
	WORD $0x50c00806
	WORD $0xe74050b0
	WORD $0x0806e750
	WORD $0x50a00806
	WORD $0xe7605080
	WORD $0x0806e770
	WORD $0x50900806
	WORD $0xe7007000
	WORD $0x32e3e777
	WORD $0x600032e3
	WORD $0xe7115000
	WORD $0x32e3e722
	WORD $0x400032e3
	WORD $0xe7333000
	WORD $0x32e3e744
	WORD $0x200032e3
	WORD $0xe7551000
	WORD $0x32e3e766
	WORD $0x000032e3
	WORD $0xe7005110
	WORD $0x0806e710
	WORD $0x51000806
	WORD $0xe7205120
	WORD $0x0806e730
	WORD $0x51300806
	WORD $0xe7405140
	WORD $0x0806e733
	WORD $0x400032e3
	WORD $0xe7223000
	WORD $0x32e3e750
	WORD $0x51500806
	WORD $0xe7445000
	WORD $0x32e3e711
	WORD $0x200032e3
	WORD $0xe7605160
	WORD $0x0806e755
	WORD $0x600032e3
	WORD $0xe7771000
	WORD $0x32e3e770
	WORD $0x51700806
	WORD $0xe71051e0
	WORD $0x0806e766
	WORD $0x700032e3
	WORD $0xe7000000
	WORD $0x32e3e700
	WORD $0x51f00806
	WORD $0xe72051d0
	WORD $0x0806e744
	WORD $0x200032e3
	WORD $0xe7551000
	WORD $0x32e3e730
	WORD $0x51c00806
	WORD $0xe74051b0
	WORD $0x0806e722
	WORD $0x400032e3
	WORD $0xe7333000
	WORD $0x32e3e750
	WORD $0x51a00806
	WORD $0xe7605180
	WORD $0x0806e777
	WORD $0x600032e3
	WORD $0xe7115000
	WORD $0x32e3e770
	WORD $0x51900806
	WORD $0x41101040
	WORD $0xa74bfffc
	WORD $0x41505200
	WORD $0xe7007000
	WORD $0x32e3e766
	WORD $0x000032e3
	WORD $0xa774ff3a
	WORD $0xec08003f
	WORD $0x007cec41
	WORD $0xfff000d9
	WORD $0xeb540003
	WORD $0x000de705
	WORD $0x30700806
	WORD $0xe7153060
	WORD $0x0806e725
	WORD $0x30500806
	WORD $0xe7353040
	WORD $0x0806e745
	WORD $0x30300806
	WORD $0xe7553000
	WORD $0x0806e765
	WORD $0x30100806
	WORD $0xe7753020
	WORD $0x0806e711
	WORD $0x700032e3
	WORD $0xe7006000
	WORD $0x32e3e777
	WORD $0x500032e3
	WORD $0xe7224000
	WORD $0x32e3e733
	WORD $0x300032e3
	WORD $0xe7442000
	WORD $0x32e3e755
	WORD $0x100032e3
	WORD $0xe7660000
	WORD $0x32e3b904
	WORD $0x00414110
	WORD $0x1010a707
	WORD $0xffc9e770
	WORD $0x2000000e
	WORD $0xe7002010
	WORD $0x000ee710
	WORD $0x2020000e
	WORD $0xe7202030
	WORD $0x000ee730
	WORD $0x2040000e
	WORD $0xe7402050
	WORD $0x000ee750
	WORD $0x2060000e
	WORD $0xe7602070
	WORD $0x000e07fe
	RET

// func sumInt32VX(a []int32) int32
TEXT ·sumInt32VX(SB), NOSPLIT, $160-28
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·sumInt32VXBody(SB)
	RET

// sumInt32VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumInt32VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xec4c000c
	WORD $0x007cec4a
	WORD $0x000e047d
	WORD $0xa7080000
	WORD $0xa7190000
	WORD $0xa7f40029
	WORD $0xa7080000
	WORD $0x50002000
	WORD $0x07feec14
	WORD $0x01bd0059
	WORD $0xe7000000
	WORD $0x0044b904
	WORD $0x0053b904
	WORD $0x0001e710
	WORD $0x50000006
	WORD $0xe7010000
	WORD $0x20f3a70b
	WORD $0xfffc4150
	WORD $0x5010a764
	WORD $0xfff6e710
	WORD $0x00000044
	WORD $0xe7001000
	WORD $0x2067e700
	WORD $0x00032021
	WORD $0xec410010
	WORD $0x8064b909
	WORD $0x0041eb11
	WORD $0x0002000d
	WORD $0xb9080031
	WORD $0x5a003000
	WORD $0x41303004
	WORD $0xa747fffc
	WORD $0x50002000
	BYTE $0x07
	BYTE $0xfe
	RET

// func prodInt32VX(a []int32) int32
TEXT ·prodInt32VX(SB), NOSPLIT, $160-28
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·prodInt32VXBody(SB)
	RET

// prodInt32VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·prodInt32VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xec4c000c
	WORD $0x007cec4a
	WORD $0x000e047d
	WORD $0xa7080001
	WORD $0xa7190000
	WORD $0xa7f4002f
	WORD $0xa7080001
	WORD $0x50002000
	WORD $0x07feec14
	WORD $0x01bd0059
	WORD $0xe7000001
	WORD $0x2045b904
	WORD $0x0053b904
	WORD $0x0001e710
	WORD $0x50000006
	WORD $0xe7010000
	WORD $0x20a2a70b
	WORD $0xfffc4150
	WORD $0x5010a764
	WORD $0xfff6e710
	WORD $0x00003060
	WORD $0xe7001000
	WORD $0x20a2e710
	WORD $0x0001204d
	WORD $0xe7001000
	WORD $0x20a2e700
	WORD $0x00002021
	WORD $0xec410010
	WORD $0x8064b909
	WORD $0x0041eb11
	WORD $0x0002000d
	WORD $0xb9080031
	WORD $0x71003000
	WORD $0x41303004
	WORD $0xa747fffc
	WORD $0x50002000
	BYTE $0x07
	BYTE $0xfe
	RET

// func dotInt32VX(a []int32, b []int32) int32
TEXT ·dotInt32VX(SB), NOSPLIT, $160-52
	NO_LOCAL_POINTERS
	MOVD $ret+48(FP), R2
	MOVD a_base+0(FP), R3
	MOVD b_base+24(FP), R4
	MOVD a_len+8(FP), R5
	BL ·dotInt32VXBody(SB)
	RET

// dotInt32VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·dotInt32VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024ec5c
	WORD $0x000c007c
	WORD $0xec5a0011
	WORD $0x047da708
	WORD $0x0000a719
	WORD $0x0000a7f4
	WORD $0x002fa708
	WORD $0x00005000
	WORD $0x2000ebef
	WORD $0xf0700004
	WORD $0x07feec15
	WORD $0x01bd0059
	WORD $0xe7000000
	WORD $0x0044a7e9
	WORD $0x0000b904
	WORD $0x0001e71e
	WORD $0x30000006
	WORD $0xe72e4000
	WORD $0x0006a70b
	WORD $0xfffce702
	WORD $0x120000aa
	WORD $0x41e0e010
	WORD $0xa764fff3
	WORD $0xe7100000
	WORD $0x0044e700
	WORD $0x10002067
	WORD $0xe7000003
	WORD $0x2021ec51
	WORD $0x00118064
	WORD $0xb9090051
	WORD $0xeb110002
	WORD $0x000d58e1
	WORD $0x400071e1
	WORD $0x30001a0e
	WORD $0x41101004
	WORD $0xa757fff9
	WORD $0x50002000
	WORD $0xebeff070
	WORD $0x000407fe
	RET

// func l1normInt32VX(a []int32) int32
TEXT ·l1normInt32VX(SB), NOSPLIT, $160-28
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·l1normInt32VXBody(SB)
	RET

// l1normInt32VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·l1normInt32VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xec4c000c
	WORD $0x007cec4a
	WORD $0x000e047d
	WORD $0xa7190000
	WORD $0xa7080000
	WORD $0xa7f4002c
	WORD $0xa7080000
	WORD $0x50002000
	WORD $0x07feec14
	WORD $0x01bd0059
	WORD $0xe7000000
	WORD $0x0044b904
	WORD $0x0053b904
	WORD $0x0001e710
	WORD $0x50000006
	WORD $0xe7110000
	WORD $0x20dfe701
	WORD $0x000020f3
	WORD $0xa70bfffc
	WORD $0x41505010
	WORD $0xa764fff3
	WORD $0xe7100000
	WORD $0x0044e700
	WORD $0x10002067
	WORD $0xe7000003
	WORD $0x2021ec41
	WORD $0x00128064
	WORD $0xb9090041
	WORD $0xeb110002
	WORD $0x000db908
	WORD $0x00315810
	WORD $0x30001011
	WORD $0x1a014130
	WORD $0x3004a747
	WORD $0xfffa5000
	WORD $0x200007fe
	RET

// func l1diffInt32VX(a []int32, b []int32) int32
TEXT ·l1diffInt32VX(SB), NOSPLIT, $160-52
	NO_LOCAL_POINTERS
	MOVD $ret+48(FP), R2
	MOVD a_base+0(FP), R3
	MOVD b_base+24(FP), R4
	MOVD a_len+8(FP), R5
	BL ·l1diffInt32VXBody(SB)
	RET

// l1diffInt32VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·l1diffInt32VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024ec5c
	WORD $0x000c007c
	WORD $0xec5a0011
	WORD $0x047da719
	WORD $0x0000a708
	WORD $0x0000a7f4
	WORD $0x0035a708
	WORD $0x00005000
	WORD $0x2000ebef
	WORD $0xf0700004
	WORD $0x07feec15
	WORD $0x01bd0059
	WORD $0xe7000000
	WORD $0x0044a7e9
	WORD $0x0000b904
	WORD $0x0001e71e
	WORD $0x30000006
	WORD $0xe72e4000
	WORD $0x0006e711
	WORD $0x200020f7
	WORD $0xe7110000
	WORD $0x20dfe701
	WORD $0x000020f3
	WORD $0xa70bfffc
	WORD $0x41e0e010
	WORD $0xa764ffed
	WORD $0xe7100000
	WORD $0x0044e700
	WORD $0x10002067
	WORD $0xe7000003
	WORD $0x2021ec51
	WORD $0x00128064
	WORD $0xb9090051
	WORD $0xeb110002
	WORD $0x000d58e1
	WORD $0x30005be1
	WORD $0x400010ee
	WORD $0x1a0e4110
	WORD $0x1004a757
	WORD $0xfff85000
	WORD $0x2000ebef
	WORD $0xf0700004
	BYTE $0x07
	BYTE $0xfe
	RET

// func sumInt64VX(a []int64) int64
TEXT ·sumInt64VX(SB), NOSPLIT, $160-32
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·sumInt64VXBody(SB)
	RET

// sumInt64VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumInt64VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xec4c003e
	WORD $0x007ca719
	WORD $0x0000a709
	WORD $0x0000ec48
	WORD $0x0025017c
	WORD $0xec1401be
	WORD $0x0059e700
	WORD $0x00000044
	WORD $0xb9040053
	WORD $0xb9040001
	WORD $0xe7105000
	WORD $0x3006e701
	WORD $0x000030f3
	WORD $0xa70bfffe
	WORD $0x41505010
	WORD $0xa774fff6
	WORD $0xe7100001
	WORD $0x304de700
	WORD $0x100030f3
	WORD $0xe7000000
	WORD $0x3021ec41
	WORD $0x00118064
	WORD $0xb9090041
	WORD $0xeb110003
	WORD $0x000db908
	WORD $0x0031e300
	WORD $0x30000008
	WORD $0x41303008
	WORD $0xa747fffb
	WORD $0xe3002000
	WORD $0x002407fe
	WORD $0xa7090000
	WORD $0xe3002000
	WORD $0x002407fe
	RET

// func l1normInt64VX(a []int64) int64
TEXT ·l1normInt64VX(SB), NOSPLIT, $160-32
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·l1normInt64VXBody(SB)
	RET

// l1normInt64VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·l1normInt64VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xa7090000
	WORD $0xec4c003f
	WORD $0x007ca719
	WORD $0x0000ec48
	WORD $0x0028017c
	WORD $0xec1401be
	WORD $0x0059e700
	WORD $0x00000044
	WORD $0xb9040053
	WORD $0xb9040001
	WORD $0xe7105000
	WORD $0x3006e711
	WORD $0x000030df
	WORD $0xe7010000
	WORD $0x30f3a70b
	WORD $0xfffe4150
	WORD $0x5010a774
	WORD $0xfff3e710
	WORD $0x0001304d
	WORD $0xe7001000
	WORD $0x30f3e700
	WORD $0x00003021
	WORD $0xec410015
	WORD $0x8064b909
	WORD $0x0041eb11
	WORD $0x0003000d
	WORD $0xb9080031
	WORD $0xe3103000
	WORD $0x0004b900
	WORD $0x0011b908
	WORD $0x00014130
	WORD $0x3008a747
	WORD $0xfff7e300
	WORD $0x20000024
	BYTE $0x07
	BYTE $0xfe
	RET

// func l1diffInt64VX(a []int64, b []int64) int64
TEXT ·l1diffInt64VX(SB), NOSPLIT, $160-56
	NO_LOCAL_POINTERS
	MOVD $ret+48(FP), R2
	MOVD a_base+0(FP), R3
	MOVD b_base+24(FP), R4
	MOVD a_len+8(FP), R5
	BL ·l1diffInt64VXBody(SB)
	RET

// l1diffInt64VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·l1diffInt64VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024a709
	WORD $0x0000ec5c
	WORD $0x0046007c
	WORD $0xa7190000
	WORD $0xec58002e
	WORD $0x017cec15
	WORD $0x01be0059
	WORD $0xe7000000
	WORD $0x0044a7e9
	WORD $0x0000b904
	WORD $0x0001e71e
	WORD $0x30003006
	WORD $0xe72e4000
	WORD $0x3006e711
	WORD $0x200030f7
	WORD $0xe7110000
	WORD $0x30dfe701
	WORD $0x000030f3
	WORD $0xa70bfffe
	WORD $0x41e0e010
	WORD $0xa774ffed
	WORD $0xe7100001
	WORD $0x304de700
	WORD $0x100030f3
	WORD $0xe7000000
	WORD $0x3021ec51
	WORD $0x00168064
	WORD $0xb9090051
	WORD $0xeb110003
	WORD $0x000de3e1
	WORD $0x30000004
	WORD $0xe3e14000
	WORD $0x0009b900
	WORD $0x00eeb908
	WORD $0x000e4110
	WORD $0x1008a757
	WORD $0xfff4e300
	WORD $0x20000024
	WORD $0xebeff070
	WORD $0x000407fe
	RET

// func sumInt8VX(a []int8) int8
TEXT ·sumInt8VX(SB), NOSPLIT, $160-25
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·sumInt8VXBody(SB)
	RET

// sumInt8VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumInt8VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xec4c000c
	WORD $0x007cec4a
	WORD $0x000e107d
	WORD $0xa7080000
	WORD $0xa7190000
	WORD $0xa7f4002c
	WORD $0xa7080000
	WORD $0x42002000
	WORD $0x07feec14
	WORD $0x01bb0059
	WORD $0xe7000000
	WORD $0x0044b904
	WORD $0x0053b904
	WORD $0x0001e710
	WORD $0x50000006
	WORD $0xe7010000
	WORD $0x00f3a70b
	WORD $0xfff04150
	WORD $0x5010a764
	WORD $0xfff6e710
	WORD $0x00000044
	WORD $0xe7001000
	WORD $0x0064e700
	WORD $0x10002067
	WORD $0xe7000003
	WORD $0x2021ec41
	WORD $0x000f8064
	WORD $0xb9090041
	WORD $0xb9080013
	WORD $0xe3301000
	WORD $0x00761a03
	WORD $0x41101001
	WORD $0xa747fffa
	WORD $0x42002000
	BYTE $0x07
	BYTE $0xfe
	RET

// func prodInt8VX(a []int8) int8
TEXT ·prodInt8VX(SB), NOSPLIT, $160-25
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·prodInt8VXBody(SB)
	RET

// prodInt8VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·prodInt8VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xec4c000c
	WORD $0x007cec4a
	WORD $0x000e107d
	WORD $0xa7180001
	WORD $0xa7090000
	WORD $0xa7f4003b
	WORD $0xa7180001
	WORD $0x42102000
	WORD $0x07feec04
	WORD $0x01bb0059
	WORD $0xe7000001
	WORD $0x0045b904
	WORD $0x0013b904
	WORD $0x0050e710
	WORD $0x10000006
	WORD $0xe7010000
	WORD $0x00a2a75b
	WORD $0xfff04110
	WORD $0x1010a764
	WORD $0xfff6e710
	WORD $0x00003060
	WORD $0xe7001000
	WORD $0x00a2e710
	WORD $0x00003094
	WORD $0xe7001000
	WORD $0x00a2e710
	WORD $0x00002094
	WORD $0xe7001000
	WORD $0x00a2e710
	WORD $0x0001004d
	WORD $0xe7001000
	WORD $0x00a2e710
	WORD $0x00000021
	WORD $0xec400010
	WORD $0x8064b909
	WORD $0x0040b908
	WORD $0x0030e300
	WORD $0x30000076
	WORD $0xb9fd1010
	WORD $0x41303001
	WORD $0xa747fff9
	WORD $0x42102000
	BYTE $0x07
	BYTE $0xfe
	RET

// func l1normInt8VX(a []int8) int8
TEXT ·l1normInt8VX(SB), NOSPLIT, $160-25
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·l1normInt8VXBody(SB)
	RET

// l1normInt8VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·l1normInt8VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xec4c000c
	WORD $0x007cec4a
	WORD $0x000e107d
	WORD $0xa7190000
	WORD $0xa7080000
	WORD $0xa7f4002f
	WORD $0xa7080000
	WORD $0x42002000
	WORD $0x07feec14
	WORD $0x01bb0059
	WORD $0xe7000000
	WORD $0x0044b904
	WORD $0x0053b904
	WORD $0x0001e710
	WORD $0x50000006
	WORD $0xe7110000
	WORD $0x00dfe701
	WORD $0x000000f3
	WORD $0xa70bfff0
	WORD $0x41505010
	WORD $0xa764fff3
	WORD $0xe7100000
	WORD $0x0044e700
	WORD $0x10000064
	WORD $0xe7001000
	WORD $0x2067e700
	WORD $0x00032021
	WORD $0xec410010
	WORD $0x8064b909
	WORD $0x0041b908
	WORD $0x0013e330
	WORD $0x10000076
	WORD $0x10331a03
	WORD $0x41101001
	WORD $0xa747fff9
	WORD $0x42002000
	BYTE $0x07
	BYTE $0xfe
	RET

// func sumInt16VX(a []int16) int16
TEXT ·sumInt16VX(SB), NOSPLIT, $160-26
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·sumInt16VXBody(SB)
	RET

// sumInt16VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumInt16VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xec4c000c
	WORD $0x007cec4a
	WORD $0x000e087d
	WORD $0xa7080000
	WORD $0xa7190000
	WORD $0xa7f4002c
	WORD $0xa7080000
	WORD $0x40002000
	WORD $0x07feec14
	WORD $0x01bc0059
	WORD $0xe7000000
	WORD $0x0044b904
	WORD $0x0053b904
	WORD $0x0001e710
	WORD $0x50000006
	WORD $0xe7010000
	WORD $0x10f3a70b
	WORD $0xfff84150
	WORD $0x5010a764
	WORD $0xfff6e710
	WORD $0x00000044
	WORD $0xe7001000
	WORD $0x1064e700
	WORD $0x10002067
	WORD $0xe7000003
	WORD $0x2021ec41
	WORD $0x00108064
	WORD $0xb9090041
	WORD $0xeb110001
	WORD $0x000db908
	WORD $0x00314a00
	WORD $0x30004130
	WORD $0x3002a747
	WORD $0xfffc4000
	WORD $0x200007fe
	RET

// func prodInt16VX(a []int16) int16
TEXT ·prodInt16VX(SB), NOSPLIT, $160-26
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·prodInt16VXBody(SB)
	RET

// prodInt16VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·prodInt16VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xec4c000c
	WORD $0x007cec4a
	WORD $0x000e087d
	WORD $0xa7080001
	WORD $0xa7190000
	WORD $0xa7f40035
	WORD $0xa7080001
	WORD $0x40002000
	WORD $0x07feec14
	WORD $0x01bc0059
	WORD $0xe7000001
	WORD $0x1045b904
	WORD $0x0053b904
	WORD $0x0001e710
	WORD $0x50000006
	WORD $0xe7010000
	WORD $0x10a2a70b
	WORD $0xfff84150
	WORD $0x5010a764
	WORD $0xfff6e710
	WORD $0x00003060
	WORD $0xe7001000
	WORD $0x10a2e710
	WORD $0x00003094
	WORD $0xe7001000
	WORD $0x10a2e710
	WORD $0x0001104d
	WORD $0xe7001000
	WORD $0x10a2e700
	WORD $0x00001021
	WORD $0xec410010
	WORD $0x8064b909
	WORD $0x0041eb11
	WORD $0x0001000d
	WORD $0xb9080031
	WORD $0x4c003000
	WORD $0x41303002
	WORD $0xa747fffc
	WORD $0x40002000
	BYTE $0x07
	BYTE $0xfe
	RET

// func dotInt16VX(a []int16, b []int16) int16
TEXT ·dotInt16VX(SB), NOSPLIT, $160-50
	NO_LOCAL_POINTERS
	MOVD $ret+48(FP), R2
	MOVD a_base+0(FP), R3
	MOVD b_base+24(FP), R4
	MOVD a_len+8(FP), R5
	BL ·dotInt16VXBody(SB)
	RET

// dotInt16VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·dotInt16VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024ec5c
	WORD $0x000c007c
	WORD $0xec5a0011
	WORD $0x087da708
	WORD $0x0000a719
	WORD $0x0000a7f4
	WORD $0x0032a708
	WORD $0x00004000
	WORD $0x2000ebef
	WORD $0xf0700004
	WORD $0x07feec15
	WORD $0x01bc0059
	WORD $0xe7000000
	WORD $0x0044a7e9
	WORD $0x0000b904
	WORD $0x0001e71e
	WORD $0x30000006
	WORD $0xe72e4000
	WORD $0x0006a70b
	WORD $0xfff8e702
	WORD $0x110000aa
	WORD $0x41e0e010
	WORD $0xa764fff3
	WORD $0xe7100000
	WORD $0x0044e700
	WORD $0x10001064
	WORD $0xe7001000
	WORD $0x2067e700
	WORD $0x00032021
	WORD $0xec510011
	WORD $0x8064b909
	WORD $0x0051eb11
	WORD $0x0001000d
	WORD $0x48e14000
	WORD $0x4ce13000
	WORD $0x1a0e4110
	WORD $0x1002a757
	WORD $0xfff94000
	WORD $0x2000ebef
	WORD $0xf0700004
	BYTE $0x07
	BYTE $0xfe
	RET

// func l1normInt16VX(a []int16) int16
TEXT ·l1normInt16VX(SB), NOSPLIT, $160-26
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·l1normInt16VXBody(SB)
	RET

// l1normInt16VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·l1normInt16VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xec4c000c
	WORD $0x007cec4a
	WORD $0x000e087d
	WORD $0xa7190000
	WORD $0xa7080000
	WORD $0xa7f4002f
	WORD $0xa7080000
	WORD $0x40002000
	WORD $0x07feec14
	WORD $0x01bc0059
	WORD $0xe7000000
	WORD $0x0044b904
	WORD $0x0053b904
	WORD $0x0001e710
	WORD $0x50000006
	WORD $0xe7110000
	WORD $0x10dfe701
	WORD $0x000010f3
	WORD $0xa70bfff8
	WORD $0x41505010
	WORD $0xa764fff3
	WORD $0xe7100000
	WORD $0x0044e700
	WORD $0x10001064
	WORD $0xe7001000
	WORD $0x2067e700
	WORD $0x00032021
	WORD $0xec410012
	WORD $0x8064b909
	WORD $0x0041eb11
	WORD $0x0001000d
	WORD $0xb9080031
	WORD $0x48103000
	WORD $0x10111a01
	WORD $0x41303002
	WORD $0xa747fffa
	WORD $0x40002000
	BYTE $0x07
	BYTE $0xfe
	RET

// func l1diffInt16VX(a []int16, b []int16) int16
TEXT ·l1diffInt16VX(SB), NOSPLIT, $160-50
	NO_LOCAL_POINTERS
	MOVD $ret+48(FP), R2
	MOVD a_base+0(FP), R3
	MOVD b_base+24(FP), R4
	MOVD a_len+8(FP), R5
	BL ·l1diffInt16VXBody(SB)
	RET

// l1diffInt16VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·l1diffInt16VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024ec5c
	WORD $0x000c007c
	WORD $0xec5a0011
	WORD $0x087da719
	WORD $0x0000a708
	WORD $0x0000a7f4
	WORD $0x0038a708
	WORD $0x00004000
	WORD $0x2000ebef
	WORD $0xf0700004
	WORD $0x07feec15
	WORD $0x01bc0059
	WORD $0xe7000000
	WORD $0x0044a7e9
	WORD $0x0000b904
	WORD $0x0001e71e
	WORD $0x30000006
	WORD $0xe72e4000
	WORD $0x0006e711
	WORD $0x200010f7
	WORD $0xe7110000
	WORD $0x10dfe701
	WORD $0x000010f3
	WORD $0xa70bfff8
	WORD $0x41e0e010
	WORD $0xa764ffed
	WORD $0xe7100000
	WORD $0x0044e700
	WORD $0x10001064
	WORD $0xe7001000
	WORD $0x2067e700
	WORD $0x00032021
	WORD $0xec510014
	WORD $0x8064b909
	WORD $0x0051eb11
	WORD $0x0001000d
	WORD $0x48e13000
	WORD $0x4be14000
	WORD $0xb92700ee
	WORD $0x10ee1a0e
	WORD $0x41101002
	WORD $0xa757fff6
	WORD $0x40002000
	WORD $0xebeff070
	WORD $0x000407fe
	RET

// func sumUint8VX(a []byte) byte
TEXT ·sumUint8VX(SB), NOSPLIT, $160-25
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·sumUint8VXBody(SB)
	RET

// sumUint8VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumUint8VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xec4c000c
	WORD $0x007cec4a
	WORD $0x000e107d
	WORD $0xa7080000
	WORD $0xa7190000
	WORD $0xa7f4002c
	WORD $0xa7080000
	WORD $0x42002000
	WORD $0x07feec14
	WORD $0x01bb0059
	WORD $0xe7000000
	WORD $0x0044b904
	WORD $0x0053b904
	WORD $0x0001e710
	WORD $0x50000006
	WORD $0xe7010000
	WORD $0x00f3a70b
	WORD $0xfff04150
	WORD $0x5010a764
	WORD $0xfff6e710
	WORD $0x00000044
	WORD $0xe7001000
	WORD $0x0064e700
	WORD $0x10002067
	WORD $0xe7000003
	WORD $0x2021ec41
	WORD $0x000f8064
	WORD $0xb9090041
	WORD $0xb9080013
	WORD $0xe3301000
	WORD $0x00761a03
	WORD $0x41101001
	WORD $0xa747fffa
	WORD $0x42002000
	BYTE $0x07
	BYTE $0xfe
	RET

// func prodUint8VX(a []byte) byte
TEXT ·prodUint8VX(SB), NOSPLIT, $160-25
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·prodUint8VXBody(SB)
	RET

// prodUint8VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·prodUint8VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xec4c000c
	WORD $0x007cec4a
	WORD $0x000e107d
	WORD $0xa7180001
	WORD $0xa7090000
	WORD $0xa7f4003b
	WORD $0xa7180001
	WORD $0x42102000
	WORD $0x07feec04
	WORD $0x01bb0059
	WORD $0xe7000001
	WORD $0x0045b904
	WORD $0x0013b904
	WORD $0x0050e710
	WORD $0x10000006
	WORD $0xe7010000
	WORD $0x00a2a75b
	WORD $0xfff04110
	WORD $0x1010a764
	WORD $0xfff6e710
	WORD $0x00003060
	WORD $0xe7001000
	WORD $0x00a2e710
	WORD $0x00003094
	WORD $0xe7001000
	WORD $0x00a2e710
	WORD $0x00002094
	WORD $0xe7001000
	WORD $0x00a2e710
	WORD $0x0001004d
	WORD $0xe7001000
	WORD $0x00a2e710
	WORD $0x00000021
	WORD $0xec400010
	WORD $0x8064b909
	WORD $0x0040b908
	WORD $0x0030e300
	WORD $0x30000076
	WORD $0xb9fd1010
	WORD $0x41303001
	WORD $0xa747fff9
	WORD $0x42102000
	BYTE $0x07
	BYTE $0xfe
	RET

// func l1normUint8VX(a []byte) byte
TEXT ·l1normUint8VX(SB), NOSPLIT, $160-25
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·l1normUint8VXBody(SB)
	RET

// l1normUint8VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·l1normUint8VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xec4c000c
	WORD $0x007cec4a
	WORD $0x000e107d
	WORD $0xa7190000
	WORD $0xa7080000
	WORD $0xa7f4002c
	WORD $0xa7080000
	WORD $0x42002000
	WORD $0x07feec14
	WORD $0x01bb0059
	WORD $0xe7000000
	WORD $0x0044b904
	WORD $0x0053b904
	WORD $0x0001e710
	WORD $0x50000006
	WORD $0xe7010000
	WORD $0x00f3a70b
	WORD $0xfff04150
	WORD $0x5010a764
	WORD $0xfff6e710
	WORD $0x00000044
	WORD $0xe7001000
	WORD $0x0064e700
	WORD $0x10002067
	WORD $0xe7000003
	WORD $0x2021ec41
	WORD $0x000f8064
	WORD $0xb9090041
	WORD $0xb9080013
	WORD $0xe3301000
	WORD $0x00761a03
	WORD $0x41101001
	WORD $0xa747fffa
	WORD $0x42002000
	BYTE $0x07
	BYTE $0xfe
	RET

// func sumUint16VX(a []uint16) uint16
TEXT ·sumUint16VX(SB), NOSPLIT, $160-26
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·sumUint16VXBody(SB)
	RET

// sumUint16VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumUint16VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xec4c000c
	WORD $0x007cec4a
	WORD $0x000e087d
	WORD $0xa7080000
	WORD $0xa7190000
	WORD $0xa7f4002c
	WORD $0xa7080000
	WORD $0x40002000
	WORD $0x07feec14
	WORD $0x01bc0059
	WORD $0xe7000000
	WORD $0x0044b904
	WORD $0x0053b904
	WORD $0x0001e710
	WORD $0x50000006
	WORD $0xe7010000
	WORD $0x10f3a70b
	WORD $0xfff84150
	WORD $0x5010a764
	WORD $0xfff6e710
	WORD $0x00000044
	WORD $0xe7001000
	WORD $0x1064e700
	WORD $0x10002067
	WORD $0xe7000003
	WORD $0x2021ec41
	WORD $0x00108064
	WORD $0xb9090041
	WORD $0xeb110001
	WORD $0x000db908
	WORD $0x00314a00
	WORD $0x30004130
	WORD $0x3002a747
	WORD $0xfffc4000
	WORD $0x200007fe
	RET

// func prodUint16VX(a []uint16) uint16
TEXT ·prodUint16VX(SB), NOSPLIT, $160-26
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·prodUint16VXBody(SB)
	RET

// prodUint16VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·prodUint16VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xec4c000c
	WORD $0x007cec4a
	WORD $0x000e087d
	WORD $0xa7080001
	WORD $0xa7190000
	WORD $0xa7f40035
	WORD $0xa7080001
	WORD $0x40002000
	WORD $0x07feec14
	WORD $0x01bc0059
	WORD $0xe7000001
	WORD $0x1045b904
	WORD $0x0053b904
	WORD $0x0001e710
	WORD $0x50000006
	WORD $0xe7010000
	WORD $0x10a2a70b
	WORD $0xfff84150
	WORD $0x5010a764
	WORD $0xfff6e710
	WORD $0x00003060
	WORD $0xe7001000
	WORD $0x10a2e710
	WORD $0x00003094
	WORD $0xe7001000
	WORD $0x10a2e710
	WORD $0x0001104d
	WORD $0xe7001000
	WORD $0x10a2e700
	WORD $0x00001021
	WORD $0xec410010
	WORD $0x8064b909
	WORD $0x0041eb11
	WORD $0x0001000d
	WORD $0xb9080031
	WORD $0x4c003000
	WORD $0x41303002
	WORD $0xa747fffc
	WORD $0x40002000
	BYTE $0x07
	BYTE $0xfe
	RET

// func dotUint16VX(a []uint16, b []uint16) uint16
TEXT ·dotUint16VX(SB), NOSPLIT, $160-50
	NO_LOCAL_POINTERS
	MOVD $ret+48(FP), R2
	MOVD a_base+0(FP), R3
	MOVD b_base+24(FP), R4
	MOVD a_len+8(FP), R5
	BL ·dotUint16VXBody(SB)
	RET

// dotUint16VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·dotUint16VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024ec5c
	WORD $0x000c007c
	WORD $0xec5a0011
	WORD $0x087da708
	WORD $0x0000a719
	WORD $0x0000a7f4
	WORD $0x0032a708
	WORD $0x00004000
	WORD $0x2000ebef
	WORD $0xf0700004
	WORD $0x07feec15
	WORD $0x01bc0059
	WORD $0xe7000000
	WORD $0x0044a7e9
	WORD $0x0000b904
	WORD $0x0001e71e
	WORD $0x30000006
	WORD $0xe72e4000
	WORD $0x0006a70b
	WORD $0xfff8e702
	WORD $0x110000aa
	WORD $0x41e0e010
	WORD $0xa764fff3
	WORD $0xe7100000
	WORD $0x0044e700
	WORD $0x10001064
	WORD $0xe7001000
	WORD $0x2067e700
	WORD $0x00032021
	WORD $0xec510011
	WORD $0x8064b909
	WORD $0x0051eb11
	WORD $0x0001000d
	WORD $0x48e14000
	WORD $0x4ce13000
	WORD $0x1a0e4110
	WORD $0x1002a757
	WORD $0xfff94000
	WORD $0x2000ebef
	WORD $0xf0700004
	BYTE $0x07
	BYTE $0xfe
	RET

// func l1normUint16VX(a []uint16) uint16
TEXT ·l1normUint16VX(SB), NOSPLIT, $160-26
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·l1normUint16VXBody(SB)
	RET

// l1normUint16VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·l1normUint16VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xec4c000c
	WORD $0x007cec4a
	WORD $0x000e087d
	WORD $0xa7190000
	WORD $0xa7080000
	WORD $0xa7f4002c
	WORD $0xa7080000
	WORD $0x40002000
	WORD $0x07feec14
	WORD $0x01bc0059
	WORD $0xe7000000
	WORD $0x0044b904
	WORD $0x0053b904
	WORD $0x0001e710
	WORD $0x50000006
	WORD $0xe7010000
	WORD $0x10f3a70b
	WORD $0xfff84150
	WORD $0x5010a764
	WORD $0xfff6e710
	WORD $0x00000044
	WORD $0xe7001000
	WORD $0x1064e700
	WORD $0x10002067
	WORD $0xe7000003
	WORD $0x2021ec41
	WORD $0x00108064
	WORD $0xb9090041
	WORD $0xeb110001
	WORD $0x000db908
	WORD $0x00314a00
	WORD $0x30004130
	WORD $0x3002a747
	WORD $0xfffc4000
	WORD $0x200007fe
	RET

// func l1diffUint16VX(a []uint16, b []uint16) uint16
TEXT ·l1diffUint16VX(SB), NOSPLIT, $160-50
	NO_LOCAL_POINTERS
	MOVD $ret+48(FP), R2
	MOVD a_base+0(FP), R3
	MOVD b_base+24(FP), R4
	MOVD a_len+8(FP), R5
	BL ·l1diffUint16VXBody(SB)
	RET

// l1diffUint16VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·l1diffUint16VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024ec5c
	WORD $0x000c007c
	WORD $0xec5a0011
	WORD $0x087da719
	WORD $0x0000a708
	WORD $0x0000a7f4
	WORD $0x0035a708
	WORD $0x00004000
	WORD $0x2000ebef
	WORD $0xf0700004
	WORD $0x07feec15
	WORD $0x01bc0059
	WORD $0xe7000000
	WORD $0x0044a7e9
	WORD $0x0000b904
	WORD $0x0001e71e
	WORD $0x30000006
	WORD $0xe72e4000
	WORD $0x0006e701
	WORD $0x000010f3
	WORD $0xe7002000
	WORD $0x10f7a70b
	WORD $0xfff841e0
	WORD $0xe010a764
	WORD $0xfff0e710
	WORD $0x00000044
	WORD $0xe7001000
	WORD $0x1064e700
	WORD $0x10002067
	WORD $0xe7000003
	WORD $0x2021ec51
	WORD $0x00108064
	WORD $0xb9090051
	WORD $0xeb110001
	WORD $0x000d4a01
	WORD $0x30004b01
	WORD $0x40004110
	WORD $0x1002a757
	WORD $0xfffa4000
	WORD $0x2000ebef
	WORD $0xf0700004
	BYTE $0x07
	BYTE $0xfe
	RET

// func sumUint32VX(a []uint32) uint32
TEXT ·sumUint32VX(SB), NOSPLIT, $160-28
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·sumUint32VXBody(SB)
	RET

// sumUint32VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumUint32VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xec4c000c
	WORD $0x007cec4a
	WORD $0x000e047d
	WORD $0xa7080000
	WORD $0xa7190000
	WORD $0xa7f40029
	WORD $0xa7080000
	WORD $0x50002000
	WORD $0x07feec14
	WORD $0x01bd0059
	WORD $0xe7000000
	WORD $0x0044b904
	WORD $0x0053b904
	WORD $0x0001e710
	WORD $0x50000006
	WORD $0xe7010000
	WORD $0x20f3a70b
	WORD $0xfffc4150
	WORD $0x5010a764
	WORD $0xfff6e710
	WORD $0x00000044
	WORD $0xe7001000
	WORD $0x2067e700
	WORD $0x00032021
	WORD $0xec410010
	WORD $0x8064b909
	WORD $0x0041eb11
	WORD $0x0002000d
	WORD $0xb9080031
	WORD $0x5a003000
	WORD $0x41303004
	WORD $0xa747fffc
	WORD $0x50002000
	BYTE $0x07
	BYTE $0xfe
	RET

// func prodUint32VX(a []uint32) uint32
TEXT ·prodUint32VX(SB), NOSPLIT, $160-28
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·prodUint32VXBody(SB)
	RET

// prodUint32VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·prodUint32VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xec4c000c
	WORD $0x007cec4a
	WORD $0x000e047d
	WORD $0xa7080001
	WORD $0xa7190000
	WORD $0xa7f4002f
	WORD $0xa7080001
	WORD $0x50002000
	WORD $0x07feec14
	WORD $0x01bd0059
	WORD $0xe7000001
	WORD $0x2045b904
	WORD $0x0053b904
	WORD $0x0001e710
	WORD $0x50000006
	WORD $0xe7010000
	WORD $0x20a2a70b
	WORD $0xfffc4150
	WORD $0x5010a764
	WORD $0xfff6e710
	WORD $0x00003060
	WORD $0xe7001000
	WORD $0x20a2e710
	WORD $0x0001204d
	WORD $0xe7001000
	WORD $0x20a2e700
	WORD $0x00002021
	WORD $0xec410010
	WORD $0x8064b909
	WORD $0x0041eb11
	WORD $0x0002000d
	WORD $0xb9080031
	WORD $0x71003000
	WORD $0x41303004
	WORD $0xa747fffc
	WORD $0x50002000
	BYTE $0x07
	BYTE $0xfe
	RET

// func dotUint32VX(a []uint32, b []uint32) uint32
TEXT ·dotUint32VX(SB), NOSPLIT, $160-52
	NO_LOCAL_POINTERS
	MOVD $ret+48(FP), R2
	MOVD a_base+0(FP), R3
	MOVD b_base+24(FP), R4
	MOVD a_len+8(FP), R5
	BL ·dotUint32VXBody(SB)
	RET

// dotUint32VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·dotUint32VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024ec5c
	WORD $0x000c007c
	WORD $0xec5a0011
	WORD $0x047da708
	WORD $0x0000a719
	WORD $0x0000a7f4
	WORD $0x002fa708
	WORD $0x00005000
	WORD $0x2000ebef
	WORD $0xf0700004
	WORD $0x07feec15
	WORD $0x01bd0059
	WORD $0xe7000000
	WORD $0x0044a7e9
	WORD $0x0000b904
	WORD $0x0001e71e
	WORD $0x30000006
	WORD $0xe72e4000
	WORD $0x0006a70b
	WORD $0xfffce702
	WORD $0x120000aa
	WORD $0x41e0e010
	WORD $0xa764fff3
	WORD $0xe7100000
	WORD $0x0044e700
	WORD $0x10002067
	WORD $0xe7000003
	WORD $0x2021ec51
	WORD $0x00118064
	WORD $0xb9090051
	WORD $0xeb110002
	WORD $0x000d58e1
	WORD $0x400071e1
	WORD $0x30001a0e
	WORD $0x41101004
	WORD $0xa757fff9
	WORD $0x50002000
	WORD $0xebeff070
	WORD $0x000407fe
	RET

// func l1normUint32VX(a []uint32) uint32
TEXT ·l1normUint32VX(SB), NOSPLIT, $160-28
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·l1normUint32VXBody(SB)
	RET

// l1normUint32VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·l1normUint32VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xec4c000c
	WORD $0x007cec4a
	WORD $0x000e047d
	WORD $0xa7190000
	WORD $0xa7080000
	WORD $0xa7f40029
	WORD $0xa7080000
	WORD $0x50002000
	WORD $0x07feec14
	WORD $0x01bd0059
	WORD $0xe7000000
	WORD $0x0044b904
	WORD $0x0053b904
	WORD $0x0001e710
	WORD $0x50000006
	WORD $0xe7010000
	WORD $0x20f3a70b
	WORD $0xfffc4150
	WORD $0x5010a764
	WORD $0xfff6e710
	WORD $0x00000044
	WORD $0xe7001000
	WORD $0x2067e700
	WORD $0x00032021
	WORD $0xec410010
	WORD $0x8064b909
	WORD $0x0041eb11
	WORD $0x0002000d
	WORD $0xb9080031
	WORD $0x5a003000
	WORD $0x41303004
	WORD $0xa747fffc
	WORD $0x50002000
	BYTE $0x07
	BYTE $0xfe
	RET

// func l1diffUint32VX(a []uint32, b []uint32) uint32
TEXT ·l1diffUint32VX(SB), NOSPLIT, $160-52
	NO_LOCAL_POINTERS
	MOVD $ret+48(FP), R2
	MOVD a_base+0(FP), R3
	MOVD b_base+24(FP), R4
	MOVD a_len+8(FP), R5
	BL ·l1diffUint32VXBody(SB)
	RET

// l1diffUint32VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·l1diffUint32VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024ec5c
	WORD $0x000c007c
	WORD $0xec5a0011
	WORD $0x047da719
	WORD $0x0000a708
	WORD $0x0000a7f4
	WORD $0x0032a708
	WORD $0x00005000
	WORD $0x2000ebef
	WORD $0xf0700004
	WORD $0x07feec15
	WORD $0x01bd0059
	WORD $0xe7000000
	WORD $0x0044a7e9
	WORD $0x0000b904
	WORD $0x0001e71e
	WORD $0x30000006
	WORD $0xe72e4000
	WORD $0x0006e701
	WORD $0x000020f3
	WORD $0xe7002000
	WORD $0x20f7a70b
	WORD $0xfffc41e0
	WORD $0xe010a764
	WORD $0xfff0e710
	WORD $0x00000044
	WORD $0xe7001000
	WORD $0x2067e700
	WORD $0x00032021
	WORD $0xec510010
	WORD $0x8064b909
	WORD $0x0051eb11
	WORD $0x0002000d
	WORD $0x5a013000
	WORD $0x5b014000
	WORD $0x41101004
	WORD $0xa757fffa
	WORD $0x50002000
	WORD $0xebeff070
	WORD $0x000407fe
	RET

// func sumUint64VX(a []uint64) uint64
TEXT ·sumUint64VX(SB), NOSPLIT, $160-32
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·sumUint64VXBody(SB)
	RET

// sumUint64VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·sumUint64VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xec4c003e
	WORD $0x007ca719
	WORD $0x0000a709
	WORD $0x0000ec48
	WORD $0x0025017c
	WORD $0xec1401be
	WORD $0x0059e700
	WORD $0x00000044
	WORD $0xb9040053
	WORD $0xb9040001
	WORD $0xe7105000
	WORD $0x3006e701
	WORD $0x000030f3
	WORD $0xa70bfffe
	WORD $0x41505010
	WORD $0xa774fff6
	WORD $0xe7100001
	WORD $0x304de700
	WORD $0x100030f3
	WORD $0xe7000000
	WORD $0x3021ec41
	WORD $0x00118064
	WORD $0xb9090041
	WORD $0xeb110003
	WORD $0x000db908
	WORD $0x0031e300
	WORD $0x30000008
	WORD $0x41303008
	WORD $0xa747fffb
	WORD $0xe3002000
	WORD $0x002407fe
	WORD $0xa7090000
	WORD $0xe3002000
	WORD $0x002407fe
	RET

// func l1normUint64VX(a []uint64) uint64
TEXT ·l1normUint64VX(SB), NOSPLIT, $160-32
	NO_LOCAL_POINTERS
	MOVD $ret+24(FP), R2
	MOVD a_base+0(FP), R3
	MOVD a_len+8(FP), R4
	BL ·l1normUint64VXBody(SB)
	RET

// l1normUint64VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·l1normUint64VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xa7090000
	WORD $0xec4c0038
	WORD $0x007ca719
	WORD $0x0000ec48
	WORD $0x0025017c
	WORD $0xec1401be
	WORD $0x0059e700
	WORD $0x00000044
	WORD $0xb9040053
	WORD $0xb9040001
	WORD $0xe7105000
	WORD $0x3006e701
	WORD $0x000030f3
	WORD $0xa70bfffe
	WORD $0x41505010
	WORD $0xa774fff6
	WORD $0xe7100001
	WORD $0x304de700
	WORD $0x100030f3
	WORD $0xe7000000
	WORD $0x3021ec41
	WORD $0x00118064
	WORD $0xb9090041
	WORD $0xeb110003
	WORD $0x000db908
	WORD $0x0031e300
	WORD $0x30000008
	WORD $0x41303008
	WORD $0xa747fffb
	WORD $0xe3002000
	WORD $0x002407fe
	RET

// func l1diffUint64VX(a []uint64, b []uint64) uint64
TEXT ·l1diffUint64VX(SB), NOSPLIT, $160-56
	NO_LOCAL_POINTERS
	MOVD $ret+48(FP), R2
	MOVD a_base+0(FP), R3
	MOVD b_base+24(FP), R4
	MOVD a_len+8(FP), R5
	BL ·l1diffUint64VXBody(SB)
	RET

// l1diffUint64VXBody is the compiled kernel, called by the trampoline above so that
// the register save area it writes belongs to the trampoline's frame.
TEXT ·l1diffUint64VXBody(SB), NOSPLIT|NOFRAME, $0-0
	WORD $0xebeff070
	WORD $0x0024a709
	WORD $0x0000ec5c
	WORD $0x003f007c
	WORD $0xa7190000
	WORD $0xec58002b
	WORD $0x017cec15
	WORD $0x01be0059
	WORD $0xe7000000
	WORD $0x0044a7e9
	WORD $0x0000b904
	WORD $0x0001e71e
	WORD $0x30003006
	WORD $0xe72e4000
	WORD $0x3006e701
	WORD $0x000030f3
	WORD $0xe7002000
	WORD $0x30f7a70b
	WORD $0xfffe41e0
	WORD $0xe010a774
	WORD $0xfff0e710
	WORD $0x0001304d
	WORD $0xe7001000
	WORD $0x30f3e700
	WORD $0x00003021
	WORD $0xec510012
	WORD $0x8064b909
	WORD $0x0051eb11
	WORD $0x0003000d
	WORD $0xe3013000
	WORD $0x0008e301
	WORD $0x40000009
	WORD $0x41101008
	WORD $0xa757fff8
	WORD $0xe3002000
	WORD $0x0024ebef
	WORD $0xf0700004
	BYTE $0x07
	BYTE $0xfe
	RET

